...
Motivation: 现有RRG方法在真实临床环境中质量不足，而大型视觉语言模型在通用领域通过采用对齐技术取得了显著进展，这启发了将类似技术应用于医学领域。

Method: 采用随机对比采样构建训练对，使用直接偏好优化（DPO）来增强模型性能，该方法不需要额外训练数据或人工标注。

Result: 在三个最先进模型上应用随机DPO后，临床性能指标提升了高达5%。

Conclusion: 随机DPO是一种有效的模型增强方法，能够显著提升放射学报告生成的质量，为临床部署提供了可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有自闭症诊断方法存在凝视特征性能差和缺乏现实世界泛化能力的问题，需要开发更可靠的计算辅助诊断工具。

Method: 使用包含168名ASC参与者和157名非自闭症参与者的标准化视频数据集，进行多模态分析，并引入新的统计描述符来量化凝视角度变异性，采用后期融合方法整合各模态特征。

Result: 凝视分类准确率从64%提升到69%，多模态融合后总体分类准确率达到74%，验证了多模态行为标记在ASC诊断中的有效性。

Conclusion: 研究表明基于视频的多模态筛查工具具有支持自闭症评估的潜力，为开发可扩展的筛查工具提供了基础。
...
<details>
  <summary>Details</summary>
Motivation: VLA模型在长时推理中存在注意力二次成本和KV内存无限增长的问题，现有方法往往忽视实时部署的推理效率需求。

Method: 将KV缓存划分为固定大小的块，使用循环门控模块根据学习到的效用分数对历史上下文进行总结和过滤，保留近期细粒度细节，同时积极修剪过时、低相关性的内存。

Result: 理论上实现1.21倍推理加速和36% KV内存减少，对任务成功率影响最小。
...
<details>
  <summary>Details</summary>
Motivation: 大规模视觉语言模型生成的医学报告存在事实错误和幻觉，阻碍了临床应用

Method: 通过扰动真实报告生成合成数据集，训练多标签跨模态对比回归网络进行事实核查

Result: 在多个X光数据集上验证了方法的有效性，与基于真实标注的验证达到0.997的一致性

Conclusion: 该方法在放射学工作流程中具有临床推理的应用价值
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在捕捉视网膜疾病诊断所需的关键低层空间细节方面存在困难，特别是对于青光眼、糖尿病视网膜病变和色素性视网膜炎等疾病。

Method: MDF-MLLM将U-Net编码器四层的跳跃特征整合到LLaMA 3.2 11B MLLM的交叉注意力块中，采用补丁级投影、缩放交叉注意力和基于FiLM的U-Net调制进行特征融合。

Result: 模型在1305个眼底图像-文本对数据集上评估，基线MLLM准确率为60%，MDF-MLLM达到94%准确率，召回率和F1分数分别提升67%和35%。消融研究证实多深度融合方法显著提升了空间推理和分类能力。

Conclusion: MDF-MLLM为眼底图像分类提供了一个可泛化、可解释和模块化的框架，通过多尺度特征融合优于传统MLLM基线，在临床决策支持系统中具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有越狱方法主要操纵文本提示，对基于图像输入的漏洞探索不足，且文本方法难以绕过模型的安全过滤器。

Method: MPDA包含三个核心步骤：1）LLM将不安全提示解耦为伪安全提示和有害提示；2）LLM将有害提示重写为自然对抗提示以绕过过滤器；3）视觉语言模型生成图像描述确保语义一致性。

Result: 该方法能够有效绕过T2I模型的安全过滤器，生成与原始不安全提示语义一致的NSFW图像内容。

Conclusion: MPDA揭示了多模态输入在越狱攻击中的潜力，为T2I模型的安全防护提供了新的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在显著目标检测中取得了很大进展，但预测的显著图仍存在由于目标内部复杂性导致的不完整预测，以及卷积和池化操作步长导致的边界不准确问题。

Method: 提出相互学习模块（MLM），通过多任务监督方式同时训练显著目标检测、前景轮廓检测和边缘检测任务。MLM包含多个网络分支，以相互学习方式训练，显著提升性能。

Result: 在七个具有挑战性的数据集上进行了广泛实验，证明该方法在显著目标检测和边缘检测方面都取得了最先进的结果。

Conclusion: 通过多任务监督和相互学习机制，能够生成具有均匀高亮的显著图，并实现精确的前景轮廓预测，同时减少边缘预测的局部噪声。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态相关性评估指标通常基于预训练对比学习模型的嵌入能力，但仅限于双模态数据分析，这严重限制了多模态相似性的评估。

Method: 提出MAJORScore，首次通过多模态联合表示来评估多个模态（N≥3）的相关性。多模态联合表示能够将多个模态整合到相同的潜在空间中，在一个尺度上准确表示不同模态，为公平的相关性评分提供支持。

Result: 大量实验表明，与现有方法相比，MAJORScore在一致性模态上提高了26.03%-64.29%，在不一致性模态上降低了13.28%-20.54%。
...
<details>
  <summary>Details</summary>
Motivation: 传统脚手架安全检查主要依赖人工目视检查，耗时且容易出错，可能导致安全隐患。需要更准确高效的自动化检测方法。

Method: 开发基于云的AI平台，处理脚手架结构的点云数据，通过对比认证参考数据和最新扫描数据来检测结构修改和偏差。

Result: 提出的系统能够自动监测脚手架结构变化，减少人工检查所需的时间和精力。

Conclusion: AI和数字化技术能够显著提升脚手架安全检查的效率和准确性，有助于改善建筑工地的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 当前文本到图像生成在细粒度控制方面存在挑战，特别是反事实可控性（生成违背常识模式的图像）对于创造性和探索性应用至关重要。

Method: 框架包含三个组件：图像评估器（指导数据集构建）、监督提示重写器（生成修订后的提示）、DPO训练的排序器（选择最佳修订提示）。扩展了Grounded SAM以改进图像评估器。

Result: 图像评估器性能相比其骨干网络提升了114%，实验表明该方法优于现有基线方法和ChatGPT-4o。
...
<details>
  <summary>Details</summary>
Motivation: 光学显微镜存在横向分辨率限制（约200nm），传统超分辨率技术需要昂贵设备或特殊荧光标记。本研究旨在探索一种经济有效的非荧光超分辨率显微方法。

Method: 使用两种先前开发的深度神经网络架构（O-Net和Theta-Net），在Zernike相位对比和微分干涉对比显微镜下对定制纳米尺度测试靶进行超分辨率重建，测试靶通过原子力显微镜校准。

Result: 两种网络都能有效实现超分辨率，但表现互补：高信噪比图像适合O-Net模型，低信噪比图像更适合Theta-Net模型。

Conclusion: 模型架构和源图像信噪比对超分辨率质量有重要影响，即使使用相同训练数据和训练周期，不同架构在不同条件下表现各异，说明需要根据具体应用场景选择合适的模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有工作对视觉和音频数据的基本融合忽略了更深层次的感知上下文，需要更有效地利用多模态线索来指导导航

Method: 使用多目标架构结合改进的Transformer机制，对跨模态信息进行过滤和选择性融合

Result: 在Replica和Matterport3D数据集上，DMTF-AVN在成功率、路径效率和场景适应方面均优于现有方法
...
<details>
  <summary>Details</summary>
Motivation: 现有稀疏自编码器虽然减少了神经元多义性，但概念表示仍分布在多个潜在特征中，需要大量搜索过程进行概念遗忘，效率低下。

Method: 提出SAEmnesia方法，通过监督训练促进一对一概念-神经元映射，缓解特征分裂并促进特征集中化，仅引入交叉熵计算的计算开销。

Result: 在UnlearnCanvas基准测试中比现有技术提升9.22%，在9对象顺序遗忘任务中遗忘准确率提升28.4%，超参数搜索减少96.67%。

Conclusion: SAEmnesia通过专门化神经元学习实现了更有效的概念遗忘，显著降低了计算开销并提升了可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型训练需要大量计算资源和时间，特别是在大数据集上进行超参数搜索时。随机采样方法存在对主导类别的偏见，且无法捕捉类内多样性，需要更智能的核心集选择机制

Method: 引入一种智能轻量级核心集选择机制，通过提取类内多样性，形成每个类别的聚类，然后利用这些聚类进行最终采样

Result: 在知名生物医学影像数据集上的分类实验表明，该方案在多个性能指标上优于随机采样方法

Conclusion: 提出的智能核心集选择方法能有效解决随机采样的局限性，在保持数据集代表性的同时显著减少计算需求
...
<details>
  <summary>Details</summary>
Motivation: 目前大多数深度学习模型仅使用单次或有限次数的既往乳腺X光检查，且缺乏对现实世界环境中不平衡结果分布和异质性随访的适应性。

Method: 开发了LongiMam模型，结合卷积神经网络（捕捉空间模式）和循环神经网络（捕捉时间模式），在大型人群筛查数据集上训练和评估，该数据集具有典型的临床筛查中不平衡的病例-对照比例。

Result: 在多种不同数量和组成的既往检查场景下，LongiMam在包含既往乳腺X光检查时一致提高了预测性能。结合既往和当前访视的表现优于单次访视模型，而仅使用既往检查表现较差，突显了结合历史和近期信息的重要性。

Conclusion: 纵向建模增强了乳腺癌预测，支持使用重复乳腺X光检查来优化筛查项目中的风险分层。LongiMam已作为开源软件公开发布。
...
<details>
  <summary>Details</summary>
Motivation: 研究CNN与人类大脑在社交认知这一更复杂大脑过程中的对应关系，而不仅仅是通用视觉感知。

Method: 通过相关性分析评估CNN架构与人类行为和fMRI数据的对齐程度，并提出了Object2Brain框架结合GradCAM和对象检测来研究不同对象类别对CNN-人类相关性的影响。

Result: CNN在图像情感评估任务中主要进行简单视觉处理，无法反映高阶大脑处理；不同CNN架构显示出不同的对象类别敏感性。

Conclusion: CNN在处理复杂社交认知任务时与人类大脑处理存在差异，需要更先进的模型来捕捉高阶认知过程。
...
<details>
  <summary>Details</summary>
Motivation: CBMs虽然允许专家干预概念预测，但无法解决模型与专家推理之间的系统性偏差，如模型从有偏数据中学习捷径。

Method: 提出两步框架：1) Removal步骤：专家使用概念解释识别并移除不良概念；2) Retraining步骤：引入CBDebug方法，将概念级用户反馈转化为样本级辅助标签，应用监督偏差缓解和定向增强。

Result: 在多个CBM架构（PIP-Net、Post-hoc CBM）和已知伪相关基准测试中，CBDebug显著优于先前的重训练方法。

Conclusion: CBDebug框架有效减少了CBMs对不良概念的依赖，提升了模型的可解释性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 沉船是海事历史的重要标记，但传统的手动检查测深数据方法耗时且需要专家分析

Method: 开发基于深度学习的开源工具，包括数据预处理、深度学习推理、模型输出阈值处理，使用来自五大湖和爱尔兰海岸的沉船数据进行训练，并采用合成数据生成技术增强数据集

Result: 与基于深度学习的ArcGIS工具包和传统逆沉洞检测方法相比，该工具在分割性能上表现更优

Conclusion: ShipwreckFinder为沉船检测提供了一个高效、自动化的开源解决方案，显著提高了检测效率
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明神经网络可以大幅剪枝而不损失性能，但剪枝对模型可解释性的影响尚不明确。

Method: 使用在ImageNette上训练的ResNet-18，比较不同剪枝水平下Vanilla Gradients和Integrated Gradients的显著性图，并应用CRAFT进行概念提取分析。

Result: 轻到中度剪枝提高了显著性图的聚焦度和忠实度，保持了语义明确的概念；而过度剪枝会合并异质特征，降低显著性图稀疏性和概念一致性。

Conclusion: 剪枝可以塑造内部表征使其更符合人类注意力模式，但过度剪枝会削弱可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖点云输入，但大多数消费级相机缺乏深度传感器，视觉数据更为常见。需要解决仅使用多视图图像进行室内场景理解的问题。

Method: 基于轻量级稀疏卷积主干网络，使用两个专用头部：一个用于3D物体检测，一个用于布局估计，采用新颖有效的参数化墙壁表示方法。

Result: 在三个具有挑战性的场景理解基准测试中均达到最先进性能：使用真实点云、使用带位姿图像和使用无位姿图像。在布局估计方面显著提升，为整体室内场景理解设定了新基准。

Conclusion: TUN3D能够仅从多视图图像创建紧凑且语义丰富的空间表示，在3D物体检测方面与专门方法相当，在布局估计方面显著推进了室内场景理解技术。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法往往忽视图像不同区域的重要性差异，可能影响视觉关键内容的重建质量。

Method: 图像分割为关键和非关键区域，关键区域使用图像语义编码器，非关键区域采用图像到文本建模压缩，结合模型量化和低秩适应微调等轻量级部署技术。

Result: 仿真结果表明，该系统在语义保真度和视觉质量方面均优于传统方法。

Conclusion: 该系统在图像传输任务中表现出有效性，实现了语义保真度和视觉质量的提升。
...
<details>
  <summary>Details</summary>
Motivation: 为毫米波ISAC系统的人类感知研究提供标准化的标注数据集，推动信号处理和深度学习在该领域的发展。

Method: 开发了测试平台和实验设置，收集了多种信号特征数据，并采用参数高效微调技术来适应不同任务。

Result: 构建了完整的数据集并验证了其在下游任务中的实用性，通过微调技术显著降低了计算复杂度。

Conclusion: mmHSense数据集为毫米波ISAC研究提供了重要资源，参数高效微调方法有助于模型在不同任务间的迁移应用。
...
<details>
  <summary>Details</summary>
Motivation: 中轴（骨架）作为形状描述符对噪声敏感，传统修剪方法存在局限性。受稀疏化尺度空间启发，希望构建满足尺度空间特性的骨架简化框架。

Method: 结合中轴稀疏化构建骨架化尺度空间，包括连续和离散理论框架，并引入致密化概念实现逆向尺度进展。

Result: 实验证明该框架在鲁棒骨架化、形状压缩和增材制造刚度增强等实际任务中有效。

Conclusion: 骨架化尺度空间提供了一种理论严谨的层次化形状简化方法，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有气候预测空间分辨率低（12.5公里），限制了地方决策的实用性，需要高分辨率气候预测支持本地决策。

Method: 在缺乏高分辨率气候预测训练数据的情况下，使用高分辨率观测网格数据集训练单图像超分辨率模型，然后应用于低分辨率气候预测。提出基于气候指标的评估方法，使用气象站观测的气候指数来评估降尺度结果。

Result: 日平均温度实验表明，单图像超分辨率模型能够降尺度气候预测，且不会增加气候指标的误差。

Conclusion: 该方法能够有效提升气候预测的空间分辨率，为地方决策提供更精细的气候信息。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型(VLMs)在多模态推理任务中表现出色，但存在被滥用的风险。研究表明基于图像的扰动特别有效，现有越狱技术性能不稳定且扰动明显，需要更有效的攻击方法。

Method: 提出JaiLIP方法，在图像空间中最小化联合目标函数，结合干净图像与对抗图像之间的均方误差损失和模型有害输出损失。

Result: 实验结果表明，该方法生成的对抗图像高度有效且难以察觉，在毒性指标上优于现有方法，并在交通领域验证了其实际应用价值。

Conclusion: 研究强调了基于图像越狱攻击的实际挑战，以及为VLMs开发高效防御机制的必要性。
...
<details>
  <summary>Details</summary>
Motivation: 量化自动识别系统与人类专家在生物识别任务中的不确定性差异，为计算机科学家和自然学家提供性能对比参考。

Method: 设计ExpertCLEF挑战赛，评估19个由4个研究团队实现的深度学习系统，并与9位法国植物区系专家进行对比。

Result: 最先进的深度学习模型在植物识别任务上的表现已接近最先进的人类专家水平。

Conclusion: 深度学习在自动生物识别领域取得了显著进展，其性能已接近人类专家，但仍需考虑图像信息不完整带来的识别不确定性。
...
TL;DR: QuadGPT是首个端到端的四边形网格生成自回归框架，通过统一标记化方法和强化学习微调，直接生成高质量四边形网格，显著优于传统三角网格转换方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通过先生成三角网格再合并为四边形的方式会产生较差的拓扑结构，需要开发直接生成高质量四边形网格的方法。

Method: 提出QuadGPT框架，采用序列预测范式，包含统一标记化方法处理混合拓扑结构，以及专门的强化学习微调方法tDPO提升生成质量。

Result: 实验证明QuadGPT在几何精度和拓扑质量上都显著超越之前的三角网格转换流程。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像扩散模型会无意中复制受版权保护的风格和视觉概念，而现有的概念擦除方法无法扩展到需要擦除多个可能冲突概念的实用场景。

Method: 提出DyME框架，训练概念特定的LoRA适配器，并在推理时动态组合需要的适配器；引入双级正交性约束（特征级和参数级）来减少适配器间的干扰；开发了新的分层基准ErasureBench-H。

Result: 在ErasureBench-H和标准数据集上的实验表明，DyME在保持多概念擦除保真度的同时，显著减少了副作用，优于现有最先进方法。

Conclusion: DyME通过动态模块化设计和正交性约束，有效解决了多概念擦除的可扩展性和干扰问题，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频理解评估指标（如BLEU、ROUGE等）无法准确反映人类判断的精细度，而人工评估成本高昂，需要开发更有效的自动化评估方法。

Method: 通过生成器-评估器的交互机制进行训练：生成器根据目标评分生成响应，不匹配评估器评分的响应被丢弃，从而构建训练数据。

Result: 在四个元评估基准中的三个上，VideoJudge-7B超越了更大的MLLM基线模型（如Qwen2.5-VL 32B和72B），证明了视频输入对于视频理解任务评估的重要性。

Conclusion: 多模态模型在视频理解评估中优于纯语言模型，视频输入是评估视频理解任务的关键因素，长链思维推理并不能提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体协同感知通过共享信息提升场景理解能力，但通信带宽限制了系统可扩展性。需要开发高效的特征压缩方法来降低通信负载。

Method: 端到端方法：使用简单瓶颈网络压缩特征维度，然后采用多阶段残差向量量化（RVQ），仅传输每个像素的代码索引，将特征从8192 bpp压缩到6-30 bpp。

Result: 在DAIR-V2X数据集上，ReVQom实现273倍（30 bpp）到1365倍（6 bpp）压缩。在18 bpp（455倍压缩）时性能匹配或优于原始特征协同感知，在6-12 bpp时实现超低带宽运行。
...
<details>
  <summary>Details</summary>
Motivation: 研究当代文本到图像AI模型在生成沙特职业图像时是否延续性别刻板印象和文化不准确性。

Method: 使用ImageFX、DALL-E V3和Grok三个模型为56种沙特职业生成1,006张图像，由两名沙特注释员从五个维度评估，第三位资深研究员裁决分歧，共获得10,100个判断。

Result: 所有模型都显示强烈性别失衡（ImageFX 85%男性，Grok 86.6%男性，DALL-E V3 96%男性），领导和技术角色失衡最明显，文化不准确性普遍存在。

Conclusion: 当前模型反映了训练数据中的社会偏见，需要更多样化的训练数据、更公平的算法和更文化敏感的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要为每种不当内容类型单独训练小型分类模型，需要大量人工标注数据且缺乏跨问题泛化能力

Method: 引入三个预训练任务：Caption（增强视频细节感知）、VQA（加深问题定义理解）、Chain-of-Thought（增强推理能力）

Result: 实验结果显示该方法显著提升了MLLM在零样本和监督微调设置下的性能，并对新兴未见问题表现出强泛化能力

Conclusion: 该预训练方法有效解决了短视频内容与MLLM原始预训练数据之间的分布差异问题，实现了统一的不当内容检测
...
<details>
  <summary>Details</summary>
Motivation: 传统基于坐标预测的GUI定位方法在处理高分辨率复杂布局时精度不足，需要更鲁棒的解决方案。

Method: 提出GUI-Cursor模型，将GUI定位重构为交互式搜索任务：模型生成动作移动光标，基于空间关系和移动历史逐步接近目标元素。使用多步在线强化学习和密集轨迹奖励函数进行训练。

Result: 基于Qwen2.5-VL-7B的GUI-Cursor在ScreenSpot-v2（88.8%→93.9%）和ScreenSpot-Pro（26.8%→56.5%）上显著提升准确率，达到SOTA。95%的实例可在两步内解决，对困难样本能自适应增加步骤。

Conclusion: 交互式搜索方法比直接坐标预测更有效，视觉反馈和强化学习训练使模型能够更准确地定位GUI元素，特别是在复杂布局场景下。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有文本-视频检索系统的两个局限：1）低质量文本-视频数据对难以识别和检查，2）余弦相似度缺乏对排序结果的解释性，限制了模型的可解释性。

Method: 提出X-CoT框架，首先扩展现有基准数据集，添加视频注释以支持语义理解并减少数据偏差；设计检索思维链，通过成对比较步骤产生详细推理和完整排序。

Result: X-CoT在实证中提高了检索性能，并生成详细推理依据；同时有助于分析模型行为和数据质量。

Conclusion: X-CoT框架通过可解释的推理过程有效解决了传统检索系统的局限性，为模型评估和数据检查提供了新的途径。
...
<details>
  <summary>Details</summary>
Motivation: 手术器械的安全检测需要可靠的视觉缺陷检测，但人工检查容易出错，现有的自动化方法由于领域迁移问题无法有效应用于手术器械图像。

Method: 整合背景掩码、基于补丁的分析策略和高效的领域自适应技术，专门针对手术器械图像优化无监督缺陷检测方法。

Result: 该方法能够可靠地检测手术器械图像中的细粒度缺陷，克服了误报检测、对小而细微缺陷敏感度不足以及领域迁移问题。

Conclusion: 提出的方法成功解决了手术器械缺陷检测中的关键挑战，为手术器械的安全检测提供了有效的自动化解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于表示对齐的方法虽然能提升扩散模型的判别特征表示质量，但依赖于计算成本高昂的大型预训练编码器，这限制了其应用效率。

Method: 提出LSEP正则化方法，通过促进中间层表示的线性可分性，将线性探测直接融入网络学习过程，无需外部编码器和对齐操作。

Result: 在基于流的Transformer架构（如SiTs）上实现了显著改进，在256×256 ImageNet数据集上获得了1.46的FID分数。

Conclusion: LSEP方法有效消除了对外部编码器的依赖，同时提升了训练效率和生成质量，为大规模扩散模型训练提供了更高效的替代方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的对比学习方法在全局图像地理定位中忽略了地理空间中的空间依赖性，导致无法有效处理假阴性（视觉和地理相似但被标记为负样本的图像对）和难负样本（视觉相似但地理距离远的图像对）的问题。

Method: 提出空间正则化对比学习策略，整合半变异函数来建模空间相关性。通过将特征空间中的图像距离与地理距离关联来拟合半变异函数，定义给定空间距离下的预期视觉差异作为参考来识别难负样本和假阴性。该方法集成到GeoCLIP框架中。

Result: 在OSV5M数据集上的评估表明，显式建模空间先验能提高图像地理定位性能，特别是在更细粒度上表现更好。

Conclusion: 通过整合空间先验建模，能够有效提升对比学习在图像地理定位任务中的性能，特别是在处理空间依赖性和区分相似但地理距离不同的图像对方面表现出优势。
...
<details>
  <summary>Details</summary>
Motivation: 构建能够进行无限文本、语音和视频交互的数字人类代理，实现从静态肖像到持久智能视听交互的转化。

Method: 采用Thinker-Actor双Transformer架构，Thinker模块基于预训练大语言-语音模型进行感知推理，Actor模块使用分块自回归扩散模型，通过交叉注意力机制生成时间对齐的多模态响应。

Result: 系统在两张A100 GPU上实现实时运行，支持从任意肖像进行数小时一致性的视频聊天体验。

Conclusion: X-Streamer为交互式数字人类的统一世界建模开辟了新途径，实现了持久稳定的多模态交互体验。
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张图像预测物体运动的问题，现有视频生成器虽然被视为世界模型，但在简单物理场景中预测运动效果不佳，因为其需要生成像素而非直接建模运动。

Method: 采用条件生成密集轨迹网格的模型，架构类似现代视频生成器但输出运动轨迹而非像素，直接建模运动而非生成视觉内容。

Result: 在模拟数据上表现优异，在机器人学等下游应用中有效，在真实世界直觉物理数据集上显示出有前景的准确性，优于现有回归器和生成器。

Conclusion: 直接建模运动轨迹比生成像素能更有效地预测物体运动，该方法在运动预测任务上超越了当前最先进的视频生成器。
...
<details>
  <summary>Details</summary>
Motivation: 探索不同自监督学习架构在视频动作识别任务中的性能差异，为选择合适的特征提取方法提供实证依据。

Method: 在UCF Sports数据集上对DINOv3（独立帧处理）和V-JEPA2（联合时间建模）进行对比评估，分析分类精度、聚类性能、类内一致性和类间区分度等多个维度。

Result: DINOv3在聚类性能（Silhouette得分0.31 vs 0.21）和类间区分度（6.16倍分离比）上表现更好，特别适合姿态识别；V-JEPA2在所有动作类型上表现更一致，性能方差更低（0.094 vs 0.288）。

Conclusion: 不同架构存在根本性权衡：DINOv3适合静态姿态识别，V-JEPA2在运动依赖型动作上表现更均衡。选择方法应基于任务需求和可靠性约束。
...
<details>
  <summary>Details</summary>
Motivation: 传统灾害评估方法效率低且危险，现有计算机视觉方法只能提供分类标签或分割掩码，缺乏对灾害场景的全面理解能力。

Method: 采用双架构方法：CNN-LSTM模型（ResNet50骨干网络）处理卫星图像，Vision Transformer模型处理无人机图像，结合ConceptNet和WordNet的外部语义知识增强词汇覆盖和描述准确性。

Result: VLCE在InfoMetIC指标上达到95.33%的最高分，同时保持有竞争力的语义对齐性能，显著优于LLaVA和QwenVL等基线模型。

Conclusion: 双架构系统通过自动生成信息密集的描述，在改善灾害损害评估方面显示出巨大潜力，能够从卫星和无人机照片中提供可操作的情报。
...
Method: 使用多种表征相似性度量（几何、单元调谐、线性可解码性），并采用相似性网络融合（SNF）技术整合这些互补的度量。

Result: 几何和调谐度量能有效区分模型家族，而线性可解码信息更广泛共享。SNF融合后实现了比任何单一度量更强的家族分离，并发现了监督ResNets和ViTs形成不同簇，但所有自监督模型跨架构边界聚集在一起的有趣模式。

Conclusion: 该生物学启发框架为视觉模型提供了原则性的分类方法，表明由架构和训练目标共同塑造的新兴计算策略定义了超越表面设计类别的表征结构。
...
<details>
  <summary>Details</summary>
Motivation: 当前视频基础模型虽然具有强大的想象力先验，但缺乏明确的3D基础能力，限制了其在空间一致性和下游3D推理任务中的实用性。

Method: 引入交叉分支监督机制，几何线索指导视频生成，视频先验正则化3D预测，通过统一主干和跨分支信息交换实现3D感知视频表示。

Result: 实验表明FantasyWorld有效桥接视频想象和3D感知，在多视图一致性和风格一致性方面优于现有几何一致性基线，几何分支的潜在表示可支持新视角合成和导航等下游任务。

Conclusion: 该框架通过几何增强实现了视频模型与3D感知的统一，为AR/VR内容创建和机器人导航等应用提供了高质量的3D世界模型。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决科学机器学习中处理异构和多模态科学观测数据的挑战，需要一个灵活且强大的模型来处理不同维度、分辨率和物理场的混合数据。

Method: 采用卷积视觉Transformer架构，结合组件卷积处理标量和矢量通道，跨场交叉注意力建模不同物理场间信息传播，轴向注意力降低计算负担。

Result: 在多个PDE数据集上预训练后，MORPH在零样本和全样本泛化任务中表现优异，匹配或超越现有基线模型和最新SOTA模型。

Conclusion: MORPH为学习异构和多模态科学观测数据提供了灵活强大的基础，为可扩展和数据高效的科学机器学习开辟了新路径。
...
Result: 在FLIR ADAS V2数据集上，MS-YOLO仅需6.7 GFLOPs计算量，达到竞争性mAP和优越精度，计算开销降低1.5%
...
TL;DR: MATR是一个基于Transformer的多目标跟踪框架，通过显式预测物体运动来减少查询冲突，在多个基准测试中达到state-of-the-art性能。


<details>
  <summary>Details</summary>
Motivation: 现有的DETR-based多目标跟踪方法在单个Transformer解码器层中联合处理检测和跟踪查询，导致查询冲突和关联精度下降。

Method: 提出Motion-Aware Transformer (MATR)，显式预测跨帧的物体运动来提前更新跟踪查询，减少查询碰撞，实现更一致的训练。

Result: 在DanceTrack上HOTA提升超过9分，达到71.3；在SportsMOT上达到72.2 HOTA；在BDD100k上达到54.7 mTETA和41.6 mHOTA，均达到state-of-the-art。
...
TL;DR: DeLiVR是一种高效的视频去雨方法，通过将时空李群微分偏置直接注入网络注意力分数中，解决雨纹、模糊和噪声问题，同时增强时空一致性。
...
Method: 提出两个互补组件：1）旋转有界李相对偏置，通过紧凑预测模块预测每帧的平面内角度，实现几何一致对齐；2）微分群位移，计算相邻帧间角度差估计速度，结合时间衰减和注意力掩码聚焦帧间关系。

Result: 在公开基准测试上的广泛实验结果证明了该方法的有效性。

Conclusion: DeLiVR通过李群表示连续几何变换，为视频建模中的时空一致性提供了原则性方法，在去雨任务中表现出色。
...
TL;DR: 该论文研究了基础AI/ML模型在合成孔径雷达（SAR）目标识别任务中的可行性，通过自监督学习微调公开可用的SSL模型，在SAR数据上训练AFRL-DINOv2模型，显著超越了现有最佳SAR领域模型SARATR-X的性能。
...
Method: 首先测试了现有最强大的视觉基础模型（DINOv2、DINOv3、PE-Core）在SAR任务上的表现，然后采用自监督学习微调方法，在SAR数据上训练多个AFRL-DINOv2模型，并分析不同骨干网络和下游任务适应策略的性能权衡。

Result: AFRL-DINOv2模型在SAR基础模型方面达到了新的最先进水平，显著优于当前最佳SAR领域模型SARATR-X。实验还分析了模型在不同下游环境挑战（如扩展操作条件和少量标注数据）下的表现。

Conclusion: 虽然取得了积极成果，但SAR基础模型的开发仍有很长的路要走。作者希望这项工作能为未来的SAR基础模型构建者提供参考和启发。
...
<details>
  <summary>Details</summary>
Motivation: 现有UI测试方法依赖物理设备或静态截图分析，难以应对真实移动环境的动态多样性，阻碍了可扩展测试和智能UI代理的开发。

Method: 采用两阶段方法：给定初始屏幕图像和用户操作，首先预测下一个UI状态的抽象布局，然后基于预测布局合成视觉一致的新图像。

Result: 实验结果显示UISim在生成真实连贯的后续UI状态方面优于端到端UI生成基线方法，证明了其保真度和实用性。
...
TL;DR: 提出LFA-Net轻量化视网膜血管分割网络，通过LiteFusion-Attention模块高效捕获局部和全局上下文，在资源受限环境中实现高性能分割。


<details>
  <summary>Details</summary>
Motivation: 解决现有深度学习模型在小血管分割和计算成本高方面的挑战，特别是在计算资源有限的真实临床环境中实现轻量化视网膜血管分割。

Method: 设计LFA-Net网络，包含新型注意力模块LiteFusion-Attention，融合残差学习连接、Vision Mamba启发的动态机制和基于调制的注意力机制。

Result: 在DRIVE、STARE和CHASE_DB数据集上分别获得83.28%、87.44%、84.50%的Dice分数和72.85%、79.31%、74.70%的Jaccard指数，模型仅需11万参数、0.42MB内存和4.46GFLOPs。

Conclusion: LFA-Net在保持轻量化的同时实现了优异的血管分割性能，特别适合资源受限的临床环境应用。
...
Method: 包含视觉上下文编码模块（利用多尺度场景信息编码个体关系）和情感语义编码模块（利用大语言模型生成情感词典，通过结构化情感树生成全面语义表示），最后通过相似性感知交互对齐和整合视觉与语义信息
...
<details>
  <summary>Details</summary>
Motivation: SAM模型在医学图像分割中面临边界模糊、解剖关系建模不足和缺乏不确定性量化等挑战，限制了其在临床场景中的应用。

Method: KG-SAM集成了三个关键组件：(i)医学知识图谱编码细粒度解剖关系，(ii)基于能量的条件随机场确保解剖一致性预测，(iii)不确定性感知融合模块提升临床可靠性。

Result: 在多中心医学数据集上的实验表明，KG-SAM在前列腺分割中达到82.69%的Dice分数，在腹部MRI和CT分割中分别达到78.05%和79.68%。

Conclusion: KG-SAM建立了一个稳健且可泛化的框架，显著推进了医学图像分割技术的发展。
...
TL;DR: UniVid是一个框架，通过微调预训练的视频扩散变换器来处理各种视觉任务，无需任务特定修改，使用视觉句子表示任务，展示了预训练视频生成模型作为可扩展统一视觉基础模型的潜力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉大模型需要跨模态和跨源的任务特定预训练，成本高昂且难以扩展到未见任务。预训练视频生成模型已捕获时间序列依赖，可能提供更统一和可扩展的替代方案。

Method: 提出UniVid框架，微调视频扩散变换器，将任务表示为视觉句子，其中上下文序列定义任务和输出模态。支持跨模态推理（图像和视频组合）和跨源任务（自然到标注数据）。

Result: 尽管仅使用自然视频数据训练，UniVid在跨模态和跨源设置下都表现出良好的泛化能力。理解和生成任务可以通过简单反转视觉句子顺序来切换。

Conclusion: 预训练视频生成模型有潜力作为可扩展和统一的视觉建模基础，为未来视觉AI系统提供更高效和灵活的解决方案。
...
TL;DR: 提出了一种保持空间完整性的token合并方法，使现代ViT架构能够兼容空间结构设计，实现高效加速而性能损失极小。


<details>
  <summary>Details</summary>
Motivation: 现代ViT架构采用空间设计（如窗口注意力、相对位置编码），但现有token减少方法无法保持这些架构依赖的空间结构，导致兼容性问题。

Method: 使用2D减少策略保持结构化token布局，空间感知合并算法维持相对位置，以及最大幅度维度token表示保留关键特征。

Result: 在SAM-H上实现1.25倍加速仅损失0.7% mIOU，在DeiT-B上1.15倍加速且ImageNet top-1准确率无下降。

Conclusion: 该方法简单有效，在空间和非空间架构上均达到SOTA性能，证明了空间完整性对现代ViT架构的重要性。
...
TL;DR: 提出了GASP-ICL框架，无需训练即可提升大视觉语言模型在多模态深度伪造检测中的性能，通过任务感知的知识注入和自适应评分器选择相关样本


<details>
  <summary>Details</summary>
Motivation: 多模态深度伪造检测面临捕捉细微伪造线索、解决跨模态不一致性和执行任务对齐检索的挑战，现有大视觉语言模型在此任务上效果有限

Method: GASP-ICL框架包含MDD适配的特征提取器检索对齐的图像-文本对，设计图结构泰勒自适应评分器（GSTAS）捕捉跨样本关系并传播查询对齐信号，选择语义对齐的任务相关演示样本

Result: 在四种伪造类型上的实验表明，GASP-ICL超越了强基线方法，在不进行LVLM微调的情况下实现了性能提升

Conclusion: GASP-ICL通过训练自由的框架有效增强了多模态深度伪造检测能力，为利用大视觉语言模型进行可靠的伪造检测提供了新思路
...
TL;DR: 提出ProDA框架，通过时空场景图和动态提示模块来解耦多动作场景中的特定动作，实现更精确的动作识别
...
Method: 使用时空场景图(SSG)和动态提示模块(DPM)指导图解析神经网络(GPNN)生成动作特定表示，设计视频自适应GPNN进行动态权重聚合

Result: 在视频动作识别任务中表现出优于现有最先进方法的性能

Conclusion: ProDA框架能有效解耦多动作场景中的指定动作，为复杂视频理解提供新思路
...
TL;DR: 提出了一种多模态数据集和DeHater模型，用于识别和消除数字内容中的仇恨元素，通过水印稳定扩散技术和注意力分析生成仇恨注意力图来模糊仇恨区域。


<details>
  <summary>Details</summary>
Motivation: 有害在线内容的增加扭曲了公共话语，对维护健康数字环境构成挑战，需要开发更有效的AI驱动仇恨检测方法。

Method: 结合水印稳定扩散技术和数字注意力分析模块(DAAM)，生成仇恨注意力图来识别图像中的仇恨元素，并开发DeHater视觉语言模型进行多模态去仇恨化任务。

Result: 创建了专门用于仇恨识别的多模态数据集，开发了能够精确识别和模糊图像中仇恨区域的AI模型，为社交媒体中更道德的AI应用设立了新标准。

Conclusion: 该方法为AI驱动的图像仇恨检测设立了新标准，有助于开发更符合伦理的社交媒体AI应用，并通过共享任务促进该领域的发展。
...
TL;DR: MIRG-RL是一个用于多图像推理和定位的统一框架，通过两阶段训练（监督微调和图像感知强化学习）解决LVLMs在跨图像推理能力不足的问题，在基准测试中达到SOTA性能（64.82%）。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在多图像任务中面临两个关键挑战：缺乏跨图像推理能力和不足的跨图像参考奖励建模。

Method: 提出两阶段训练范式：结合带注释轨迹的监督微调和图像感知强化学习优化；创新性地构建轨迹数据方法，集成对象级和图像级标注信息；设计具有对象和图像双重奖励函数的图像感知RL策略。

Result: 在跨图像推理任务上达到64.82%的准确率，超过之前最佳方法1%；代码和数据集已开源。

Conclusion: MIRG-RL框架有效解决了多图像推理和定位中的跨图像模糊问题，在多图像定位基准测试中实现了最先进的性能。
...
TL;DR: LongScape是一个混合框架，结合了扩散去噪和自回归生成，通过动作引导的分块机制实现稳定长时程视频生成


<details>
  <summary>Details</summary>
Motivation: 解决当前视频生成方法在长时程生成中的稳定性问题：扩散方法存在时间不一致和视觉漂移，自回归方法则牺牲视觉细节

Method: 提出混合框架，结合intra-chunk扩散去噪和inter-chunk自回归因果生成；核心创新是动作引导的变长分块机制和上下文感知专家混合框架

Result: 实验结果表明该方法在扩展rollouts中实现了稳定一致的长时程生成
...
TL;DR: 提出MoWM框架，通过混合世界模型融合潜在模型和像素空间模型的表示，用于具身动作规划，在CALVIN基准上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成世界模型依赖像素级重建导致视觉冗余，潜在世界模型忽略精细细节，需要克服这些限制以实现精确的动作规划。

Method: 使用潜在模型的运动感知表示作为高级先验，指导从像素空间模型提取细粒度视觉特征，突出信息丰富的视觉细节。

Result: 在CALVIN基准测试中达到最先进的任务成功率，并展示出优异的泛化能力。
...
TL;DR: 提出了DiTraj，一种针对DiT的无训练轨迹控制框架，通过前景-背景分离引导和3D位置嵌入改进实现高质量轨迹可控视频生成
...
Method: 1. 前景-背景分离引导：使用LLM将用户提示转换为前景和背景提示分别指导生成；2. STD-RoPE：通过修改前景token的位置嵌入消除跨帧空间差异，增强轨迹控制；3. 通过调节位置嵌入密度实现3D感知轨迹控制

Result: 大量实验表明，该方法在视频质量和轨迹可控性方面均优于先前方法

Conclusion: DiTraj是一种简单有效的训练免费框架，能够充分利用DiT优势实现高质量的轨迹可控视频生成
...
TL;DR: 该论文对多跳问答中的检索策略进行了全面评估，提出了一种结合密集嵌入和词汇重叠的混合检索方法，在HotpotQA数据集上相比余弦相似度基线取得了50%的精确匹配和47%的F1分数相对提升。


<details>
  <summary>Details</summary>
Motivation: Transformer模型在问答领域取得了进展，但多跳推理（需要结合多个段落证据）仍然困难，需要改进检索策略来支持多跳问答。

Method: 在检索增强生成框架下比较了余弦相似度、最大边际相关性和混合方法，并采用EfficientRAG管道进行查询优化，引入标记标记和迭代优化。

Result: 混合方法在HotpotQA数据集上显著优于基线方法，提高了实体召回率和证据互补性，但在处理干扰项和时间推理方面仍有局限。
...
TL;DR: 本文提出HDR-4DGS方法，解决动态场景下的高动态范围新视角合成问题，通过动态色调映射模块实现时间辐射一致性和空间准确的颜色转换。


<details>
  <summary>Details</summary>
Motivation: 现有HDR NVS方法主要针对静态场景，但现实世界经常包含动态元素（如移动物体、变化光照等），需要同时建模时间辐射变化和3D转换。

Method: 提出基于高斯泼溅的HDR-4DGS架构，包含创新的动态色调映射模块，根据时间维度上辐射分布的变化动态调整色调映射函数。

Result: HDR-4DGS在定量性能和视觉保真度上都超越了现有最先进方法，能够从任意视角和时间点生成逼真的HDR渲染。

Conclusion: 该方法成功解决了动态场景下的HDR新视角合成挑战，实现了时间辐射一致性和空间准确的颜色转换。
...
TL;DR: SRHand是一种从低分辨率图像重建高分辨率3D手部几何和纹理的方法，通过结合隐式图像表示和显式手部网格，解决了现有方法在低分辨率图像上泛化能力差的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖高分辨率多视角图像输入，在低分辨率图像上表现不佳，且多视角超分辨率方法不适用于可变形的手部重建。

Method: 提出几何感知隐式图像函数(GIIF)，通过联合优化隐式图像函数和显式3D手部形状，实现多视角和姿态一致的高分辨率图像上采样和细节重建。

Result: 在InterHand2.6M和Goliath数据集上，SRHand在定量和定性评估中显著优于现有图像上采样和3D手部重建方法。

Conclusion: SRHand能够从低分辨率输入重建包含皱纹、指甲等细节的高质量3D手部模型，在图像上采样和3D重建方面都表现出色。
...
TL;DR: 该论文指出当前AI生成图像检测研究过度关注"假"图像而忽视了"真"图像的定义和数据收集问题，认为需要重新思考"真实图像"的概念。


<details>
  <summary>Details</summary>
Motivation: 现代图像生成技术的普及引发了对其负面社会影响的担忧，但当前检测方法主要依赖过时的低分辨率"真实"图像数据集，忽视了智能手机时代图像采集技术的变革。

Method: 通过分析当前图像检测研究的局限性，指出智能手机图像形成算法与生成模型在神经网络架构上的相似性，质疑"假"图像检测这一目标的合理性。

Result: 发现当前检测方法严重依赖ImageNet等老旧数据集，而现代智能手机90%以上的照片都经过多传感器输入的神经网络算法处理，这与生成模型技术密切相关。

Conclusion: 需要重新定义"真实图像"的技术概念，建立新的基准数据集，并引发关于"假"图像检测目标是否合理的公开讨论。
...
TL;DR: Aes-R1是一个基于强化学习的多模态美学评估框架，通过AesCoT构建高质量思维链数据，并使用RAPO算法联合优化绝对评分和相对排序，显著提升美学评分和推理能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在图像美学评估方面具有潜力，但面临美学推理数据稀缺和美学判断主观性强的问题，难以生成准确且可解释的美学判断。

Method: 提出Aes-R1框架，包含AesCoT管道构建高质量思维链数据用于冷启动，然后使用RAPO强化学习算法联合优化绝对评分回归和相对排序，使模型能够生成结构化解释和评分。

Result: 实验表明Aes-R1将骨干模型的平均PLCC/SRCC分别提升47.9%/34.8%，超越同类规模的最先进基线，在有限监督和分布外场景下具有鲁棒泛化能力。

Conclusion: Aes-R1通过统一的框架增强多模态大语言模型的美学评分和推理能力，能够生成有依据的解释和可靠的评分，有效解决了美学评估的主观性和可解释性问题。
...
TL;DR: StableDub是一个基于Stable-Diffusion的视觉配音框架，通过唇部习惯建模和遮挡鲁棒合成解决了现有方法在唇部习惯捕捉和遮挡处理方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉配音方法存在两个关键缺陷：(1)仅依赖音频驱动无法捕捉说话者特定的唇部习惯；(2)传统盲修复方法在处理遮挡时容易产生视觉伪影。

Method: 提出唇部习惯调制机制联合建模音素视听同步和说话者特异性口面部动态，引入遮挡感知训练策略，并采用混合Mamba-Transformer架构提升训练效率。

Result: 实验结果表明StableDub在唇部习惯相似性、遮挡鲁棒性、音频-唇部同步、视频质量和分辨率一致性方面优于其他方法。

Conclusion: 该方法消除了对成本密集型先验的需求，在计算密集型扩散模型上表现出优越的训练效率，扩展了视觉配音方法的适用性。
...
TL;DR: Drag4D是一个交互式框架，用于在文本驱动的3D场景生成中集成物体运动控制，支持用户为3D物体定义轨迹并无缝集成到高质量3D背景中。


<details>
  <summary>Details</summary>
Motivation: 现有方法在3D场景生成中缺乏对物体运动的精确控制能力，用户无法直观地定义和操控3D物体在场景中的运动轨迹。

Method: 三阶段管道：1）使用2D高斯泼溅和全景图像增强文本到3D背景生成；2）3D复制粘贴方法提取物体网格并物理感知定位；3）基于部件增强的运动条件视频扩散模型实现时序动画。

Result: 通过各阶段评估验证了统一架构的有效性，展示了用户控制物体运动与高质量3D背景的协调对齐。

Conclusion: Drag4D框架成功实现了3D场景中物体运动的精确控制，为交互式3D内容生成提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到视频和图像到视频生成方法在控制运动时序方面存在局限，而音频提供了与视频运动对齐的时间线索，是控制视频时序的有前景的条件。

Method: 基于预训练视频骨干网络，提出两个关键组件：运动感知损失（强调高运动区域学习）和音频同步引导（使用视觉对齐的无音频层模型指导全模型）。

Result: 在AVSync15和The Greatest Hits数据集上的实验表明，Syncphony在同步精度和视觉质量方面均优于现有方法。

Conclusion: Syncphony证明了音频作为视频生成时序控制条件的有效性，通过创新的同步机制实现了更好的音视频同步效果。
...
TL;DR: 提出了一种基于语言引导的遥感变化检测模型LG-CD，通过多模态融合提升检测精度和鲁棒性


<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的方法主要关注单模态视觉信息，忽视了文本等多模态数据提供的丰富语义信息

Method: 使用SAM2视觉基础模型提取特征，设计文本融合注意力模块对齐视觉和文本信息，通过视觉语义融合解码器生成变化检测掩码

Result: 在三个数据集上的实验表明LG-CD持续优于最先进的变化检测方法
...
Method: 基于扩散模型，提出点云确定性拖拽（3D特征映射增强潜在空间布局控制）和拖拽-文本引导去噪（动态平衡拖拽和文本条件影响）。

Result: 实验表明该方法不仅实现高保真联合编辑，在单独文本或拖拽模式下也表现优异，超越了专门的单模式方法。

Conclusion: 建立了一个多功能、可推广的可控图像编辑解决方案，支持灵活的单模式或联合编辑操作。
...
TL;DR: 提出了一种sideload-CL-adaptation框架，利用无标注数据通过对比学习预训练CNN特征提取器，然后侧载到冻结的YOLO11n骨干网络中，提升北欧地区雪地环境下无人机车辆检测性能。


<details>
  <summary>Details</summary>
Motivation: 北欧地区无人机车辆检测面临强可见性挑战和雪覆盖引起的域偏移问题，标注数据昂贵但无标注数据获取成本低，需要利用无标注数据改进轻量级模型的检测性能。

Method: 通过对比学习在无标注数据上预训练CNN特征提取器，然后将学习到的表示侧载到冻结的YOLO11n骨干网络中进行微调，比较了多种融合方法和粒度。

Result: 在NVD数据集上，提出的sideload-CL-adaptation模型将检测性能提高了3.8%到9.5%（mAP50指标）。

Conclusion: 该方法有效利用无标注数据解决了北欧雪地环境下的车辆检测挑战，为轻量级模型在恶劣天气条件下的应用提供了可行方案。
...
TL;DR: IF-V2V是一种无需反转的轻量级视频编辑方法，通过矢量场校正和结构运动保持初始化，将现成的图像到视频模型适配用于视频编辑


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法要么需要特定模型的反转，要么需要大量优化，无法充分利用最新的图像到视频模型将图像编辑能力迁移到视频领域

Method: 提出矢量场校正与样本偏差、结构运动保持初始化、偏差缓存机制，在不进行反转的情况下将源视频信息融入去噪过程

Result: 评估显示该方法在编辑质量和一致性方面优于现有方法

Conclusion: IF-V2V提供了一种轻量级的即插即用解决方案，实现了视觉创意
...
TL;DR: SSLCounter是一个用于多视角计数（MVC）的自监督学习框架，通过神经体积渲染减少对大规模标注数据的依赖，仅使用70%训练数据就能达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 大多数MVC方法依赖完全监督学习，需要大量标注数据。本文旨在通过自监督学习缓解对标注数据的依赖问题。

Method: 使用神经体积渲染学习场景的隐式表示，通过可微分神经渲染重建连续几何形状和复杂视角相关的2D投影外观。

Result: 在多个MVC基准测试中达到最先进性能，仅使用70%训练数据就能获得竞争性结果，展示了优越的数据效率。

Conclusion: SSLCounter框架具有灵活性，可无缝集成到现有框架中，为MVC任务提供了高效的数据利用方案。
...
TL;DR: 提出了一个用于评估基础模型中物体中心空间推理能力的系统性基准测试，发现目标检测器和视觉语言模型在空间定位和关系推理之间存在稳定权衡


<details>
  <summary>Details</summary>
Motivation: 现有基准测试过于强调定位精度，而忽视了模型是否真正理解场景中物体的排列和关系，这对于有效的场景理解至关重要

Method: 使用受控合成数据集，评估了最先进的视觉模型和大型视觉语言模型在三个任务上的表现：空间定位、空间推理和下游检索任务

Result: 目标检测器能提供精确的边界框但关系推理能力有限，而视觉语言模型能提供粗略布局线索和流畅描述但在细粒度空间上下文上表现不佳

Conclusion: 研究揭示了定位能力与真正空间理解之间的差距，指出了社区需要开发具有空间感知能力的基础模型
...
<details>
  <summary>Details</summary>
Motivation: 视觉上下文学习通常过度依赖单个上下文图像对，这会导致预测结果存在偏差且不稳定。

Method: 提出基于补丁的k近邻视觉上下文学习（PANICL），通过平滑多个上下文对之间的分配分数来减少偏差，无需额外训练。

Result: 在多种视觉任务（如前景分割、目标检测、着色等）上均表现出优于基线方法的性能，并对领域偏移具有强鲁棒性。

Conclusion: PANICL是一个通用且有效的框架，能够显著提升视觉上下文学习的性能，并具有良好的泛化能力。
...
TL;DR: SingRef6D是一种轻量级的6D姿态估计方法，仅需单张RGB图像作为参考，无需深度传感器或多视图图像，通过改进深度预测和深度感知匹配技术，在挑战性表面和光照条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有6D姿态估计方法依赖深度传感器（对透明/反光表面效果差）或RGB图像（在低光/无纹理场景中匹配性能不佳），SingRef6D旨在解决这些局限性。

Method: 1. 基于Depth-Anything v2的token-scaler微调机制，优化深度预测；2. 深度感知匹配过程，将空间关系整合到LoFTR中，提升匹配鲁棒性。

Result: 在REAL275深度预测上比Depth-Anything v2提升14.41%；在REAL275、ClearPose和Toyota-Light数据集上，平均召回率提升6.1%。

Conclusion: SingRef6D在资源受限环境下仍保持鲁棒性，无需深度传感器或密集模板，显著提升了在挑战性材料和光照条件下的6D姿态估计性能。
...
TL;DR: DynaNav是一个动态视觉导航框架，通过基于场景复杂度的自适应特征和层选择，显著降低了计算开销，同时提高了导航性能。


<details>
  <summary>Details</summary>
Motivation: 现有基础模型（特别是transformer解码器）存在计算开销高、可解释性差的问题，限制了在资源受限场景下的部署。

Method: 提出DynaNav框架，采用可训练的硬特征选择器进行稀疏操作，并将特征选择集成到早退机制中，使用贝叶斯优化确定最优退出阈值。

Result: 在真实世界数据集和模拟环境中，相比ViNT，DynaNav实现了FLOPs减少2.26倍，推理时间降低42.3%，内存使用降低32.8%，同时在四个公共数据集上提升了导航性能。

Conclusion: DynaNav通过动态特征和层选择机制，有效解决了视觉导航中的计算效率和可解释性问题，为资源受限场景下的部署提供了可行方案。
...
TL;DR: 提出SemanticControl方法，解决ControlNet在处理语义不匹配的视觉条件时效果不佳的问题，通过自适应抑制冲突区域的视觉条件影响，增强文本引导。


<details>
  <summary>Details</summary>
Motivation: ControlNet依赖与文本提示精确对齐的视觉条件，但在实际应用中（特别是非常见或想象场景）往往缺乏完美匹配的视觉条件，导致生成效果不佳。

Method: 训练自由的方法，首先使用与视觉条件对齐的替代提示进行辅助去噪过程提取注意力掩码，然后在实际目标提示的去噪过程中利用这些掩码来抑制冲突区域的视觉条件影响。

Result: 实验结果表明，该方法在各种视觉条件（深度图、边缘图、人体骨架）下都能在语义不匹配条件下提升性能，优于现有基线方法。

Conclusion: SemanticControl能够有效利用语义相关但不完全对齐的视觉条件，为文本到图像生成提供了更灵活的控制能力。
...
TL;DR: 该论文提出了一种新的视觉情感评估方法——情感陈述判断任务，通过自动化流程构建情感中心陈述，系统评估MLLMs在情感理解方面的能力，发现其在情感解释和基于上下文的情感判断方面表现较强，但在理解感知主观性方面存在局限。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs情感感知评估方法存在忽视合理回答、情感分类有限、忽略上下文因素和人工标注繁琐等约束，导致零样本场景下研究结果不一致。

Method: 提出情感陈述判断任务和自动化流程，高效构建情感中心陈述，最小化人工努力，系统评估主流MLLMs的情感理解能力。

Result: MLLMs在情感解释和基于上下文的情感判断方面表现较强，但在理解感知主观性方面相对有限；与人类相比，即使最佳MLLMs如GPT4o也存在显著性能差距。

Conclusion: 通过开发基础评估框架和全面MLLM评估，该工作有助于推进MLLMs情感智能的发展，识别出未来改进的关键领域。
...
TL;DR: MultiCrafter是一个多主体图像生成框架，通过位置监督、专家混合架构和强化学习来解决现有方法中的属性泄漏问题，并实现与人类偏好对齐的高保真生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的多主体图像生成方法存在严重的属性泄漏问题，导致主体保真度下降，且无法与细致的人类偏好对齐。

Method: 1. 引入显式位置监督来分离不同主体的注意力区域；2. 采用专家混合架构增强模型处理不同场景的能力；3. 设计在线强化学习框架，包含多主体保真度评分机制和针对MoE架构的稳定训练策略。

Result: 实验验证表明，该框架显著提高了主体保真度，并更好地与人类偏好对齐。

Conclusion: MultiCrafter通过解决注意力纠缠和偏好对齐问题，在多主体图像生成任务上取得了显著改进。
...
TL;DR: PartSAM是首个基于大规模3D数据训练的可提示部件分割模型，采用类SAM的双分支编码器架构，通过模型辅助标注流程获得500万+3D形状-部件对，在多个基准测试中大幅超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有开放世界部件分割方法依赖2D基础模型导致的几何信息缺失、表面理解局限和泛化能力不足的问题。

Method: 采用triplane-based双分支编码器架构，构建空间结构化token进行可扩展的部件感知表示学习；通过模型辅助标注流程从在线资源中筛选500万+3D形状-部件对。

Result: 在多个基准测试中大幅超越现有最优方法，实现高度准确的部件识别和自动分解（包括表面和内部结构）。

Conclusion: PartSAM标志着向3D部件理解基础模型迈出的决定性一步，展现了新兴的开放世界能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有无参考图像质量评估模型难以准确评估真实场景下的对比度失真，需要开发更有效的对比度质量评估方法

Method: 定制化微调EfficientNet B0、ResNet18和MobileNetV2三种预训练架构，添加对比度感知回归头，使用孪生网络，在CID2013和CCID2014数据集上进行端到端训练

Result: 定制化EfficientNet B0模型在CCID2014上达到PLCC=0.9286和SRCC=0.9178，在CID2013上达到PLCC=0.9581和SRCC=0.9369，性能优于传统方法和深度基线

Conclusion: 对比度感知的轻量级预训练网络适配能够为实时和资源受限应用提供高性能、可扩展的无参考对比度质量评估解决方案
...
TL;DR: Geo-R1提出了一种基于推理的强化微调范式，用于解决遥感图像中少样本指代表达理解的问题，通过先生成可解释的推理链来分解指代表达，然后利用这些推理来定位目标对象。


<details>
  <summary>Details</summary>
Motivation: 遥感图像中的指代表达理解需要复杂的对象-上下文关系推理。现有的监督微调方法在数据稀缺场景下泛化能力差，难以应对少样本学习挑战。

Method: Geo-R1采用推理中心的强化微调范式，强制模型先生成明确的、可解释的推理链来分解指代表达，然后利用这些推理来定位目标对象，实现"先推理后行动"的过程。

Result: 在三个精心设计的少样本地理空间指代表达基准测试中，Geo-R1持续且显著优于监督微调基线，并展现出强大的跨数据集泛化能力。

Conclusion: Geo-R1通过推理优先的方法有效利用有限标注数据，提升泛化能力并提供可解释性，为解决遥感图像少样本指代表达理解问题提供了有效方案。
...
TL;DR: 该研究评估了视觉语言模型在医疗视觉问答中的临床附和行为，提出了一个医学附和数据集和轻量级缓解策略VIPER，以减少模型基于非证据的附和响应。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型越来越多地集成到临床工作流程中，但它们经常表现出附和行为，优先考虑与用户措辞、社交线索或感知权威的一致性，而不是基于证据的推理。

Method: 研究通过从PathVQA、SLAKE和VQA-RAD构建医学附和数据集，使用心理学动机的压力模板进行对抗性实验。提出VIPER缓解策略，过滤非证据内容并生成约束性证据优先答案。

Result: 实验发现各种VLM模型普遍易受附和影响，附和响应发生率与模型准确性或大小相关性较弱。模仿和专家提供的纠正是最有效的触发因素。VIPER框架平均减少了附和行为，优于基线方法。

Conclusion: 该研究的基准分析和缓解框架为医疗VLM在现实世界临床交互中的稳健部署奠定了基础，强调了基于证据的防御的必要性。
...
TL;DR: GLARIFY是一种利用时空注视信息增强视觉语言模型在现实场景中多模态查询能力的新方法，通过处理模糊的注视模式和复杂时空关系来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着智能眼镜的普及，用户注视数据被整合到视觉语言模型中，但注视数据存在模糊性问题：用户口头查询使用代词或省略上下文，以及人类注视模式具有噪声和复杂时空关系。现有方法仅考虑单张图像作为视觉输入，无法捕捉用户注意力的动态特性。

Method: 首先分析数百个包含注视模态的查询样本，证明用户注视模式的噪声特性；然后使用GPT-4o设计自动数据合成管道生成GLARIFY-Ambi数据集，包含专门处理噪声注视模式的思维链过程；最后设计热图模块将注视信息整合到先进视觉语言模型中，同时保留其预训练知识。

Result: 在保留测试集上的实验表明，GLARIFY显著优于基线方法。

Conclusion: 通过稳健地将视觉语言模型与人类注意力对齐，GLARIFY为实现可用且直观的视觉助手交互范式铺平了道路。
...
TL;DR: 本文研究了大型视觉语言模型的空间偏见问题，发现模型对图像中相同信息在不同位置的响应不一致，并提出平衡位置分配机制来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在空间变化下的鲁棒性不足，当相同关键视觉信息放置在不同位置时，模型会产生不一致的输出，这暴露了其空间语义理解的根本局限性。

Method: 通过精心设计的探测数据集分析空间偏见，发现问题的根源在于语言模型组件中位置嵌入的不平衡设计。提出了平衡位置分配机制，为所有图像标记分配相同的位置嵌入，促进更平衡的视觉信息整合。

Result: 实验表明，BaPA机制无需重新训练即可增强LVLMs的空间鲁棒性，结合轻量级微调还能进一步提升在各种多模态基准测试中的性能。信息流分析显示BaPA产生平衡的注意力，实现更全面的视觉理解。

Conclusion: BaPA是一种简单有效的机制，能够显著改善LVLMs的空间稳健性，促进更平衡的跨模态交互，为提升多模态模型的鲁棒性提供了有价值的解决方案。
...
TL;DR: 提出了一种从预训练扩散模型骨干网络中解耦视觉和语义特征的新方法，实现了类似语义对应的视觉对应，并提出了新的视觉语义匹配(VSM)度量来量化主题驱动图像生成中的视觉不一致性。


<details>
  <summary>Details</summary>
Motivation: 扩散模型骨干网络虽然编码了丰富的语义特征，但也必须包含支持图像合成能力的视觉特征。然而由于缺乏标注数据集，分离这些视觉特征具有挑战性。

Method: 引入自动化管道构建具有标注语义和视觉对应的图像对，设计对比架构来分离两种特征类型，利用解耦表示提出VSM度量。

Result: 经验结果表明，该方法在量化视觉不一致性方面优于CLIP、DINO等全局特征度量，同时支持不一致区域的空间定位。
...
TL;DR: WAVE是首个基于LLM的统一文本、音频和视频嵌入表示，通过分层特征融合和多任务训练实现任意模态间的跨模态检索和提示感知嵌入生成。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型的嵌入表示在动态模态（如音频和视频）中的应用尚未充分探索，需要开发统一的表示空间来支持任意模态间的交互。

Method: 采用分层特征融合策略和联合多模态多任务训练方法，构建统一的文本-音频-视频嵌入空间，支持任意模态间的检索和提示感知嵌入生成。

Result: 在MMEB-v2视频基准测试中达到新SOTA，在音频和视频到音频检索中表现优异，在多模态问答任务中显著优于现有嵌入模型。

Conclusion: WAVE通过统一的嵌入表示开启了广泛的跨模态应用可能性，消融研究验证了联合训练策略的有效性。
...
TL;DR: ERGO提出了一种两阶段"粗到细"的推理管道，通过先分析下采样图像识别任务相关区域，然后仅对相关区域进行全分辨率处理，从而在保持细粒度视觉细节的同时降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有的大型视觉语言模型在处理高分辨率图像时产生大量计算开销，而新兴的"图像思维"模型需要超越文本的视觉推理能力，这促使开发更高效的推理方法。

Method: 采用两阶段推理管道：第一阶段分析下采样图像识别相关区域；第二阶段仅对相关区域进行全分辨率处理。使用强化学习框架开发奖励组件，实现推理驱动的感知。

Result: ERGO在多个数据集上比原始模型和竞争方法获得更高准确率，同时效率更高。例如在V*基准测试中超越Qwen2.5-VL-7B 4.7分，仅使用23%的视觉token，实现3倍推理加速。
...
TL;DR: DualFocus是一个新的深度聚焦（DFF）框架，通过联合建模空间和焦距维度的焦点变化，利用焦点堆栈中的梯度模式来提高复杂场景中的深度估计精度。
...
Method: 提出变分公式，包含空间约束（利用不同焦点级别的梯度模式变化区分真实深度边缘和纹理伪影）和焦距约束（强制焦点概率符合物理焦点行为的单峰单调性）。

Result: 在四个公共数据集上的实验表明，DualFocus在深度精度和感知质量上均优于现有最先进方法。

Conclusion: DualFocu通过引入符合物理规律的归纳偏置，提高了DFF在挑战性区域的鲁棒性和准确性。
...
TL;DR: 提出了基于信息论的协作感知理论框架RDcomm，通过任务熵离散编码和互信息驱动的消息选择，在DAIR-V2X和OPV2V数据集上实现SOTA性能，同时将通信量减少108倍


<details>
  <summary>Details</summary>
Motivation: 填补协作感知领域在理论基础上的空白，为多智能体系统设计性能与通信量权衡的最优通信策略提供理论支撑

Method: 引入实用的率失真理论，提出RDcomm框架：1）任务熵离散编码，根据任务相关性分配码字长度；2）互信息驱动的消息选择，实现冗余最小化

Result: 在3D目标检测和BEV分割任务上，RDcomm在DAIR-V2X和OPV2V数据集上达到最先进精度，同时通信量减少高达108倍

Conclusion: 基于信息论的率失真理论为协作感知提供了坚实的理论基础，RDcomm框架验证了该理论的有效性，实现了性能与通信效率的最佳平衡
...
TL;DR: FailureAtlas是一个主动探索文生图模型失败模式的框架，通过结构化搜索发现最小失败诱导概念，解决了静态基准测试诊断能力有限的问题。


<details>
  <summary>Details</summary>
Motivation: 静态基准测试虽然为比较文生图模型提供了基础，但其被动设计诊断能力有限，难以全面揭示系统性失败或隔离根本原因。

Method: 将错误发现构建为结构化搜索问题，寻找最小失败诱导概念，并采用新颖的加速技术解决计算爆炸问题。

Result: 在Stable Diffusion模型上发现了超过247,000个先前未知的错误切片，并首次大规模证明了这些失败与训练数据稀缺的关联。

Conclusion: FailureAtlas通过提供原则化、可扩展的深度模型审计引擎，建立了诊断优先的新方法学，指导开发更鲁棒的生成式AI。
...
TL;DR: 提出了一种无需训练的自监督幻觉缓解方法，通过文本到图像投影放大幻觉信号，使用双锚点编辑解码器隐藏状态来减少多模态大语言模型的幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但容易产生与视觉证据不一致的幻觉内容，现有方法通常需要额外微调或手工先验，且往往在信息量和可扩展性上做出妥协。

Method: 引入幻觉放大机制：将描述通过文本到图像模型投影到视觉空间以揭示隐含幻觉信号作为负锚点，原始图像作为正锚点。利用这两个锚点，通过拉近表示与忠实语义的距离、推离幻觉方向来编辑解码器隐藏状态。

Result: 在多个基准测试中显著减少了对象、属性和关系层面的幻觉，同时基本保持了召回率和描述丰富度。例如，在CHAIR基准上使用LLaVA-v1.5-7B实现了超过5%的幻觉减少。在不同架构上的结果验证了强大的跨架构泛化能力。

Conclusion: 该方法无需人工先验或额外训练成本，在应用于无幻觉描述时几乎不产生副作用，展现了其鲁棒性和实用的即插即用适用性。
...
TL;DR: CoFFT是一种无需训练的方法，通过模拟人类视觉认知来增强视觉语言模型的视觉推理能力，通过迭代的预见-聚焦思维循环来减少无关信息干扰。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理复杂冗余的视觉输入时容易受到干扰，产生任务无关的推理过程甚至幻觉，因为无法在推理过程中精确发现和处理所需区域。

Method: CoFFT包含三个迭代阶段：1)多样化样本生成：生成多样化的推理样本来探索潜在推理路径；2)双重预见解码：基于视觉聚焦和推理进展严格评估样本，将最优样本的第一步加入推理过程；3)视觉聚焦调整：精确调整视觉焦点到对未来推理最有益的区域。

Result: 在多个基准测试中使用Qwen2.5-VL、InternVL-2.5和Llava-Next模型，性能一致提升了3.1-5.8%，且计算开销可控增加。

Conclusion: CoFFT通过创建推理指导视觉聚焦、视觉聚焦指导后续推理的相互依赖循环，有效提升了视觉语言模型的推理性能。
...
TL;DR: 提出了一种轻量级的多模态框架，结合Qwen2.5-VL-3B-Instruct模型和SmolAgent编排层，用于视频场景理解，在医疗机器人领域表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在时间推理、不确定性估计和结构化输出方面存在局限，无法满足医疗机器人对动态临床环境的安全需求。
...
Conclusion: 该框架在机器人辅助手术、患者监测和决策支持等医疗应用领域具有巨大潜力。
...
TL;DR: 本文提出了一个新的第一人称视角面对面教学视频数据集，用于分析教学互动中的程序步骤分割和对话状态分类任务，并评估了多模态大语言模型在这些任务上的表现。


<details>
  <summary>Details</summary>
Motivation: 面对面教学互动在教育支持和技能转移中至关重要，但这类场景在计算机视觉领域尚未得到系统研究，主要原因是缺乏合适的数据集和有限的分析技术。

Method: 构建了一个带有标注的第一人称视角面对面教学视频数据集，比较了多模态大语言模型与特定任务模型在程序步骤分割和对话状态分类任务上的性能。

Result: 实验表明，多模态大语言模型即使在没有任务特定微调的情况下，也优于专门的基线模型。

Conclusion: 多模态大语言模型在理解教学互动方面显示出巨大潜力，能够整合处理语言和非语言沟通信息，实现更全面的教学场景理解。
...
TL;DR: DAVIS是一个基于扩散模型的音频-视觉分离框架，通过生成式学习方法解决音频-视觉声源分离任务，超越了传统基于回归的方法。
...
Method: 利用去噪扩散概率模型(DDPM)和流匹配(FM)等生成建模范式，在专门的分离U-Net架构中直接从噪声分布合成分离的声音频谱图，同时以混合音频输入和相关视觉信息为条件。

Result: 在AVE和MUSIC数据集上的评估表明，DAVIS的两个变体在分离质量上都超越了现有方法。

Conclusion: 生成式框架对于解决音频-视觉声源分离任务具有显著效果，能够产生高质量的声音分离结果。
...
TL;DR: 提出SpecXNet双域网络架构，通过空间和频谱双分支联合学习，实现鲁棒的深度伪造检测，在跨数据集和未知伪造场景下达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: GAN和扩散模型生成的伪造内容越来越逼真，现有方法仅关注空间或频域特征，难以泛化到未见过的伪造类型

Method: 提出双域特征耦合器(DDFC)分解特征为局部空间分支和全局频谱分支，引入双傅里叶注意力(DFA)动态融合双域特征，基于改进的XceptionNet架构

Result: 在多个深度伪造基准测试中达到最先进准确率，特别是在跨数据集和未知伪造场景下表现优异，同时保持实时性

Conclusion: 空间-频谱统一学习对于鲁棒和可泛化的深度伪造检测具有显著效果
...
<details>
  <summary>Details</summary>
Motivation: 当前基于3D高斯泼溅的大规模重建模型虽然能高效实现高质量3D渲染，但无法生成资产的材质属性，这对于在不同光照环境下实现真实感渲染至关重要。

Method: 首先微调一个新的多视角材质扩散模型，条件输入为深度和法线图；然后利用生成的多视角PBR图像，探索与2D高斯泼溅对齐的高斯材质表示，建模PBR材质的每个通道；最后通过重建的点云获取PBR属性，实现动态重光照。

Result: 大量实验表明，该方法生成的材质不仅比基线方法更具视觉吸引力，还增强了材质建模能力，从而支持实际的下游渲染应用。

Conclusion: MGM框架成功解决了现有3D生成模型缺乏材质属性的问题，为3D内容创建提供了更完整的解决方案，支持动态重光照等实际应用。
...
TL;DR: 提出了一种基于多视图增强的新型自监督点云补全方法，通过引入Mamba架构来提升补全质量，在合成和真实数据集上达到了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 当前点云补全方法存在各种局限性：监督方法依赖真实标注且泛化能力差；无监督方法需要完整点云；弱监督方法需要多视角观测；现有自监督方法因自监督信号能力有限而效果不佳。

Method: 设计基于单视角部分点云的多视图增强自监督信号，首次将Mamba架构引入自监督点云补全任务，提升模型学习能力。

Result: 在合成和真实数据集上的实验表明，该方法达到了最先进的性能。

Conclusion: 提出的自监督点云补全方法通过多视图增强和Mamba架构的引入，有效克服了现有方法的局限性，实现了高质量的点云补全。
...
TL;DR: Refine-Control是一个半监督蒸馏框架，通过三层次知识融合损失和利用标记/未标记数据，在降低计算成本和延迟的同时保持高质量图像生成能力。


<details>
  <summary>Details</summary>
Motivation: 条件图像生成模型虽然效果显著，但资源需求高且标注数据稀缺，导致难以部署在边缘设备上，存在巨大成本和隐私问题。

Method: 提出Refine-Control半监督蒸馏框架，采用三层次知识融合损失来传输不同层次的知识，并利用标记和未标记数据进行半监督蒸馏。

Result: 实验表明Refine-Control显著降低了计算成本和延迟，同时保持了高保真生成能力和可控性。

Conclusion: 该框架有效解决了边缘设备部署条件图像生成模型的资源限制和隐私问题，具有实际应用价值。
...
TL;DR: 提出JGEKD方法，通过联合图熵知识蒸馏处理非独立同分布的3D点云分类任务，捕捉类别间相关性并提升模型鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统3D点云分类假设数据独立同分布，但实际类别间存在相关性，这种假设会破坏类别关联。

Method: 构建基于联合图熵的损失函数，使用联合图捕捉类别间隐藏关系，设计孪生结构和自知识蒸馏/教师知识蒸馏框架处理空间变换不变性。

Result: 在ScanObject、ModelNet40、ScanntV2_cls和ModelNet-C数据集上取得有竞争力的结果。

Conclusion: JGEKD方法能有效处理非独立同分布数据，通过知识蒸馏实现类别相关性转移，提升模型对数据损坏的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的神经程序合成方法仅将图表示为文本程序，未能捕捉节点图固有的视觉空间特性，而该特性正是使节点图对人类直观易用的关键。

Method: 结合大型多模态模型处理视觉和文本图表示，并采用约束树搜索推理算法确保语法有效性，同时高效导航程序空间。

Result: 实验结果表明，多模态程序合成方法在无条件图合成和条件图合成中都更高效，具有更高的视觉质量和保真度，优于仅文本的基线方法。

Conclusion: MultiMat在多模态程序合成方面建立了新的最先进性能，证明了结合视觉和文本表示的有效性。
...
TL;DR: 本文通过整合StyleGAN、DragGAN和PCA技术，提升了GAN生成图像潜在空间的效率和可控性。该方法在AFHQ数据集上验证，能够在保持图像质量的同时显著减少优化时间，并实现跨模型图像对齐和直观编辑。


<details>
  <summary>Details</summary>
Motivation: 现有的GAN潜在空间控制方法存在效率低、解释性差的问题，需要一种更高效和直观的潜在空间操控方式。

Method: 结合StyleGAN的结构化潜在空间、DragGAN的直观图像操控能力，以及PCA的降维和跨模型对齐功能，在AFHQ数据集上进行实验验证。

Result: 在DragGAN的W+潜在层引入PCA后，总优化时间显著减少，同时保持了良好的视觉质量，SSIM指标甚至有所提升，特别是在较浅的潜在空间（W+层=3）中效果更明显。

Conclusion: 该方法展示了高效且可解释的潜在空间控制在图像合成和编辑应用中的巨大潜力，为跨模型图像对齐和操控提供了可行方案。
...
TL;DR: MinerU2.5是一个1.2B参数的文档解析视觉语言模型，采用从粗到细的两阶段解析策略，在保持计算效率的同时实现最先进的识别精度。


<details>
  <summary>Details</summary>
Motivation: 解决高分辨率文档解析中的计算效率问题，同时保持对密集文本、复杂公式和表格等细粒度内容的识别能力。
...
Conclusion: MinerU2.5展示了强大的文档解析能力，证明了从粗到细的两阶段策略在平衡计算效率和识别精度方面的有效性。
...
TL;DR: 提出了Geo-CoT框架，通过多步骤推理过程解决遥感视觉语言模型在复杂分析任务中的局限性，实现可验证的遥感分析


<details>
  <summary>Details</summary>
Motivation: 当前遥感视觉语言模型在复杂分析任务中表现不佳，主要原因是端到端训练方式忽略了关键推理步骤，导致输出不可验证

Method: 采用两阶段对齐策略：首先通过监督微调建立基础认知架构，然后使用组奖励策略优化来优化模型的推理策略，确保事实正确性

Result: 开发的RSThinker模型在各项任务中显著优于最先进模型，能够同时输出最终答案和可验证的分析轨迹

Conclusion: Geo-CoT380k数据集和RSThinker模型的公开发布为地球观测领域从模糊感知转向结构化、可验证推理提供了具体路径
...
TL;DR: MUSplat是一个无需训练的框架，将2D开放词汇理解提升到3D高斯泼溅场景中，解决了现有方法依赖昂贵逐场景训练、单语义设计限制和跨视图语义不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 现有主流方法存在三个关键缺陷：(i)依赖昂贵的逐场景重新训练，无法实现即插即用；(ii)限制性的单语义设计无法表示复杂的多概念语义；(iii)易受跨视图语义不一致影响，破坏最终语义表示。

Method: 利用预训练的2D分割模型生成多粒度2D掩码并提升到3D，通过语义熵和几何不透明度优化初始对象组边界，使用视觉语言模型从最具代表性视角解释对象外观，提取稳健的文本特征以实现开放词汇查询。

Result: MUSplat将场景适应时间从数小时减少到几分钟，在开放词汇3D对象选择和语义分割基准任务中优于基于训练的框架，同时解决了它们的单语义限制。

Conclusion: 通过消除昂贵的逐场景训练过程，MUSplat实现了高效、多语义的3D场景理解，为开放词汇3D分析提供了实用解决方案。
...
TL;DR: UrbanFeel是一个综合性基准测试，用于评估多模态大语言模型在城市发展理解和主观环境感知方面的性能，包含14.3K个视觉问题，涵盖三个认知维度。


<details>
  <summary>Details</summary>
Motivation: 城市发展影响全球一半以上人口，但目前缺乏系统探索城市环境时间演变和主观感知的基准测试，现有MLLM在城市环境中的性能评估有限。

Method: 通过空间聚类、规则生成、模型辅助提示和人工标注的混合流程，从全球11个代表性城市收集多时相街景图像，生成高质量问答对。

Result: Gemini-2.5 Pro表现最佳，准确率接近人类专家水平，平均差距仅为1.5%。模型在场景理解任务中表现良好，部分模型在像素级变化检测中甚至超越人类，但在时间推理任务中性能显著下降。

Conclusion: MLLM在城市环境理解方面具有潜力，特别是在场景感知和主观评价维度可以达到人类水平，但在时间推理方面仍需改进。
...
TL;DR: 提出EXCL方法解决无源无监督领域自适应问题，通过双专家框架和检索增强交互优化，在四个基准数据集上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有SFUDA方法要么只利用源模型预测，要么微调大型多模态模型，但都忽略了目标数据的互补洞察和潜在结构

Method: EXCL包含双专家框架和检索增强交互优化管道。双专家框架将冻结的源域模型和预训练视觉语言模型置于同等地位，从未标记目标样本中挖掘共识知识。RAIN优化管道包含三个阶段：协同检索伪源和复杂目标样本、分别微调各专家、通过共享学习结果强制学习对象一致性

Result: 在四个基准数据集上的广泛实验表明，该方法达到了最先进的性能

Conclusion: EXCL方法有效解决了SFUDA问题，通过双专家协作学习和检索增强交互机制取得了优异性能
...
TL;DR: FlashEdit是一个基于扩散模型的实时图像编辑框架，通过三步创新实现了150倍的速度提升，能在0.2秒内完成高质量编辑。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散模型的文本引导图像编辑方法虽然质量出色，但存在严重的延迟问题，阻碍了实际应用。

Method: 采用三步创新：1）一步式反演和编辑（OSIE）管道绕过昂贵的迭代过程；2）背景屏蔽（BG-Shield）技术通过选择性修改编辑区域特征保证背景保留；3）稀疏化空间交叉注意力（SSCA）机制通过抑制语义泄漏实现精确局部编辑。

Result: 实验表明FlashEdit在保持优异背景一致性和结构完整性的同时，编辑速度达到0.2秒，比现有多步方法快150倍以上。

Conclusion: FlashEdit成功解决了扩散模型图像编辑的延迟问题，实现了实时高质量编辑，为实际应用铺平了道路。
...
TL;DR: 提出Neural-MedBench基准测试，用于评估神经病学中多模态临床推理能力，发现当前先进视觉语言模型在临床推理方面存在显著性能下降


<details>
  <summary>Details</summary>
Motivation: 现有医疗基准测试过于强调分类准确性，无法真实评估临床推理能力，需要专门设计推理密集型的评估工具

Method: 开发包含多序列MRI、电子健康记录和临床笔记的基准测试，包含鉴别诊断、病灶识别和原理生成三个核心任务家族，采用混合评分管道

Result: 对GPT-4o、Claude-4和MedGemma等先进模型的评估显示相比传统数据集性能显著下降，错误分析表明推理失败而非感知错误是主要问题

Conclusion: 需要双轴评估框架：广度导向的大数据集用于统计泛化，深度导向的紧凑基准如Neural-MedBench用于推理保真度
...
TL;DR: UniMapGen是一个用于大规模地图构建的生成式框架，通过离散序列表示车道线，支持多模态输入，并采用状态更新策略实现全局连续性。


<details>
  <summary>Details</summary>
Motivation: 传统大规模地图构建方法依赖昂贵的专用数据采集车辆和人工标注，现有卫星方法存在数据遮挡、过时以及感知方法向量化效率低等问题。

Method: 提出离散序列表示车道线，建立迭代生成策略；设计支持BEV、PV和文本提示的多模态输入架构；开发状态更新策略保证地图全局连续性。

Result: 在OpenSatMap数据集上达到state-of-the-art性能，能够推断被遮挡道路并预测数据集中缺失的道路标注。

Conclusion: UniMapGen通过生成式方法解决了传统地图构建的效率和精度问题，为大规模地图构建提供了创新解决方案。
...
TL;DR: GS-2M提出了一种基于3D高斯泼溅的统一解决方案，用于从多视角图像进行网格重建和材质分解，能够处理高反射表面并避免依赖外部模型先验。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将网格重建和材质分解作为独立任务处理，难以重建高反射表面，且需要依赖外部模型先验来增强分解结果。同时，现有联合解决方案往往使用复杂的神经网络组件，限制了其在大规模场景下的性能。

Method: 通过联合优化与渲染深度和法线质量相关的属性，提出基于多视角光度变化的新型粗糙度监督策略，结合精心设计的损失函数和优化过程，消除对神经网络组件的依赖。

Result: 该方法在广泛使用的数据集上验证了有效性，与最先进的表面重建方法相比，能够生成可比较的重建结果，提供三角形网格及其关联的材质组件。

Conclusion: GS-2M框架成功解决了网格重建和材质分解的联合优化问题，在保持几何细节的同时对反射表面具有鲁棒性，为下游任务提供了高质量的网格和材质分解结果。
...
TL;DR: 本文提出了一种面向任务的桌面场景生成新任务，并开发了MesaTask框架，通过空间推理链和DPO算法生成与任务描述相符的物理合理的桌面场景。


<details>
  <summary>Details</summary>
Motivation: 传统方法创建桌面场景依赖于耗时的手动布局设计或纯粹的随机布局，在真实性和任务对齐方面存在局限。机器人需要能够解释人类指令并执行操作任务，因此需要可用的任务相关桌面场景进行训练。

Method: 提出了空间推理链，将生成过程分解为对象推理、空间互相关联推理和最终3D布局的场景图构建。开发了基于LLM的MesaTask框架，并利用DPO算法进行增强。

Result: 实验表明，MesaTask在生成符合任务要求且具有真实布局的桌面场景方面优于基线方法。

Conclusion: MesaTask能够有效弥合任务指令与桌面场景之间的差距，生成物理合理且与任务描述良好对齐的桌面场景。
...
<details>
  <summary>Details</summary>
Motivation: 尽管强化学习在文档分析领域应用不广泛，但其增强的推理能力可能对下游任务有益。作者希望探索基于规则的强化学习在文档图像分类中的效果。

Method: 使用基于规则的强化学习方法，在文档图像分类任务上进行实验，重点关注其对分布外数据的泛化能力。

Result: 强化学习在三种分布外场景（分布外图像、未见类别和不同模态）中表现出比传统方法更好的泛化性能。

Conclusion: 基于规则的强化学习在文档分析任务中具有潜力，特别是在处理分布外数据时表现出优越的泛化能力。
...
TL;DR: SceneSplit是一种针对文本到视频模型的黑盒越狱方法，通过将有害叙述分解为多个独立良性的场景，利用场景组合约束引导模型生成有害视频。


<details>
  <summary>Details</summary>
Motivation: 随着文本到视频模型的快速发展，其安全风险日益凸显。虽然已有研究探索了LLMs、VLMs和文本到图像模型的漏洞，但T2V模型的安全性问题尚未得到充分研究，存在显著的安全空白。

Method: SceneSplit通过将有害叙述分解为多个独立良性的场景，利用场景序列组合约束生成空间，使模型在看似安全的场景组合下生成有害内容。该方法还结合迭代场景操作和安全模式重用策略库来提升攻击效果。

Result: 在11个安全类别上对T2V模型进行评估，SceneSplit在Luma Ray2、Hailuo和Veo2模型上分别达到77.2%、84.1%和78.2%的平均攻击成功率，显著优于现有基线方法。
...
TL;DR: HiGS是一种基于动量的采样技术，通过整合近期模型预测来提升扩散模型的图像生成质量和效率，无需额外计算或训练。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像生成方面取得了显著进展，但其输出在较少神经函数评估次数或较低引导尺度下仍显得不真实且缺乏细节。

Method: HiGS利用当前预测与过去预测加权平均之间的差异来引导采样过程，使生成图像更真实且细节更丰富。该方法无需额外计算，可无缝集成到现有扩散框架中。

Result: 实验表明HiGS在不同模型和架构下均能提升图像质量，使用预训练SiT模型在ImageNet 256×256无引导生成中仅需30步采样即达到1.61的FID新纪录。

Conclusion: HiGS是一种即插即用的标准扩散采样增强方法，可实现更快生成和更高保真度。
...
TL;DR: VeloxSeg提出了一种轻量级3D医学图像分割方法，通过双流CNN-Transformer架构和知识迁移技术，在保持高效性的同时显著提升了分割精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决轻量级3D医学图像分割中存在的"效率/鲁棒性冲突"问题，特别是在处理复杂解剖结构和异质模态时，传统轻量方法表示能力脆弱。

Method: 采用可部署的双流CNN-Transformer架构，包含配对窗口注意力(PWA)和Johnson-Lindenstrauss引理引导卷积(JLC)，结合"扫视-聚焦"原则和多模态交互，并通过空间解耦知识迁移(SDKT)注入纹理先验。

Result: 在多模态基准测试中，VeloxSeg实现了26%的Dice系数提升，GPU吞吐量提高11倍，CPU吞吐量提高48倍。

Conclusion: 该方法成功解决了轻量级3D医学图像分割的效率与鲁棒性平衡问题，为实际临床应用提供了高效可靠的解决方案。
...
TL;DR: NIFTY是一个结合扩散模型和传统基于块的纹理优化技术的混合框架，用于基于样例的纹理合成，无需神经网络训练即可生成高质量纹理。


<details>
  <summary>Details</summary>
Motivation: 解决基于样例的纹理合成问题，结合扩散模型和传统块匹配方法的优势，避免神经网络训练需求，同时改进传统方法中的初始化问题和视觉伪影。

Method: NIFTY是一个非参数流匹配模型，基于非局部块匹配构建，结合了卷积神经网络训练的扩散模型见解和经典基于块的纹理优化技术。

Result: 实验结果表明，该方法相比文献中的代表性方法具有更好的效果，能够有效生成高质量纹理。

Conclusion: NIFTY框架成功地将扩散模型与经典纹理合成技术相结合，在不需神经网络训练的情况下实现了高质量的纹理合成效果。
...
TL;DR: RAPID3是一个零更新的扩散变换器加速框架，通过三个轻量级策略头实现图像自适应加速，达到近3倍采样速度提升且保持生成质量


<details>
  <summary>Details</summary>
Motivation: 现有扩散变换器采样速度慢，训练免费加速器使用统一启发式策略导致质量损失，而动态神经网络需要高微调成本

Method: 使用三个策略头（步跳过、缓存重用、稀疏注意力）观察去噪状态并独立决策，通过GRPO在线训练策略参数，生成器保持冻结，同时使用对抗性判别器增强奖励信号

Result: 在包括Stable Diffusion 3和FLUX在内的最先进DiT骨干网络上，RAPID3实现了近3倍更快的采样速度，同时保持竞争力的生成质量

Conclusion: RAPID3框架成功解决了扩散变换器采样速度问题，通过图像自适应策略实现了高效的零更新加速
...
TL;DR: 该论文提出了一种多模态知识图构建方法，通过挖掘局部视觉特征与文本之间的关系，以及属性与广泛视觉上下文样本之间的关系，来增强行人属性识别（PAR）任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前的行人属性识别算法未能充分利用属性知识和上下文信息进行更准确的识别。虽然近期工作开始考虑使用属性文本作为额外输入来增强视觉和语义信息的关联，但这些方法仍处于初级阶段。

Method: 提出了一个有效的多模态知识图构建方法，充分考虑属性之间的关系以及属性与视觉标记之间的关系。引入了知识图引导的跨模态超图学习框架来增强标准行人属性识别框架。

Result: 在多个PAR基准数据集上的综合实验充分证明了所提出知识图对PAR任务的有效性，为知识引导的行人属性识别奠定了坚实基础。

Conclusion: 该方法通过构建多模态知识图，有效提升了行人属性识别的准确性和鲁棒性，源代码将在GitHub上开源。
...
TL;DR: CircuitSense是一个评估多模态大语言模型在电路理解方面能力的综合基准，包含8000多个问题，涵盖从组件级原理图到系统级框图的完整工程工作流程。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在自然图像任务上表现出色，但其从技术图表中提取数学模型的能力尚未被探索。工程设计需要视觉理解与数学推理的结合，而现有模型在这方面存在明显不足。

Method: 提出了一个分层合成生成管道，包括基于网格的原理图生成器和带有自动推导符号方程标签的框图生成器。通过三个层次评估模型能力：感知（组件识别和拓扑识别）、分析（符号推导和解析推理）和设计（电路合成）。

Result: 评估了六个最先进的多模态大语言模型，发现闭源模型在感知任务上准确率超过85%，但在符号推导和解析推理任务上表现低于19%，暴露出视觉解析与符号推理之间的关键差距。
...
TL;DR: HierLight-YOLO：基于YOLOv8的层次特征融合轻量化模型，用于无人机场景下的小目标实时检测，通过HEPAN特征融合和轻量化模块提升检测精度和效率。


<details>
  <summary>Details</summary>
Motivation: 解决无人机摄影场景中小目标检测（<32像素）和实时效率的双重挑战，YOLO系列检测器在小目标检测中假阴性率较高。

Method: 提出HierLight-YOLO模型，包含HEPAN多尺度特征融合方法、IRDCB和LDown轻量化模块，以及专门的小目标检测头。

Result: 在VisDrone2019基准测试中达到最先进性能，显著降低模型参数和计算复杂度。

Conclusion: HierLight-YOLO有效提升了无人机场景下小目标的实时检测能力，在保持检测性能的同时实现了模型轻量化。
...
TL;DR: 本研究探讨了利用GPT-4o大型多模态模型检测多模态虚假信息的潜力，提出了优化的提示工程、结构化分析框架和六项评估标准，并在多个数据集上进行了性能分析。


<details>
  <summary>Details</summary>
Motivation: 多模态环境（结合文本和图像）中虚假信息的扩散对数字平台构成了重大挑战，需要开发有效的检测和缓解方法。

Method: 采用GPT-4o模型，开发优化提示工程技巧，构建结构化多模态分析框架，定义六项评估标准，并在Gossipcop、Politifact等数据集上进行测试。

Result: 研究提供了GPT-4o在多模态虚假信息检测中的性能分析，揭示了模型的优势和局限性，并评估了分类的稳定性。

Conclusion: 该研究为自动化多模态虚假信息分析提供了一个稳健且可复现的方法论框架。
...
TL;DR: 利用预训练的GPT-4模型通过特定提示来分析图像，生成遮挡顺序预测，构建遮挡矩阵以辅助其他遮挡处理任务。
...
Method: 通过设计特定提示词，利用预训练的GPT-4模型分析输入图像并生成顺序预测，然后将响应解析构建遮挡矩阵。
...
TL;DR: 提出了一种基于显著边界增强的多焦点图像融合方法，通过梯度域模型和Tenengrad梯度检测来提升边界质量，在四个公开数据集上优于12种现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有多焦点图像融合方法在保持清晰焦点-散焦边界方面存在困难，容易导致模糊过渡和焦点细节丢失。
...
TL;DR: 提出了TDOA方法，通过聚类代理模型将动态输出场景转换为静态单输出场景，实现了对动态输出标签空间的文本对抗攻击。


<details>
  <summary>Details</summary>
Motivation: 现有文本对抗攻击方法主要针对静态场景，无法处理动态输出标签空间的问题。

Method: 使用聚类代理模型训练方法，将动态输出转换为静态单输出；提出最远标签目标攻击策略，选择偏离模型粗粒度标签的对抗向量。

Result: 在4个数据集和8个受害者模型上评估，单次查询最大攻击成功率50.81%；在静态场景中最大ASR达82.68%；在生成式任务中超越先前结果0.64 RDBLEU和0.62 RDchrF。

Conclusion: TDOA方法有效解决了动态输出场景的文本对抗攻击问题，在有限访问条件下对大型语言模型具有强破坏潜力。
...
TL;DR: 提出使用逻辑张量网络（LTNs）将医学背景知识编码到语义分割模型的损失函数中，以提升医学图像分割性能，特别是在训练数据稀缺的情况下。


<details>
  <summary>Details</summary>
Motivation: 当前的深度学习医学图像分割系统虽然取得了进展，但仍有改进空间。作者认为通过将医学知识融入模型可以提升性能。

Method: 使用逻辑张量网络（LTNs）将一阶逻辑规则编码为医学背景知识，并与SwinUNETR分割模型结合形成端到端框架。

Result: 在脑部MRI海马体分割任务中，LTNs方法相比基线提升了分割性能，特别是在训练数据稀缺时效果更明显。

Conclusion: 尽管还处于初步阶段，但神经符号方法具有足够的通用性，可以适应并应用于其他医学语义分割任务。
...
TL;DR: 提出VARE框架和S-VARE方法，解决自回归视觉模型的概念擦除安全问题，通过辅助视觉标记和过滤交叉熵损失实现精准概念擦除。


<details>
  <summary>Details</summary>
Motivation: 现有概念擦除技术主要针对扩散模型，无法适用于自回归视觉模型的下一个标记预测范式，存在安全隐患。

Method: VARE框架利用辅助视觉标记降低微调强度，S-VARE方法结合过滤交叉熵损失精确识别不安全视觉标记，并加入保持损失维持语义保真度。

Result: 实验表明该方法能实现精准概念擦除，同时保持生成质量，解决了语言漂移和多样性降低问题。

Conclusion: 该方法填补了自回归文本到图像生成的安全空白，为VAR模型提供了有效的安全防护方案。
...
TL;DR: RAU是一个基于视觉语言模型(VLM)的框架，用于医学图像中的参考式解剖结构理解、定位和分割，通过相对空间推理实现小解剖区域的精确定位和分割。


<details>
  <summary>Details</summary>
Motivation: 医学图像中的解剖理解需要大量专家标注数据，但这类数据稀缺。利用参考图像指导未标注目标图像的解释是一个有前景的解决方案。

Method: RAU框架首先通过参考图像和目标图像之间的相对空间推理训练VLM识别解剖区域，然后将VLM生成的空间线索与SAM2的细粒度分割能力相结合，实现像素级分割。

Result: 在四个数据集（两个分布内和两个分布外）上，RAU在相同内存设置下优于SAM2微调基线，产生更准确的分割和更可靠的定位，并展现出强大的泛化能力。

Conclusion: RAU是首个探索VLM在医学图像中参考式识别、定位和分割解剖结构能力的方法，其优异性能表明VLM驱动的方法在自动化临床工作流程中具有巨大潜力。
...
TL;DR: 本文提出了FreqDebias框架，通过频率去偏技术解决深度伪造检测器中的光谱偏差问题，显著提升了跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测器在有限训练数据下容易学习到频率域的光谱偏差，过度依赖特定频带，导致对未见伪造类型的泛化能力受限。

Method: 提出FreqDebias框架，包含两个互补策略：1) Fo-Mixup增强，动态多样化训练样本的频率特征；2) 双一致性正则化，通过类激活图的局部一致性和超球嵌入空间的全局一致性来缓解频率依赖。

Result: 大量实验表明，FreqDebias在跨域和域内设置下均显著优于现有最先进方法，有效提升了检测器的泛化性能。

Conclusion: FreqDebias成功解决了深度伪造检测中的光谱偏差问题，为提升检测器对未知伪造类型的鲁棒性提供了有效解决方案。
...
TL;DR: LucidFlux是一个无需图像描述的通用图像恢复框架，通过轻量级双分支调节器和时间步/层自适应调制策略，在大规模扩散Transformer上实现鲁棒的图像恢复。


<details>
  <summary>Details</summary>
Motivation: 解决当前通用图像恢复方法在未知混合退化条件下容易产生过度平滑、幻觉或语义漂移的问题，特别是避免对图像描述的依赖。

Method: 采用双分支调节器分别锚定几何结构和抑制伪影，设计时间步和层自适应调制策略进行分层控制，利用SigLIP特征实现无描述的语义对齐，并通过可扩展数据筛选管道进行监督。

Result: 在合成和真实场景基准测试中，LucidFlux始终优于强大的开源和商业基线，消融研究验证了各组件的重要性。

Conclusion: 对于大型扩散Transformer，控制何时、何地以及如何注入条件信号（而非增加参数或依赖文本提示）是实现鲁棒、无描述通用图像恢复的关键因素。
...
TL;DR: 该论文提出了一种改进多模态大语言模型可解释性的方法，通过多尺度解释聚合和激活排名相关性来增强视觉和文本模态的内部依赖关系理解。


<details>
  <summary>Details</summary>
Motivation: 现有可解释性研究主要关注跨模态归因，但忽略了模态内依赖关系。视觉模态中孤立图像块归因忽略了空间上下文，文本模态中依赖前文标记引入了虚假激活，这些干扰降低了归因的保真度。

Method: 提出了MSEA方法在视觉分支聚合多尺度输入的归因来动态调整感受野，以及ARC方法在文本分支通过top-k预测排名对齐来测量上下文标记的相关性并抑制虚假激活。

Result: 在先进MLLMs和基准数据集上的广泛实验表明，该方法一致优于现有可解释性方法，产生了更忠实和细粒度的模型行为解释。

Conclusion: 通过利用模态内交互来增强可解释性，能够更准确地揭示多模态大语言模型的内部决策机制。
...
TL;DR: U-MAN是一种改进的医学图像分割架构，通过引入渐进注意力引导特征融合和多尺度自适应KAN模块，解决了传统U-Net在细节保留和边界精度方面的不足。


<details>
  <summary>Details</summary>
Motivation: 传统U-Net架构存在两个主要问题：简单的跳跃连接忽略了编码器-解码器之间的语义差距，以及缺乏深层多尺度特征提取能力，导致医学图像分割中细节丢失和边界不准确。

Method: 提出U-MAN架构，包含两个核心模块：PAGF模块用注意力机制替代传统跳跃连接，实现编码器-解码器特征融合；MAN模块实现多尺度自适应特征处理，提升对不同尺寸目标的识别能力。

Result: 在BUSI、GLAS和CVC三个公开数据集上的实验表明，U-MAN在边界定义准确性和细节保留方面优于现有最先进方法。

Conclusion: U-MAN通过结合注意力机制和多尺度处理，有效提升了医学图像分割的性能，特别是在复杂解剖结构和病理区域的精细分割方面表现出色。
...
TL;DR: 提出γ-Quant方法，通过学习非线性量化在低比特深度下实现模式识别，在4比特原始数据上达到与12比特数据相当的性能


<details>
  <summary>Details</summary>
Motivation: 传统模式识别模型使用预处理数据（如RGB图像、归一化传感器数据），但在自动化分析场景中这种预处理可能不是最优的，且高比特数据在低带宽、能耗受限场景下效率低下

Method: 提出γ-Quant方法，通过学习任务特定的非线性量化，直接在原始低比特数据上进行模式识别，应用于原始图像目标检测和可穿戴设备的人类活动识别

Result: 实验表明，使用仅4比特的原始数据配合可学习量化，可以达到与12比特原始数据相当的性能水平

Conclusion: γ-Quant方法在低比特深度设置下有效，为低带宽、能耗受限的传感器应用提供了高效解决方案
...
TL;DR: 提出了一种自监督训练框架SSVIF，用于解决分割导向的可见光和红外图像融合方法需要大量标注数据的问题。


<details>
  <summary>Details</summary>
Motivation: 传统VIF方法只关注融合图像质量，而应用导向的VIF方法需要考虑下游任务性能但需要标注数据，数据获取成本高。

Method: 利用特征级融合分割和像素级融合分割之间的一致性，提出跨分割一致性自监督任务，设计两阶段训练策略和动态权重调整方法。

Result: 在公开数据集上的实验表明，SSVIF仅使用未标注的可见光-红外图像对训练，就能超越传统VIF方法，与有监督的分割导向方法相当。

Conclusion: SSVIF框架有效解决了分割导向VIF方法的标注数据依赖问题，为无监督图像融合提供了新思路。
...
TL;DR: 提出了一种名为Bézier Meets Diffusion的统一框架，用于跨域医学图像生成，通过Bézier曲线风格迁移和条件扩散模型生成高质量标注图像，提升跨域分割性能。


<details>
  <summary>Details</summary>
Motivation: 医学影像领域存在较大的域间差异，现有基于GAN的风格迁移方法难以处理高变异性区域，需要更有效的跨域图像生成方法。

Method: 首先使用Bézier曲线进行风格迁移减少域间差异，然后基于分割模型生成伪标签，训练条件扩散模型合成高质量目标域图像，并采用不确定性引导的分数匹配方法提升鲁棒性。

Result: 在公开数据集上的实验表明，该方法能生成逼真的标注图像，显著增强目标域数据并改善分割性能。

Conclusion: Bézier Meets Diffusion框架有效解决了医学影像跨域适应问题，通过结合Bézier风格迁移和扩散模型，实现了高质量的跨域图像生成和分割性能提升。
...
TL;DR: PSTTS是一种无需额外参数的即插即用模块，通过利用事件数据的时空分布特性来识别和丢弃冗余token，在保持任务精度的同时显著提升计算效率。


<details>
  <summary>Details</summary>
Motivation: 主流的事件时空表示学习方法将事件流转换为事件帧序列，但忽略了事件帧序列中存在的高空间稀疏性和帧间运动冗余，导致大量计算开销。现有的RGB视频token稀疏化方法依赖不可靠的中间token表示且未考虑事件噪声的影响，无法直接应用于事件数据。

Method: PSTTS包含两个阶段：空间token净化（通过评估每个事件帧内事件的时空一致性来丢弃噪声和非事件区域）和时间token选择（通过评估相邻事件帧之间的运动模式相似性来识别和移除冗余时间信息）。

Result: 在HARDVS、DailyDVS-200和SeACT数据集上，PSTTS将FLOPs降低了29-43.6%，FPS提高了21.6-41.3%，同时保持了任务精度。

Conclusion: PSTTS能够有效识别事件数据中的时空冗余信息，实现精度和效率的最佳平衡，为事件数据处理提供了高效解决方案。
...
TL;DR: 本文提出GCPO方法，通过识别关键图像token并实施有效的token-wise优化，提升基于RLVR的自回归视觉生成性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法对所有图像token进行统一优化，但不同token对RLVR训练的贡献差异未被探索。关键挑战在于如何识别AR生成中的关键token并实施有效的token-wise优化。

Method: 提出GCPO方法，从三个角度识别关键token：因果依赖关系、熵诱导的空间结构、RLVR关注的token多样性。引入动态token-wise优势权重来鼓励探索。

Result: 仅使用30%的图像token，GCPO在多个文本到图像基准测试中优于使用全部token的GRPO方法，适用于AR模型和统一多模态模型。

Conclusion: GCPO通过关注关键token实现了更有效的策略优化，在自回归视觉生成中表现出显著优势。
...
TL;DR: EAGLE是一个轻量级黑盒框架，用于解释多模态大语言模型中的自回归token生成，通过紧凑感知区域归因和模态感知分析来提高模型可解释性。


<details>
  <summary>Details</summary>
Motivation: 目前对多模态大语言模型生成token对视觉模态依赖程度的理解不足，限制了模型的可解释性和可靠性。

Method: EAGLE框架通过优化目标函数统一充分性和必要性评分，使用贪心搜索在稀疏化图像区域上进行归因，同时进行模态感知分析来解耦token依赖关系。

Result: 实验表明EAGLE在忠实性、定位能力和幻觉诊断方面优于现有方法，且GPU内存需求显著降低。

Conclusion: EAGLE框架有效提升了多模态大语言模型的可解释性，具有实用价值。
...
TL;DR: 本文首次系统评估了视觉语言模型（VLMs）的颜色命名能力，发现虽然VLMs在原型颜色上表现良好，但在非原型颜色上表现显著下降，且存在严重的跨语言训练不平衡问题。
...
Method: 使用957个颜色样本对五个代表性模型进行系统评估，采用经典颜色命名方法，并进行跨语言分析和消融研究。

Result: VLMs在原型颜色上准确率高，但在扩展的非原型颜色集上性能显著下降；识别出21个常见颜色术语；发现语言模型架构独立于视觉处理能力显著影响颜色命名。

Conclusion: VLMs的颜色命名能力存在局限性，特别是在非原型颜色和跨语言场景中，需要改进训练数据平衡和模型架构以更好地匹配人类颜色命名模式。
...
TL;DR: 提出EfficientDepth系统，结合Transformer架构和轻量级卷积解码器，通过双模态密度头和LPIPS损失函数实现高效的单目深度估计，在保持几何一致性和细节的同时显著降低计算资源需求。


<details>
  <summary>Details</summary>
Motivation: 现有单目深度估计方法在3D重建和视图合成中难以满足几何一致性、细节保留、鲁棒性和边缘设备效率等关键需求，需要开发更高效的解决方案。

Method: 使用Transformer架构与轻量级卷积解码器结合，引入双模态密度头；采用合成和真实图像混合训练数据，结合多阶段优化策略；引入基于LPIPS的损失函数提升细节质量。

Result: EfficientDepth在性能上达到或超越现有最先进模型，同时显著减少计算资源消耗。

Conclusion: 该方法成功解决了单目深度估计中的关键挑战，在保持高质量结果的同时实现了计算效率的提升，为实际应用提供了可行方案。
...
TL;DR: 该论文是一篇关于类别发现（CD）任务的综述性文章，系统回顾了该领域的研究进展，提出了分类框架，分析了不同方法，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 类别发现作为新兴的开放世界学习任务，近年来受到广泛关注但缺乏系统性的综述。本文旨在填补这一空白，为研究者提供全面的文献回顾和分析。

Method: 首先提出基于NCD和GCD两种基本设置及其衍生设置的分类框架；其次从表征学习、标签分配和类别数估计三个核心组件详细分析各种方法；最后通过基准测试提炼关键见解。

Result: 研究发现大规模预训练主干网络、层次化辅助线索和课程式训练对类别发现有益，但在标签分配设计、类别数估计和复杂多目标场景扩展方面仍面临挑战。

Conclusion: 本文系统梳理了类别发现领域的研究现状，提供了分类框架和基准测试，指出了当前挑战和未来研究方向，并建立了持续更新的文献库。
...
TL;DR: HyCoVAD是一个混合SSL-LLM模型，用于复杂视频异常检测，结合自监督学习和大型语言模型的优势，在ComplexVAD数据集上达到72.5%的帧级AUC，优于现有基线12.5%。


<details>
  <summary>Details</summary>
Motivation: 传统自监督学习方法难以理解复杂异常的语义含义，而大型语言模型虽然具有强大的上下文推理能力，但计算成本高且缺乏细粒度空间定位能力。

Method: 使用基于nnFormer的多任务SSL时序分析器识别可疑异常帧，然后通过LLM进行语义上下文验证，结合结构化规则推理来确认异常。

Result: 在ComplexVAD数据集上实现了72.5%的帧级AUC，比现有基线提高了12.5%，同时减少了LLM计算量。

Conclusion: HyCoVAD成功结合了SSL和LLM的优势，为复杂视频异常检测提供了有效解决方案，并发布了交互异常分类法、自适应阈值协议和代码以促进未来研究。
...
TL;DR: JanusVLN提出了一种新颖的视觉语言导航框架，采用双隐式神经记忆来建模空间几何和视觉语义记忆，解决了现有方法中空间信息丢失、计算冗余和内存膨胀的问题，在多个基准测试中实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的视觉语言导航方法依赖显式语义记忆（如文本认知地图或历史视觉帧），存在空间信息丢失、计算冗余和内存膨胀等问题，阻碍了高效导航。受人类导航中隐式场景表示的启发，作者提出了双隐式神经记忆框架。

Method: JanusVLN框架包含双隐式神经记忆：1）扩展MLLM以融入来自空间几何编码器的3D先验知识，增强基于RGB输入的空间推理能力；2）构建空间几何和视觉语义编码器的历史键值缓存作为双隐式记忆，仅保留初始和滑动窗口中的token KV，避免冗余计算，实现高效增量更新。

Result: 实验表明JanusVLN在20多个最新方法中达到SOTA性能：相比使用多种数据类型输入的方法，成功率提高10.5-35.5；相比使用更多RGB训练数据的方法，成功率提高3.6-10.8。

Conclusion: 提出的双隐式神经记忆作为一种新颖范式，为未来VLN研究探索了有前景的新方向，证明了隐式记忆在解决空间信息损失和计算效率问题上的有效性。
...
TL;DR: SpikeMatch是首个面向SNN的半监督学习框架，利用SNN的时间动态特性进行多样化伪标签生成，在多个标准基准测试中优于现有方法


<details>
  <summary>Details</summary>
Motivation: SNN具有生物合理性和高能效优势，但针对SNN的半监督学习方法相比ANN仍未被充分探索，需要填补这一研究空白

Method: 通过SNN的泄漏因子利用时间动态特性，在协同训练框架内进行多样化伪标签生成。利用单个SNN的多个预测一致性，从弱增强的无标签样本生成可靠伪标签来训练强增强样本

Result: 实验表明SpikeMatch在多个标准基准测试中优于现有适配到SNN骨干网络的SSL方法

Conclusion: SpikeMatch通过有效利用SNN的时间动态特性，在有限标签情况下成功捕捉判别性特征，为SNN的半监督学习提供了有效解决方案
...
TL;DR: 探索2D高斯飞溅（2DGS）作为视觉语言模型的替代视觉表示，解决RGB图像传输成本高和注意力序列长度爆炸的问题，实现语义强大且传输高效的边缘云学习。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言管道存在两个结构性低效问题：密集RGB图像从边缘设备传输到云端能耗高、成本大；基于补丁的标记化导致序列长度爆炸，增加注意力预算和上下文限制压力。

Method: 开发可扩展的2DGS管道，包括结构化初始化、亮度感知剪枝和批处理CUDA内核；通过轻量级飞溅感知输入干和感知器重采样器，将对比语言图像预训练（CLIP）适配到2DGS，仅训练约7%的总参数。

Result: 在大型DataComp子集上，GS编码器在压缩输入3到20倍（相对于像素）的同时，实现了有意义的零样本ImageNet-1K性能；虽然准确率目前落后于RGB编码器，但2DGS拟合速度提高90倍以上，GPU利用率达到约97%。

Conclusion: 2DGS被确立为可行的多模态基板，指明了架构瓶颈，并为实现语义强大且传输高效的边缘云学习表征开辟了道路。
...
TL;DR: LongLive是一个用于实时交互式长视频生成的帧级自回归框架，通过KV重缓存机制、流式长视频训练和短窗口注意力等技术，在保持高质量的同时实现高效生成。


<details>
  <summary>Details</summary>
Motivation: 解决长视频生成在效率和质量上的挑战，特别是支持交互式功能（如流式提示输入）的需求，确保在提示切换时的视觉一致性和语义连贯性。

Method: 采用因果帧级自回归设计，集成KV重缓存机制以平滑切换提示；使用流式长视频训练实现训练与推理对齐；结合短窗口注意力和帧级注意力汇聚点以保持长距离一致性。

Result: 在32 GPU天内将13亿参数的短片段模型微调至分钟级生成；在单个NVIDIA H100上实现20.7 FPS，支持最长240秒视频生成；INT8量化推理仅带来边际质量损失。

Conclusion: LongLive通过关键设计成功平衡了长视频生成的效率与质量，支持实时交互式应用，为动态内容创作提供了可行解决方案。
...
TL;DR: SPARK框架通过协同策略和奖励模型共同进化，解决了RLHF的高成本和RLVR的监督浪费问题，实现了无需外部奖励模型的高效强化学习。


<details>
  <summary>Details</summary>
Motivation: 传统RLHF依赖人类偏好导致高成本和奖励策略不匹配，而RLVR在每次更新后丢弃回放数据和正确性信号造成监督浪费。需要一种更高效、稳定的方法来同时优化策略和奖励模型。

Method: SPARK框架在RLVR基础上，回收利用回放数据和正确性信号，同时训练模型作为生成式奖励模型。采用点式奖励评分、成对比较和基于反思响应的评估等混合目标，让模型学会评估和改进自身响应。

Result: SPARK在多个LLM和LVLM模型上取得显著性能提升，SPARK-VL-7B在7个推理基准上平均提升9.7%，在2个奖励基准上提升12.1%，在8个通用基准上提升1.5%。

Conclusion: SPARK创建了正向协同进化反馈循环，消除了对单独奖励模型和人类偏好数据的需求，支持通过自我反思进行测试时扩展，具有鲁棒性和广泛泛化能力。
...
TL;DR: 提出了一种新的自监督卷积方法CCNeXt，在深度估计任务中超越了现有CNN和ViT模型，同时平衡了计算成本，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 深度估计在机器人、自动驾驶和AR等应用中至关重要，但这些场景通常受计算能力限制。由于难以获取可靠的地面真实深度数据，自监督技术成为解决方案。

Method: CCNeXt架构采用现代CNN特征提取器，在编码器中引入新颖的窗口化极线交叉注意力模块，并全面重新设计了深度估计解码器。

Result: 在KITTI Eigen Split测试数据上达到竞争性指标，比当前最佳模型快10.18倍，在KITTI Eigen Split改进地面真实数据和Driving Stereo数据集的所有指标中均达到SOTA。

Conclusion: CCNeXt在保持计算效率的同时，实现了卓越的深度估计性能，为计算受限场景提供了有效的解决方案。
...
TL;DR: UML-CoT框架使用统一建模语言（UML）生成结构化思维链和可执行动作计划，在杂乱房间清理任务中优于非结构化CoT方法


<details>
  <summary>Details</summary>
Motivation: 传统CoT方法依赖非结构化文本，在具身任务中可解释性和可执行性受限，现有结构化CoT方法建模能力有限，缺乏继承、行为抽象等构造，也没有标准化的顺序或条件规划语义

Method: 提出UML-CoT框架，利用UML类图捕捉组合对象语义，活动图建模过程控制流。采用三阶段训练管道，结合监督微调和GRPO策略优化，包括从仅答案数据中学习奖励
...
TL;DR: Labeling Copilot是首个用于计算机视觉的数据整理深度研究代理，通过多模态大语言模型驱动的中央协调器，实现校准发现、可控合成和共识标注三大核心功能，有效解决高质量领域特定数据集构建的瓶颈问题。
...
Method: 采用中央协调器代理，通过多步推理执行三大核心功能：校准发现从大型存储库中获取相关数据；可控合成生成稀有场景的新数据；共识标注通过非极大值抑制和投票机制协调多个基础模型产生准确标签。

Result: 在COCO数据集上平均每张图像产生14.2个候选提案，达到37.1%的mAP；在Open Images数据集上发现903个新边界框类别；校准发现在1000万样本规模上比替代方案计算效率高40倍。

Conclusion: 基于优化、可扩展工具的代理工作流程为工业规模数据集整理提供了稳健基础。
...
TL;DR: DIPSY提出了一种无需训练的方法，利用IP-Adapter进行图像到图像转换，仅使用少量样本生成高区分度的合成图像，在少样本图像分类任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决少样本图像分类中标记样本有限的问题，避免现有方法需要大量模型微调或外部信息源的依赖。

Method: 提出三个关键创新：扩展的无分类器引导方案、基于类别相似性的采样策略、无需微调的简单流程。

Result: 在十个基准数据集上的实验表明，该方法达到最先进或可比的性能，无需生成模型适应或外部工具。

Conclusion: 证明了利用正负引导的双图像提示在生成类别区分特征方面的有效性，特别适用于细粒度分类任务。
...
TL;DR: 本文提出SRDD方法，将自回归变换器VAR重新解释为离散扩散模型，建立了两者之间的理论桥梁，从而能够将扩散模型的优势直接引入VAR中，实现更快的收敛、更低的推理成本和更好的零样本重建效果。


<details>
  <summary>Details</summary>
Motivation: 自回归变换器在视觉生成中表现出色，甚至超越了基于扩散的模型。但作者发现当VAR配备马尔可夫注意力掩码时，在数学上等同于离散扩散，这一理论洞察为改进VAR提供了新的视角。

Method: 通过建立VAR与离散扩散模型之间的数学等价关系，将扩散模型的迭代优化等优势直接引入VAR架构，减少架构低效性。

Result: 在多个数据集上的实验表明，基于扩散视角的VAR在效率和生成质量方面都取得了持续提升，实现了更快的收敛速度、更低的推理成本和改进的零样本重建能力。

Conclusion: SRDD为自回归变换器和扩散模型之间建立了原则性桥梁，通过这一新视角能够显著提升视觉生成模型的性能，为未来的模型设计提供了重要启示。
...
TL;DR: HERMAN方法通过利用LLM生成层次化文本描述符，增强CLIP模型的语义空间，在类增量学习中实现更好的细粒度识别和缓解灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 现有的CLIP类增量学习方法使用简单模板（如"a photo of a [CLASS]"）忽略了视觉概念的层次性，且仅使用最后一层特征，忽视了早期层的层次信息。

Method: 提出HiErarchical Representation MAtchiNg (HERMAN)方法：1）利用LLM递归生成区分性文本描述符；2）将描述符与语义层次的不同级别匹配；3）基于任务需求自适应路由，实现精确区分。

Result: 在多个基准测试上的广泛实验表明，该方法持续达到最先进的性能。

Conclusion: HERMAN方法通过引入层次化表示匹配机制，有效提升了CLIP在类增量学习中的表现，特别是在处理细粒度识别和缓解灾难性遗忘方面表现出色。
...
TL;DR: DeeptraceReward是首个细粒度、时空感知的基准数据集，用于标注人类感知的AI生成视频伪迹，包含4.3K标注和3.3K高质量生成视频，训练的多模态奖励模型在伪迹识别、定位和解释方面优于GPT-5 34.7%。


<details>
  <summary>Details</summary>
Motivation: 现有研究忽视了人类能否检测生成视频中的深度伪造痕迹这一关键维度，即揭示视频为机器生成的时空视觉伪迹。

Method: 构建DeeptraceReward数据集，包含自然语言解释、边界框区域定位和精确时间戳标注，将标注整合为9类深度伪造痕迹，训练多模态语言模型作为奖励模型来模仿人类判断和定位。

Result: 7B奖励模型在伪迹识别、定位和解释任务上平均优于GPT-5 34.7%，发现存在一致的难度梯度：二元真伪分类最容易，细粒度伪迹检测中自然语言解释最简单，空间定位次之，时间标注最难。
...
TL;DR: 提出了CapRL框架，通过强化学习范式解决图像描述任务中主观评价的挑战，利用非视觉语言模型基于描述回答问题的准确性作为奖励信号，显著提升了描述质量。
...
Method: CapRL采用解耦的两阶段流程：LVLM生成描述，然后通过独立的无视觉LLM基于描述回答多选题的准确性作为客观奖励函数。这是首个将RLVR应用于主观图像描述任务的研究。

Result: 在CapRL-5M数据集上预训练后在12个基准测试中获得显著提升，在Prism框架评估中性能接近Qwen2.5-VL-72B，平均超出基线8.4%。

Conclusion: CapRL通过重新定义描述质量的标准，成功将强化学习应用于主观的图像描述任务，为LVLM预训练提供了可扩展的高质量数据生成方案。
...
TL;DR: 本文提出了一种无需训练或架构修改的引用分割方法RefAM，利用扩散变换器的特征和注意力机制，通过处理全局注意力汇和注意力重分配策略，在零样本设置下实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的引用分割方法通常需要微调或多个预训练模型组合，增加了训练成本和架构复杂性。而大规模生成扩散模型编码了丰富的语义信息，可作为通用特征提取器，但如何有效利用其注意力特征进行下游任务仍需探索。

Method: 提出RefAM框架，利用扩散变换器的交叉注意力图，识别并处理全局注意力汇（GAS），通过添加停用词进行注意力重分配，将背景激活分割成更小的簇，从而获得更清晰和定位更准确的热图。

Result: 在零样本引用图像和视频分割基准测试中，该方法始终优于先前的方法，无需微调或额外组件即可达到新的最先进水平。

Conclusion: 扩散变换器的注意力特征可直接用于下游任务，通过适当的注意力机制处理（如GAS处理和重分配策略），可以在不进行训练的情况下实现高效的引用分割。
...
### [160] [SGAligner++: Cross-Modal Language-Aided 3D Scene Graph Alignment](https://arxiv.org/abs/2509.20401)
*Binod Singh,Sayan Deb Sarkar,Iro Armeni*

Main category: cs.GR

TL;DR: SGAligner++是一个跨模态的3D场景图对齐框架，通过语言辅助和联合嵌入空间学习，在低重叠和噪声条件下实现准确的场景图对齐
...
Method: 采用轻量级单模态编码器和基于注意力的融合机制，学习统一的联合嵌入空间来处理跨模态场景图对齐

Result: 在真实世界数据集上的评估显示，SGAligner++在噪声真实世界重建任务上比现有最优方法性能提升高达40%，并实现跨模态泛化

Conclusion: SGAligner++通过语言辅助的跨模态框架有效解决了3D场景图对齐的挑战，为视觉定位、3D重建和导航等任务提供了更好的场景理解能力
...
### [161] [ControlHair: Physically-based Video Diffusion for Controllable Dynamic Hair Rendering](https://arxiv.org/abs/2509.21541)
*Weikai Lin,Haoxiang Li,Yuhao Zhu*

Main category: cs.GR

TL;DR: ControlHair是一个结合物理模拟器和条件视频扩散的混合框架，用于实现可控的动态头发渲染。


<details>
  <summary>Details</summary>
Motivation: 现有的视频扩散模型虽然能生成高质量视频，但缺乏对头发动态的细粒度控制。头发模拟和渲染面临复杂动态、多样化材质属性和复杂光发交互的挑战。

Method: 采用三阶段流水线：1）通过物理模拟器将物理参数编码为每帧几何；2）提取每帧控制信号；3）将控制信号输入视频扩散模型生成具有所需头发动态的视频。

Result: 在精心策划的10K视频数据集上训练，ControlHair在文本和姿态条件基线方法上表现更优，能够精确控制头发动态。

Conclusion: ControlHair是首个用于可控动态的物理感知视频扩散框架，支持动态发型试戴、子弹时间效果和电影图形等应用。
...
### [162] [Rigidity-Aware 3D Gaussian Deformation from a Single Image](https://arxiv.org/abs/2509.22222)
*Jinhyeok Kim,Jaehun Bang,Seunghyun Seo,Kyungdon Joo*

Main category: cs.GR

TL;DR: DeformSplat是一个从单张图像重建物体变形的框架，通过3D高斯变形和刚性区域分割技术，解决了现有方法依赖多视角视频的限制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常需要多视角视频来恢复变形，这在受限场景下适用性有限。本文旨在开发仅从单张图像就能有效重建物体变形的技术。

Method: 提出了两个关键技术：1) 高斯到像素匹配，弥合3D高斯表示和2D像素观测之间的领域差距；2) 刚性部分分割（初始化和细化），明确识别刚性区域以保持几何一致性。

Result: 大量实验表明，该方法显著优于现有方法，并能自然扩展到帧插值和交互式物体操作等应用。

Conclusion: DeformSplat框架通过结合高斯到像素匹配和刚性部分分割，能够从单张图像重建一致的变形，为受限场景下的变形重建提供了有效解决方案。
...
### [163] [Aerial Path Planning for Urban Geometry and Texture Co-Capture](https://arxiv.org/abs/2509.22227)
*Weidan Xiong,Bochuan Zeng,Ziyu Hu,Jianwei Guo,Ke Xie,Hui Huang*

Main category: cs.GR

TL;DR: 提出一种创新的航拍路径规划框架，用于在有限先验知识下协同采集城市几何结构和高质量纹理图像，解决现有技术忽视纹理质量导致视觉伪影的问题。


<details>
  <summary>Details</summary>
Motivation: 当前城市场景重建技术往往忽略纹理质量的重要性，导致纹理模型出现明显视觉伪影。需要在站点访问前有限先验知识下，解决城市几何和纹理协同采集问题。

Method: 1) 生成高质量垂直俯视和水平平面视图；2) 提出多目标优化策略最大化纹理保真度、几何精度并最小化航拍成本；3) 开发考虑纹理一致性的序列路径规划算法。

Result: 在大规模合成和真实城市数据集上的广泛实验表明，该方法能有效生成适合几何和纹理并发重建的图像集，以低成本创建逼真的纹理场景代理。

Conclusion: 该框架能够以低运营成本实现几何结构和纹理的协同重建，为城市建模提供实用解决方案。
...
### [164] [Visual Multi-Agent System: Mitigating Hallucination Snowballing via Visual Flow](https://arxiv.org/abs/2509.21789)
...
TL;DR: 本文提出ViF方法，通过视觉流和注意力重分配来缓解多智能体视觉语言模型中的幻觉雪球效应问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体视觉语言系统存在幻觉雪球效应问题，即单个智能体的幻觉会在后续智能体中被放大，这源于对文本流的过度依赖导致视觉注意力分配减少。

Method: 通过注意力分析识别出具有单峰注意力峰值的关键视觉标记，提出ViF方法：使用选定的视觉中继标记构建视觉流，并应用注意力重分配来增强这一模式。

Result: 实验结果表明，该方法显著减少了幻觉雪球效应，在基于四种常见MAS结构和十个基础模型的八个基准测试中一致提升了性能。
...
### [165] [Cross-Modal Retrieval with Cauchy-Schwarz Divergence](https://arxiv.org/abs/2509.21339)
*Jiahao Zhang,Wenzhe Yin,Shujian Yu*

Main category: cs.IR

TL;DR: 提出了基于Cauchy-Schwarz散度的跨模态检索方法，通过广义CS散度实现多模态直接对齐，无需逐对比较


<details>
  <summary>Details</summary>
Motivation: 现有跨模态检索方法存在数值不稳定、超参数敏感、无法捕捉分布完整结构等问题，需要更稳健的对齐技术

Method: 引入Cauchy-Schwarz散度作为超参数无关的度量，并提出基于Hölder不等式的广义CS散度，通过双向循环比较实现多模态直接对齐

Result: 在六个基准数据集上的实验表明，该方法在双模态和三模态检索任务中均有效

Conclusion: CS/GCS散度为跨模态检索提供了稳定高效的分布对齐解决方案，支持多模态直接对齐
...
### [166] [Language-in-the-Loop Culvert Inspection on the Erie Canal](https://arxiv.org/abs/2509.21370)
*Yashom Dighe,Yash Turkar,Karthik Dantu*

Main category: cs.RO

TL;DR: VISION是一个端到端的语言辅助自主系统，结合视觉语言模型和约束视角规划，用于自主检查运河涵洞，无需领域特定微调即可生成专家对齐的高分辨率检测报告。


<details>
  <summary>Details</summary>
Motivation: 传统人工检查运河涵洞存在年龄、几何形状、照明差、天气和难以接近等挑战，需要开发自动化解决方案。

Method: 使用网络级视觉语言模型接收简短提示生成开放词汇的感兴趣区域提案，融合立体深度恢复尺度，约束感知规划器命令重新定位以捕获目标特写。

Result: 在伊利运河涵洞的四足机器人部署中，初始ROI提案与专家达成61.4%一致，最终重新成像后评估达到80%一致。

Conclusion: VISION能够将初步假设转化为基于实地的专家对齐发现，证明语言辅助自主系统在涵洞检查中的有效性。
...
### [167] [RoboView-Bias: Benchmarking Visual Bias in Embodied Agents for Robotic Manipulation](https://arxiv.org/abs/2509.22356)
*Enguang Liu,Siyuan Liang,Liming Lu,Xiyu Zeng,Xiaochun Cao,Aishan Liu,Shuchao Pang*

Main category: cs.RO

TL;DR: 提出了RoboView-Bias基准，这是首个专门用于系统量化机器人操作中视觉偏见的基准，通过因子隔离原则创建了2,127个任务实例。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注泛化性和鲁棒性，缺乏对视觉偏见的系统量化，这限制了对感知如何影响决策稳定性的深入理解。

Method: 利用结构化变体生成框架和感知公平性验证协议，创建了能够稳健测量单个视觉因素及其相互作用引起的偏见的任务实例。

Result: 评估了三个代表性具身智能体发现：(i)所有智能体都表现出显著的视觉偏见，相机视角是最关键因素；(ii)智能体在高度饱和颜色上获得最高成功率；(iii)视觉偏见表现出强烈的不对称耦合。基于语义接地层的缓解策略将视觉偏见降低了约54.5%。
...
### [168] [MINT-RVAE: Multi-Cues Intention Prediction of Human-Robot Interaction using Human Pose and Emotion Information from RGB-only Camera Data](https://arxiv.org/abs/2509.22573)
*Farida Mohsen,Ali Safa*

Main category: cs.RO

TL;DR: 提出了一种仅使用RGB输入的创新管道，用于预测人类交互意图，具有帧级精度，通过MINT-RVAE合成序列生成方法解决类别不平衡问题，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多依赖多模态输入（如RGB-D）来检测人类交互意图，但需要更快的机器人响应和仅使用RGB输入的更高效解决方案。

Method: 开发了MINT-RVAE合成序列生成方法，结合新的损失函数和训练策略，专门处理HRI数据集中的类别不平衡问题，实现帧级精确的意图预测。

Result: 在AUROC指标上达到0.95，优于之前工作的0.90-0.912，同时仅需RGB输入并支持精确的帧起始预测。

Conclusion: 该方法为高效人机交互提供了仅需RGB输入的解决方案，公开了带有帧级标注的新数据集以支持未来研究。
...
### [169] [WoW: Towards a World omniscient World model Through Embodied Interaction](https://arxiv.org/abs/2509.22642)
...
TL;DR: 本文提出WoW模型，通过机器人交互数据训练世界模型，证明大规模真实交互是发展AI物理直觉的关键。


<details>
  <summary>Details</summary>
Motivation: 当前视频模型（如Sora）依赖被动观察，难以理解物理因果关系。人类通过主动交互发展物理直觉，因此需要基于真实世界交互来构建AI的物理直觉。

Method: 训练14B参数的WoW生成世界模型，使用200万条机器人交互轨迹数据。通过SOPHIA机制（视觉语言模型代理评估DiT生成输出并迭代优化语言指令）约束物理真实性，配合逆动力学模型将计划转化为可执行动作。

Result: 在WoWBench基准测试中达到SOTA性能，在物理因果关系、碰撞动力学和物体持久性方面表现优异。模型表现出概率性物理理解和物理幻觉特性。

Conclusion: 大规模真实世界交互是发展AI物理直觉的基石，为构建具有物理常识的AI系统提供了新路径。
...
### [170] [Pixel Motion Diffusion is What We Need for Robot Control](https://arxiv.org/abs/2509.22652)
*E-Ro Nguyen,Yichi Zhang,Kanchana Ranasinghe,Xiang Li,Michael S. Ryoo*

Main category: cs.RO

TL;DR: DAWN是一个基于扩散模型的统一框架，用于语言条件下的机器人操作，通过结构化的像素运动表示连接高层运动意图和低层机器人动作。


<details>
  <summary>Details</summary>
Motivation: 解决机器人控制中高层运动意图与低层动作之间的连接问题，利用扩散模型的可解释性和可训练性构建端到端系统。

Method: 将高层和低层控制器都建模为扩散过程，使用结构化的像素运动表示作为中间抽象层，实现完全可训练的端到端系统。

Result: 在CALVIN基准测试中达到最先进水平，在MetaWorld上验证有效性，并展示了从仿真到真实世界的可靠迁移能力。

Conclusion: 扩散模型与运动中心表示的结合为可扩展和鲁棒的机器人学习提供了强大的基线方法。
...
### [171] [See, Point, Fly: A Learning-Free VLM Framework for Universal Unmanned Aerial Navigation](https://arxiv.org/abs/2509.22653)
...
TL;DR: SPF是一个基于视觉语言模型的无需训练空中视觉语言导航框架，将动作预测视为2D空间定位任务，通过分解指令为2D路径点并转换为3D位移向量来实现无人机导航。


<details>
  <summary>Details</summary>
Motivation: 现有基于VLM的方法将动作预测视为文本生成任务，但作者认为空中视觉语言导航更适合作为2D空间定位任务来处理，以提高导航精度和效率。

Method: SPF利用VLM将模糊语言指令分解为输入图像上的迭代2D路径点标注，结合预测的飞行距离，将2D路径点转换为3D位移向量作为无人机动作指令，并采用闭环控制方式实现动态目标跟踪。

Result: 在DRL仿真基准测试中，SPF比之前最佳方法绝对提升了63%，在真实世界评估中也大幅优于强基线，且对不同VLM具有良好泛化能力。

Conclusion: SPF通过将AVLN重新定义为空间定位任务，实现了无需训练的高效导航框架，在仿真和真实环境中都表现出色，验证了设计选择的有效性。
...
### [172] [Clinical Uncertainty Impacts Machine Learning Evaluations](https://arxiv.org/abs/2509.22242)
...
TL;DR: 论文主张在机器学习评估中考虑标注不确定性，使用概率性指标来直接处理分布，以更好地反映临床数据的真实性能。


<details>
  <summary>Details</summary>
Motivation: 临床数据集的标注很少是确定的，因为标注者之间存在分歧且不同案例的置信度不一致。传统的聚合方法（如多数投票）掩盖了这种变异性。

Method: 提出使用概率性指标来显式处理标注不确定性，这些指标可以独立于标注生成过程（如简单计数、主观置信度评级或概率响应模型）应用，并且计算轻量，具有线性时间实现。
...
### [173] [Guidance Watermarking for Diffusion Models](https://arxiv.org/abs/2509.22126)
*Enoal Gesny,Eva Giboulot,Teddy Furon,Vivien Chappelier*

Main category: cs.CR

TL;DR: 本文提出了一种基于梯度引导的扩散模型水印方法，可将后处理水印方案转换为生成过程中的水印嵌入，提高鲁棒性且不损失生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型水印方法多为后处理方案，缺乏在生成过程中直接嵌入水印的能力。需要一种能够将任意后处理水印方案转换为生成过程中水印嵌入的方法。

Method: 通过使用现成水印解码器计算的梯度来引导扩散过程，梯度计算包含多种图像增强技术，无需重新训练或微调即可提高对未见过攻击的鲁棒性。

Result: 该方法在不同扩散模型和检测器上验证有效，水印引导不会显著改变给定种子和提示生成的图像，保持了生成的多样性和质量。

Conclusion: 该水印引导方法与修改变分自编码器的水印技术互补，为扩散模型提供了一种有效的生成过程中水印嵌入方案。
...
### [174] [Patch-Based Diffusion for Data-Efficient, Radiologist-Preferred MRI Reconstruction](https://arxiv.org/abs/2509.21531)
*Rohan Sanda,Asad Aali,Andrew Johnston,Eduardo Reis,Jonathan Singh,Gordon Wetzstein,Sara Fridovich-Keil*

Main category: eess.IV

TL;DR: 提出了一种基于patch的扩散模型PaDIS-MRI，用于加速MRI重建，在数据稀缺的临床环境中表现出色，诊断质量优于现有方法。


<details>
  <summary>Details</summary>
Motivation: MRI采集时间长、成本高且易受运动伪影影响。现有扩散模型需要大量训练数据，而临床环境中数据稀缺。patch-based扩散模型在小数据集上有效，但尚未在MRI中证明临床价值。

Method: 扩展Patch-based Diffusion Inverse Solver (PaDIS) 到复数多线圈MRI重建，在FastMRI脑数据集上进行7倍欠采样重建，并与FastMRI-EDM等基线方法比较。

Result: PaDIS-MRI仅需25个k空间图像的小数据集训练，在图像质量指标(PSNR、SSIM、NRMSE)、像素级不确定性、跨对比度泛化性和对严重欠采样的鲁棒性方面均优于FastMRI-EDM。在盲法研究中，91.7%的病例被放射科医生评为诊断质量更优。

Conclusion: patch-based扩散先验在数据稀缺的临床环境中具有高保真MRI重建潜力，特别是在诊断置信度至关重要的场景中。
...
### [175] [Comparative Analysis of GAN and Diffusion for MRI-to-CT translation](https://arxiv.org/abs/2509.22049)
*Emily Honey,Anders Helbo,Jens Petersen*

Main category: eess.IV

TL;DR: 比较两种MRI到CT转换架构的性能：条件生成对抗网络（cGAN）和条件去噪扩散概率模型（cDDPM），发现多通道条件输入和cDDPM架构效果更好。


<details>
  <summary>Details</summary>
Motivation: CT图像在治疗和诊断中至关重要，但当CT难以获取时，需要从MRI生成合成CT（sCT）的方法，因此需要确定最有效的MRI到CT转换策略。

Method: 使用Pix2Pix代表cGAN，Palette代表cDDPM，将3D转换问题分解为2D横向切片序列，研究单通道和多通道条件输入的影响，并使用新的切片相似性度量SIMOS评估性能。

Result: 分析显示，MRI到CT生成模型受益于多通道条件输入和cDDPM架构的使用。

Conclusion: cDDPM架构在MRI到CT转换任务中表现优于cGAN，多通道条件输入能提升生成质量。
...
### [176] [COMPASS: Robust Feature Conformal Prediction for Medical Segmentation Metrics](https://arxiv.org/abs/2509.22240)
*Matt Y. Cheung,Ashok Veeraraghavan,Guha Balakrishnan*

Main category: eess.IV

TL;DR: COMPASS是一个用于医学图像分割的实用框架，通过利用深度神经网络的特征表示空间，为下游指标（如器官大小）生成高效的符合性预测区间，比传统方法产生更紧凑的区间。


<details>
  <summary>Details</summary>
Motivation: 在临床应用中，分割模型的实用性通常基于下游指标（如器官大小）的准确性，而非像素级分割精度。因此，对这些指标进行不确定性量化对决策至关重要。传统的符合性预测方法将分割到指标的复杂非线性管道视为黑盒处理，效率低下。

Method: COMPASS框架直接在模型的表示空间中进行校准，通过沿着对目标指标最敏感的低维子空间扰动中间特征。该方法利用深度神经网络的归纳偏置，估计重要性权重，并能在协变量偏移下恢复目标覆盖。

Result: 在四个医学图像分割任务上的实验表明，COMPASS产生的区间比传统符合性预测基线显著更紧凑。同时，该方法在协变量偏移下也能保持有效的目标覆盖。

Conclusion: COMPASS为医学图像分割提供了实用的基于指标的不确定性量化方法，通过利用学习到的内部特征实现了更高效的不确定性估计。
...
### [177] [Deep Learning-Based Cross-Anatomy CT Synthesis Using Adapted nnResU-Net with Anatomical Feature Prioritized Loss](https://arxiv.org/abs/2509.22394)
*Javier Sequeiro González,Arthur Longuefosse,Miguel Díaz Benito,Álvaro García Martín,Fabien Baldacci*

Main category: eess.IV

TL;DR: 本文提出了一种基于3D nnUNet的MR到CT和CBCT到CT图像转换方法，使用多中心SynthRAD2025数据集，涵盖头颈、胸部和腹部区域。通过标准UNet和残差UNet两种网络配置，结合新提出的解剖特征优先损失函数，显著提升了临床相关结构的重建质量。


<details>
  <summary>Details</summary>
Motivation: 解决医学图像跨模态合成中的挑战，特别是在保持解剖结构保真度方面。传统的基于强度的损失函数在保留临床相关结构方面存在不足，需要开发能够优先重建重要解剖特征的损失函数。

Method: 采用基于nnUNet的3D patch训练方法，使用标准UNet和残差UNet两种网络架构。引入解剖特征优先损失函数，通过预训练的紧凑分割网络提取多层特征进行比较。采用针对不同解剖区域定制的3D patch训练策略，使用zscore归一化和特定的训练策略。

Result: 残差网络结合AFP损失在解剖保真度方面表现最佳，特别是在MR到CT的骨骼结构和CBCT到CT的病灶重建方面。L1损失网络在强度指标上略有优势。残差网络+AFP组合产生了更清晰的图像重建效果。

Conclusion: 该方法为医学图像跨模态合成提供了稳定解决方案，证明了将自动nnUNet流程与残差学习和解剖引导特征损失相结合的有效性。
...
### [178] [VoiceAssistant-Eval: Benchmarking AI Assistants across Listening, Speaking, and Viewing](https://arxiv.org/abs/2509.22651)
*Ke Wang,Houxing Ren,Zimu Lu,Mingjie Zhan,Hongsheng Li*

Main category: cs.CL

TL;DR: VoiceAssistant-Eval是一个综合性基准测试，用于评估AI助手在听、说、看三个维度的能力，包含10,497个示例和13个任务类别。评估结果显示专有模型并不总是优于开源模型，大多数模型在说话任务上表现良好但在音频理解方面落后，且设计良好的小模型可以媲美大模型。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试无法充分评估语音优先AI助手的能力范围，需要开发更全面的评估框架来指导下一代AI助手的发展。

Method: 构建VoiceAssistant-Eval基准测试，包含10,497个经过筛选的示例，涵盖13个任务类别（自然声音、音乐、对话等听力任务；多轮对话、角色扮演等说话任务；异质图像等视觉任务）。评估了21个开源模型和GPT-4o-Audio，测量响应内容质量、语音质量和一致性。

Result: 专有模型并非普遍优于开源模型；大多数模型擅长说话任务但音频理解能力较弱；设计良好的小模型（如7B的Step-Audio-2-mini）在听力准确率上超过大模型（如32B的LLaMA-Omni2）。当前模型在多模态输入和角色扮演语音模仿任务上仍有困难，鲁棒性和安全对齐存在显著差距。

Conclusion: VoiceAssistant-Eval识别了当前AI助手的性能差距，为评估和指导下一代AI助手的发展建立了严格框架。代码和数据将公开发布。
...
### [179] [Perception-Consistency Multimodal Large Language Models Reasoning via Caption-Regularized Policy Optimization](https://arxiv.org/abs/2509.21854)
*Songjun Tu,Qichao Zhang,Jingbo Sun,Yuqian Fu,Linjing Li,Xiangyuan Lan,Dongmei Jiang,Yaowei Wang,Dongbin Zhao*

Main category: cs.MM

TL;DR: 提出Caption-Regularized Policy Optimization (CapPO)框架，通过caption一致性正则化和KL加权优势估计来解决多模态大语言模型中视觉感知错误传播问题，显著提升数学和通用推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型存在视觉感知错误在推理链中传播的关键漏洞，现有强化学习微调方法未能有效解决视觉基础与后续推理过程之间的错位问题。

Method: CapPO框架包含两个关键机制：(1) caption一致性正则化，最小化基于原始图像和基于caption的响应之间的差异；(2) KL加权优势估计，自适应缩放强化信号以增强感知一致性轨迹。

Result: 在5个数学相关和5个通用推理基准测试中，CapPO相比基础模型Qwen2.5-VL-7B在数学任务上提升6.0%准确率，在通用推理任务上提升2.4%。消融研究证实各组件有效性，错误分析显示CapPO显著减少感知相关错误。

Conclusion: CapPO提供了一个简单而有效的框架来改善多模态推理，通过显式强制感知一致性来提升模型性能。
...
### [180] [Are Hallucinations Bad Estimations?](https://arxiv.org/abs/2509.21473)
*Hude Liu,Jerry Yao-Chieh Hu,Jennifer Yuntong Zhang,Zhao Song,Han Liu*

Main category: cs.LG

TL;DR: 本文形式化地将生成模型中的幻觉定义为无法将估计与任何合理原因联系起来。研究表明，即使是最优的损失最小化估计器也会产生幻觉，并提供了通用数据分布下幻觉率的高概率下界。


<details>
  <summary>Details</summary>
Motivation: 重新审视生成模型中的幻觉问题，将其解释为损失最小化与人类可接受输出之间的结构不匹配，从而揭示幻觉是由校准错误引起的估计误差。

Method: 通过理论分析证明最优估计器仍会产生幻觉，并给出幻觉率的下界。在硬币聚合、开放式问答和文生图任务上进行实验验证。

Result: 实验结果表明，即使在最优估计条件下，生成模型仍会产生幻觉，支持了理论分析。

Conclusion: 幻觉是生成模型固有的结构性问题，源于损失最小化目标与人类期望输出之间的不匹配，需要通过更好的校准方法来解决。
...
### [181] [VISION: Prompting Ocean Vertical Velocity Reconstruction from Incomplete Observations](https://arxiv.org/abs/2509.21477)
*Yuan Gao,Hao Wu,Qingsong Wen,Kun Wang,Xian Wu,Xiaomeng Huang*

Main category: cs.LG

TL;DR: 提出KD48高分辨率海洋动力学基准和VISION重建范式，通过动态提示解决海洋观测数据缺失问题，显著优于现有方法


<details>
  <summary>Details</summary>
Motivation: 解决海洋科学中长期缺乏标准化基准的问题，以及从表面观测重建海洋动力学（如垂直速度场）的挑战

Method: 构建KD48基准数据集，提出VISION重建范式，采用动态提示和状态条件提示模块，结合几何和尺度感知算子

Result: 在KD48基准上的广泛实验表明，VISION显著优于最先进模型，在极端数据缺失情况下表现出强泛化能力

Conclusion: 通过提供高质量基准和稳健模型，为数据不确定性下的海洋科学研究建立了坚实基础
...
### [182] [SlimDiff: Training-Free, Activation-Guided Hands-free Slimming of Diffusion Models](https://arxiv.org/abs/2509.21498)
*Arani Roy,Shristi Das Biswas,Kaushik Roy*

Main category: cs.LG

TL;DR: SlimDiff是一种无需梯度、无需微调的无训练扩散模型压缩框架，通过激活感知的谱近似方法实现结构和参数压缩，在保持生成质量的同时显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然生成性能优秀，但计算成本高昂，现有压缩技术依赖微调或重训练来恢复性能，存在瓶颈。

Method: 将扩散模型压缩重构为谱近似任务，利用激活协方差定义低秩子空间，在固定压缩预算下进行动态剪枝，采用模块化分解和自适应稀疏分配策略。

Result: 相比基线模型实现35%加速和约1亿参数减少，生成质量与未压缩模型相当，仅需约500个校准样本，比现有方法减少70倍以上。

Conclusion: 这是首个无需训练、基于闭式解的激活引导扩散模型结构压缩方法，兼具理论清晰性和实际效率。
...
### [183] [DistillKac: Few-Step Image Generation via Damped Wave Equations](https://arxiv.org/abs/2509.21513)
*Weiqiao Han,Chenlin Meng,Christopher D. Manning,Stefano Ermon*

Main category: cs.LG

TL;DR: DistillKac是一种快速图像生成器，利用阻尼波方程及其随机Kac表示以有限速度移动概率质量。相比扩散模型，它实现了有限速度传输和全局有界动能，并通过端点蒸馏技术实现高质量样本生成。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的反向时间速度可能变得僵硬并隐含允许无限传播速度，而Kac动力学强制有限速度传输并产生全局有界动能，这提供了更好的数值稳定性。

Method: 使用阻尼波方程和随机Kac表示；引入速度空间的分类器自由引导；提出端点蒸馏方法，训练学生模型在长间隔内匹配冻结的教师模型。

Result: 实验表明DistillKac能够以极少的函数评估次数生成高质量样本，同时保持有限速度概率流的数值稳定性优势。

Conclusion: DistillKac通过有限速度概率流和端点蒸馏技术，在保持数值稳定性的同时实现了高效的图像生成。
...
### [184] [TRiCo: Triadic Game-Theoretic Co-Training for Robust Semi-Supervised Learning](https://arxiv.org/abs/2509.21526)
*Hongyang He,Xinyuan Song,Yangfan He,Zeyu Zhang,Yanshu Li,Haochen You,Lifan Sun,Wenqiao Zhang*

Main category: cs.LG

TL;DR: TRiCo是一个新的三元博弈论协同训练框架，通过整合教师、两个学生和一个对抗生成器来重新构建半监督学习结构，在低标签场景下实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有半监督学习框架的关键限制，如静态视图交互、不可靠的伪标签以及缺乏困难样本建模，提供一个原则性且可推广的解决方案。

Method: 将SSL建模为三个角色的结构化交互：两个在互补表示上训练的学生分类器、一个通过验证反馈自适应调节伪标签选择和损失平衡的元学习教师，以及一个扰动嵌入以发现决策边界弱点的非参数生成器。使用互信息而非置信度选择伪标签，并将三元交互形式化为Stackelberg博弈。

Result: 在CIFAR-10、SVHN、STL-10和ImageNet上的广泛实验表明，TRiCo在低标签场景下始终实现最先进性能，同时保持架构无关性并与冻结视觉骨干网络兼容。

Conclusion: TRiCo通过创新的三元博弈框架，为半监督学习提供了一个更稳健、更有效的训练范式，解决了现有方法的多个关键问题。
...
### [185] [Closing the Oracle Gap: Increment Vector Transformation for Class Incremental Learning](https://arxiv.org/abs/2509.21898)
*Zihuan Qiu,Yi Xu,Fanman Meng,Runtong Zhang,Linfeng Xu,Qingbo Wu,Hongliang Li*

Main category: cs.LG

TL;DR: 该论文提出了一种名为IVT（增量向量变换）的新型即插即用框架，通过保持与先前任务最优解的线性连接性来缓解类增量学习中的灾难性遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 当前类增量学习方法与能够访问全部历史数据的oracle模型相比仍存在显著性能差距。受线性模式连接性启发，研究发现oracle解通常与先前任务的最优解保持低损失的线性连接。

Method: 提出IVT框架，通过Fisher信息矩阵高效近似变换，将模型参数周期性转移到能够保持与先前任务最优解线性连接性的变换解上，而不是直接遵循CIL更新。

Result: 在CIFAR-100、FGVCAircraft等数据集上的实验表明，IVT能显著提升强基线方法的性能。如在CIFAR-100上，IVT将PASS基线的最终准确率提升+5.12%，遗忘减少2.54%。

Conclusion: IVT框架通过保持线性连接性有效缓解了灾难性遗忘，适用于无示例和有示例场景，且与各种初始化策略兼容，能够持续增强强CIL基线的性能。
...
### [186] [Enriching Knowledge Distillation with Intra-Class Contrastive Learning](https://arxiv.org/abs/2509.22053)
*Hua Yuan,Ning Xu,Xin Geng,Yong Rui*

Main category: cs.LG

TL;DR: 提出在教师模型训练中引入类内对比损失和边界损失，以增强软标签中的类内多样性信息，提高知识蒸馏效果


<details>
  <summary>Details</summary>
Motivation: 现有知识蒸馏方法中教师模型主要依赖真实标签作为目标，没有考虑同一类别内的多样化表示，限制了学生模型的泛化能力

Method: 在教师训练过程中引入类内对比损失来丰富软标签中的类内信息，同时加入边界损失来解决训练不稳定和收敛慢的问题

Result: 理论分析证明类内对比损失能够丰富类内多样性，实验结果表明该方法有效提升了知识蒸馏效果

Conclusion: 通过结合类内对比学习和边界损失的教师训练策略，能够生成包含更丰富类内信息的软标签，从而提升学生模型的泛化性能
...
### [187] [Adaptive Dual-Mode Distillation with Incentive Schemes for Scalable, Heterogeneous Federated Learning on Non-IID Data](https://arxiv.org/abs/2509.22507)
*Zahid Iqbal*

Main category: cs.LG

TL;DR: 该论文针对联邦学习中的统计异构性、模型异构性和激励机制挑战，提出了DL-SH、DL-MH和I-DL-MH三种方法，显著提升了模型精度并降低了通信成本。


<details>
  <summary>Details</summary>
Motivation: 联邦学习面临三个关键挑战：1）客户端的模型异构性（不同业务需求和计算资源导致无法训练相同模型）；2）统计异构性（非IID数据）导致全局模型性能下降；3）需要有效的激励机制促进客户端参与。

Method: 提出了三种方法：DL-SH（处理统计异构性，实现高效、隐私保护和通信高效的联邦学习）、DL-MH（处理完全异构模型并解决统计差异）、I-DL-MH（在DL-MH基础上加入激励机制促进客户端参与）。

Result: 在多种实验设置下（不同模型架构、IID和非IID数据分布、多个数据集）的测试表明，所提方法相比现有技术显著提升了精度并降低了通信成本。DL-SH在非IID条件下将全局模型精度提升了153%，I-DL-MH提升了225%。

Conclusion: 该论文提出的方法有效解决了联邦学习中的统计异构性和模型异构性问题，同时通过激励机制促进了客户端参与，在保持隐私保护和通信效率的同时显著提升了模型性能。
...
### [188] [JointDiff: Bridging Continuous and Discrete in Multi-Agent Trajectory Generation](https://arxiv.org/abs/2509.22522)
*Guillem Capellera,Luis Ferraz,Antonio Rubio,Alexandre Alahi,Antonio Agudo*

Main category: cs.LG

TL;DR: JointDiff是一个新的扩散框架，统一生成连续时空数据和同步离散事件，在体育领域展示了同时建模多智能体轨迹和关键控球事件的有效性。
...
Result: 在足球和美式足球数据集上实现了最先进的性能，创建了带有文本描述的统一体育基准。

Conclusion: 联合建模对于构建真实可控的交互系统生成模型至关重要。
...
### [189] [Activation Function Design Sustains Plasticity in Continual Learning](https://arxiv.org/abs/2509.22562)
*Lute Lillo,Nick Cheney*

Main category: cs.LG

TL;DR: 在持续学习中，激活函数的选择是缓解塑性损失的关键因素，本文提出了两种新的非线性激活函数（Smooth-Leaky和Randomized Smooth-Leaky）来维持模型的可塑性。


<details>
  <summary>Details</summary>
Motivation: 传统i.i.d.训练中激活函数差异不大，但在持续学习中，模型会逐渐失去适应能力（塑性损失），而激活函数在这一失效模式中的作用尚未充分探索。

Method: 基于负分支形状和饱和行为的属性分析，引入两种新的非线性激活函数，并在监督类增量基准和强化学习非平稳环境中进行评估，同时提供简单的压力测试协议和诊断方法。

Result: 实验表明，精心设计的激活函数可以在持续学习中维持可塑性，无需额外容量或任务特定调优。

Conclusion: 激活函数设计是持续学习中维持可塑性的轻量级、领域通用的有效方法。
...
### [190] [Learn the Ropes, Then Trust the Wins: Self-imitation with Progressive Exploration for Agentic Reinforcement Learning](https://arxiv.org/abs/2509.22601)
...
TL;DR: SPEAR是一种基于课程学习的自模仿学习框架，用于训练智能LLM代理，通过渐进式探索-利用平衡解决RL训练中的探索挑战。


<details>
  <summary>Details</summary>
Motivation: 传统RL方法通过策略熵最大化来促进探索，但这种方法容易导致训练不稳定，因为多轮分布漂移会导致熵崩溃或发散失控。

Method: SPEAR扩展了基本的自模仿学习框架，通过课程管理探索过程：早期使用内在奖励促进技能级探索，后期加强自模仿以利用成功模式进行动作级探索。同时引入优势重校准和正则化来稳定训练。

Result: 该方法能够在保持适当熵范围的前提下，实现渐进式的探索-利用平衡，避免熵崩溃或无界熵增长，提高训练稳定性。

Conclusion: SPEAR通过课程化自模仿学习有效解决了智能代理训练中的探索-利用权衡问题，为LLM代理的长期战略任务训练提供了稳定有效的解决方案。
