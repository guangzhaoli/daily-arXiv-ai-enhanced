...
<details>
  <summary>Details</summary>
Motivation: 消除手动数据标注需求，实现跨任务和领域的通用视觉表示学习。

Method: 结合数据扩展、Gram锚定和后期策略，优化模型训练和特征质量。
...
TL;DR: 本文提出了一种多模态学习方法，用于提供变形攻击检测的文本描述，并通过零样本评估展示了其通用性和预测能力。
...
Method: 采用对比语言-图像预训练（CLIP）进行零样本评估，设计了十种不同的文本提示，包括短文本和长文本，并结合人类可理解的文本片段。

Result: 在公开的人脸生物特征数据集上进行了广泛实验，评估了五种不同的变形生成技术和三种不同媒介下的性能。

Conclusion: 提出的框架在零样本评估中表现出色，能够通用化变形攻击检测并预测最相关的文本片段。
...
Result: 在公开基准测试中取得了Top-10准确率和优异的零样本破译能力，并提供逻辑分析过程，可能为未破译甲骨文提供考古参考。
...
TL;DR: 开发并验证了一种用于TBDXA扫描的深度学习自动标记方法，准确率达99.5%，并用于健康标记研究。


<details>
  <summary>Details</summary>
Motivation: TBDXA扫描是一种低成本全身成像技术，但缺乏自动标记方法，限制了其在健康研究中的应用。

Method: 使用1,683个手动标注的TBDXA扫描训练深度学习模型，并在外部测试集上验证。

Result: 模型在外部测试集上达到99.5%的关键点正确率，并用于分析35,928次扫描的健康标记关联。
...
TL;DR: 本文提出了一种新型的多模态注意力归一化流（MANGO）方法，通过可逆交叉注意力层（ICA）和三种新的交叉注意力机制，实现了显式、可解释且易处理的多模态融合学习。


<details>
  <summary>Details</summary>
Motivation: 当前多模态融合方法依赖Transformer的注意力机制，难以捕捉多模态输入的本质特征和复杂结构，因此需要一种更显式和可解释的方法。

Method: 提出了可逆交叉注意力层（ICA）和三种新的交叉注意力机制（MMCA、IMCA、LICA），并设计了多模态注意力归一化流以处理高维数据。

Result: 在语义分割、图像到图像翻译和电影类型分类三个任务中，该方法取得了最先进的性能。

Conclusion: MANGO方法通过显式建模多模态特征的相关性，显著提升了多模态学习的性能，并具有可解释性和可扩展性。
...
TL;DR: 研究表明，结合少量真实图像与大量合成图像可显著提升西瓜病害分类模型的性能。


<details>
  <summary>Details</summary>
Motivation: 评估真实与合成图像结合对提升作物病害分类模型性能的有效性。

Method: 使用EfficientNetV2-L模型，比较不同真实与合成图像比例的训练数据集（H0-H4）。

Result: H3和H4处理（少量真实图像加大量合成图像）的F1分数从0.65提升至1.00。

Conclusion: 合成图像不能完全替代真实图像，需结合使用以优化模型性能。
...
TL;DR: 提出了一种基于合成数据的框架，用于提升视觉语言模型（VLMs）在工业泄漏检测等安全关键领域的性能，解决了真实数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 由于隐私、数据敏感性和事件罕见性，工业泄漏检测等领域缺乏足够的真实数据，导致传统微调方法难以应用。
...
TL;DR: 3D高斯泼溅（3DGS）是一种新兴的视图合成方法，具有快速训练/渲染和高质量视觉效果的优点。本文提出了一种因子化和参数化的熵编码方法EntropyGS，用于压缩3DGS高斯属性，实现了30倍的码率降低，同时保持相似的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 3DGS的高斯创建和视图渲染任务通常在时间或设备上分离，因此需要存储/传输和压缩3DGS高斯属性。本文通过分析高斯属性的统计特性，发现其符合特定分布，从而提出高效的压缩方法。

Method: 通过分析3DGS高斯属性的相关性和统计特性，提出EntropyGS方法，利用拉普拉斯分布和高斯混合分布对属性进行建模，并采用自适应量化的熵编码技术。
...
TL;DR: Xenium平台和CellSymphony框架结合空间转录组学和组织学图像，实现单细胞分辨率的多模态分析，揭示肿瘤微环境。


<details>
  <summary>Details</summary>
Motivation: 解决从组织学图像中提取细胞级特征并与空间转录组学数据整合的挑战。

Method: 利用基础模型嵌入，融合空间基因表达和形态学信息，开发CellSymphony框架。

Result: 在三种癌症类型中实现准确的细胞类型注释，并发现不同的微环境生态位。
...
TL;DR: 综述了基于深度学习的裂缝检测领域的最新趋势，包括学习范式的转变、泛化能力的提升和数据采集的多样化，并引入了一个新的3D激光扫描数据集3DCrack。
...
Result: 总结了代表性工作，并通过实验建立了常用深度学习方法的基线性能。
...
Method: 提出MRFD，利用交叉注意力识别显著区域，生成初始响应，基于JSD计算可靠性权重，并通过一致性感知融合预测。

Result: 实验表明MRFD显著减少幻觉并提高事实性，无需模型更新。

Conclusion: MRFD为LVLMs提供了一种有效的幻觉缓解方法。
...
TL;DR: 论文研究了基于外观的视线估计（PoG）在个体差异和头部姿态变化下的挑战，提出了动态校准策略以提高估计器的鲁棒性。
...
Method: 通过对比学习模块增强跨模态对齐约束，结合结构先验（如语义布局图或边缘草图）引导生成器进行空间级结构建模，并联合优化对比损失、结构一致性损失和语义保留损失。

Result: 在COCO-2014数据集上的实验表明，该方法在CLIP Score、FID和SSIM指标上表现优越，有效平衡了语义对齐和结构保真度。
...
TL;DR: 提出JRDB-Reasoning基准和自适应查询引擎，用于评估视觉推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉推理基准缺乏复杂性定义、难度控制和结构化注释。

Method: 形式化推理复杂性，开发自适应查询引擎，扩展JRDB数据集。

Result: 创建JRDB-Reasoning基准，支持细粒度评估和动态测试。

Conclusion: 新工具填补了视觉推理评估的空白，适用于复杂环境。
...
<details>
  <summary>Details</summary>
Motivation: 多模态光学图像匹配精度受非线性辐射和几何变形差异影响，需要一种更准确的方法。

Method: 采用两步法：粗匹配使用结构相似性指数（SSIM），细匹配使用WLAD，并结合相位一致性和互结构滤波。

Result: 在三种数据集上测试，PCWLAD在正确匹配率和均方根误差上优于现有方法，平均匹配精度达0.4像素。

Conclusion: PCWLAD方法在多模态图像匹配中表现出色，且相关软件和数据集已开源。
...
TL;DR: InterSyn是一种新颖的运动合成框架，通过联合学习单人及多人动态，生成更真实的交互运动。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将单人及多人动态分开处理，而InterSyn旨在捕捉真实场景中自然的动态交互和细微协调。

Method: InterSyn包含两个模块：INS（联合建模单人及交互行为）和REC（优化角色间的动态协调）。

Result: 实验表明，InterSyn生成的运动序列在文本对齐和多样性上优于现有方法。
...
TL;DR: 该论文综述了OoD分割在自动驾驶中的应用，将其方法分为四类，并讨论了挑战与未来方向。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型通常使用替代目标（如似然或重建损失）训练，但这些目标与感知质量、语义准确性或物理真实感不一致。强化学习提供了一种优化这些目标的框架。

Method: 综述系统回顾了强化学习在图像、视频和3D/4D生成中的应用，包括其作为微调机制和结构组件的角色。

Result: 强化学习在增强生成任务的可控性、一致性和人类对齐方面表现出有效性。

Conclusion: 强化学习与生成模型的结合仍面临挑战，未来研究需进一步探索其在高层次目标对齐中的应用。
...
TL;DR: 论文提出了一种针对视觉语言指令调优的定向训练数据选择方法，通过匹配基准测试的概念或技能，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现视觉语言基准测试在训练时主要受益于相似技能或视觉概念的指令，因此需要优化数据选择方法。

Method: 提取基准测试的概念/技能，确定其受益类型（概念或技能），并选择匹配度最高的指令进行训练。
...
TL;DR: 论文提出了一种名为Glo-DMU的框架，用于自动化量化肾小球超微结构特征，结合深度学习模型，提高了肾脏疾病诊断的效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注单个超微结构的识别，难以满足实际诊断需求，因此需要一种能够同时量化多种超微结构特征的方法。

Method: Glo-DMU基于三个深度学习模型：超微结构分割模型、肾小球滤过屏障区域分类模型和电子致密沉积物检测模型，量化了三种广泛使用的超微结构特征。
...
TL;DR: 本文介绍了使用深度学习技术在OCR和文档布局分析中的三种任务的方法和发现，包括历史希伯来文本、16至18世纪会议决议和现代英文手写体的识别。


<details>
  <summary>Details</summary>
Motivation: 研究旨在通过先进技术提升不同时期和类型文本的识别精度，为历史文献和现代手写体识别提供解决方案。

Method: 分别采用Kraken和TrOCR模型、结合DeepLabV3+与双向LSTM的CRNN，以及基于ResNet34编码器的CRNN模型。

Result: 通过数据增强和伪标签技术提升了模型性能，有效捕捉了序列依赖关系。

Conclusion: 研究为未来相关领域提供了有价值的见解和潜在方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以明确分离STEM数据中的空间漂移和辐射衰减效应，导致材料动态分析的困难。

Method: 提出AtomDiffuser框架，通过预测仿射变换和空间变化的衰减图来解耦降解效应。

Result: 在合成和真实cryo-STEM数据上表现良好，支持高分辨率降解推断和漂移对齐。

Conclusion: AtomDiffuser为辐射诱导的原子不稳定性提供了可视化和量化工具，提升了材料动态分析的准确性。
...
TL;DR: 本文提出了一种新方法，通过直接提示多模态视觉语言模型（VLMs）来估计其对比敏感度函数（CSF），以评估其与人类感知的一致性。


<details>
  <summary>Details</summary>
Motivation: 评估VLMs与人类感知的一致性，以了解它们如何处理低层次视觉特征。
...
TL;DR: 提出一种通过共同生成图像和其对应的内在场景属性（如深度、分割图）来提升图像空间一致性和真实性的方法。
...
Method: NSegment+仅对分割标签施加弹性变形，保持原始图像不变，从而鼓励模型学习更鲁棒的表示。
...
TL;DR: 论文提出了一种基于姿态驱动的质量控制数据增强框架（PQ-DAF），通过结合视觉语言模型和渐进条件扩散模型，有效解决了驾驶员分心检测中数据稀缺和领域偏移问题。


<details>
  <summary>Details</summary>
Motivation: 驾驶员分心检测对提升交通安全至关重要，但现有模型在真实场景中泛化能力不足，主要源于数据标注成本高和训练数据与目标场景的领域偏移。

Method: 提出PQ-DAF框架，利用渐进条件扩散模型（PCDMs）生成多样化的训练样本，并通过基于CogVLM视觉语言模型的样本质量评估模块过滤低质量样本。

Result: 实验表明，PQ-DAF在少样本驾驶员分心检测中显著提升了模型泛化能力。

Conclusion: PQ-DAF通过高质量数据增强和样本过滤，有效解决了数据稀缺和领域偏移问题，提升了模型的泛化性能。
...
Method: 引入delta向量修改文本嵌入，提出选择性抑制方法（SSDV），并在个性化T2I模型中优化delta向量。

Result: 实验结果表明，该方法在定量和定性指标上均显著优于现有方法。

Conclusion: 该方法有效抑制了不想要的内容，并在个性化模型中实现了更精确的抑制。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖固定坡度锚点，难以适应多样化的道路几何形状，且缺乏时间一致性，影响实际驾驶场景中的稳定性。

Method: 提出Slope-Aware Adaptive Feature模块动态融合多坡度特征，并引入Height Consistency Module确保时间一致性。

Result: 在OpenLane基准测试中，SC-Lane以64.3%的F-score达到最优性能，显著超越现有方法。

Conclusion: SC-Lane通过自适应坡度特征和时间一致性模块，为3D车道检测设定了新标准。
...
TL;DR: STRIDE-QA是一个大规模视觉问答数据集，用于自动驾驶中的时空推理，填补了现有视觉语言模型在动态交通场景理解中的不足。
...
Method: 构建了STRIDE-QA数据集，包含100小时的多传感器驾驶数据，提供1600万QA对和285K帧图像，支持对象中心和自我中心推理。
...
Method: 采用实例跟踪建模、自适应残差语义提示框架和对比学习损失，结合任务间查询空间初始化策略。

Result: 在YouTube-VIS数据集上表现优异，避免了灾难性遗忘，提升了分割和分类性能。

Conclusion: CRISP是持续视频实例分割的有效方法，适用于长期增量学习任务。
...
TL;DR: 论文提出了一种自动化识别电气布局图中符号的方法，并开发了一个轻量级开源工具包SkeySpot，用于实时检测、分类和量化电气符号。
...
Result: YOLOv8在DELP数据集上表现最佳，mAP达到82.5%。SkeySpot能够生成标准化输出，支持下游应用和监管平台的兼容性。

Conclusion: 该方法降低了企业对专有CAD系统的依赖，减少了人工标注工作量，尤其适用于中小型企业，并支持建筑环境的标准化、互操作性和可持续性。
...
Result: 编码器阶段（类似V1皮层）在图像失真方面与人类感知判断高度相关，且在中等噪声、模糊和稀疏性下表现最佳。

Conclusion: 视觉系统可能被优化以去除特定水平的失真和稀疏性，生物启发模型可以无监督学习感知度量。
...
<details>
  <summary>Details</summary>
Motivation: 现有在线视频超分辨率方法仅使用相邻前一帧进行时间对齐，限制了长程时间建模能力。

Method: TS-Mamba通过构建视频轨迹选择相似token，并利用轨迹感知的移位Mamba聚合模块（TSMA）进行信息聚合，设计了基于Hilbert扫描的移位SSMs块以增强空间连续性。

Result: 在三个广泛使用的VSR测试数据集上，TS-Mamba在大多数情况下优于六个基准模型，且复杂度降低了22.7%。

Conclusion: TS-Mamba通过结合轨迹建模和Mamba，显著提升了在线视频超分辨率的性能和效率。
...
Result: 在约140万张训练图像上表现优异，最终排名第三。
...
Method: 1. 自动体素强度分箱生成合成图像；2. 使用卷积自编码器评估预测脑掩模质量；3. 选择高质量伪标记微调网络。

Result: 在分布外数据上，性能接近使用更多标记图像训练的模型，且自编码器排名与分割准确性相关性更强。

Conclusion: 结合领域随机化和自编码器质量控制，可在极有限标记数据下实现有效的半监督分割，减轻标记负担。
...
Result: 结果揭示了各方法的优缺点及组合效果，为未来毫米波HAR系统提供指导。
...
Conclusion: STAMP为STAS诊断提供了高效工具，具有临床推广潜力。
...
TL;DR: TweezeEdit是一个无需调优和反转的图像编辑框架，通过正则化整个去噪路径，显著提升语义保留和目标对齐效率。
...
Method: 提出TweezeEdit框架，通过梯度驱动正则化在一致性模型上直接注入目标语义，避免依赖反转锚点。

Result: 实验表明TweezeEdit在语义保留和目标对齐上优于现有方法，仅需12步（1.6秒/次）完成编辑。

Conclusion: TweezeEdit高效且一致，适用于实时应用，解决了现有方法的局限性。
...
Result: 在多个基准测试中实现了最先进的细节保留效果，同时保持实时渲染效率。

Conclusion: 该方法在结构相似性和感知质量上显著优于基线方法，适用于高频纹理和锐利边界的重建。
...
Method: 提出Bolt-SAM分割模型和MOD-LaMa编辑模型，结合ERA策略生成缺陷图像。

Result: 实验表明SBDE生成的缺陷图像优于现有方法，显著提升检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用第一人称视频或音乐作为输入，而联合利用两者的任务尚未充分探索。

Method: 开发了EgoMusic Motion Network，结合扩散模型和Mamba，利用新数据集EgoAIST++进行训练。

Result: 实验表明，该方法明显优于现有技术，并能有效泛化到真实数据。

Conclusion: 该方法在舞蹈动作预测中表现出色，解决了遮挡和音乐同步问题。
...
TL;DR: 这篇综述论文旨在填补视觉推理领域的空白，通过分类和分析五种主要推理类型（关系、符号、时间、因果和常识推理），并探讨其实现方法和评估协议的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有研究往往孤立地探讨不同类型的视觉推理，缺乏统一的分析和比较。本文试图提供一个系统化的视角，以促进视觉推理领域的进一步发展。

Method: 论文将视觉推理分为五种类型，并系统性地分析了其实现方法（如图模型、记忆网络、注意力机制和神经符号系统）和评估协议。

Result: 研究发现现有评估协议在泛化性、可重复性和解释性方面存在局限性，并指出了视觉推理领域的关键挑战（如复杂场景的可扩展性、符号与神经范式的深度融合等）。

Conclusion: 论文提出了未来研究的议程，强调感知与推理的结合对于构建透明、可信赖且跨领域适应的AI系统至关重要，尤其是在自动驾驶和医疗诊断等关键领域。
...
<details>
  <summary>Details</summary>
Motivation: 现有医学图像定位研究存在模态覆盖有限、标注粒度粗糙和缺乏统一框架的问题，限制了其在智能诊断、视觉问答和报告生成中的应用。
...
Conclusion: Med-GLIP-5M数据集和Med-GLIP框架为医学图像定位提供了更全面的解决方案，并在下游任务中展示了实际应用价值。
...
TL;DR: AR手术导航系统通过实时工具跟踪提高导管放置精度。


<details>
  <summary>Details</summary>
Motivation: 传统导航系统在手术中存在深度感知和遮挡处理的局限性，AR技术有望解决这些问题。

Method: 使用HoloLens 2的传感器，结合表面追踪和实时红外工具跟踪，提供静态和动态AR引导。

Result: 实时工具跟踪在所有精度指标上表现更优，用户主观评价也更高。

Conclusion: AR实时工具跟踪在手术导航中具有显著优势，未来可进一步优化。
...
Method: RAP通过检索外部知识动态更新OOD提示，增强语义监督，并在测试时实时调整以适应测试环境。

Result: 在ImageNet-1k数据集上，RAP在1-shot OOD检测中FPR95降低7.05%，AUROC提升1.71%。

Conclusion: RAP通过检索增强的提示方法显著提升了OOD检测性能，实验验证了其有效性。
...
TL;DR: PTQAT是一种新型混合量化算法，结合PTQ和QAT的优势，通过选择性微调关键层，提高量化模型性能，同时减少计算资源消耗。


<details>
  <summary>Details</summary>
Motivation: 解决PTQ导致的性能下降和QAT的高资源消耗问题，提出一种更高效的混合量化方法。

Method: 选择关键层进行QAT微调，其余层采用PTQ，通过补偿量化误差传播提升精度。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法因依赖隐式建模音频-面部运动关联而缺乏明确的发音先验，导致视频质量不佳。

Method: HM-Talker 使用混合运动表示，结合动作单元（AUs）和隐式特征，并通过跨模态解缠模块（CMDM）和混合运动建模模块（HMMM）优化特征提取与泛化。

Result: 实验表明，HM-Talker 在视觉质量和唇同步准确性上优于现有方法。

Conclusion: HM-Talker 通过混合运动表示和身份无关学习，显著提升了说话头部视频的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于视觉的自动驾驶系统在恶劣天气、部分遮挡和精确速度估计方面存在局限性，而安全关键场景需要准确的运动理解和长时程轨迹预测以避免碰撞。

Method: 提出SpaRC-AD框架，通过相机和雷达的稀疏3D特征对齐以及多普勒速度估计，优化代理锚点、地图折线和运动建模。

Result: 在多个自动驾驶任务中显著优于纯视觉基线，包括3D检测（+4.8% mAP）、多目标跟踪（+8.3% AMOTA）、在线建图（+1.8% mAP）、运动预测（-4.0% mADE）和轨迹规划（-0.1m L2和-9% TPC）。

Conclusion: SpaRC-AD在安全关键场景中展示了雷达融合的有效性，特别是在运动理解和长时程轨迹预测方面，为自动驾驶提供了更可靠的解决方案。
...
Conclusion: 该方法通过结合SAM和新型损失函数，显著提升了遥感变化检测的鲁棒性和准确性。
...
TL;DR: 提出了一种基于多模态代理的灵活自适应方法，用于解决基于参考的视频对象分割任务，通过动态工作流和专用工具提升性能。
...
Method: 引入HumanSense基准测试，结合多模态输入（视觉、音频、文本），并采用多阶段、模态渐进强化学习来增强推理能力。

Result: 领先的MLLMs在高级交互任务中仍有改进空间，全模态模型表现更优；推理能力是提供适当反馈的关键。

Conclusion: 通过增强推理能力和设计提示，可以显著提升模型性能，且成功的推理过程表现出高度一致的思维模式。
...
TL;DR: EvTurb利用高速事件流解耦湍流引起的模糊和倾斜效应，通过两步事件引导网络提升图像质量。
...
Method: 提出EvTurb框架，通过事件积分减少模糊，方差图消除倾斜，并发布首个真实湍流数据集TurbEvent。

Result: 实验表明EvTurb在计算效率下优于现有方法。

Conclusion: EvTurb有效解决湍流问题，为下游计算机视觉任务提供高质量图像。
...
TL;DR: P$^3$A提出了一种针对自动驾驶中2D目标检测的强大且实用的对抗补丁攻击框架，通过新指标PASR和损失函数LCSL提升攻击效果。
...
Method: 提出PASR指标、LCSL损失函数和PSPP数据预处理方法，优化高分辨率数据集攻击。
...
TL;DR: FGA是一种轻量级上采样模块，通过频率引导注意力机制提升单图像超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 传统上采样方法（如子像素卷积）难以重建高频细节并易引入混叠伪影，FGA旨在解决这些问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在MAR任务中难以同时抑制伪影和保留结构细节，FIND-Net旨在解决这一问题。

Method: FIND-Net通过集成快速傅里叶卷积（FFC）层和可训练高斯滤波，在频域和空间域中处理MAR任务。

Result: 在合成数据集上，FIND-Net在MAE、SSIM和PSNR指标上均优于现有方法；在真实临床CT扫描中也表现优异。
...
Method: 利用少量真实样本（如2或32张）通过Chamfer Guidance优化生成过程，无需训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型忽视环境传感器的因果信号，依赖单一来源的标注，缺乏交互式场景推理。

Method: 构建177k图像数据集，使用GPT-4o和Gemini 2.0标注，通过LoRA微调Qwen-2.5-VL。

Result: 在时序和假设推理中表现优异（BERT-F1 0.903），支持交互式分析。

Conclusion: ChatENV是传感器感知的环境监测工具，性能优于现有时序模型。
...
<details>
  <summary>Details</summary>
Motivation: 研究视觉编码器对细微或不可察觉的图像采集和处理参数的敏感性，及其对语义预测的影响。

Method: 分析图像采集和处理参数在视觉表示中的编码方式，并评估其对语义预测的影响。

Result: 发现这些参数被系统编码，且对语义预测有显著影响，取决于参数与语义标签的相关性。

Conclusion: 图像采集和处理参数对视觉编码器的语义预测有重要影响，需在模型设计中考虑。
...
Conclusion: 结合姿态估计和BLSTM的方法高效、准确，适用于小数据集和短序列。
...
TL;DR: SemPT是一种新的视觉迁移学习框架，通过共享属性级知识解决未见类别的泛化问题，表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决视觉迁移学习中保留类别特定表示与获取可迁移知识之间的冲突。

Method: 采用两步提示策略提取共享视觉属性并生成属性级描述，结合视觉引导权重和联合嵌入对齐。

Result: 在15个基准数据集上实现最先进性能，涵盖多种迁移学习场景。

Conclusion: SemPT通过属性级知识增强文本嵌入，平衡了判别性和可迁移性，适用于多种迁移学习任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在训练和测试中存在不一致性，导致性能下降，需统一基准和优化整合方式。

Method: 引入UniBench300统一基准，采用串行整合和持续学习（CL）方法。

Result: 实验显示UniBench300显著提升性能，CL支持稳定整合，性能下降与网络容量负相关。
...
TL;DR: AddressVLM通过结合卫星图像和街景图像，提升LVLM在细粒度地址定位中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有LVLM在城市级粗粒度定位表现良好，但在街道级细粒度定位中表现不佳。

Method: 提出跨视角对齐调优和地址定位调优两阶段训练，结合卫星图像和街景图像。

Result: 在Pittsburgh和San Francisco数据集上，AddressVLM平均地址定位准确率分别提升9%和12%。
...
TL;DR: 本文提出了一种构建高质量人脸数据集的方法，通过清理现有数据、生成合成数据并结合课程学习策略，最终在竞赛中获得第一名。


<details>
  <summary>Details</summary>
Motivation: 构建一个不与现有公开数据集重叠的高质量人脸数据集，以提升人脸识别模型的性能。

Method: 结合Mixture-of-Experts策略清理数据，使用Stable Diffusion生成合成数据，并通过Vec2Face扩展数据，采用课程学习策略训练模型。

Result: 最终数据集在竞赛中获得第一名，并在不同规模的身份数据集上提升了模型性能。

Conclusion: 通过混合真实与合成数据的方法，高效构建了多样且高质量的数据集，显著提升了人脸识别模型的性能。
...
TL;DR: HyperTea结合全局和局部时间视角，利用超图神经网络提升红外小目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅建模低阶特征关联且局限于单一时间尺度，超图在高阶关联学习中的潜力未被充分挖掘。

Method: 提出HyperTea，包含全局时间增强模块（GTEM）、局部时间增强模块（LTEM）和时间对齐模块（TAM），结合CNN、RNN和HGNN。

Result: 在DAUB和IRDST数据集上达到SOTA性能。

Conclusion: HyperTea首次整合CNN、RNN和HGNN，显著提升了红外小目标检测效果。
...
TL;DR: 提出了一种联合重建跨TE数据的方法，结合隐式神经表示和物理信息正则化，显著减少了胎儿脑MRI中T2映射的扫描时间和运动敏感性。


<details>
  <summary>Details</summary>
Motivation: 胎儿脑MRI中T2映射在0.55T中场强下具有潜力，但现有方法因多次采集和运动干扰导致扫描时间长且敏感。

Method: 结合隐式神经表示和物理信息正则化，联合重建跨TE数据，保留解剖和T2定量信息。

Result: 在模拟胎儿脑和成人数据中表现优异，首次实现0.55T下胎儿T2映射。

Conclusion: 该方法通过利用解剖冗余，有望减少每个TE所需的堆栈数量。
...
TL;DR: 提出了一种名为IADGPT的统一框架，用于少样本工业异常检测（FS-IAD），通过三阶段训练策略模拟人类学习过程，结合语言输出和注意力图实现异常推理。


<details>
  <summary>Details</summary>
Motivation: 现有的大视觉语言模型（LVLMs）缺乏工业知识和推理能力，无法胜任专业的人类质检任务。

Method: 采用三阶段渐进训练策略，前两阶段学习基础工业知识和差异意识，第三阶段通过上下文学习范式提升泛化能力。

Result: IADGPT在异常检测、定位和推理任务中表现优异，实验验证了其性能提升。

Conclusion: IADGPT通过模拟人类学习过程，显著提升了FS-IAD任务的性能，并展示了在工业质检中的潜力。
...
TL;DR: 提出了一种轻量级的视图转换框架，利用预训练扩散模型的高保真生成能力，从单张输入图像合成新视图。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要微调大型扩散模型或从头训练，成本高且重建模糊、泛化能力差。

Method: 使用相机姿态条件化的U-Net（TUNet）预测目标视图的潜在表示，并提出融合策略保留细节。

Result: 在MVImgNet上实验表明，该方法优于现有方法。

Conclusion: 该方法高效且能生成高质量的新视图。
...
TL;DR: 提出MCFNet，结合事件相机和RGB相机，通过运动线索融合网络提升复杂光照下的目标检测性能。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机动态范围有限，在复杂交通环境中（如夜间驾驶、隧道）导致全局对比度降低和高频细节丢失，影响目标检测。
...
TL;DR: 论文提出了一种名为CountCluster的方法，通过优化对象交叉注意力图，使其在早期去噪步骤中准确反映输入提示中的对象数量，从而提升图像生成的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的扩散模型在生成图像时难以准确反映输入提示中指定的对象数量，尤其是在早期去噪步骤中对象数量的确定。

Method: 提出CountCluster方法，通过聚类对象交叉注意力图，定义理想分布并优化潜在表示，无需外部工具或额外训练。

Result: 实验表明，该方法在对象计数准确性上平均提升了18.5%，并在多种提示下表现出优越的数量控制性能。

Conclusion: CountCluster方法有效解决了扩散模型在对象数量控制上的问题，为文本到图像生成提供了更准确的解决方案。
...
TL;DR: NextStep-1是一种14B自回归模型，结合157M流匹配头，通过离散文本和连续图像令牌实现文本到图像生成，性能领先。
...
Method: 使用离散文本令牌和连续图像令牌，通过下一令牌预测目标训练。

Result: 在文本到图像生成任务中达到SOTA性能，支持高保真图像合成和编辑。

Conclusion: 展示了统一方法的强大和多功能性，代码和模型将开源。
...
TL;DR: 论文提出了一种用于实时处理未聚焦SAR数据的神经网络，解决了卫星资源限制问题，并验证了在FPGA上部署的可行性。


<details>
  <summary>Details</summary>
Motivation: 实时监测海上船只需要大量数据传输和处理，传统方法受限于卫星资源，因此需要开发高效的机载处理方法。

Method: 设计了针对Stripmap和IW模式SAR数据的神经网络，支持实时推理，并在FPGA上部署验证。

Result: 实验证明模型可行，且能完成船只与风力发电机的二分类任务。

Conclusion: 该研究为机载SAR数据处理提供了高效解决方案，减少了数据传输需求，降低了延迟。
...
TL;DR: 论文提出了一种新的跨视角定位框架，通过改进图像匹配和定位方法，解决了现有方法在几何一致性匹配上的不足。


<details>
  <summary>Details</summary>
Motivation: 在GNSS受限环境中，跨视角定位至关重要，但现有方法无法建立严格的跨视角对应关系，导致匹配粗糙或不一致。

Method: 引入表面模型（Surface Model）进行准确的鸟瞰图投影，以及SimRefiner模块通过局部-全局残差校正优化相似性矩阵。

Result: 实验表明，该方法显著提高了定位精度和图像匹配质量，并在极端视角差异下设定了新的基准。

Conclusion: 论文通过改进匹配和定位方法，解决了跨视角定位中的关键问题，并提供了首个带有像素级对应标注的基准数据集CVFM。
...
TL;DR: 论文提出了一种名为DCPE的方法，替代k-means聚类，更有效地利用代码本中的token相似性信息，提升自回归模型的训练速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法仅使用索引值训练自回归生成模型，忽略了代码本中丰富的token相似性信息，且k-means聚类在代码本特征空间中表现不佳。

Method: 提出DCPE方法，采用基于实例的距离替代基于质心的距离，并使用凝聚合并技术解决token空间差异问题。

Result: DCPE将LlamaGen-B的训练速度提升42%，并改善了FID和IS性能。

Conclusion: DCPE是一种即插即用的方法，能有效利用代码本中的判别性先验信息，提升自回归模型的训练效率和性能。
...
TL;DR: 本文提出了EgoCross基准测试，用于评估多模态大语言模型在跨域自我中心视频问答中的泛化能力，发现现有模型在非日常生活领域表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要局限于日常活动，而实际应用中存在领域偏移问题，因此需要评估模型在多样化领域的泛化能力。

Method: 构建了包含四个多样化领域的EgoCross基准测试，涵盖1,000个QA对和798个视频片段，支持OpenQA和CloseQA格式。

Result: 实验表明，现有模型在非日常生活领域泛化能力有限，通过微调和强化学习等方法探索改进方向。

Conclusion: EgoCross为推进领域自适应、鲁棒的自我中心视频理解提供了基础，未来可进一步优化模型跨域性能。
...
TL;DR: ConGCD提出了一种基于视觉基元和共识调度的新方法，用于广义类别发现（GCD），通过分解对象并建立跨知识比较，模拟人类认知过程。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习的对象识别能力远不及人类感知系统，尤其是在处理已知和未知类别时。现有GCD方法主要关注优化目标函数，缺乏对人类认知过程的模拟。

Method: ConGCD通过高级语义重构建立基元导向表示，并通过解构绑定类内共享属性。采用主导和上下文共识单元分别捕捉类判别模式和分布不变性，动态优化激活路径。

Result: 在粗粒度和细粒度基准测试中，ConGCD表现出色，证明了其作为共识感知范式的有效性。

Conclusion: ConGCD通过模拟人类认知过程，提供了一种新的GCD解决方案，显著提升了机器学习的对象识别能力。
...
Method: 竞赛分为两个赛道：纯合成数据训练和混合数据训练，共有九组提交了多种模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 复杂场景中的对称轴检测仍具挑战性，现有热图方法在定位单个对称轴时精度不足。

Method: 使用双分支架构，分别处理反射和旋转对称，引入方向锚点和反射/旋转匹配机制。

Result: 实验表明，该方法在对称轴检测上优于现有方法，达到最先进性能。

Conclusion: 通过几何基元表示和双分支架构，有效提升了对称轴检测的精度和鲁棒性。
...
TL;DR: DARC是一种新型的模型驱动群体配准框架，用于高效构建无偏且高保真的图谱，并支持多种应用。
...
Method: 采用监督微调结合基于规则的强化学习方法，对Qwen2.5-VL-7B模型进行优化。

Result: 改进后的模型在时空物理推理任务上表现显著提升，超越了主流专有模型。

Conclusion: 尽管模型性能有所提升，但其在新物理场景中的泛化能力仍有限，表明需要进一步研究新的方法。
...
TL;DR: AEGIS是一个大规模基准测试，专注于检测高度逼真的AI生成视频，填补了现有基准测试在真实性和复杂性上的不足。


<details>
  <summary>Details</summary>
Motivation: AI生成内容的发展带来了高度逼真的合成视频，对社会信任和数字完整性构成威胁，现有基准测试无法有效评估现代视觉语言模型。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在高质量视频生成中表现优异，但其迭代去噪过程和长序列的二次注意力成本导致推理瓶颈。现有加速策略（如步蒸馏和稀疏注意力）单独使用效果有限，联合训练又面临数据需求高的问题。

Method: BLADE引入自适应块稀疏注意力（ASA）机制动态生成内容感知的稀疏掩码，并结合基于轨迹分布匹配（TDM）的稀疏感知步蒸馏范式，将稀疏性直接融入蒸馏过程。

Result: 在CogVideoX-5B和Wan2.1-1.3B模型上，BLADE分别实现8.89x和14.10x的推理加速，并在VBench-2.0基准测试中提升生成质量分数。

Conclusion: BLADE通过联合训练框架有效解决了视频生成模型的推理瓶颈，同时提升速度和生成质量，具有广泛的应用潜力。
...
TL;DR: TriFlowSR is a novel framework for Reference-based Image Super-Resolution (RefSR) that improves pattern matching between low-resolution (LR) and reference high-resolution (HR) images, and introduces the Landmark-4K dataset for UHD scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based RefSR methods struggle with alignment between LR and reference HR images, and current datasets lack sufficient fine-grained details for high-quality restoration.

Method: TriFlowSR uses a Reference Matching Strategy to align LR and reference HR images effectively, and introduces the Landmark-4K dataset for UHD scenarios.

Result: TriFlowSR outperforms previous methods in utilizing semantic and texture information from reference HR images.

Conclusion: TriFlowSR is the first diffusion-based RefSR pipeline for UHD landmark scenarios under real-world degradation, with promising results.
...
Method: 设计了用户缓慢将面部靠近摄像头的交互场景，结合光流分析和神经网络分类器处理RGB帧和光流数据，提取时空特征。

Result: 该方法显著提高了活体检测的可靠性，优于被动方法。

Conclusion: 通过特定交互动作和光流分析，该方法在活体检测中表现出色，为安全认证提供了新思路。
...
TL;DR: VasoMIM是一种针对X射线血管造影的自监督学习框架，通过整合血管解剖知识改进传统MIM的不足，显著提升了血管分割性能。


<details>
  <summary>Details</summary>
Motivation: X射线血管造影中血管分割的标注数据稀缺，且传统MIM方法因血管与背景像素的严重不平衡导致血管表示能力弱。

Method: 提出VasoMIM框架，包含解剖引导的掩蔽策略和解剖一致性损失，优先掩蔽血管区域并强化血管语义一致性。

Result: 在三个数据集上达到最先进性能。

Conclusion: VasoMIM有望促进X射线血管造影分析，尤其在数据稀缺场景下表现突出。
...
TL;DR: OF-Diff 是一种用于高精度可控遥感图像生成的新方法，通过提取对象先验形状和双分支扩散模型，显著提升了生成图像的保真度和多样性。
...
Method: 提出 OF-Diff，首次基于布局提取对象先验形状，并引入双分支扩散模型和扩散一致性损失，无需真实图像即可生成高保真图像。此外，使用 DDPO 微调扩散过程以增强多样性和语义一致性。

Result: OF-Diff 在遥感图像生成中优于现有方法，关键质量指标显著提升，例如飞机、船只和车辆的 mAP 分别提高了 8.3%、7.7% 和 4.0%。

Conclusion: OF-Diff 通过改进对象保真度和生成多样性，为遥感图像生成提供了高效且可靠的解决方案。
...
TL;DR: 开发了一种移动友好的解决方案，用于准确分类33种作物中的101种植物病害，采用高效轻量级架构，EfficientNet-B1表现最佳，准确率达94.7%。
...
Result: EfficientNet-B1表现最佳，分类准确率达94.7%，在准确性和计算效率之间取得了最佳平衡。

Conclusion: EfficientNet-B1适合在移动设备上实际部署，为植物病害早期检测提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 提升UI代理的性能，减少对大量训练数据的依赖，并通过强化微调和高效数据清理策略优化模型。

Method: 采用Qwen2.5-VL进行强化微调，设计奖励函数和数据清理策略，并提出自进化轨迹历史对齐和稀疏动作增强技术。

Result: 7B和72B模型在Screenspot-V2/Pro基准测试中分别达到94.1%/50.8%和95.3%/61.9%，在AndroidWorld导航任务中分别达到49.1%和65.9%的成功率。

Conclusion: UI-Venus通过创新方法在UI任务中表现优异，推动了开源UI代理的发展，并提供了数据清理和自进化框架的贡献。
...
TL;DR: BaCon-Stereo提出了一种基于对比学习的自监督立体匹配框架，通过教师-学生范式和多基线输入解决遮挡区域的问题。
...
Method: 采用教师-学生范式，利用多基线输入，教师和学生的输入共享参考视图但目标视图不同，通过教师预测监督学生学习遮挡补全。

Result: 在KITTI 2015和2012基准测试中优于现有自监督方法，提升了遮挡和非遮挡区域的预测效果。
...
TL;DR: 本文提出了AdaptFED，一种改进的联邦学习框架，通过任务感知的客户端嵌入和低秩超网络条件化，提升了泛化性和效率。
...
Method: 引入任务感知客户端嵌入和低秩超网络条件化，优化焦点调制层的动态适应。
...
Conclusion: 该研究首次探索了细粒度偏好对齐和数据选择，为更真实的视频生成开辟了新途径。
...
Method: 使用GPT-4o、GPT-5-nano、GPT-5-mini和GPT-5在脑肿瘤VQA基准上进行零样本链式思考评估。

Result: GPT-5-mini表现最佳（44.19%），但不同肿瘤亚型间性能差异显著。
...
TL;DR: TexVerse是一个大规模高分辨率纹理3D数据集，填补了现有数据集的空白，包含超过858K个独特模型和1.6M实例，支持多种3D视觉和图形任务。


<details>
  <summary>Details</summary>
Motivation: 当前大规模3D数据集主要关注几何生成，高分辨率纹理生成缺乏合适的数据集支持，TexVerse旨在解决这一问题。

Method: TexVerse从Sketchfab收集了858K个高分辨率3D模型，包括158K个PBR材质模型，并提供骨架和动画子集。

Result: 数据集包含1.6M实例，提供详细注释，支持纹理合成、PBR材质开发、动画等任务。
...
Method: 使用Kvasir-VQA-x1数据集，包含6,500张图像和159,549个复杂问答对，分为两个子任务：视觉问答和多模态解释生成。

Result: 挑战赛提供了数据访问和参与指南，旨在促进XAI在临床决策中的应用。

Conclusion: Medico 2025挑战赛通过多模态解释和临床相关性，推动了医学影像分析中可解释AI的进步。
...
TL;DR: ToonComposer是一个生成模型，将动画制作中的中间帧生成和上色阶段统一为一个后关键帧阶段，减少人工工作量并提高灵活性。


<details>
  <summary>Details</summary>
Motivation: 传统动画制作需要大量手动操作，现有AI方法将各阶段分开处理，导致错误累积和伪影。ToonComposer旨在解决这些问题。

Method: ToonComposer通过稀疏草图注入机制提供精确控制，并利用空间低秩适配器将现代视频基础模型适配到卡通领域。
...
Method: 采用解码器-only Transformer，利用因果注意力机制和几何先验进行流式处理。
...
Conclusion: MAESTRO为遥感数据的自监督学习提供了新思路，显著提升了多时相任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频类增量学习方法要么内存效率低，要么牺牲关键时间信息导致性能下降。

Method: ESSENTIAL通过情景记忆存储稀疏时间特征，语义记忆存储可学习提示的通用知识，并引入记忆检索模块整合两者。
...
TL;DR: 提出了一种统一跨领域3D人体运动模型HiC，通过单阶段训练和上下文学习，解决了现有方法依赖领域特定组件和多阶段训练的问题。
...
TL;DR: Puppeteer是一个自动绑定和动画框架，通过预测骨骼结构和皮肤权重，生成高质量3D动画。


<details>
  <summary>Details</summary>
Motivation: 解决静态3D模型转换为动态动画的瓶颈，减少对专家干预的依赖。

Method: 使用自回归变换器预测骨骼结构，注意力架构推断皮肤权重，并结合可微分优化动画管道。

Result: 在多个基准测试中显著优于现有技术，生成稳定且高质量的动画。

Conclusion: Puppeteer能够高效处理多样化的3D内容，消除现有方法中的抖动问题。
...
TL;DR: QINRs（量子隐式神经表示）在量子计算机上学习与执行，但存在架构、训练效率等问题。本文提出QVF（量子视觉场），通过神经振幅编码和全纠缠设计，实现高效训练和稳定收敛，并在实验中表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决QINRs在架构设计、量子力学属性利用、训练效率等方面的挑战，提升量子视觉场的学习能力。

Method: 提出QVF，使用神经振幅编码将经典数据嵌入量子态向量，采用全纠缠的可学习参数化量子电路，直接在实希尔伯特空间操作。

Result: QVF在量子硬件模拟器上表现优于现有量子方法和经典基线，尤其在细节学习方面，并展示了在2D/3D场补全和形状插值中的应用。

Conclusion: QVF是一种高效且稳定的量子视觉场表示方法，具有实际应用潜力。
...
### [108] [Invisible Watermarks, Visible Gains: Steering Machine Unlearning with Bi-Level Watermarking Design](https://arxiv.org/abs/2508.10065)
*Yuhao Sun,Yihua Zhang,Gaowen Liu,Hongtao Xie,Sijia Liu*

Main category: cs.CR

TL;DR: 论文提出了一种基于数字水印的机器遗忘方法Water4MU，通过数据级调整提升遗忘效果，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 随着被遗忘权需求的增加，机器遗忘（MU）成为增强信任和合规性的重要工具，但现有方法主要依赖模型权重调整，数据级调整的潜力未被充分探索。

Method: 提出Water4MU框架，利用数字水印技术通过数据内容修改实现精确遗忘，采用双层优化（BLO）框架优化水印网络和模型训练。

Result: 实验证明Water4MU在图像分类和生成任务中有效，尤其在“挑战性遗忘”场景中表现优于现有方法。
...
### [109] [Insights from the Algonauts 2025 Winners](https://arxiv.org/abs/2508.10784)
*Paul S. Scotti,Mihir Tripathy*

Main category: q-bio.NC

TL;DR: Algonauts 2025 Challenge focused on predicting human brain activity from long, multimodal movies, with winners announced based on out-of-distribution tests.


<details>
  <summary>Details</summary>
Motivation: Advance computational neuroscience by building models that predict brain activity from complex stimuli, pushing beyond previous editions' focus on simpler stimuli.

Method: Teams predicted fMRI responses across 1,000 brain parcels using 80 hours of naturalistic movie stimuli, including training and validation data from the CNeuroMod project.

Result: Winners were determined by their ability to predict brain activity for held-out films, with top team reports now available.

Conclusion: The challenge highlights progress in brain encoding models and sets the stage for future advancements in understanding complex stimuli's impact on brain activity.
...
TL;DR: 提出了一种用于肺癌筛查的深度学习框架，结合可解释性技术，在CT图像上实现了高准确率和临床透明度。


<details>
  <summary>Details</summary>
Motivation: 肺癌早期检测对提高生存率至关重要，但传统方法可能缺乏准确性和可解释性。
...
Conclusion: 结合可解释性的CNN方法可为肺癌筛查提供快速、准确且透明的支持，尤其在资源有限的环境中。
...
TL;DR: 该博士研究通过开发半监督框架和多任务数据集，解决了手术视频分析中的标注稀缺和领域差距问题，提升了模型的鲁棒性和数据效率。


<details>
  <summary>Details</summary>
Motivation: 手术视频分析在临床部署中面临标注稀缺、时空复杂性和跨领域差距等挑战，研究旨在开发更鲁棒且可扩展的解决方案。

Method: 通过基准测试选择最佳神经网络架构，并提出新架构和半监督框架（如DIST、SemiVT-Surge和ENCORE），利用未标注数据提升性能。

Result: 提出的方法在多个手术数据集上取得领先结果，并发布了GynSurg和Cataract-1K两个多任务数据集。

Conclusion: 研究为手术视频分析提供了鲁棒且数据高效的解决方案，为可推广的AI系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在大偏差和可解释性方面存在挑战，需要一种更高效、鲁棒且可解释的运动跟踪方法。

Method: 结合DINOv2的强大特征表示和LoRA层的参数效率，通过自动检测对应标志点实现图像配准。

Result: 在肾脏、肝脏和肺部的实验中，Dice分数分别为92.07%、90.90%和95.23%，Hausdorff距离分别为5.47 mm、8.31 mm和6.72 mm，处理速度约为30ms/扫描。

Conclusion: DINOMotion在实时运动跟踪中表现出色，特别是在处理大偏差时优于现有方法，具有临床应用潜力。
...
Method: 利用Haar变换和t-SVD统一捕捉全局和局部图像块相关性，无需学习局部基，结合CNN噪声估计增强鲁棒性。

Result: 实验验证了Haar-tSVD在去噪速度和性能上的平衡，有效去除噪声并保留细节。

Conclusion: Haar-tSVD是一种高效、并行化的去噪方法，适用于实际应用，代码和数据已开源。
...
<details>
  <summary>Details</summary>
Motivation: 用户生成视频内容快速增长，需要无参考视频质量评估方法进行大规模质量监控。

Method: 利用帧间差异，多层级分析质量敏感区域，整合2D和3D特征捕捉时空变化。

Result: 在五个数据集上表现优异，平均排名前二，计算效率高。
...
TL;DR: 分析2D DSA中多标注者对颅脑血管分割的变异性，量化分割不确定性，并探讨其在指导额外标注和开发不确定性感知自动分割方法中的应用。


<details>
  <summary>Details</summary>
Motivation: 研究多标注者在2D DSA中对颅脑血管分割的变异性，以量化和利用分割不确定性。

Method: 分析多标注者的分割结果，量化不确定性，并探讨其应用。

Result: 量化了分割不确定性，并提出了其在指导标注和自动分割中的潜在用途。
...
### [117] [AI-Driven Detection and Analysis of Handwriting on Seized Ivory: A Tool to Uncover Criminal Networks in the Illicit Wildlife Trade](https://arxiv.org/abs/2508.10219)
*Will Fein,Ryan J. Horwitz,John E. Brown III,Amit Misra,Felipe Oviedo,Kevin White,Juan M. Lavista Ferres,Samuel K. Wasser*

Main category: cs.LG

TL;DR: AI驱动的管道分析象牙上的手写标记，为打击野生动物犯罪提供低成本、可扩展的取证证据。


<details>
  <summary>Details</summary>
Motivation: 象牙贸易导致非洲象数量下降，但现有取证方法（如DNA分析）成本高且有时不可行。手写标记虽易获取，却很少被分析。

Method: 收集6,085张象牙照片，使用目标检测模型提取17,000多个标记，并通过AI工具识别184个重复的“签名标记”。

Result: 发现20个签名标记出现在多次查获中，建立了不同查获之间的取证联系。

Conclusion: AI在野生动物取证中具有变革潜力，手写标记分析可补充其他取证方法，帮助打击有组织犯罪。
...
### [118] [SynBrain: Enhancing Visual-to-fMRI Synthesis via Probabilistic Representation Learning](https://arxiv.org/abs/2508.10298)
...
Method: 引入BrainVAE和语义到神经映射器，通过概率分布建模神经表示。

Result: 在视觉到fMRI编码性能上超越现有方法，适应新受试者能力强。

Conclusion: SynBrain揭示了跨试验和受试者的功能一致性，并生成高质量fMRI信号。
...
### [119] [Improving Learning of New Diseases through Knowledge-Enhanced Initialization for Federated Adapter Tuning](https://arxiv.org/abs/2508.10299)
*Danni Peng,Yuan Wang,Kangning Cai,Peiyan Ning,Jiming Xu,Yong Liu,Rick Siow Mong Goh,Qingsong Wei,Huazhu Fu*

Main category: cs.LG

TL;DR: FedKEI是一种联邦学习框架，通过跨客户端和跨任务知识转移，为新任务提供初始化适配器，提升医疗领域对新疾病的适应能力。


<details>
  <summary>Details</summary>
Motivation: 医疗环境中，客户需要快速适应新任务或疾病，同时利用过去经验。FedKEI旨在通过知识转移优化初始化过程。

Method: FedKEI采用全局聚类和双层优化方案，学习全局和局部聚合权重，个性化知识转移。

Result: 在皮肤病、胸部X光和视网膜OCT数据集上，FedKEI优于现有方法，适应新疾病能力更强。

Conclusion: FedKEI通过知识增强初始化，显著提升了联邦学习在医疗任务中的适应性和效率。
...
### [120] [On the Complexity-Faithfulness Trade-off of Gradient-Based Explanations](https://arxiv.org/abs/2508.10490)
*Amir Mehrpanah,Matteo Gamba,Kevin Smith,Hossein Azizpour*

Main category: cs.LG

TL;DR: 本文提出了一个统一的光谱框架，用于分析和量化解释的平滑性、忠实性及其权衡，并揭示了基于代理的平滑方法如何扭曲解释。
...
Method: 引入光谱框架，系统分析解释的平滑性和忠实性，并量化ReLU网络对高频信息的贡献，通过正则化方法识别权衡关系。

Result: 理论分析揭示了基于代理的平滑方法会扭曲解释，导致“解释差距”，并通过实验验证了这些发现。

Conclusion: 本文提供了一种原则性的方法来分析和优化解释的平滑性与忠实性之间的权衡，为未来的解释方法设计提供了理论基础。
...
### [121] [On Spectral Properties of Gradient-based Explanation Methods](https://arxiv.org/abs/2508.10595)
...
<details>
  <summary>Details</summary>
Motivation: 理解深度网络行为对增强结果可信度至关重要，但现有解释方法存在可靠性问题，需更严格的形式化分析。

Method: 采用概率和谱视角分析解释方法，提出标准扰动尺度和SpectralLens聚合方法。

Result: 揭示了梯度使用中的谱偏差，验证了改进方法的有效性。

Conclusion: 通过形式化分析和实验验证，论文为解释方法提供了更可靠的理论基础和改进方向。
...
### [122] [Geospatial Diffusion for Land Cover Imperviousness Change Forecasting](https://arxiv.org/abs/2508.10649)
*Debvrat Varshney,Vibhas Vats,Bhartendu Pandey,Christa Brelsford,Philipe Dias*

Main category: cs.LG

TL;DR: 提出了一种利用生成式AI（GenAI）预测土地覆盖变化的新方法，通过将LULC预测视为数据合成问题，并基于历史和辅助数据源进行建模。
...
Result: 在12个大都市区的实验中，模型在分辨率≥0.7×0.7km²时，MAE低于基线，表明其能捕捉历史数据中的时空模式以预测未来变化。

Conclusion: 生成模型能有效预测土地覆盖变化，未来研究将整合地球物理属性和驱动变量以支持多情景模拟。
...
Result: 在图像营养预测任务中，微调大型视觉语言模型（如ChatGPT 5、Qwen-Max）相比基线模型有显著提升。

Conclusion: MM-Food-100K数据集公开免费提供，剩余90%数据可能用于商业用途，收益与贡献者共享。
...
Method: We-Math 2.0整合了四部分：1) 五级数学知识系统；2) 标准与进阶数据集；3) 两阶段强化学习框架；4) 全面评估基准。
...
### [125] [Agentic Design Review System](https://arxiv.org/abs/2508.10745)
*Sayan Nag,K J Joseph,Koustava Goswami,Vlad I Morariu,Balaji Vasan Srinivasan*

Main category: cs.AI

TL;DR: 提出AgenticDRS系统，通过多智能体协作评估平面设计，结合图匹配和提示扩展方法，效果优于现有基准。


<details>
  <summary>Details</summary>
Motivation: 平面设计评估需多维度综合分析，现有方法依赖专家反馈，缺乏自动化解决方案。

Method: 提出AgenticDRS系统，由元智能体协调多智能体协作分析设计，采用图匹配和提示扩展方法。

Result: 实验表明AgenticDRS优于现有基准，能生成可操作反馈。

Conclusion: 该工作为平面设计评估提供了实用且高效的解决方案，呼吁更多研究关注此方向。
...
### [126] [ReconVLA: Reconstructive Vision-Language-Action Model as Effective Robot Perceiver](https://arxiv.org/abs/2508.10333)
*Wenxuan Song,Ziyang Zhou,Han Zhao,Jiayi Chen,Pengxiang Ding,Haodong Yan,Yuxin Huang,Feilong Tang,Donglin Wang,Haoang Li*

Main category: cs.RO

TL;DR: ReconVLA是一种通过重构视觉注意力区域来提升视觉-语言-动作模型性能的隐式接地方法。
...
Result: 实验表明，该方法在模拟和真实场景中均表现出色，实现了精确操作和泛化能力。
...
### [127] [CorrectNav: Self-Correction Flywheel Empowers Vision-Language-Action Navigation Model](https://arxiv.org/abs/2508.10416)
...
Method: 利用错误轨迹作为数据源，开发了识别偏差和自动生成自校正数据的技术，通过多次迭代提升模型。

Result: 在R2R-CE和RxR-CE基准测试中，CorrectNav模型分别达到65.1%和69.3%的成功率，优于之前最佳模型。

Conclusion: Self-correction Flywheel范式显著提升了模型的错误校正能力和性能，适用于复杂环境。
