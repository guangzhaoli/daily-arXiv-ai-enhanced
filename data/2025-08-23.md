...
Motivation: 传统方法使用Soft-argmax作为可微近似，但作者发现这不是实现高性能的唯一途径，因此探索了替代方案。

Method: 提出了一种基于结构化预测框架的新训练目标，避免了Soft-argmax的使用。

Result: 在三个面部关键点检测基准（WFLW、COFW、300W）上实现了SOTA性能，训练收敛速度提升2.2倍，同时保持更好/竞争性精度。

Conclusion: 结构化预测框架是Soft-argmax的有效替代方案，能够显著提升训练效率并保持高性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络（CNNs）在处理复杂数据结构和关系时存在局限性，因此需要一种更有效的方法来建模图像中的像素或区域关系。

Method: 将图像表示为图结构，其中像素或区域作为顶点，并通过Delaunay三角剖分优化图的表示，结合GCNs和Voronoi图进行分类。

Result: 在多个基准数据集上，该方法在预处理效率和分类准确性方面均显著优于现有技术，尤其是在复杂场景和细粒度分类任务中。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖伪深度、CAD模型或多阶段级联，需要更简单的RGB-only解决方案。
...
Conclusion: YOPO证明了RGB-only方法在类别级9自由度姿态估计中的高效性，缩小了与RGB-D系统的差距。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型对抗攻击通常生成低质量图像且计算资源消耗大，扩散模型虽能高质量生成但需要数百步采样。

Method: TAIGen通过在混合步骤区间注入扰动，结合选择性RGB通道策略（红通道注意力图，绿蓝通道GradCAM引导扰动），保留图像结构的同时最大化目标模型的误分类。

Result: TAIGen在ImageNet上对ResNet、MNASNet和ShuffleNet的攻击成功率分别为70.6%、80.8%和97.8%，生成速度比现有方法快10倍。

Conclusion: TAIGen是最具影响力的攻击方法，防御机制对其生成的图像净化效果最差。
...
<details>
  <summary>Details</summary>
Motivation: 现有CVP方法多局限于掩码域，RGB域潜力未充分挖掘，RUN++旨在跨域应用可逆建模。

Method: RUN++将CVP任务建模为优化问题，展开为多阶段网络，包含CORE、CARE和FINE模块，结合扩散模型细化不确定区域。

Result: RUN++显著减少误报和漏报，高效处理模糊区域，并在真实退化条件下保持鲁棒性。

Conclusion: RUN++通过可逆建模和生成细化，为CVP任务提供了高效且鲁棒的解决方案。
...
Conclusion: GasTwinFormer是实时畜牧业排放监测的实用解决方案，通过消融研究验证了其有效性。
...
<details>
  <summary>Details</summary>
Motivation: Existing rectified flow models use linear trajectories, which may force generation through low-probability regions, potentially harming semantic alignment.

Method: CurveFlow incorporates curvature guidance into flow paths and uses curvature regularization to smooth trajectories.

Result: CurveFlow outperforms baselines on MS COCO datasets, achieving better semantic consistency (BLEU, METEOR, ROUGE, CLAIR) and image quality.

Conclusion: CurveFlow's curvature-aware approach enhances instruction-following ability and image generation quality.
...
<details>
  <summary>Details</summary>
Motivation: 现有无参考图像质量评估方法存在数据集偏差和依赖主观标签的问题，限制了其泛化性能。

Method: HiRQA结合排名和对比学习，提出高阶排名损失和嵌入距离损失，并通过结构化文本提示增强表示。

Result: HiRQA在合成和真实失真数据集上表现出色，泛化能力强，且轻量级版本HiRQA-S实现实时处理。

Conclusion: HiRQA在性能、泛化和可扩展性方面达到SOTA水平，适用于多种真实场景。
...
Method: 采用投资组合理论优化问题，并通过遗传算法快速求解，选择相机以应对时空相关中断。

Result: 实验表明，该方法在时空中断下优于传统基线策略，能保证可靠的3D重建质量。

Conclusion: 提出的相机选择策略在多视角3D重建中具有可靠性优势，适用于动态边缘环境。
...
Method: 整合医学视觉编码器和视觉语言大模型，采用多任务提示工程和多阶段微调技术。

Result: 在 DDR 数据集上达到 84.55% 的平衡准确率和 79.92% 的 F1 分数，概念检测表现优异。

Conclusion: XDR-LVLM 通过生成详细的诊断报告，填补了自动化诊断与临床需求之间的鸿沟。
...
Conclusion: MeSS方法有效提升了城市网格模型的纹理生成质量，适用于虚拟导航和自动驾驶场景。
...
Method: 采用学习基于的方法，为规则代理生成对抗行为，以测试其在失败场景中的表现。

Result: 对抗代理能够有效降低规则代理的累积奖励，证明了其对抗行为的有效性。

Conclusion: 该方法为安全关键应用中的规则代理测试提供了新思路，能够更全面地评估代理的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集和自动化方法仅提供二元（语义）掩码，限制了肾脏病理功能单元形态学量化的精确性。

Method: DyMorph-B2I结合了分水岭、骨架化和形态学操作，并引入自适应几何细化和可调超参数优化。

Result: 实验表明，该方法在分离粘附和异质结构方面优于传统方法，提升了实例分割和形态学分析的准确性。

Conclusion: DyMorph-B2I为肾脏病理学提供了更精确的实例分割工具，其开源实现有助于进一步研究和应用。
...
<details>
  <summary>Details</summary>
Motivation: 提高道路安全，通过低成本且易部署的行车记录仪视频预测事故，减少伤害和财产损失。

Method: 结合更好的时空特征，并通过循环网络聚合，改进现有的图神经网络。

Result: 在三个公开数据集上，STAGNet的平均精度和平均碰撞时间优于之前的方法。

Conclusion: STAGNet是一种高效且通用的交通事故预测模型。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D生成模型多局限于单一模态或3D结构，忽略了多模态数据的互补优势，限制了训练数据的范围。

Method: TriMM采用协作多模态编码整合不同模态特征，引入2D和3D辅助监督提升鲁棒性，并基于三平面潜在扩散模型生成高质量3D资产。

Result: 实验表明，TriMM在小规模训练数据下仍能媲美大规模数据集训练的模型，并在RGB-D数据集上验证了多模态数据整合的可行性。

Conclusion: TriMM通过有效利用多模态数据，显著提升了3D生成的质量和细节表现。
...
Method: 使用软原型对比模块计算类别原型，双重一致性学习模块对齐样本变换和邻域信息。
...
<details>
  <summary>Details</summary>
Motivation: 无人机在户外环境中导航时，由于轨迹长和机动性复杂，传统方法需要过多人工干预或详细指令。高低空协作可以兼顾多粒度视角和可控运动空间。

Method: 提出DuAl-VLN任务，高低空无人机分工协作：高空负责环境推理，低空负责精确导航。构建HaL-13k数据集，并提出AeroDuo框架，结合Pilot-LLM和多阶段策略。

Result: 构建了包含13,838条轨迹的HaL-13k数据集，并提出AeroDuo框架，通过高低空无人机协作实现高效导航。

Conclusion: DuAl-VLN任务和AeroDuo框架通过高低空协作，显著提升了无人机视觉与语言导航的性能和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成任务中表现优异，但其顺序生成过程耗时较长。现有方法（如DDIM）通过非马尔可夫过程减少采样步骤，但缺乏对原始扩散模型是否也能高效生成的理解。

Method: 引入跳过步采样机制，绕过中间去噪步骤，直接从预训练扩散模型中加速采样。该方法与标准扩散模型的训练目标一致。

Result: 在多个预训练扩散模型（如OpenAI ADM、Stable Diffusion等）上实验，证明该方法能以更少步骤实现高质量生成。
...
Method: 提出多功能编码框架统一多种编码模式，设计交互式编码代理通过上下文学习理解编码请求，并建立IIC-bench基准测试。

Result: 实验表明Comp-X能高效理解编码请求并保持压缩性能，展示了AGI在图像压缩中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有异常检测方法在病理学中面临计算限制、组织结构多样性和缺乏可解释性的问题。

Method: Ano-NAViLa基于预训练视觉-语言模型，加入轻量级可训练MLP，结合正常和异常病理知识。

Result: 在两个淋巴结数据集上，Ano-NAViLa在异常检测和定位中达到最先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法基于一对一监督策略，导致拓扑推理性能受限，RATopo通过冗余分配策略扩展监督范围。

Method: 改进Transformer解码器结构，交换交叉注意力和自注意力层，并实例化多个并行交叉注意力块。

Result: 在OpenLane-V2数据集上验证，RATopo显著提升了车道间及车道与交通元素的拓扑推理性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统专利分析依赖图像数据，但专利图像常缺乏全面视觉和语义信息，导致评估模糊。CLIP等视觉语言模型为专利分析提供了新机会。

Method: DesignCLIP结合类感知分类、对比学习、生成详细图像描述和多视图学习，处理专利数据特性。

Result: DesignCLIP在专利分类和检索任务中均优于基线及SOTA模型，并探索了多模态检索以激发设计创新。

Conclusion: 多模态方法在专利分析中具有潜力，DesignCLIP为设计专利应用提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 先天性心脏病（CHD）超声检测受图像噪声和探头位置变化影响，现有方法常忽略时间信息且缺乏预测校准。

Method: TPA通过图像编码器提取视频帧特征，用可训练时间提取器聚合特征以捕捉心脏运动，并通过对比损失对齐视频表示与文本提示。引入CVAESM模块量化不确定性。

Result: TPA在CHD诊断中达到85.40%的宏F1分数，校准误差显著降低；在EchoNet-Dynamic数据集上宏F1提升4.73%。

Conclusion: TPA整合时间建模、提示感知学习和不确定性量化，为CHD超声分类提供了高效可靠的框架。
...
<details>
  <summary>Details</summary>
Motivation: 传统多摄像头系统在实时3D轨迹跟踪中存在局限性，尤其是在篮球这种复杂场景中。

Method: 构建了BasketLiDAR数据集，并开发了基于LiDAR的实时跟踪和多模态跟踪算法。

Result: 实验表明，该方法在遮挡条件下仍能实现实时跟踪和优越性能。

Conclusion: BasketLiDAR为体育MOT领域提供了首个多模态数据集，解决了传统方法的实时性和精度问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法依赖大量训练和计算资源的问题，同时提升伪装目标检测的效率和性能。

Method: 分为两阶段：RAG生成粗掩码作为提示，SAM进行细化分割。通过无监督聚类构建检索数据库。

Result: 在基准数据集上表现优异，计算效率高，可在个人笔记本电脑上运行。

Conclusion: RAG-SEG提供了一种高效、实用的伪装目标检测解决方案，无需训练且性能优越。
...
<details>
  <summary>Details</summary>
Motivation: 由于文本到视频扩散模型可能被滥用生成有害或误导性内容，需要一种方法来控制这些内容的生成。

Method: VideoEraser通过选择性提示嵌入调整（SPEA）和抗对抗噪声引导（ARNG）两阶段过程，无缝集成到现有模型中。

Result: 实验表明，VideoEraser在四种任务中平均减少46%的不良内容生成，优于现有方法。

Conclusion: VideoEraser在抑制不良内容生成方面表现出色，具有高效性、完整性和鲁棒性。
...
Result: GRU在预测行人意图上优于LSTM，1D CNN在速度上表现最佳。

Conclusion: 研究为自动驾驶车辆提供了一种高效的行人意图预测方法，1D CNN适合实时应用。
...
<details>
  <summary>Details</summary>
Motivation: 三维物体检测在自动驾驶和机器人技术中至关重要，但需要有效融合相机和雷达的多模态数据。

Method: 提出RCDINO模型，利用DINOv2预训练模型的语义表示增强视觉特征，同时保持与基线架构的兼容性。

Result: 在nuScenes数据集上，RCDINO实现了雷达-相机模型中的最先进性能（56.4 NDS和48.1 mAP）。

Conclusion: RCDINO通过融合多模态数据和预训练模型，显著提升了三维物体检测的性能。
...
Method: 采用注意力敲除作为主要分析工具，设计了三种变体（时间、空间、语言到视频敲除），并在不同层数上应用，分为全局和细粒度两种设置。

Result: 研究发现视频信息提取主要在早期层完成，某些中间层是关键异常点，时空建模更依赖语言引导检索。这些发现可用于减少Video-LLMs的注意力计算。
...
<details>
  <summary>Details</summary>
Motivation: 传统微调方法需要更新所有模型参数，可能导致过拟合和计算成本高。BioTune旨在通过选择性微调层来解决这些问题。

Method: BioTune使用进化算法识别需要微调的层，优化目标任务的模型性能。

Result: 在九个图像分类数据集上的评估表明，BioTune在准确性和效率上优于或与现有方法（如AutoRGN和LoRA）相当。

Conclusion: BioTune通过减少可训练参数数量，降低了计算成本，提升了迁移学习效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有 3DGS 压缩方法在存储受限设备上的应用存在两个主要限制：压缩后文件仍较大且缺乏对存档后场景变化的适应机制。

Method: 提出 ICGS-Quantizer，通过联合利用高斯和属性相关性、共享码本以及基于图像的场景解码，提高压缩效率和适应性。

Result: ICGS-Quantizer 将 3DGS 存储需求降至千字节级，同时在压缩效率和场景变化适应性上优于现有方法。

Conclusion: ICGS-Quantizer 显著提升了 3DGS 的压缩效率和适应性，适用于大规模场景和长期存档需求。
...
<details>
  <summary>Details</summary>
Motivation: 驾驶场景中存在快速移动的车辆、行人和大规模静态背景，现有方法在解耦动态和静态组件时忽略了背景优化和几何关系，导致新视角渲染的鲁棒性和几何表示准确性不足。

Method: DriveSplat采用区域体素初始化方案（近、中、远区域分区）和可变形神经高斯模型，结合预训练模型的深度和法线先验进行监督。

Result: 在Waymo和KITTI数据集上验证，DriveSplat在新视角合成任务中达到了最先进的性能。

Conclusion: DriveSplat通过动态-静态解耦和几何优化，显著提升了驾驶场景重建的质量和准确性。
...
Method: 采用因果链建模分析RPM任务，设计DIO模型，并提出三种改进方法以克服其局限性。

Result: 实验表明DIO模型的优化目标无法有效学习人类推理逻辑。

Conclusion: 通过改进方法逐步提升模型在RPM任务中的表现，为机器智能的抽象推理能力提供新思路。
...
<details>
  <summary>Details</summary>
Motivation: 短视频内容兴起，现有方法难以捕捉全局时间依赖性和语义连贯性，且多通道特征融合易受噪声影响。

Method: 结合脉冲神经网络（SNN）提取关键帧，动态聚合图推理器解耦上下文对象一致性与语义连贯性，变分推理重建模块优化多通道特征分布。

Result: 在SumMe、TVSum、VideoXum和QFVS等多个数据集上表现优于现有方法。

Conclusion: SpiVG通过SNN和变分推理有效提升了视频摘要的信息密度和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 尽管MAEs在视觉基础模型中表现出色，但在新数据集上应用时需要大量超参数调优，且超参数与性能之间的关系尚未充分探索。

Method: 通过分析线性MAE学习的特征，推导出掩码比例和块大小如何选择捕捉短程和长程空间相关性的特征，并将分析扩展到非线性MAE。

Result: 研究表明，MAE表示能够适应数据集中的空间相关性，超越二阶统计量。

Conclusion: 提供了关于如何选择MAE超参数的实用见解。
...
Method: 设计了局部和全局时间运动融合模块，双向传播信息，并结合检测头和时空融合损失进行优化。
...
<details>
  <summary>Details</summary>
Motivation: 传统汽车损伤检测方法耗时、不可靠且难以检测微小缺陷，因此需要一种更快、更精确的自动化解决方案。

Method: 使用YOLOv8框架及其定制变体（YOLOv8m-t4和YOLOv8m-t42），通过定制数据集和实时数据增强技术训练模型。

Result: YOLOv8m-t42模型在检测微小缺陷时表现最佳，精度0.86，召回率0.84，F1分数0.85，mAP@0.5为0.60。

Conclusion: YOLOv8m-t42模型在检测微小凹痕方面具有更高的准确性和实用性，适合实时应用如自动化保险评估和汽车检测。
...
<details>
  <summary>Details</summary>
Motivation: 3DGS依赖密集多视角输入和精确相机姿态，但在实际场景中难以获取。稀疏视角下SfM初始化失败，导致渲染质量下降。

Method: 提出密集立体模块替代SfM，逐步估计相机姿态并重建全局密集点云；引入相干视图插值模块生成额外监督信号；采用多尺度拉普拉斯正则化和自适应空间感知几何正则化。

Result: 在极稀疏视角（仅2个训练视图）下，PSNR提升2.75dB，合成图像失真小且保留高频细节。

Conclusion: 该方法在稀疏视角下显著优于现有技术，实现了高质量的3D场景重建和渲染。
...
Result: 在六个公共数据集上，LGMSNet优于现有方法，并在零样本泛化测试中表现优异。

Conclusion: LGMSNet在资源受限的医疗场景中具有实际部署潜力。
...
<details>
  <summary>Details</summary>
Motivation: 多模态图像融合（MMIF）面临模态不对齐、高频细节破坏和任务特定限制等挑战。

Method: 提出AdaWAT进行频率解耦和空间-频率Mamba块实现高效融合。

Result: 在四种MMIF任务中表现优异，平衡性能和效率。

Conclusion: AdaSFFuse显著提升了多模态特征的融合效果，同时保持低计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 从记录的驾驶日志中合成外推视图对于自动驾驶车辆的模拟场景至关重要，但现有方法在几何一致性和渲染质量上存在不足。

Method: ExtraGS采用基于混合高斯-SDF设计的Road Surface Gaussian(RSG)表示和Far Field Gaussians(FFG)，并结合自监督不确定性估计框架。

Result: 在多个数据集和生成先验上的实验表明，ExtraGS显著提升了外推视图的真实性和几何一致性。

Conclusion: ExtraGS通过整合几何和生成先验，有效解决了外推视图的挑战，同时保持了高保真度。
...
Method: 采用两阶段流程：1）运动初始化，将草图分为语义组并定义关键帧；2）运动细化，通过基于组的位移网络（GDN）预测位移场，并利用文本到视频模型的先验。GDN还包含上下文条件特征增强模块（CCFE）以提高时间一致性。

Result: 实验表明，GroupSketch在多对象复杂草图的动画生成中显著优于现有方法，生成高质量且时间一致的动画。

Conclusion: GroupSketch扩展了草图动画的实际应用范围，为复杂多对象场景提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率遥感图像中的狭窄道路提取面临宽度有限、拓扑结构碎片化和频繁遮挡等挑战，需要一种专门的方法来解决这些问题。
...
Result: 在多种分布偏移基准测试中，ADAPT实现了最先进的性能，具有出色的可扩展性和鲁棒性。

Conclusion: ADAPT通过概率推断和轻量级正则化，解决了现有测试时自适应方法的局限性，为实际部署提供了高效可靠的解决方案。
...
Conclusion: 通过频率感知的像素权重分配，为解决频谱偏差问题提供了新思路。
...
TL;DR: 提出了一种新的线性时间凸松弛方法和承包商方法，显著提高了点云配准的速度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的全局最优方法（如SDP松弛）在解决点云配准问题时速度较慢，无法满足实时需求。

Method: 采用线性时间凸松弛和承包商方法加速分支定界（BnB）算法。

Result: 在旋转轴已知的情况下，能够在0.5秒内完成100个点的3D点云配准，速度比现有SDP求解器快两个数量级。

Conclusion: 该方法在保证全局最优性的同时，显著提高了计算效率，适用于高离群率场景。
...
TL;DR: 该研究通过结合相机陷阱和无人机热成像技术，利用深度学习模型自动识别野生动物和人类活动，揭示了尼泊尔奇特旺国家公园内人类与野生动物的冲突区域。


<details>
  <summary>Details</summary>
Motivation: 理解野生动物和人类活动的空间分布对于评估人类与野生动物的互动及制定有效的保护计划至关重要。

Method: 研究结合可见光和近红外相机陷阱以及热红外无人机图像，构建深度学习模型（如YOLOv11s和增强的Faster RCNN）进行目标检测，并进行空间模式分析。

Result: YOLOv11s模型在相机陷阱图像中表现最佳（精确度96.2%），无人机热成像提供了补充视角。空间分析揭示了潜在的冲突区域。
...
TL;DR: Grounded VideoDiT是一种视频LLM，通过DTL编码器、对象接地表示和混合令牌方案解决了现有模型在时间感知上的不足，并在多个基准测试中取得了先进成果。


<details>
  <summary>Details</summary>
Motivation: 现有视频LLM在时间感知上存在不足，如时间戳编码不明确、帧级特征连续性差、语言视觉对齐漂移。

Method: 引入DTL编码器增强边界敏感性，对象接地表示强化对齐，混合令牌方案提供显式时间戳建模。
...
TL;DR: 提出了一种弱监督方法，通过人类操作员对初始分割结果进行质量评分，训练评分模型并优化分割模型，显著提高了树木实例识别的准确性。


<details>
  <summary>Details</summary>
Motivation: 由于ALS数据的变化性和精确标注数据的高成本，需要一种更高效的方法来改进树木实例分割。

Method: 使用非微调模型或闭式算法生成初始分割结果，人类操作员进行质量评分，训练评分模型并反馈优化分割模型。

Result: 分割模型的树木实例识别准确率提高了34%，同时减少了非树木实例的预测。

Conclusion: 该方法在稀疏林区或复杂环境中仍存在挑战，但整体显著提升了分割性能。
...
Method: 通过关键特征引导（CFG）正则化对抗点云的搜索，优先破坏不同架构可能采用的关键特征，并在损失函数中约束最大偏差。

Result: 在ModelNet40和ScanObjectNN数据集上，CFG显著优于现有攻击方法。

Conclusion: CFG是一种高效且无需目标模型信息的对抗攻击方法，具有强迁移性和不可感知性。
...
TL;DR: 提出了一种名为MapKD的多级跨模态知识蒸馏框架，通过教师-教练-学生（TCS）范式，将多模态模型的知识迁移到轻量级视觉学生模型中，显著提升了性能和推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有在线高清地图构建方法依赖离线地图和多模态传感器，导致计算开销大且不灵活。

Method: 采用知识蒸馏策略，设计了教师-教练-学生（TCS）范式，并引入两种知识蒸馏策略（TGPD和MSRD）进行特征对齐和语义学习指导。

Result: 在nuScenes数据集上，学生模型的mIoU和mAP分别提升了6.68和10.94，同时加速了推理速度。

Conclusion: MapKD框架有效解决了多模态依赖问题，提升了轻量级视觉模型的性能，适用于自动驾驶系统。
...
TL;DR: 提出了一种名为CM2LoD3的新方法，通过冲突地图（CMs）和合成生成的语义冲突地图（SCMG）来自动重建LoD3建筑模型，提高了3D重建的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有的LoD1和LoD2建筑模型缺乏详细的立面元素（如窗户、门等），而LoD3模型虽然解决了这一问题，但其生成通常需要手动建模，难以大规模应用。

Method: 利用从射线到模型先验分析中获得的冲突地图（CMs），并结合合成的语义冲突地图（SCMG）进行语义分割。此外，通过置信度分数将纹理模型的分割结果与CMs融合，进一步提升分割性能。

Result: 实验结果表明，CM2LoD3方法在分割和重建建筑开口方面表现良好，结合纹理分割的不确定性感知融合后性能达到61%。
...
TL;DR: MDPR框架通过多维动态提示路由解决VLM微调中的类别不平衡问题，提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 预训练视觉语言模型（VLM）在微调时存在类别不平衡导致的偏差，现有方法忽视预训练中的固有偏差。

Method: 提出MDPR框架，构建五维视觉语义知识库，动态路由对齐全局类别并平衡细粒度语义。

Result: 在CIFAR-LT等长尾基准测试中达到SOTA水平，动态路由计算开销低。

Conclusion: MDPR是数据不平衡下VLM微调的高效灵活增强方法。
...
TL;DR: StreamMem是一种用于流式视频理解的查询无关KV缓存内存机制，通过动态压缩KV缓存并保持固定大小的KV内存，在长视频场景中实现高效问答。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法在长视频生成中因依赖RGB信号而导致的结构和运动误差累积问题。

Method: 联合预测感知条件和颜色信息，利用深度线索构建记忆库，并采用分段噪声调度。

Result: 在扩散和整流流模型中有效减少了时间漂移，提升了生成视频的保真度。

Conclusion: WorldWeaver通过统一的长时建模方案，显著改善了长视频生成的质量和一致性。
...
TL;DR: 提出了一种基于分子增强的All-in-SAM模型，通过结合分子导向学习和SAM适配器，显著提升了细胞分类性能，同时降低了标注工作量。


<details>
  <summary>Details</summary>
Motivation: 解决通用视觉基础模型在细粒度语义分割（如特定细胞亚型识别）中的不足，并减少对详细像素级标注的依赖。

Method: 采用全栈方法，包括分子增强学习、SAM适配器调整语义重点，以及分子导向纠正学习（MOCL）提升分割精度。

Result: 实验结果表明，All-in-SAM模型在细胞分类性能上显著提升，且对不同标注质量具有鲁棒性。

Conclusion: 该方法不仅减轻了标注负担，还扩展了精准生物医学图像分析在资源有限环境中的应用，推动了医学诊断和病理图像分析的自动化。
...
<details>
  <summary>Details</summary>
Motivation: 通过统一的框架提升视频生成的质量和效率，解决现有开源模型和商业解决方案在复杂运动和一致性方面的不足。

Method: 采用Hybrid Stream DiT架构增强模态对齐和训练收敛，建立数据筛选流程并使用MLLM视频质量模型过滤高质量样本。
...
TL;DR: ATLAS提出了一种高保真的人体模型，通过解耦形状和骨骼基础，提升了形状表达能力和身体属性的细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 现有的人体网格建模方法由于训练数据多样性不足和建模假设限制，难以捕捉多样姿势和形状的细节变化，且内部骨骼与外部软组织之间存在依赖关系。

Method: ATLAS基于600k高分辨率扫描数据，显式解耦形状和骨骼基础，将网格表示建立在人体骨骼上。

Result: ATLAS在拟合未见过的多样化姿势时表现更优，非线性姿势校正比线性模型更有效地捕捉复杂姿势。

Conclusion: ATLAS通过解耦形状和骨骼，实现了更高的形状表达能力和独立于软组织的关键点拟合，优于现有方法。
...
Method: 提出SceneGen框架，结合局部和全局场景信息的特征聚合模块，以及位置头，实现单次前向生成3D资产及其空间位置。

Result: 实验证明SceneGen在单图像和多图像输入下均表现高效且生成质量高。
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在图像编辑中存在全局去噪过程导致编辑区域与整个图像上下文纠缠的问题，而自回归模型通过序列化处理避免了这一问题。

Method: VAREdit将图像编辑重构为下一尺度预测问题，并引入Scale-Aligned Reference（SAR）模块，通过尺度匹配的调节信息提升编辑效果。

Result: VAREdit在编辑准确性和效率上均有显著提升，GPT-Balance得分比领先的扩散方法高30%+，且编辑速度更快。

Conclusion: VAREdit通过自回归框架和SAR模块，显著提升了图像编辑的精确性和效率，为指令引导的图像编辑提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 在现实应用中，用户通常需要一组多样化的输出，而独立采样往往导致结果冗余，限制了选择和探索。

Method: 将群体推理建模为二次整数分配问题，通过逐步修剪候选集来提高效率。
...
TL;DR: CineScale是一种新的推理范式，用于实现更高分辨率的视觉生成，支持8k图像和4k视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有视觉扩散模型因高分辨率数据缺乏和计算资源限制，难以生成高质量高分辨率内容，且现有方法易产生重复模式。

Method: 提出CineScale，针对不同视频生成架构设计专用变体，支持高分辨率I2V和V2V合成。

Result: 实验验证了CineScale在图像和视频模型高分辨率生成中的优越性，支持8k图像和4k视频生成。
...
### [66] [Exploring the Landscape of Non-Equilibrium Memories with Neural Cellular Automata](https://arxiv.org/abs/2508.15726)
*Ethan Lake,Ehsan Pajouheshgar*

Main category: cond-mat.stat-mech

TL;DR: 研究发现二维多体记忆的多样性远超预期，包括不同于Toom规则的新型记忆机制。


<details>
  <summary>Details</summary>
Motivation: 探索非平衡动力学中信息存储的多样性，揭示多体记忆的丰富物理现象。

Method: 结合严格证明和机器学习方法，分析二维记忆的动力学行为。

Result: 发现多种新型记忆机制，包括噪声依赖的信息存储和涨落稳定的有序相。

Conclusion: 多体记忆的物理机制比已知更丰富，为信息存储提供了新思路。
...
TL;DR: 论文提出了首个开源手术伤口数据集SurgWound，并建立了诊断基准，同时提出了三阶段学习框架WoundQwen，用于手术伤口诊断和报告生成。


<details>
  <summary>Details</summary>
Motivation: 手术部位感染（SSI）是常见且高成本的医疗相关感染，现有研究因数据隐私和专家标注成本高而受限，缺乏公开数据集和开源工具。

Method: 1. 发布SurgWound数据集；2. 建立手术伤口诊断基准；3. 提出三阶段学习框架WoundQwen，结合多模态大语言模型（MLLM）进行特征预测、诊断和报告生成。

Result: SurgWound数据集包含697张手术伤口图像和八种临床属性标注；WoundQwen框架能分析伤口特征并提供个性化护理建议。

Conclusion: 该研究填补了手术伤口筛查工具的空白，为个性化护理和及时干预提供了新途径。
...
### [68] [See it. Say it. Sorted: Agentic System for Compositional Diagram Generation](https://arxiv.org/abs/2508.15222)
*Hantao Zhang,Jingyang Liu,Ed Li*

Main category: cs.AI

TL;DR: 论文提出了一种无训练的代理系统，将视觉语言模型与大型语言模型结合，用于将手绘草图转换为可编辑的矢量图程序。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在生成逼真图像方面表现优异，但在流程图等需要空间精度和对齐的符号结构任务中表现不佳。

Method: 系统通过迭代循环，结合批评VLM、候选LLM和评判VLM，生成并选择最佳的SVG更新策略。

Result: 在10个流程图草图上，该方法比前沿闭源图像生成模型更准确地重建了布局和结构。

Conclusion: 该方法支持人类干预，输出为可编程SVG，易于扩展和定制。
...
### [69] [Self-supervised physics-informed generative networks for phase retrieval from a single X-ray hologram](https://arxiv.org/abs/2508.15530)
...
TL;DR: 提出了一种基于物理信息生成对抗网络的自学习方法，用于从单次强度测量中恢复相位和吸收信息，无需配对或模拟训练数据。
...
Method: 使用物理信息生成对抗网络从单次全息图中重建样本平面中的未传播波场的相位和吸收信息。
...
### [70] [A Vision-Based Shared-Control Teleoperation Scheme for Controlling the Robotic Arm of a Four-Legged Robot](https://arxiv.org/abs/2508.14994)
...
TL;DR: 提出一种基于视觉的直观远程控制方法，通过映射操作者手腕动作到机器人手臂，实现实时控制，并确保安全性。


<details>
  <summary>Details</summary>
Motivation: 在危险和远程环境中，四足机器人需要更安全和高效的操作方法，传统控制方式复杂且易碰撞。

Method: 利用外部摄像头和机器学习模型检测操作者手腕位置，映射到机器人手臂命令，结合轨迹规划避免碰撞。
...
### [71] [Decentralized Vision-Based Autonomous Aerial Wildlife Monitoring](https://arxiv.org/abs/2508.15038)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器人解决方案多从群体角度出发或依赖人工操作，难以大规模应用。

Method: 开发了新颖的基于视觉的协调和跟踪算法，适用于动态、非结构化环境，无需集中通信或控制。

Result: 通过真实环境实验验证了系统在多样化野外条件下的可靠部署。
...
### [72] [Lang2Lift: A Framework for Language-Guided Pallet Detection and Pose Estimation Integrated in Autonomous Outdoor Forklift Operation](https://arxiv.org/abs/2508.15427)
*Huy Hoang Nguyen,Johannes Huemer,Markus Murschitz,Tobias Glueck,Minh Nhat Vu,Andreas Kugi*

Main category: cs.RO

TL;DR: Lang2Lift利用基础模型实现自然语言引导的托盘检测与6D姿态估计，解决了户外环境中托盘自动化搬运的难题。


<details>
  <summary>Details</summary>
Motivation: 劳动力短缺、安全隐患以及手动定位和检索托盘的效率低下，推动了自动化托盘搬运的需求。

Method: 结合Florence-2、SAM-2和FoundationPose，实现语言引导的分割与姿态估计，并通过运动规划模块实现全自动叉车操作。

Result: 在真实测试数据集上达到0.76 mIoU的分割准确率，验证了系统在物流和建筑环境中的可行性。

Conclusion: Lang2Lift展示了在复杂户外环境中自动化托盘搬运的潜力，具备实际部署的可行性。
...
### [73] [On the Effectiveness of Graph Reordering for Accelerating Approximate Nearest Neighbor Search on GPU](https://arxiv.org/abs/2508.15436)
*Yutaro Oguri,Mai Nishimura,Yusuke Matsui*

Main category: cs.IR

TL;DR: 本文首次系统研究了GPU上基于图的近似最近邻搜索（ANNS）的图重排序效果，提出了一种统一的评估框架，通过优化内存布局实现了15%的查询性能提升。
...
Method: 开发了一个统一的评估框架，包括图适配器和GPU优化的图遍历引擎，用于全面评估不同图索引的多样化重排序策略。

Result: 在保持搜索准确性的前提下，GPU优化的重排序策略实现了高达15%的查询性能提升。

Conclusion: 内存布局优化与现有算法创新正交，为未来研究提供了新的方向，所有代码将开源以促进可复现性。
...
### [74] ["Does the cafe entrance look accessible? Where is the door?" Towards Geospatial AI Agents for Visual Inquiries](https://arxiv.org/abs/2508.15752)
...
TL;DR: Geo-Visual Agents是一种多模态AI代理，通过分析地理空间图像和传统GIS数据，回答关于世界的视觉空间问题。


<details>
  <summary>Details</summary>
Motivation: 现有交互式数字地图依赖预结构化GIS数据，无法满足对世界外观的视觉空间问题需求。

Method: 结合地理空间图像（如街景、照片、卫星图像）和传统GIS数据，定义Geo-Visual Agents的愿景、感知与交互方法。

Result: 提供了三个示例，并列举了未来研究的关键挑战与机遇。

Conclusion: Geo-Visual Agents有望通过多模态分析扩展数字地图的功能，解决更复杂的视觉空间问题。
...
### [75] [End-to-End Agentic RAG System Training for Traceable Diagnostic Reasoning](https://arxiv.org/abs/2508.15746)
*Qiaoyu Zheng,Yuze Sun,Chaoyi Wu,Weike Zhao,Pengcheng Qiu,Yongguo Yu,Kun Sun,Yanfeng Wang,Ya Zhang,Weidi Xie*

Main category: cs.CL

TL;DR: Deep-DxSearch是一种基于强化学习的检索增强生成系统，用于医疗诊断，通过端到端训练提升诊断准确性和可追溯性。


<details>
  <summary>Details</summary>
Motivation: 医疗大语言模型在诊断中存在知识鸿沟和幻觉问题，现有检索增强方法对外部知识的利用不足且反馈推理可追溯性差。

Method: 构建大规模医疗检索语料库，将LLM作为核心代理，通过强化学习训练优化检索、推理结构和诊断准确性。

Result: Deep-DxSearch在多个数据中心表现优于其他方法，诊断准确性显著提升，尤其在罕见病诊断中表现突出。

Conclusion: Deep-DxSearch通过强化学习和检索增强设计，显著提升了医疗诊断的准确性和可解释性，为临床提供了更可靠的初步诊断支持。
...
### [76] [Fusing Structural Phenotypes with Functional Data for Early Prediction of Primary Angle Closure Glaucoma Progression](https://arxiv.org/abs/2508.14922)
...
TL;DR: 该研究通过结合视神经头（ONH）结构特征和视野（VF）功能参数，使用机器学习分类器对原发性闭角型青光眼（PACG）患者的进展速度进行分类。


<details>
  <summary>Details</summary>
Motivation: 旨在提高对PACG患者进展速度的分类准确性，以优化临床监测和治疗策略。

Method: 研究纳入超过5年随访的PACG患者，通过AI分割OCT数据提取31个ONH参数，并结合VF区域敏感性训练多种机器学习模型。

Result: 随机森林模型结合结构和功能特征表现最佳（AUC=0.87），SHAP分析确定了6个关键预测因子。

Conclusion: 结合ONH结构和VF功能参数显著提高了PACG进展风险的分类准确性，强调了ONH形态在疾病监测中的重要性。
...
TL;DR: 本文揭示了概念擦除技术（CETs）的局限性，包括易被规避和副作用，并提出了一个评估基准（SEE）来量化这些副作用。


<details>
  <summary>Details</summary>
Motivation: 研究CETs在隐私、版权和安全方面的应用时，发现其易被规避且存在副作用，因此需要系统性评估其鲁棒性。

Method: 提出了SEE评估基准，包含分层和组合提示，通过自动化评估管道量化CETs的副作用，包括对邻近概念的影响、目标规避和属性泄漏。

Result: 实验表明CETs可通过超类-子类层次结构和语义相似提示规避，且存在属性泄漏和注意力集中/分散现象。

Conclusion: CETs的局限性需进一步研究，作者发布了数据集和工具以支持未来工作。
...
TL;DR: 该论文探讨了生成模型中基于似然的OOD检测方法的有效性，并验证了在表示空间中该方法的表现。


<details>
  <summary>Details</summary>
Motivation: 尽管AI发展迅速，但安全性仍是部署机器学习系统的主要瓶颈。OOD检测是关键的安全组件，而数据似然是最自然的OOD评分方法。然而，先前的研究表明似然方法在实践中常失败，引发对其有效性的质疑。

Method: 作者训练了一个变分扩散模型，不是基于图像，而是基于预训练的ResNet-18的表示空间，以评估基于似然的检测器性能，并与OpenOOD套件中的最先进方法进行比较。

Result: 研究发现，在表示空间中，基于似然的OOD检测方法表现优于或与现有方法相当。

Conclusion: 论文表明，似然方法在表示空间中仍具有实用性，其失败可能更多源于像素空间的限制，而非密度估计本身的问题。
...
Method: 采用多模态专家混合（MoE）架构，结合离线与在线强化学习（RL）训练，提出混合奖励（MoR）方法。

Result: 在通用推理任务中表现优异，科学领域超越开源模型，部分任务超越闭源模型。

Conclusion: Intern-S1为科学领域提供了强大的开源模型支持，推动了AGI的发展。
...
### [80] [\textit{adder-viz}: Real-Time Visualization Software for Transcoding Event Video](https://arxiv.org/abs/2508.14996)
*Andrew C. Freeman,Luke Reinkensmeyer*

Main category: cs.MM

TL;DR: 论文介绍了对ADΔER事件视频表示的改进，并发布了用于实时事件转码和可视化应用的adder-viz软件。


<details>
  <summary>Details</summary>
Motivation: 解决现有事件视频表示在灵活性、速度和压缩性方面的局限性。

Method: 改进ADΔER表示，并开发adder-viz软件用于实时事件转码和可视化。

Result: 发布了MIT许可的adder-viz软件，可从GitHub获取。

Conclusion: ADΔER表示及其改进工具为事件视频研究提供了更高效的解决方案。
...
### [81] [Scalable Event-Based Video Streaming for Machines with MoQ](https://arxiv.org/abs/2508.15003)
*Andrew C. Freeman*

Main category: eess.IV

TL;DR: 该论文探讨了基于事件的视频系统，提出了一种新的低延迟事件流格式，以解决传统视频流与新型神经形态传感器数据之间的传输问题。


<details>
  <summary>Details</summary>
Motivation: 传统视频流采用有损压缩和速率自适应技术，而新型神经形态传感器以异步像素采样记录视频，主要用于计算机视觉应用。目前研究多关注应用开发，忽略了数据传输问题。

Method: 论文调查了基于事件的视频系统现状，结合近期可扩展事件流工作，提出了一种基于Media Over QUIC协议草案的低延迟事件流格式。

Result: 提出了一种新的低延迟事件流格式，适用于神经形态传感器数据的高效传输。

Conclusion: 该研究填补了神经形态传感器数据传输的空白，为未来相关应用提供了技术基础。
...
### [82] [Zero-shot Volumetric CT Super-Resolution using 3D Gaussian Splatting with Upsampled 2D X-ray Projection Priors](https://arxiv.org/abs/2508.15151)
...
<details>
  <summary>Details</summary>
Motivation: 高分辨率CT成像受限于辐射风险，现有监督方法需要大量配对数据，而零样本方法难以恢复精细解剖细节。

Method: 利用扩散模型生成高分辨率2D X射线投影先验，结合3D高斯溅射和负alpha混合技术进行3D CT重建。

Result: 在两个数据集上的实验表明，该方法在定量和定性上均优于现有方法。

Conclusion: 该方法通过结合外部先验和残差学习，有效提升了零样本3D CT超分辨率的效果。
...
### [83] [Pathology-Informed Latent Diffusion Model for Anomaly Detection in Lymph Node Metastasis](https://arxiv.org/abs/2508.15236)
*Jiamu Wang,Keunho Byeon,Jinsol Song,Anh Nguyen,Sangjeong Ahn,Sung Hak Lee,Jin Tae Kwak*

Main category: eess.IV

TL;DR: 提出了一种结合视觉语言模型和扩散模型的非监督异常检测方法，用于数字病理学中的异常检测。


<details>
  <summary>Details</summary>
Motivation: 数字病理学中数据标注稀缺，非监督异常检测方法可以避免对大量标注数据的依赖。

Method: 利用视觉语言模型和扩散模型，结合病理学提示进行重建，以区分正常和异常组织。

Result: 在胃淋巴结数据集和公共乳腺淋巴结数据集上验证了方法的有效性，展示了跨器官的泛化能力。

Conclusion: 该方法在数字病理学中具有潜力，能够有效进行非监督异常检测。
...
### [84] [Explainable Knowledge Distillation for Efficient Medical Image Classification](https://arxiv.org/abs/2508.15251)
*Aqib Nazir Mir,Danish Raza Rizvi*

Main category: eess.IV

TL;DR: 该研究探索了基于知识蒸馏的COVID-19和肺癌分类框架，使用高容量教师模型指导轻量级学生模型的训练，平衡了准确性和计算效率。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决医疗AI在资源受限环境中的实际应用问题，通过知识蒸馏实现高效且可解释的分类模型。

Method: 采用VGG19和轻量级Vision Transformers作为教师模型，指导基于OFA-595超网的学生模型训练，结合混合监督和Score-CAM可视化。

Result: 蒸馏后的学生模型在保持高分类性能的同时，显著减少了参数和推理时间。

Conclusion: 研究强调了模型效率与可解释性结合的重要性，为医疗AI提供了实用且可信的解决方案。
...
### [85] [Bladder Cancer Diagnosis with Deep Learning: A Multi-Task Framework and Online Platform](https://arxiv.org/abs/2508.15379)
*Jinliang Yu,Mingduo Xie,Yue Wang,Tianfan Fu,Xianglai Xu,Jiajun Wang*

Main category: eess.IV

TL;DR: 提出了一种基于深度学习的多任务框架，用于膀胱癌诊断，包括分类、分割和分子分型，并通过在线平台实现高效、准确的临床诊断。
...
Method: 结合EfficientNet-B0、ResNet34-UNet++和ConvNeXt-Tiny模型，设计多任务框架，并开发Gradio在线平台。

Result: 分类准确率93.28%，F1-score 82.05%，AUC 96.41%；分割Dice系数0.9091。

Conclusion: 该框架和平台提升了膀胱癌诊断的可靠性和效率，推动了AI辅助决策在泌尿学中的应用。
...
### [86] [DoSReMC: Domain Shift Resilient Mammography Classification using Batch Normalization Adaptation](https://arxiv.org/abs/2508.15452)
...
TL;DR: DoSReMC通过仅微调BN和FC层，提升乳腺X光分类的跨域泛化能力，无需重新训练整个模型。


<details>
  <summary>Details</summary>
Motivation: 解决AI在乳腺X光分类中因域偏移导致的性能下降问题，以促进AI在临床中的安全公平应用。

Method: 提出DoSReMC框架，通过微调BN和FC层并结合对抗训练，增强跨域泛化能力。

Result: 实验证明BN层是域依赖的主要来源，DoSReMC显著提升了跨域性能。

Conclusion: DoSReMC为构建更鲁棒和通用的乳腺X光分类系统提供了实用方案。
...
### [87] [Deep Equilibrium Convolutional Sparse Coding for Hyperspectral Image Denoising](https://arxiv.org/abs/2508.15553)
*Jin Ye,Jingran Wang,Fengchao Xiong,Jingzhou Chen,Yuntao Qian*

Main category: eess.IV

TL;DR: 提出了一种基于深度平衡卷积稀疏编码（DECSC）的高光谱图像去噪方法，结合局部空间-光谱相关性、非局部空间自相似性和全局空间一致性，显著提升了去噪性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像（HSIs）常受复杂噪声影响，现有深度展开方法缺乏收敛保证，而深度平衡（DEQ）模型通过固定点问题自然符合优化需求。

Method: 在卷积稀疏编码（CSC）框架下，结合2D和3D卷积稀疏表示，嵌入Transformer块利用非局部自相似性，并通过细节增强模块提升图像细节保留。

Result: 实验表明，DECSC方法在去噪性能上优于现有先进方法。

Conclusion: DECSC框架通过统一多种特征表示和优化方法，为高光谱图像去噪提供了高效且收敛性强的解决方案。
...
### [88] [Are Virtual DES Images a Valid Alternative to the Real Ones?](https://arxiv.org/abs/2508.15594)
*Ana C. Perre,Luís A. Alexandre,Luís C. Freire*

Main category: eess.IV

TL;DR: 研究探讨了三种模型（预训练U-Net、端到端训练U-Net和CycleGAN）用于从低能量图像生成虚拟双能量减影图像（DES），并评估其对CESM分类的影响。预训练U-Net表现最佳，但真实DES图像仍更优。


<details>
  <summary>Details</summary>
Motivation: 通过图像生成技术减少患者辐射暴露，同时评估虚拟DES图像在CESM分类中的可行性。

Method: 使用三种模型生成虚拟DES图像，并比较其与真实DES图像在恶性与非恶性分类中的性能。

Result: 预训练U-Net的F1分数为85.59%，低于真实DES图像的90.35%，但虚拟DES图像潜力显著。

Conclusion: 虚拟DES图像生成具有临床潜力，未来技术改进可能缩小与真实图像的差距。
...
### [89] [Label Uncertainty for Ultrasound Segmentation](https://arxiv.org/abs/2508.15635)
...
TL;DR: 论文提出了一种利用专家提供的像素级置信度值来标注和训练AI模型的新方法，以提高医学影像分割的准确性和临床实用性。


<details>
  <summary>Details</summary>
Motivation: 医学影像中，放射科医生之间的观察差异导致标签不确定性，尤其是在视觉解释主观性强的模态中（如肺部超声）。传统的绝对标注方法无法捕捉这种不确定性。

Method: 设计了一种数据标注协议，记录放射科医生对每个标注区域的置信度，并在训练过程中利用这些置信度值。通过不同置信度阈值的实验，验证了高阈值（如60%）的有效性。

Result: 实验表明，利用置信度值训练模型可以提高分割性能，并显著改善下游临床任务（如S/F氧合比估计、分类和30天患者再入院预测）的表现。
...
### [90] [Hessian-based lightweight neural network for brain vessel segmentation on a minimal training dataset](https://arxiv.org/abs/2508.15660)
*Alexandra Bernadotte,Elfimov Nikita,Mikhail Shutov,Ivan Menshikov*

Main category: eess.IV

TL;DR: 提出了一种名为HessNet的轻量级半监督神经网络，用于3D脑部血管分割，显著降低了资源需求并达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 当前脑部血管分割主要依赖手动或传统方法（如Frangi滤波器），准确性不足，且缺乏公开的详细标注数据集。

Method: 采用基于Hessian矩阵的轻量级神经网络（仅6000参数），支持CPU运行，结合半监督学习。

Result: 在少量训练数据上达到先进分割精度，并基于IXI数据集创建了200张半手动标注的脑部血管数据集。

Conclusion: HessNet高效且准确，支持专家专注于复杂病例，数据集已公开。
...
### [91] [Scalable FPGA Framework for Real-Time Denoising in High-Throughput Imaging: A DRAM-Optimized Pipeline using High-Level Synthesis](https://arxiv.org/abs/2508.14917)
*Weichien Liao*
...
<details>
  <summary>Details</summary>
Motivation: 高通量成像工作流（如PRISM）产生的数据速率超过传统实时处理能力，需要低延迟解决方案。

Method: 通过高级综合（HLS）实现FPGA预处理流水线，优化DRAM缓冲，利用突发模式AXI4接口减少延迟。

Result: 该架构在帧间隔内完成去噪，减少数据集大小，适用于PRISM规模采集。

Conclusion: 模块化FPGA框架为光谱和显微镜中的延迟敏感成像工作流提供了实用解决方案。
