...
TL;DR: DES是一种动态调整对抗训练扰动预算的框架，通过结合决策边界距离、预测置信度和模型不确定性，显著提升了对抗鲁棒性和标准准确性。
...
Method: DES通过梯度代理、softmax熵和蒙特卡洛dropout动态调整每个实例和训练迭代的扰动预算。
...
Method: 采用两阶段框架：推理阶段生成区域建议，分割阶段通过视觉语言分割模块优化分割结果。

Result: 在ReasonSeg和SegInW数据集上达到SOTA性能，分别提升6.5 gIoU和9.2 cIoU，零样本设置下mAP为49.7。
...
TL;DR: MMRB是首个评估多图像结构化视觉推理的基准，包含92个子任务，覆盖空间、时间和语义推理，并展示了开源MLLMs在多图像推理任务中显著落后于商业模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要关注单图像推理或多图像理解任务，缺乏对多图像输入推理能力的深入评估。

Method: 提出MMRB基准，包含92个子任务，使用GPT-4o生成多解法和CoT风格标注，并设计句子级匹配框架支持快速评估。
...
TL;DR: 提出了一种弱监督流程，通过图像扩散模型生成可控属性的真实人类图像数据集，并利用基于Transformer的架构将图像特征映射到3D点云，最后训练点云扩散模型，显著提升了3D人类生成的速度、文本对齐性和渲染质量。
...
Method: 1. 使用图像扩散模型生成可控属性的真实人类图像数据集；2. 提出基于Transformer的图像特征到3D点云的映射方法；3. 训练点云扩散模型。
...
TL;DR: ReXVQA是胸部放射学视觉问答（VQA）的最大综合基准，包含约696,000个问题和160,000张胸部X光片，评估了8种多模态大语言模型，其中MedGemma表现最佳（83.24%准确率），甚至超过放射科住院医师（77.27%）。


<details>
  <summary>Details</summary>
Motivation: 现有VQA基准多依赖模板化查询，缺乏临床真实性。ReXVQA旨在提供多样化的临床任务，评估放射学推理能力，推动AI系统模拟专家级临床推理。

Method: 构建包含696,000问题和160,000胸部X光片的数据集，涵盖五种放射学推理技能。评估8种多模态大语言模型，并与3名放射科住院医师进行对比研究。

Result: MedGemma以83.24%准确率领先其他模型，在人类对比研究中表现优于放射科住院医师（83.84% vs 77.27%）。

Conclusion: ReXVQA为评估放射学AI系统设定了新标准，为下一代能模拟专家级临床推理的AI系统奠定基础。数据集将开源。
...
Method: 通过区分正确的动作或动作序列与反事实干扰项，评估不同类型的世界模型和规划器。
...
TL;DR: PLUCC是一种利用球员行为上下文线索进行冰球定位的新方法，显著提升了检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 提高孟加拉手语（BdSL）识别的准确性和可扩展性，以改善听力障碍者的沟通无障碍性。

Method: 使用视频Transformer架构（VideoMAE、ViViT、TimeSformer）进行微调，应用数据增强技术（随机裁剪、水平翻转等），并采用10折分层交叉验证和独立用户评估。

Result: VideoMAE在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，显著优于传统方法。

Conclusion: 视频Transformer模型在孟加拉手语识别中表现出色，性能受数据集规模、视频质量和模型架构等因素影响。
...
TL;DR: 论文提出了一种方法，通过激活最大化技术解释DNN编码模型，生成能驱动特定脑区反应的图像，并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有DNN编码模型虽能预测脑响应，但难以解释具体特征。本文旨在通过激活最大化技术揭示驱动脑响应的视觉特征。

Method: 使用预训练的Inception V3网络提取特征，通过线性回归预测fMRI响应，并应用激活最大化生成优化图像。

Result: 生成的图像能可靠驱动目标脑区活动，且与已知选择性一致。

Conclusion: 激活最大化可成功应用于DNN编码模型，为视觉系统的响应特征提供了灵活的表征和调控方法。
...
TL;DR: 研究发现，基于扰动的图像保护方法无法完全阻止扩散模型的编辑，反而可能增强图像与文本提示的关联性。


<details>
  <summary>Details</summary>
Motivation: 探讨现有基于扰动的图像保护方法在防止扩散模型滥用方面的有效性。

Method: 通过实验评估多种扰动方法在不同领域和编辑任务中的表现。

Result: 扰动保护未能完全阻止编辑，反而可能改善编辑效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过精心归一化以避免失真，而深度学习中的卷积滤波器缺乏此类约束，导致在大气传输中响应失真。

Method: 提出滤波器归一化方法，结合可学习的缩放和平移，确保滤波器的大气等变性。

Result: 在人工和自然强度变化基准测试中取得显著改进，ResNet34甚至大幅超越CLIP。

Conclusion: 滤波器归一化不仅提升了性能，还正则化学习、促进多样性，增强了鲁棒性和泛化能力。
...
TL;DR: HMAR是一种新的图像生成算法，通过改进VAR的并行生成问题，提高了图像质量和生成速度。
...
Method: HMAR采用马尔可夫过程和多步掩码生成技术，仅依赖前一分辨率尺度的信息生成图像。
...
TL;DR: 论文研究了使用自我中心设备进行高动态范围场景真实感重建的挑战，提出了视觉惯性束调整（VIBA）和基于高斯泼溅的物理图像形成模型，显著提升了重建质量。
...
Method: 提出使用VIBA校准滚动快门RGB相机的高频轨迹，并引入基于高斯泼溅的物理图像形成模型。

Result: 实验表明，VIBA和图像形成模型分别带来+1 dB的PSNR提升。

Conclusion: 该方法在多种光照条件下均表现优异，适用于广泛使用的高斯泼溅表示变体。
...
TL;DR: 提出了一种称为“面部去正面化”的新方法，通过增强训练数据集来改进极端头部姿态下的人脸识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖复杂的面部正面化技术，但在实际应用中不实用且在小数据集上评估。
...
Conclusion: 当前方法可能在小数据集上过拟合，去正面化方法在大规模数据集上更有效。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR 3D检测方法依赖稀疏卷积或Transformer，在边缘设备上运行效率低。
...
TL;DR: AuthGuard结合语言指导和视觉编码器，提升深度伪造检测的泛化能力和准确性。
...
Method: 结合分类和图像-文本对比学习，利用MLLMs生成文本指导，并集成数据不确定性学习。

Result: 在多个数据集上取得SOTA性能，AUC提升显著。
...
Result: 在高FLOPs减少下保持或提升准确率，ResNet56和ResNet110分别减少86.37%和95.82% FLOPs，碳排放减少83.31%。

Conclusion: 该方法为剪枝领域开辟新方向，实现高效、环保的模型优化。
...
TL;DR: 提出EECD-Net方法，结合SRCNN、SCU和GAT模块，显著提升道路裂缝检测的准确性和能效。


<details>
  <summary>Details</summary>
Motivation: 智能终端设备因能量有限和低分辨率成像难以实现实时监测，需提升检测性能和能效。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过间接方式（如专用损失函数）提升SISR性能，而LSKs旨在直接优化图像频率成分。

Method: LSKs是形式上的秩一矩阵，可分解为正交且可合并的一维核，显著减少参数和计算量。
...
TL;DR: RACRO是一种通过强化学习优化视觉描述生成的方法，以支持多模态大语言模型的高效推理，无需昂贵的重新训练。
...
Method: 提出RACRO方法，通过推理目标引导的强化学习优化视觉描述生成，实现感知与推理的解耦。

Result: 在数学和科学多模态基准测试中，RACRO实现了最先进的性能，并支持更高级推理模型的即插即用。

Conclusion: RACRO通过优化视觉描述生成，显著提升了多模态推理的效率和性能，同时降低了成本。
...
TL;DR: 提出了一种轻量级全局建模网络LGM-Pose，通过MobileViM Block和LARM模块提取全局信息，结合SFusion模块整合多尺度信息，在减少参数量的同时提升了性能和处理速度。


<details>
  <summary>Details</summary>
Motivation: 当前多分支并行CNN架构难以捕捉全局上下文且延迟高，需要一种更高效的轻量级方法。

Method: 设计了MobileViM Block和LARM模块，结合NPT-Op提取全局信息；引入SFusion模块整合多尺度信息。

Result: 在COCO和MPII数据集上，参数更少且性能更优，处理速度更快。

Conclusion: LGM-Pose通过单分支结构和创新模块，有效解决了全局建模和多尺度整合问题，实现了高效轻量化的姿态估计。
...
Method: 通过深度点云渲染生成不可见性掩码，结合用户编辑掩码构建复合掩码数据集，采用自迭代调优策略处理大相机运动下的时间一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法主要关注基于人类命令执行低级动作，缺乏对高级规划和多级知识学习的能力。

Method: 提出HEC框架，将学习过程分为高级指令和低级动作两层，并定义五个子任务。Task-aware MoILE方法通过聚类视觉-文本嵌入实现任务识别，并使用任务级和令牌级路由器选择LoRA专家。通过SVD保留关键参数以减少灾难性遗忘。

Result: 实验表明，该方法在减少旧任务遗忘方面优于其他方法，有效支持代理在持续学习新任务时保留先验知识。

Conclusion: HEC和Task-aware MoILE方法在分层持续学习中表现出色，解决了灾难性遗忘问题，支持多级知识学习。
...
TL;DR: 提出了一种名为Perfecting Depth的两阶段框架，用于传感器深度增强，结合随机不确定性建模与确定性细化，生成高质量深度图。


<details>
  <summary>Details</summary>
Motivation: 传感器深度测量常存在不可靠区域，需要一种方法自动检测并修复这些区域，同时保留几何线索。

Method: 框架分为两阶段：1) 随机估计阶段，利用扩散模型的随机性检测不可靠区域并推断几何结构；2) 确定性细化阶段，利用不确定性图进行结构一致性和像素级精度优化。

Result: 实验证明该方法在多种真实场景中有效，生成密集、无伪影的深度图，并验证了其鲁棒性和可扩展性。
...
TL;DR: 这篇论文综述了深度学习如何逐步改变传统的图像匹配流程，包括替换传统步骤和合并为端到端模块，并评估了代表性方法，讨论了未来研究方向。
...
Method: 论文采用分类法，分为替换传统步骤（如可学习的检测器-描述符）和合并为端到端模块（如稀疏匹配器），并评估了代表性方法。

Result: 通过基准测试，论文展示了深度学习在相对位姿恢复、单应性估计和视觉定位任务中的优势。

Conclusion: 论文总结了深度学习驱动的图像匹配策略，提供了清晰的领域发展概览，并指出了未来创新的关键方向。
...
TL;DR: STARE是一个评估多模态大语言模型在空间认知任务中表现的基准，发现模型在复杂任务中表现不佳，而人类通过视觉模拟显著提升效率。


<details>
  <summary>Details</summary>
Motivation: 现有AI基准主要关注语言推理，忽视了非语言、多步视觉模拟的复杂性，因此需要专门评估空间认知能力的基准。

Method: STARE包含4K个任务，涵盖几何变换、空间推理和现实场景，通过对比人类和模型的表现进行分析。

Result: 模型在简单2D任务中表现良好，但在复杂3D任务中接近随机水平；人类通过视觉模拟显著提升速度和准确性。

Conclusion: 模型在利用视觉信息进行多步推理方面存在不足，需进一步优化以提升空间认知能力。
...
TL;DR: TADiSR是一种基于扩散模型的超分辨率框架，通过文本感知注意力和联合分割解码器提升文本区域的保真度。


<details>
  <summary>Details</summary>
Motivation: 解决生成模型在图像超分辨率中文本结构失真的问题。

Method: 结合文本感知注意力和联合分割解码器，提出合成高质量图像的完整流程。

Result: 显著提升超分辨率图像中文本的可读性，并在多个评估指标上达到最优性能。
...
<details>
  <summary>Details</summary>
Motivation: 扩散生成模型在高质量视频生成中表现优异，但推理速度慢和计算需求高限制了实际部署。量化和稀疏化可以独立加速推理，但现有方法缺乏联合优化导致性能下降。

Method: FPSAttention通过统一的3D分块粒度、去噪步骤感知策略和硬件友好的内核设计，实现了FP8量化和稀疏化的协同优化。

Result: 在720p分辨率下，FPSAttention实现了注意力操作7.09倍和端到端视频生成4.96倍的加速，且生成质量未下降。
...
TL;DR: 本文提出了一种结合特征提取和对象分割的方法，将群分解理论应用于更现实的场景，以解决传统表示学习无法处理条件独立性的问题。


<details>
  <summary>Details</summary>
Motivation: 传统表示学习假设解耦的独立特征轴是好的表示，但无法解释条件独立性。本文旨在通过群分解理论改进表示学习，使其更适用于现实世界场景。

Method: 结合特征提取和对象分割，将像素平移替换为特征平移，并将对象分割视为同一变换下的特征分组。

Result: 在包含真实世界对象和背景的数据集上验证了方法的有效性。

Conclusion: 该方法有助于更好地理解人类在现实世界中对象识别的发展。
...
Method: 结合Mixture of LoRA Experts (MoLE)和跨模块概念指导，实现参数高效适应和特征对齐；引入多层次特征保留策略和几何感知概念判别损失。

Result: 在六个基准数据集上，相对现有方法提升了4.2%-8.7%的5-way 5-shot分类性能。
...
TL;DR: Gen-n-Val 是一个新型的数据生成框架，利用 Layer Diffusion、LLMs 和 VLLMs 生成高质量的单对象掩码和多样背景，显著减少无效数据并提升模型性能。
...
Method: Gen-n-Val 包含两个代理：LD 提示代理（LLM）优化提示以生成高质量前景实例图像和分割掩码；数据验证代理（VLLM）过滤低质量合成数据。
...
Result: MARS在不同场景和天线位置下均优于基线模型，MSE和SSIM表现更优，计算成本低。
...
TL;DR: 论文提出了HoliSafe数据集和SafeLLaVA模型，解决了现有视觉语言模型（VLMs）安全性的不足，通过全面的数据集和创新的架构提升了模型的安全性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数据集覆盖和架构创新上存在不足，导致VLMs易受攻击且安全性有限。

Method: 引入HoliSafe数据集覆盖所有安全/不安全图像-文本组合，并提出SafeLLaVA模型，包含可学习的安全元标记和专用安全头。

Result: SafeLLaVA在多个VLM基准测试中达到最先进的安全性能，HoliSafe数据集揭示了现有模型的关键漏洞。

Conclusion: HoliSafe和SafeLLaVA为多模态对齐提供了新方向，推动了VLM安全性的进一步研究。
...
TL;DR: 论文探讨了多模态语言模型LlaVA-Next中图像概念的表示方式，发现线性可解码特征，并通过稀疏自编码器增强特征多样性。


<details>
  <summary>Details</summary>
Motivation: 研究多模态模型如何在隐藏激活中表示图像，以增强模型的可解释性和跨模态共享能力。

Method: 使用线性解码和稀疏自编码器分析LlaVA-Next模型的隐藏激活，并进行目标编辑验证特征的因果性。

Result: 发现图像和文本特征在深层逐渐共享，尽管初始表示较为分离。

Conclusion: 多模态模型在深层表现出跨模态特征共享，稀疏自编码器可提升特征多样性。
...
TL;DR: 论文研究了预训练视觉语言模型（VLM）在少样本适应中的鲁棒性，提出了SRAPF方法，显著提升了ID和OOD准确率。


<details>
  <summary>Details</summary>
Motivation: 预训练VLM在少样本适应中遇到OOD数据时性能下降，需要提升其OOD泛化能力。

Method: 通过对比不同适应方法，提出部分微调视觉编码器，并结合检索增强和对抗扰动技术，最终开发了SRAPF方法。

Result: SRAPF在ImageNet OOD基准测试中达到了最先进的ID和OOD准确率。

Conclusion: SRAPF方法有效平衡了ID和OOD性能，为少样本适应提供了新思路。
...
TL;DR: iDPOE方法通过隐式扩散策略和等变表示改进内镜黏膜下剥离术（ESD）轨迹预测，提升手术技能训练效果。


<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的剥离轨迹可优化手术培训，但现有模仿学习方法在不确定未来动作、几何对称性和泛化性方面存在不足。

Method: 提出iDPOE方法，结合扩散模型和等变表示，通过联合状态动作分布建模专家行为，改进视觉表示学习和泛化能力。

Result: 在近2000个ESD视频片段上，iDPOE在轨迹预测上优于现有方法。

Conclusion: iDPOE首次将模仿学习应用于剥离轨迹预测，为手术技能发展提供了新思路。
...
TL;DR: AI辅助的自动标注系统用于显示屏缺陷检测，通过上下文学习能力提升SegGPT架构，引入涂鸦标注机制，显著提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 多数据集目标检测存在类语义和标注框不一致的问题，现有方法无法同时解决这两类问题。

Method: 提出LAT框架，包括伪标签生成、特权提案生成器和语义特征融合模块，实现标签空间对齐和特征优化。

Result: 在多个基准测试中，LAT比半监督基线方法提升高达4.8AP。

Conclusion: LAT无需共享标签空间或手动标注，能有效解决类级和标注框不一致问题。
...
TL;DR: SRD是一种强化学习框架，用于防御视觉语言模型中的后门攻击，无需预先知道触发器。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在图像描述任务中表现出色，但容易被后门攻击，导致生成恶意描述。

Method: 提出Semantic Reward Defense (SRD)，通过强化学习框架应用离散扰动，破坏恶意路径的激活。

Result: 实验表明，SRD将攻击成功率降至5.6%，且在干净输入上的性能下降小于10%。

Conclusion: SRD提供了一种无需触发器知识的可解释防御方法，适用于多模态生成模型中的后门攻击。
...
TL;DR: 论文提出了一种名为RAP的数据选择方法，通过识别能触发多模态推理的稀疏样本（认知样本），显著减少了训练数据量和计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为多模态大语言模型（MLLMs）需要大量训练数据才能提升推理能力，但作者发现仅少数样本对推理有实质性贡献，其余样本冗余且计算成本高。

Method: 提出RAP方法，包含两个互补的评估器（CDE和ACE）和一个难度感知替换模块（DRM），用于筛选认知样本并替换简单样本。

Result: 实验表明，RAP仅需9.3%的训练数据即可实现优于全数据集的性能，同时减少43%以上的计算成本。

Conclusion: RAP证明了高质量小数据集可以替代大数据集，为多模态推理的高效训练提供了新思路。
...
TL;DR: 提出了一种新的SSIM形式，通过加法而非乘法组合其组件，优化了无监督单目深度学习的训练效果。


<details>
  <summary>Details</summary>
Motivation: 传统SSIM函数中乘法组合的组件和超参数对训练的影响被忽视，影响了性能。

Method: 采用加法组合SSIM中的亮度、对比度和结构相似性组件，构建新的损失函数。

Result: 新SSIM损失函数在KITTI-2015数据集上显著优于基线。

Conclusion: 加法组合的SSIM形式能生成更平滑的梯度，提升无监督深度估计性能。
...
TL;DR: HypeVPR是一种新颖的层次化嵌入框架，利用双曲空间处理P2E VPR的挑战，显著提升检索速度和准确性。


<details>
  <summary>Details</summary>
Motivation: 解决P2E VPR中多视角查询图像的层次结构问题，利用双曲空间保留特征关系。

Method: 提出层次化特征聚合机制和粗到细搜索策略，优化速度和准确性。

Result: 在多个基准数据集上表现优于现有方法，检索速度提升5倍。

Conclusion: HypeVPR通过双曲空间和层次化设计，显著提升了VPR的性能和效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常依赖于特定任务的嵌入，无法同时支持几何重建和跨任务复用。

Method: Object-X通过将捕获的多模态信息几何地锚定在3D体素网格中，并学习融合体素与对象属性的非结构化嵌入。
...
TL;DR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算相似向量表快速删除冗余。


<details>
  <summary>Details</summary>
Motivation: ANNS在某些场景下可能返回过于相似的结果，需要同时保证相似性和多样性。

Method: 预计算相似向量表，并在过滤阶段贪婪查找以删除冗余向量。

Result: 在类似真实RAG应用中，LotusFilter处理速度快（0.02 ms/查询）。
...
<details>
  <summary>Details</summary>
Motivation: 尽管RANSAC及其变体是处理异常值数据的黄金标准，但在不同任务中实现一致高性能仍具挑战性。SupeRANSAC旨在解决这一问题。
...
<details>
  <summary>Details</summary>
Motivation: 解决中文超大类别字符识别（mega-category recognition）的数据集缺失问题，支持文化传承和数字应用。

Method: 构建MegaHan97K数据集，包含手写、历史和合成子集，提供平衡样本。

Result: 数据集覆盖97,455类汉字，支持GB18030-2022标准，揭示了存储需求、形态相似字符识别和零样本学习等新挑战。
...
TL;DR: Spike-TBR是一种基于时间二进制表示（TBR）的新型事件编码策略，通过整合脉冲神经元来增强对噪声的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 事件相机具有高时间分辨率、低延迟和动态范围等优势，但如何高效地将事件流转换为兼容标准计算机视觉管道的格式仍是一个挑战，尤其是在噪声存在的情况下。

Method: 提出Spike-TBR，结合TBR的帧基优势与脉冲神经网络的噪声过滤能力，创建更鲁棒的事件流表示。评估了四种使用不同脉冲神经元的变体。

Result: 在多个数据集上展示了在噪声场景下的优越性能，同时在干净数据上也有改进。

Conclusion: Spike-TBR填补了基于脉冲和基于帧的处理之间的差距，为事件驱动视觉应用提供了简单且抗噪声的解决方案。
...
TL;DR: 该论文提出了一种针对交通灯检测CNN的对抗性攻击方法，通过打印的补丁实现攻击，并在真实环境中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 现有研究对交通灯检测器的攻击较少，本文填补了这一空白。

Method: 提出了一种威胁模型，通过在交通灯下方放置补丁进行攻击，并描述了训练策略。

Result: 实验展示了成功的对抗性补丁攻击，包括标签翻转攻击和象形图分类攻击，并在真实环境中验证了攻击效果。

Conclusion: 该方法在实验室和测试区域均有效，代码已开源。
...
Conclusion: DualX-VSR通过新颖的注意力机制和简化结构，显著提升了视频超分辨率的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的感知系统在2D推理分割方面取得了显著进展，但缺乏类似的3D推理分割框架。

Method: 引入SEG token和对象标识符，处理点云数据和文本提示，生成高精度3D分割掩码。

Result: 在ScanNet数据集上的实验验证了OpenMaskDINO3D在各种任务中的有效性。

Conclusion: OpenMaskDINO3D能够直接从自然语言指令生成准确的3D分割结果，为3D理解提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 从稀疏观测数据中重建多维地质场，结合张量补全和地质统计学以提高精度。

Method: 将部分观测的空间场建模为多维张量，通过全局低秩结构恢复缺失值。

Result: 在合成地质场实验中，张量补全方法在不同观测数据比例下均优于普通克里金法。

Conclusion: 该方法为地质场重建提供了更精确的优化框架。
...
TL;DR: 本文提出了一种新型的后门攻击框架，通过在水印模型中嵌入不可见触发器，实现在图像编辑过程中的隐蔽攻击。
...
Conclusion: 本文提出的方法为图像编辑领域的后门攻击提供了新的研究方向，并展示了不可见触发器的潜力。
...
Result: 模型仅使用6层束搜索，Elo评分达到2593。性能随模型规模和嵌入维度提升，表明潜在规划是传统搜索的可行替代方案。

Conclusion: 潜在规划方法可推广到其他完美信息游戏，且源代码已开源。
...
TL;DR: Anywhere3D-Bench是一个全面的3D视觉基准测试，涵盖四个不同层次的视觉定位任务，揭示了当前模型在空间和部分级别任务上的显著不足。
...
Method: 构建Anywhere3D-Bench基准测试，评估现有3D视觉定位方法、LLMs和MLLMs的性能。
...
TL;DR: 提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，相比NeRF方法更高效，并展示了在零样本泛化基准测试中的竞争力。


<details>
  <summary>Details</summary>
Motivation: 探索利用显式3D表示和FoundationStereo模型的深度估计，以低成本生成高保真数据集并快速微调立体模型。

Method: 结合3DGS重建的几何形状和FoundationStereo的深度估计，通过专家知识转移生成数据集。

Result: 3DGS生成的几何形状存在噪声，而FoundationStereo的视差估计更干净，零样本泛化性能更好。

Conclusion: 3DGS方法在低成本数据集生成和快速微调方面潜力巨大，但在复杂场景中的鲁棒性仍需进一步研究。
...
TL;DR: 论文探讨了文化遗产数字化中3D摄影方法的多样性，强调没有单一方法适用于所有对象，需根据对象特性和未来用途选择合适工具。
...
Result: 研究表明，没有绝对分类的3D数字化方法，需根据对象特性和用途定制工具。

Conclusion: 文化遗产数字化需结合遗产和数字领域专家的协作，选择最适合的工具。
...
TL;DR: CzechLynx是首个大规模、开放访问的数据集，用于欧亚猞猁的个体识别、2D姿态估计和实例分割。


<details>
  <summary>Details</summary>
Motivation: 为猞猁的个体识别和姿态估计提供高质量数据集，支持跨时空域的研究。

Method: 收集30k相机陷阱图像并标注，生成100k合成图像，定义三种评估协议。

Result: 数据集包含219个独特个体，覆盖15年监测数据，支持多种任务。

Conclusion: CzechLynx将推动猞猁识别和姿态估计的模型开发与基准测试。
...
TL;DR: 提出了一种基于视频的胚胎分级任务，利用全长延时监测视频预测胚胎质量，并设计了互补时空模式挖掘框架（CoSTeM）来提升评估效果。
...
Method: 提出CoSTeM框架，结合形态学和形态动力学分支，分别捕捉局部结构特征和全局发育轨迹。
...
TL;DR: 提出一种通过架构设计而非数据驱动的方法来提升图像质量评估（IQA）模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统IQA模型易受对抗攻击，现有防御方法依赖数据驱动，但可能并非最佳途径。

Method: 通过正交信息流和范数保持操作重构模型内部结构，结合剪枝和微调提升稳定性。

Result: 设计出一种无需对抗训练的鲁棒IQA架构，能有效抵御攻击。

Conclusion: 建议从数据优化转向设计优化，以提升模型鲁棒性。
...
TL;DR: APVR框架通过分层视觉信息检索解决长视频理解的内存限制问题，无需训练即可高效处理小时级视频。
...
Method: APVR包含两个互补组件：Pivot Frame Retrieval通过语义扩展和多模态置信度评分识别相关帧；Pivot Token Retrieval在选定帧内进行查询感知的注意力驱动令牌选择。

Result: 在LongVideoBench和VideoMME上的实验表明，APVR在训练无关和训练相关方法中均达到最优性能。

Conclusion: APVR通过双粒度检索实现了长视频的高效处理，并支持与现有MLLM架构的即插即用集成。
...
Method: 提出FEAT，采用顺序空间-时间-通道注意力机制、线性复杂度设计和残差值引导模块，优化全局依赖建模和噪声适应。
...
Result: BalSAM在人工林场景中表现优异，但未经调整的SAM表现不如定制化的Mask R-CNN。

Conclusion: 端到端调整SAM和集成DSM数据是提升树冠分割模型的有效途径。
...
TL;DR: TextVidBench是一个专为长视频文本问答设计的基准测试，填补了现有数据集在视频时长和评估范围上的不足，并提出了改进大模型的方法。
...
Method: 提出TextVidBench基准，包含跨域长视频、三阶段评估框架和高质量标注；改进大模型的方法包括IT-Rope机制、非均匀位置编码和轻量微调。

Result: 实验表明TextVidBench对现有模型构成挑战，提出的方法有效提升了长视频文本理解能力。

Conclusion: TextVidBench为长视频文本问答提供了更真实的评估场景，改进方法为提升模型能力提供了新思路。
...
TL;DR: 论文提出了一种基于视觉自回归（VAR）模型的图像超分辨率（ISR）方法，通过多尺度图像标记化和直接偏好优化（DPO）解决了现有方法的局限性，实现了单次前向传递的超分辨率，并在小模型和无外部数据的情况下取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率限制、依赖大规模模型和数据集的问题，本文旨在通过新组件解决这些局限性。

Method: 提出两种新组件：1）分层图像标记化方法，实现多尺度图像表示；2）DPO正则化项，优化模型输出偏好。

Result: 模型在单次前向传递中实现超分辨率，使用小模型（300M参数）且无需外部数据，取得了最先进的结果。

Conclusion: 新方法在ISR任务中表现出色，解决了VARSR的局限性，为未来研究提供了新思路。
...
TL;DR: PATS是一种新的视频采样策略，用于自动化运动技能评估，通过保留完整的基础动作来提高评估准确性。


<details>
  <summary>Details</summary>
Motivation: 当前视频采样方法破坏了评估所需的时序连续性，影响了技能评估的准确性。

Method: PATS自适应地分割视频，确保每个分析部分包含完整的关键动作执行，并在多个片段中重复此过程。

Result: 在EgoExo4D基准测试中，PATS在所有视角配置下均优于现有方法，并在具有挑战性的领域（如攀岩、音乐和篮球）中表现显著提升。

Conclusion: PATS是一种有效的自适应时序采样方法，适用于多样化的活动特性，推动了自动化技能评估的实际应用。
...
Result: 在现有基准上准确率提升18.82%，推理速度更快，且在多样化布局数据集DL-CSVTR上表现优异。
...
TL;DR: 论文探讨了单目深度估计的进展，重点介绍了深度学习方法及其在零样本场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计在缺乏显式深度线索时难以准确估计度量深度，但能有效提取结构信息。

Method: 采用多尺度融合网络、分类问题重新解释、附加先验和更有效的目标函数等方法。

Result: Depth Anything在零样本单目深度估计中表现领先，能提取未见图像的结构信息。

Conclusion: 尽管在度量深度估计上仍有挑战，但深度学习方法在结构信息提取方面表现出色。
...
TL;DR: 提出了一种利用3D高斯泼溅和高斯不透明度场生成合成数据的管道，用于训练3D点云语义分割模型，效果显著。


<details>
  <summary>Details</summary>
Motivation: 真实点云数据获取和标注成本高，需要一种低成本且灵活的合成数据生成方法。

Method: 使用3D高斯泼溅和高斯不透明度场生成农业车辆的3D资产，模拟LiDAR生成点云数据。

Result: PTv3模型在仅使用合成数据训练时，mIoU达到91.35%，某些情况下优于真实数据训练的模型。

Conclusion: 合成数据训练模型具有泛化能力，可应用于未训练过的网格模型。
...
TL;DR: UAV4D框架解决了无人机拍摄动态场景的渲染问题，通过3D基础模型和人体网格重建模型，结合SMPL模型和高斯点云，实现了高质量的新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法有效处理无人机拍摄的单目视频中的动态场景，尤其是多小移动人体的重建问题。

Method: 结合3D基础模型和人体网格重建模型，利用人体-场景接触点解决尺度模糊问题，并通过SMPL模型和高斯点云初始化实现整体渲染。
...
TL;DR: 本文介绍了一种新型物理标注系统，用于生成自动化光学检测的训练数据，通过指针交互直接捕捉专家知识，提高标注效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法效率低且不直观，无法充分利用专家的物理操作经验。该系统旨在通过物理交互直接捕捉专家知识，提升数据标注的质量和效率。

Method: 系统使用校准的追踪指针记录用户在物体上的物理轨迹，并通过投影界面提供视觉引导，将空间交互转化为标准化的标注格式。

Result: 初步评估证实系统能有效捕捉详细标注轨迹，并与CVAT集成，简化后续机器学习任务的工作流程。

Conclusion: 该系统填补了人类专家知识与自动化数据生成之间的空白，为非IT专家参与机器学习训练提供了可能，未来有望推动自动化光学检测的数据生成。
...
TL;DR: FlowDirector是一种无需反转的视频编辑框架，通过ODE在数据空间中直接演化视频，保持时间一致性和结构细节，并通过注意力引导掩码和增强编辑策略实现局部可控编辑。
...
Method: 提出FlowDirector框架，利用ODE在数据空间中直接演化视频，引入注意力引导掩码和增强编辑策略。

Result: 在多个基准测试中表现优异，实现了指令遵循、时间一致性和背景保留的最佳性能。

Conclusion: FlowDirector为无需反转的高效且连贯的视频编辑建立了新范式。
...
TL;DR: 本文综述了越南文档分析与识别（DAR）的现状，探讨了传统OCR和深度学习方法面临的挑战，以及大型语言模型（LLMs）和视觉语言模型带来的新机遇。


<details>
  <summary>Details</summary>
Motivation: 越南文本识别因复杂的变音符号、声调变化和缺乏大规模标注数据集而面临独特挑战，传统方法难以应对现实文档的多样性。

Method: 综述了现有技术，包括传统OCR、深度学习方法以及LLMs和视觉语言模型的应用。

Result: 指出了领域适应、多模态学习和计算效率等挑战，并探讨了LLMs在越南DAR中的潜力。

Conclusion: 提出了未来研究方向，包括数据集开发、模型优化和多模态方法整合，以推动越南DAR的进步。
...
TL;DR: SeedEdit 3.0与Seedream 3.0模型结合，显著提升了编辑指令跟随和图像内容保留能力，并通过数据增强和联合学习管道进一步优化性能。


<details>
  <summary>Details</summary>
Motivation: 改进现有图像编辑模型在真实图像输入上的编辑指令跟随和内容保留能力。

Method: 1. 开发增强的数据整理管道，采用元信息范式嵌入策略；2. 引入联合学习管道，结合扩散损失和奖励损失。

Result: 在真实图像编辑测试中，SeedEdit 3.0实现了56.1%的高可用率，优于其他版本和竞品。

Conclusion: SeedEdit 3.0在多个方面实现了最佳平衡，显著提升了图像编辑的实用性和性能。
...
Method: 使用视觉Transformer（VisualGLM-6B）和大型语言模型（GPT-4）结合，通过LoRA和P-Tuning v2进行参数高效微调。

Result: 模型在客观特征上F1得分为0.84，与居民感知的吻合度为89.3%，并能捕捉上下文相关的矛盾和非线性模式。
...
<details>
  <summary>Details</summary>
Motivation: 随着FAA工具的普及，其信任问题（如偏见、隐私等）日益突出，需要研究解决。

Method: 通过研讨会形式，汇集研究者探讨FAA任务中的信任挑战，如微表情识别、人机交互等。

Result: 研讨会支持FG2025的伦理目标，推动可信FAA的研究与讨论。

Conclusion: 该研讨会为FAA的信任问题提供了研究平台，促进伦理与技术结合的发展。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变压器（vDiT）在文本到视频生成方面取得了显著进展，但其高计算需求限制了实际部署。现有加速方法依赖启发式方法，适用性有限。

Method: ASTRAEA提出轻量级令牌选择机制和GPU并行稀疏注意力策略，并通过进化算法自动优化令牌预算分配。

Result: 在单GPU上实现2.4倍推理加速，8GPU上可达13.2倍，且视频质量损失极小（VBench得分损失<0.5%）。
...
Method: 引入Does-it/Can-it框架（DIM-CIM），构建COCO-DIMCIM基准测试，结合COCO概念和大语言模型。

Result: 发现模型在参数增加时泛化能力提升但默认模式多样性下降，并识别细粒度失败案例。训练数据多样性与其默认模式多样性相关性为0.85。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型在非理想条件下表现不稳定，容易被规避。受图像超分辨率领域退化模型的启发，作者旨在开发更全面的伪造模型以提升检测器的泛化能力。

Method: 通过Poisson混合、多样化掩码、生成器伪影和干扰物扩展伪伪造空间，并在训练图像中添加强退化以增强检测器的鲁棒性。

Result: 在DFDC和DFDCP数据集上，分别比现有最佳LAA骨干网络提高了3.51%和6.21%的AUC。

Conclusion: PMM不仅显著提升了模型对常见图像退化的鲁棒性，还在标准基准数据集上表现出性能提升，揭示了先前检测器的鲁棒性不足并提出了改进方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的小型无人机时存在局限性，事件相机虽提供高时间分辨率和动态范围，但现有基准缺乏精细时间分辨率或无人机特定运动模式。

Method: 提出Florence RGB-Event Drone数据集（FRED），包含RGB视频和事件流，涵盖5种无人机模型及挑战性场景（如雨天和不良光照）。

Result: FRED数据集包含7小时密集标注的无人机轨迹，并提供详细评估协议和标准指标。
...
<details>
  <summary>Details</summary>
Motivation: 作者认为当前领域过于依赖神经网络，而早期基于模板匹配的方法具有更强的物理可解释性，因此尝试回归原始路径。

Method: 通过生成距离-时间图和多普勒-时间图，使用角点检测和多相主动轮廓模型分割微多普勒特征，再通过Mapper算法计算拓扑相似性。

Result: 数值模拟和实测实验验证了方法的有效性。

Conclusion: 该方法展示了不使用神经网络也能实现智能识别的潜力，并开源了代码。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法忽略细粒度分析（如异常像素），限制了检测范围。

Method: 提出TAO框架，将多细粒度异常对象检测集成到统一流程中，通过像素级跟踪异常对象。
...
TL;DR: TAPFM提出了一种单GPU任务适应病理基础模型的方法，通过ViT注意力机制优化特征表示和注意力权重，显著提升了WSI分析的性能。


<details>
  <summary>Details</summary>
Motivation: 病理基础模型（PFMs）在分析全切片图像（WSIs）时表现强大，但针对特定临床任务进行适应时面临挑战，主要由于仅能获得弱标签（WSI级别）且需要多实例学习（MIL）。

Method: TAPFM使用ViT注意力进行MIL聚合，同时优化特征表示和注意力权重，并通过分离MIL聚合器和PFM的计算图实现稳定训练。

Result: 在膀胱癌和肺腺癌的突变预测任务中，TAPFM表现优于传统方法，尤其在多标签分类任务中表现突出。

Conclusion: TAPFM使得在标准硬件上适应预训练PFM成为可能，适用于多种临床应用。
...
TL;DR: 论文提出了一种多模态低秩适应方法（MokA），针对当前多模态微调方法的局限性，通过模态特定参数压缩单模态信息并增强跨模态交互，实现了更高效的多模态微调。


<details>
  <summary>Details</summary>
Motivation: 当前高效多模态微调方法多直接借鉴LLMs，忽视了多模态场景的固有差异，导致无法充分利用所有模态。

Method: 提出MokA方法，结合单模态适应和跨模态适应，通过模态特定参数压缩单模态信息并显式增强跨模态交互。

Result: 在多种多模态场景和LLM骨干上的实验表明，MokA方法具有一致性的改进效果，并通过消融研究和效率评估验证了其有效性。

Conclusion: MokA为多模态大模型的高效适应提供了更有针对性的解决方案，为未来研究奠定了基础。
...
Result: 实验结果显示，在60GHz频段下，接收信号强度平均提升了23dB，且在室内环境中保持信号接收高于-65dB的概率为0.89。
...
TL;DR: 研究探讨了多模态模型中的跨模态记忆特性，发现模态间信息传递存在显著差距，并提出了一种缓解方法。
...
Method: 通过合成人物数据集进行控制实验，量化单模态训练后模型在另一模态中的表现，评估跨模态可转移性。

Result: 发现模态间信息可传递，但源模态和目标模态间存在显著差距，且这一差距在不同场景下普遍存在。

Conclusion: 研究为开发更鲁棒的多模态学习技术提供了启示，并提出了缓解跨模态传递差距的基线方法。
...
TL;DR: DEGround 是一种新的方法，通过共享 DETR 查询来同时处理检测和定位任务，显著提升了 3D 定位性能。


<details>
  <summary>Details</summary>
Motivation: 研究发现，现有的检测模型在未经过指令训练的情况下，其定位性能甚至优于专门训练的定位模型，这表明当前方法在类别级定位任务上仍有不足。

Method: 提出 DEGround，共享 DETR 查询用于检测和定位，并引入区域激活定位模块和查询调制模块以增强语言指令的上下文理解。

Result: DEGround 在 EmbodiedScan 验证集上的整体准确率比现有最佳模型 BIP3D 高出 7.52%。

Conclusion: DEGround 通过结合检测和定位任务，显著提升了 3D 定位性能，尤其是在上下文感知的语言指令理解方面表现突出。
...
TL;DR: OGGSplat是一种基于开放高斯生长的方法，用于从稀疏视图重建语义感知的3D场景，通过RGB-语义一致性修复模块和双向控制扩散模型实现高效重建。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要密集输入视图且计算成本高，而通用方法难以重建输入视图锥之外的区域。OGGSplat旨在解决这些问题，满足虚拟现实和具身AI等应用需求。

Method: 提出开放高斯生长方法，结合RGB-语义一致性修复模块和双向控制扩散模型，将修复区域提升到3D空间进行高斯参数优化。

Result: 建立了高斯外推（GO）基准，评估重建开放词汇场景的语义和生成质量，并在智能手机拍摄的两视图图像上展示了语义感知重建能力。

Conclusion: OGGSplat在稀疏视图下实现了语义一致性和视觉合理性的3D场景重建，为虚拟现实和具身AI提供了高效解决方案。
...
TL;DR: 提出Follow-Your-Motion框架，通过时空解耦LoRA和稀疏运动采样解决视频运动迁移中的不一致性和效率问题。
...
Method: 提出时空解耦LoRA和稀疏运动采样技术，加速调优并保持运动一致性。

Result: 在MotionBench上验证了方法的优越性。

Conclusion: Follow-Your-Motion框架高效且性能优越。
...
TL;DR: DSG-World是一种新型端到端框架，通过双状态观测显式构建3D高斯世界模型，解决遮挡问题并提升重建稳定性。
...
Conclusion: DSG-World在真实世界3D重建和模拟中表现出色，无需密集观测或多阶段流程。
...
TL;DR: MonkeyOCR是一种基于SRR三元范式的视觉语言模型，用于文档解析，通过简化流程和高效处理，在准确性和速度上取得平衡，并在多个任务上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 解决现有文档解析方法中复杂多工具流程和低效全页处理的不足，提出更高效且精确的解决方案。

Method: 采用SRR（结构-识别-关系）三元范式，将文档解析分解为布局分析、内容识别和逻辑排序三个问题。

Result: 在MonkeyDoc数据集上表现优异，平均性能提升5.1%，处理速度更快（0.84页/秒），且3B参数模型超越更大模型。

Conclusion: MonkeyOCR在文档解析任务中实现了高效、精确和可扩展的解决方案，适用于多语言和多类型文档。
...
<details>
  <summary>Details</summary>
Motivation: 由于SAM在医学领域的适应性有限，现有方法如MedSAM虽提升了性能但牺牲了泛化能力，因此需要一种新方法。

Method: SAM-TTA包含SBCT（自适应转换输入）和DUMT（一致性学习对齐语义），无需额外监督或重训练。

Result: 在五个公开数据集上，SAM-TTA优于现有TTA方法，甚至在某些场景超越MedSAM。
...
TL;DR: MoViX是一个自监督的跨视图视频定位框架，用于GPS缺失的越野环境，通过学习和季节不变表示实现高精度定位。


<details>
  <summary>Details</summary>
Motivation: 解决GPS缺失环境中因植被重复和季节变化导致的定位挑战。

Method: 采用姿态依赖的正采样策略、时间对齐的硬负采样、运动信息帧采样器和轻量级时间聚合器。

Result: 在TartanDrive 2.0数据集上，93%的情况下定位误差在25米内，100%在50米内。

Conclusion: MoViX在未进行环境特定调优的情况下优于现有方法，并展示了在真实越野环境中的泛化能力。
...
TL;DR: 论文提出LeanPO方法，通过重新定义奖励和动态标签平滑策略，优化Video-LLMs的性能，减少非目标响应的概率提升。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs偏好对齐技术（如DPO）在训练中会导致目标和非目标响应的概率同时下降，加剧视频内容冗余复杂性的问题。

Method: 提出LeanPO方法，包括重新定义隐式奖励为响应相对于策略模型的平均似然，以及动态标签平滑策略以减少噪声影响。
...
TL;DR: 论文探讨了如何利用基础模型（FM）提升ID卡展示攻击检测（PAD）的泛化能力，尤其是在面对不同国家ID卡时的表现。


<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护问题通常仅针对少数ID卡训练，导致在新国家ID卡上表现不佳，需要提升泛化能力。

Method: 通过零样本学习和微调两种方式，使用两个数据集（智利ID和芬兰、西班牙、斯洛伐克ID）进行测试。
...
TL;DR: TF-CoVR是一个专注于时间细粒度视频检索的新基准，提出了一个两阶段训练框架TF-CoVR-Base，显著提升了检索性能。


<details>
  <summary>Details</summary>
Motivation: 现有CoVR基准缺乏对时间细粒度变化的测试能力，限制了实际应用。

Method: 提出TF-CoVR基准，并设计TF-CoVR-Base框架，通过预训练视频编码器和对比学习对齐查询与候选视频。

Result: TF-CoVR-Base在零样本和微调后分别将mAP@50从5.92提升到7.51和从19.83提升到25.82。

Conclusion: TF-CoVR填补了时间细粒度视频检索的空白，TF-CoVR-Base框架显著提升了检索性能。
...
TL;DR: Rectified Point Flow 是一种统一的参数化方法，将点云配准和多部件形状组装视为单一条件生成问题，通过学习连续点速度场实现目标位置恢复。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在部件姿态回归和对称性处理上的不足，无需对称标签即可学习组装对称性。

Method: 通过自监督编码器和连续点速度场学习，将噪声点传输到目标位置，恢复部件姿态。

Result: 在六个基准测试中达到最新性能，支持多样数据集联合训练，提升几何先验学习。

Conclusion: 统一框架有效提升点云配准和形状组装的准确性，共享几何先验进一步优化结果。
...
TL;DR: 论文提出了一种基于几何基础的长时空间记忆框架，以增强视频世界模型的长期一致性，解决了现有模型因时间上下文窗口有限而导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型在生成视频帧时，由于时间上下文窗口有限，难以维持场景一致性，尤其是在重新访问时会出现严重的遗忘现象。

Method: 受人类记忆机制启发，作者提出了一种几何基础的长时空间记忆框架，包含存储和检索信息的机制，并通过定制数据集训练和评估带有显式3D记忆机制的模型。

Result: 实验表明，该方法在生成质量、一致性和上下文长度方面优于相关基线模型。

Conclusion: 该研究为长期一致的视频世界生成提供了新思路。
...
TL;DR: RaySt3R将3D形状补全问题重新定义为新视角合成问题，通过单张RGB-D图像和多视角查询射线，预测深度图、物体掩码和置信度，实现高效且一致的3D重建。
...
Result: 在合成和真实数据集上，RaySt3R在3D chamfer距离上比基线方法提升高达44%，达到SOTA性能。

Conclusion: RaySt3R通过新视角合成方法，显著提升了3D形状补全的效率和效果。
...
TL;DR: 论文提出VCT和SVCT模型，解决现有概念瓶颈模型在医学领域中的效用和稳定性问题，同时保持可解释性。


<details>
  <summary>Details</summary>
Motivation: 医学领域对透明性的需求促使研究可解释AI，但现有概念瓶颈模型忽视图像特征且解释不稳定。

Method: 提出VCT和SVCT模型，结合概念特征与图像特征，并引入去噪扩散平滑提升稳定性。

Result: 在四个医学数据集上验证，VCT和SVCT在保持准确性的同时提供稳定解释。

Conclusion: SVCT满足医学领域对可解释性和稳定性的需求，尤其在面对扰动时表现优越。
...
TL;DR: EOC-Bench是一个创新的基准测试，用于评估多模态大语言模型在动态自我中心场景中的物体认知能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要关注静态场景，忽略了用户交互引起的动态变化，EOC-Bench填补了这一空白。

Method: EOC-Bench包含3,277个标注的QA对，涵盖11个评估维度和3种视觉物体引用类型，采用混合格式标注框架和多尺度时间准确性指标。

Result: 通过EOC-Bench对多种MLLM进行了全面评估，验证了其在动态场景中的物体认知能力。

Conclusion: EOC-Bench为提升MLLM的物体认知能力提供了重要工具，为开发可靠的嵌入式系统核心模型奠定了基础。
...
Method: 使用因果解码器建立单向依赖，结合前缀标记和两阶段分词器训练，提升重建一致性。

Result: 在ImageNet-256基准上，AliTok在177M参数下gFID为1.50，IS为305.9；662M参数时gFID为1.35，超越扩散方法且采样速度快10倍。

Conclusion: AliTok在生成友好性和性能上均优于现有方法，代码和权重已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的视频修复方法计算成本高，且在高分辨率视频修复中表现不足，SeedVR2旨在解决这些问题。

Method: 提出动态窗口注意力机制和多种损失函数，优化模型架构和训练流程，实现一步修复。

Result: 实验表明，SeedVR2在一步修复中性能优于或接近现有方法。
...
Method: 引入Semantic Perceiver转换视觉特征为多模态token，支持多粒度理解。

Result: PAM在速度和内存消耗上优于现有方法，性能强大。

Conclusion: PAM为区域级视觉理解提供了高效且实用的解决方案。
...
TL;DR: 提出了一种通过3D感知伪标签改进语义匹配的方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 解决预训练视觉模型在对称物体或重复部件上的模糊性问题。

Method: 使用3D感知链式伪标签训练适配器，结合松弛循环一致性和3D球形原型映射约束。

Result: 在SPair-71k上实现了4%的绝对提升，比类似监督需求的方法高出7%。

Conclusion: 方法通用性强，简化了扩展到其他数据源的训练过程。
...
<details>
  <summary>Details</summary>
Motivation: 改进基于示例的材料编辑方法，通过找到去噪UNet中负责材料归因的模块，实现更精确的材料控制。

Method: 利用CLIP空间中的方向混合材料，并通过浅层网络预测细粒度材料属性（如粗糙度、金属感、透明度和发光）的变化方向。

Result: 通过定性和定量分析验证了方法的有效性，支持单次前向传递中的多次编辑，并适用于绘画。
...
Conclusion: ProJo4D通过渐进式优化策略，显著提升了稀疏输入下的物理参数估计效果，推动了物理基础的4D场景理解。
...
TL;DR: 论文探讨了3D视觉语言模型（VLMs）的性能差距，发现3D场景中心模型对3D编码器的依赖有限，并提出新数据集以促进真正的3D场景理解。
...
Method: 通过分析3D VLMs的编码器设计，分类为3D对象中心、2D图像基础和3D场景中心方法，并进行深入性能分析。

Result: 发现3D场景中心模型对3D编码器依赖不足，预训练效果较差，且数据扩展收益不明显。

Conclusion: 提出新数据集以解决捷径学习问题，强调需要改进评估和策略以提升3D理解能力。
...
TL;DR: 论文提出了一种基于点图预测的PM-Loss正则化损失，用于解决深度图中物体边界处的几何不连续性问题，显著提升了3D高斯泼溅的渲染质量。


<details>
  <summary>Details</summary>
Motivation: 深度图在3D高斯泼溅（3DGS）中常用于生成3D点云，但物体边界处的深度不连续性会导致点云稀疏或碎片化，影响渲染质量。

Method: 通过预训练的transformer预测点图，并设计PM-Loss正则化损失，强制几何平滑性，尤其是物体边界处。
...
TL;DR: CG-AV-Counting是一个新的视频计数基准，AV-Reasoner模型通过强化学习在多个基准上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 当前MLLMs在计数任务上表现不佳，现有基准存在视频短、查询封闭、缺乏线索标注和多模态覆盖不足的问题。

Method: 提出CG-AV-Counting基准和AV-Reasoner模型，后者使用GRPO和课程学习提升计数能力。

Result: AV-Reasoner在多个基准上达到最优，但在域外基准上语言空间推理未带来性能提升。

Conclusion: 强化学习有效提升计数能力，但语言推理在域外任务中效果有限。
...
TL;DR: MINT-CoT通过动态插入视觉标记到文本推理步骤中，提升了多模态数学问题的解决能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法在数学问题解决中存在视觉区域粗粒度、视觉编码器感知有限和依赖外部视觉修改能力的局限性。

Method: 提出MINT-CoT方法，通过Interleave Token动态选择视觉区域，并构建了54K的数据集和三阶段训练策略。

Result: MINT-CoT-7B在MathVista、GeoQA和MMStar上分别提升了34.08%、28.78%和23.2%。

Conclusion: MINT-CoT在多模态数学推理中表现出色，代码和数据已开源。
...
TL;DR: VideoMarathon数据集填补了长视频标注数据的空白，Hour-LLaVA模型通过内存增强模块实现了高效的长视频训练和推理。


<details>
  <summary>Details</summary>
Motivation: 现有长视频标注数据稀缺，限制了视频大模型在小时级视频上的训练和性能。

Method: 提出VideoMarathon数据集（9,700小时长视频，3.3M QA对）和Hour-LLaVA模型（内存增强模块支持1-FPS采样）。

Result: Hour-LLaVA在多个长视频基准测试中表现最佳。

Conclusion: VideoMarathon数据集质量高，Hour-LLaVA模型在长视频理解任务中具有优越性。
...
TL;DR: VideoMolmo是一个多模态大模型，用于基于文本描述的细粒度时空定位，通过时间模块和掩码融合提升一致性和准确性。
...
Method: 结合Molmo架构，引入时间模块和双向点传播的掩码融合管道，分两步生成坐标和分割。

Result: 在多个真实场景和任务中显著提升了时空定位准确性和推理能力。

Conclusion: VideoMolmo通过多模态设计和数据集支持，在复杂场景中表现出色。
...
TL;DR: 提出了一种生成室内空间去家具化副本的流程，通过纹理网格和多视角全景图像实现。


<details>
  <summary>Details</summary>
Motivation: 现有方法如神经辐射场（NeRF）和RGB-D修复易产生模糊或幻觉问题，需要更高质量的去家具化资产。

Method: 首先从网格中分割并移除家具，生成简化去家具化网格（SDM），利用SDM提取边缘信息，通过ControlNet修复全景图像。

Result: 相比NeRF和RGB-D修复，该方法生成更高质量的资产，避免了模糊和幻觉问题。

Conclusion: 该方法为室内场景的去家具化提供了一种高效且高质量的解决方案。
...
TL;DR: DirectLayout是一个通过大型语言模型直接生成3D室内场景布局的框架，解决了现有方法在开放词汇和细粒度用户指令对齐上的不足。
...
Method: DirectLayout分三阶段生成布局：鸟瞰图生成、3D空间提升和对象放置优化，利用Chain-of-Thought激活和奖励机制增强空间推理。

Result: 实验表明，DirectLayout在语义一致性、泛化性和物理合理性上表现优异。

Conclusion: DirectLayout通过直接生成和优化布局，实现了高质量、用户可控的3D场景合成。
...
TL;DR: 提出了一种新的任务Omnimodal Referring Expression Segmentation (ORES)，通过文本或文本加视觉实体的任意提示生成一组掩码，并提出了RAS框架来增强分割模型的多模态交互能力。
...
Result: 在ORES任务以及经典的RES和GRES任务中，RAS表现出优越性能。
...
TL;DR: ContentV是一个8B参数的文本到视频模型，通过高效训练策略在4周内达到SOTA性能。
...
Result: 在VBench上得分85.14，支持多分辨率和时长的高质量视频生成。
...
TL;DR: 研究发现MLLMs中仅有约5%的注意力头对视觉理解有贡献，提出了一种无需训练的方法识别这些视觉头，并基于此设计了KV-Cache优化策略SparseMM，显著提升了推理效率和内存利用率。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs如何处理视觉输入，尤其是注意力机制的作用，以优化其推理效率。

Method: 通过分析注意力机制发现视觉头的稀疏性，设计了一种量化视觉相关性的框架，并提出了KV-Cache优化策略SparseMM。
...
TL;DR: 首个基于物理的神经逆向渲染系统，用于多视角视频中的光传播分析。


<details>
  <summary>Details</summary>
Motivation: 解决多视角视频中光传播的逆向渲染问题，尤其是强间接光下的3D重建。

Method: 采用时间分辨的神经辐射缓存技术，加速逆向渲染过程。

Result: 实现了高精度的直接和间接光传输效果，支持光传播的视图合成和场景重光照。

Conclusion: 该系统在多视角时间分辨光传播分析中表现出色，具有创新性应用潜力。
...
TL;DR: 提出了一种名为FreeTimeGS的新方法，通过4D表示和动态高斯基元改进复杂运动场景的3D重建。
...
Method: 使用4D表示，允许高斯基元在任意时间和位置出现，并为每个基元赋予运动函数以减少时间冗余。

Result: 在多个数据集上的实验表明，渲染质量显著优于现有方法。

Conclusion: FreeTimeGS通过灵活的动态表示和运动函数，有效提升了复杂动态场景的建模能力。
...
TL;DR: VideoMathQA是一个评估模型在视频中进行跨模态数学推理能力的基准测试，涵盖10个数学领域，强调多模态和时序推理。


<details>
  <summary>Details</summary>
Motivation: 解决视频中数学推理的挑战，需要整合视觉、音频和文本信息，现有方法在此类任务上表现不足。

Method: 创建VideoMathQA基准，包含多样化的视频和问题类型，由专家标注，强调多模态和时序推理。

Result: 基准测试揭示了现有方法的局限性，并提供了系统评估框架。

Conclusion: VideoMathQA为跨模态数学推理任务提供了新的评估标准，推动模型能力的提升。
...
TL;DR: 对比流匹配（Contrastive Flow Matching）通过增强条件分离，提升条件扩散模型的训练速度和生成质量。


<details>
  <summary>Details</summary>
Motivation: 在条件设置下（如类别条件模型），流匹配的唯一性不再保证，导致生成结果模糊。

Method: 引入对比目标函数，最大化不同样本对预测流之间的差异。

Result: 实验显示，对比流匹配显著提升训练速度（最高9倍）、减少去噪步骤（最高5倍）并降低FID（最高8.9）。

Conclusion: 对比流匹配有效解决了条件流重叠问题，提升了模型性能。
...
### [128] [RoboRefer: Towards Spatial Referring with Reasoning in Vision-Language Models for Robotics](https://arxiv.org/abs/2506.04308)
...
TL;DR: RoboRefer是一种3D感知的视觉语言模型，通过深度编码器和强化微调实现精确的空间理解和推理，在RefSpatial-Bench上表现优异。
...
Conclusion: RoboRefer能集成多种控制策略，适用于复杂场景中的长时程动态任务。
...
### [129] [Learning Smooth State-Dependent Traversability from Dense Point Clouds](https://arxiv.org/abs/2506.04362)
*Zihao Dong,Alan Papalia,Leonard Jung,Alenna Spiro,Philip R. Osteen,Christa S. Robison,Michael Everett*

Main category: cs.RO

TL;DR: SPARTA方法通过几何结构和傅里叶基函数预测地形可通行性，显著提高了成功率和计算效率。


<details>
  <summary>Details</summary>
Motivation: 越野自主性中，地形的可通行性常依赖于车辆状态（如接近角度），传统方法需要大量数据和重复计算。

Method: SPARTA利用点云数据，通过傅里叶基函数输出平滑的解析函数，预测任意接近角度的风险分布。

Result: 在高保真模拟中，SPARTA在40米巨石场中的成功率为91%（基线为73%），并在硬件测试中验证了泛化能力。

Conclusion: SPARTA通过几何结构和傅里叶基函数，高效解决了依赖车辆状态的地形可通行性问题。
...
### [130] [MineInsight: A Multi-sensor Dataset for Humanitarian Demining Robotics in Off-Road Environments](https://arxiv.org/abs/2506.04842)
*Mario Malizia,Charles Hamesse,Ken Hasselmann,Geert De Cubber,Nikolaos Tsiogkas,Eric Demeester,Rob Haelterman*

Main category: cs.RO

TL;DR: 论文介绍了MineInsight数据集，用于改进人道主义排雷中的计算机视觉技术。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏多样化和真实的数据集，算法验证成为研究社区的挑战。

Method: 创建了一个多传感器、多光谱的公开数据集，包含多种目标和环境条件。

Result: 数据集包含大量多光谱图像和LiDAR数据，提供了多样化的测试环境。
...
### [131] [Synthetic Dataset Generation for Autonomous Mobile Robots Using 3D Gaussian Splatting for Vision Training](https://arxiv.org/abs/2506.05092)
*Aneesh Deogan,Wout Beks,Peter Teurlings,Koen de Vos,Mark van den Brand,Rene van de Molengraft*

Main category: cs.RO

TL;DR: 提出了一种在Unreal Engine中自动生成标注合成数据的新方法，用于训练目标检测算法，尤其在机器人足球等动态多变环境中表现优异。
...
Method: 利用逼真的3D高斯样条快速生成合成数据，结合真实世界数据提升目标检测性能。

Result: 合成数据集性能与真实数据集相当，且结合两者能显著提升检测性能。

Conclusion: 该方法为传统数据集创建提供了可扩展且高效的替代方案，特别适用于需要多样化和可扩展训练数据的机器人应用。
...
### [132] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
...
Result: 该方法在感知质量上表现更优，尤其在斑点重建存在伪影时，且能充分利用图像序列信息。

Conclusion: 该方法在提供完整图像序列时能实现最佳重建效果，具有高效性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的噪声调度，可能影响感知一致性，因此需要一种更优的方法。

Method: 采用连续时间扩散模型，引入SSIMBaD技术，通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上，方法在像素精度和感知质量上优于现有技术。

Conclusion: 该方法在保持结构保真度的同时，能有效迁移参考图像的风格属性。
...
<details>
  <summary>Details</summary>
Motivation: 现有网格变形方法存在输出质量低、需手动调参或依赖数据训练的问题，本文旨在解决这些局限性。

Method: 通过锥形奇点检测识别潜在手柄，利用VLM选择可变形子部分和手柄，并通过多视角投票减少预测不确定性。
...
TL;DR: VoxDet提出了一种实例为中心的3D语义占用预测框架，通过解耦任务提升实例级完整性。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视实例级区分性，导致实例不完整和相邻模糊。VoxDet利用体素标签隐含的实例信息，提出免费转换方法。

Method: VoxDet将体素级预测解耦为偏移回归和语义预测，使用空间解耦编码器和任务解耦预测器。

Result: 在SemanticKITTI测试集上达到63.0 IoU，排名第一。

Conclusion: VoxDet高效且性能优越，适用于相机和LiDAR输入。
...
TL;DR: 提出了一种快速且无监督的多边形近似闭合数字曲线的方案，速度优于现有技术，且在Rosin度量与美学方面表现优异。
...
Method: 方案分为三个阶段：初始分割、迭代顶点插入、迭代合并及顶点调整。
...
### [137] [Ontology-based knowledge representation for bone disease diagnosis: a foundation for safe and sustainable medical artificial intelligence systems](https://arxiv.org/abs/2506.04756)
...
Method: 开发了基于本体的框架，包括分层神经网络、VQA系统和多模态模型，结合临床知识。

Result: 框架展示了扩展潜力，但实验验证因数据和资源限制尚未完成。

Conclusion: 未来工作将扩展数据集并进行系统验证，框架有望推广至其他疾病领域。
...
### [138] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*

Main category: cs.LG

TL;DR: YOTO方法通过一次性自动优化损失权重超参数，减少训练次数，优于传统网格搜索。
...
Method: 将损失权重超参数视为网络参数，通过可微复合损失和软最大操作优化，并引入正则化损失。

Result: 在3D估计和语义分割任务中，YOTO优于网格搜索的最佳模型。

Conclusion: YOTO提供了一种高效的一次性训练方法，显著优于传统方法。
...
### [139] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
...
Method: 利用SVD奇异值捕获任务特定权重分布，通过轻量级学习器StatsMergeLearner建模权重分布，并引入任务特定教师蒸馏。
...
### [140] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: 该论文通过推导扩展定律（scaling laws）比较了CLIP和MaMMUT两种语言-视觉学习方法的性能，发现MaMMUT在扩展性和样本效率上优于CLIP，并验证了其在多种任务和数据集上的稳定性。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于通过扩展定律对模型和数据集进行系统比较，避免仅基于单一参考尺度的误导性结论，从而为开放基础模型和数据集的改进提供方法。

Method: 方法包括密集测量不同模型和样本规模下的扩展定律，并在多种下游任务（分类、检索、分割）和数据集（DataComp、DFN、Re-LAION）上验证比较结果。

Result: 结果显示MaMMUT在扩展性和样本效率上优于CLIP，且在不同任务和数据集上表现一致。此外，论文还发布了预训练模型和代码。

Conclusion: 结论表明扩展定律的准确推导为模型和数据集的跨尺度比较提供了有效手段，有助于系统改进开放基础模型和数据集。
...
<details>
  <summary>Details</summary>
Motivation: 能量模型（EBMs）训练困难，且现有方法通过变分下界训练可能导致不稳定。

Method: 提出使用双向边界（最大化下界和最小化上界），并研究了四种不同的对数似然边界。

Result: 双向边界稳定了EBM训练，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界方法有效提升了EBM的训练稳定性和性能。
...
### [142] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
*Zeming Wei,Yiwen Guo,Yisen Wang*

Main category: cs.LG

TL;DR: 论文通过类间特征归因的视角研究对抗训练（AT），发现跨类特征在AT初期对鲁棒性至关重要，但随着训练深入，模型更依赖类特定特征。


<details>
  <summary>Details</summary>
Motivation: 探究对抗训练的机制和动态，尤其是跨类特征在鲁棒性中的作用。

Method: 通过类间特征归因分析AT，理论验证跨类特征对鲁棒分类的作用，并进行多模型架构和设置的实验。
...
### [143] [Aligning Latent Spaces with Flow Priors](https://arxiv.org/abs/2506.05240)
*Yizhuo Li,Yuying Ge,Yixiao Ge,Ying Shan,Ping Luo*

Main category: cs.LG

TL;DR: 提出了一种利用基于流的生成模型作为先验，将可学习潜在空间与任意目标分布对齐的新框架。


<details>
  <summary>Details</summary>
Motivation: 传统方法在潜在空间对齐时需要进行昂贵的似然评估或ODE求解，本文旨在解决这一问题。

Method: 通过预训练流模型捕获目标分布，并设计对齐损失来正则化潜在空间，避免似然评估和ODE求解。
...
### [144] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
*Fanxiao Li,Jiaying Wu,Canyuan He,Wei Zhou*

Main category: cs.MM

TL;DR: 论文提出了一种名为CMIE的新框架，用于检测多模态大语言模型（MLLM）中的上下文无关（OOC）虚假信息，解决了现有方法在捕捉深层语义关系和证据噪声问题上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM在检测OOC虚假信息时难以捕捉图像与文本之间的深层语义关系，且证据噪声影响检测准确性。

Method: 提出CMIE框架，包含共存关系生成（CRG）策略和关联评分（AS）机制，通过识别图像与文本的共存关系并选择性利用证据来增强检测。

Result: 实验结果表明，CMIE优于现有方法。

Conclusion: CMIE通过改进语义关系捕捉和证据利用，有效提升了OOC虚假信息的检测性能。
...
### [145] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
...
<details>
  <summary>Details</summary>
Motivation: 探讨PEFT在联邦学习中的隐私问题，揭示恶意设计可能导致的用户数据泄露风险。

Method: 通过梯度反演攻击，利用可访问的适配器梯度重构用户的本地数据样本。
...
### [146] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
...
<details>
  <summary>Details</summary>
Motivation: 解决低光条件下泊松噪声的图像去噪问题，传统高斯噪声假设不适用。

Method: 集成Retinex分解与泊松去噪的编码器-解码器网络，结合泊松去噪损失函数。

Result: 显著提升低光条件下的可见性和亮度，保持图像结构和颜色一致性。

Conclusion: 方法有效且实用，适用于低光图像增强。
...
### [147] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
...
<details>
  <summary>Details</summary>
Motivation: 现有2D CNN在高光谱图像超分辨率任务中依赖局部邻域信息，缺乏全局上下文理解，且受限于波段相关性和数据稀缺性。

Method: DACN结合多头注意力捕捉局部和全局特征依赖，推断通道和空间维度的注意力图，并提出优化的损失函数（L2正则化与空间-光谱梯度损失结合）。
...
### [148] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
Conclusion: PixCell为计算病理学研究提供了强大的生成工具，模型已公开以加速研究。
...
### [149] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TL;DR: DM-SegNet是一种双Mamba架构，通过方向性状态转换和解剖感知分层解码，解决了现有医学SSM在编码器-解码器兼容性上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有医学SSM在序列建模中因1D序列扁平化破坏了空间结构，且传统解码器无法利用Mamba的状态传播能力。

Method: 提出四方向3D扫描的空间Mamba模块、门控空间卷积层和Mamba驱动的解码框架。
...
### [150] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
Method: 通过多领域专家与自动化代理协作生成和迭代优化样本，构建了包含13,000多个样本的HSSBench，涵盖六种联合国官方语言。

Result: 对20多个主流MLLMs的测试表明，HSSBench对最先进模型仍具挑战性。

Conclusion: HSSBench旨在推动MLLMs在跨学科推理能力上的研究，尤其是知识整合与连接能力的提升。
...
### [151] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
...
Method: 改进现有技术，从原始视频自动生成AVSR数据集。
...
### [152] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
...
Method: MMRefine通过六种场景和错误类型评估MLLMs的细化性能，并分析瓶颈和改进方向。
...
### [153] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
...
### [154] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TL;DR: 该研究提出了一种基于图像的可解释框架，分析中国哈尔滨社区街道的商业活力与车辆可达性、环境质量和行人感知的关系，发现适度车辆存在有助于商业，但过多停车会降低步行性和满意度。


<details>
  <summary>Details</summary>
Motivation: 研究旨在探索社区街道的商业活力如何受车辆可达性、环境质量和行人感知的影响，为城市设计和规划提供科学依据。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B），结合美团和大众点评数据构建社区商业活力指数（CCVI），并通过GPT-4感知建模分析其与空间属性的关系。

Result: 研究发现，适度车辆存在可提升商业可达性，但过多停车会降低步行性和满意度；绿化和清洁度显著提高满意度，但对价格影响较弱；街道宽度调节车辆影响。

Conclusion: 研究强调了车辆活动在社区商业中的条件性作用，展示了多模态AI在感知城市诊断中的可行性，为城市设计、停车管理和社区振兴提供了工具和理论支持。
