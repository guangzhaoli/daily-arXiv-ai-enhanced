...
Method: 提出DES框架，动态调整每个实例和训练迭代的扰动预算，结合梯度代理、softmax熵和Monte Carlo dropout。

Result: 在CIFAR-10和CIFAR-100上，DES显著优于固定预算和现有自适应方法。

Conclusion: DES为实例感知、数据驱动的对抗训练开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型缺乏明确的视觉定位与分割机制，导致认知推理与视觉感知之间存在差距。

Method: RSVP采用两阶段框架：推理阶段通过多模态链式视觉提示生成区域建议；分割阶段通过视觉语言分割模块优化分割掩码。

Result: 在ReasonSeg上超越现有方法+6.5 gIoU和+9.2 cIoU，SegInW零样本设置下达到49.7 mAP。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLM基准主要关注单图像推理或多图像理解任务的最终答案评估，缺乏对多图像输入推理能力的深入评估。
...
Method: 1. 使用图像扩散模型生成可控属性的真实人类图像数据集；2. 提出基于transformer的图像特征到3D点云的高效映射方法；3. 训练点云扩散模型，实现闭环生成。
...
<details>
  <summary>Details</summary>
Motivation: 填补胸部放射学视觉问答领域的空白，提供多样化和临床真实的任务，评估AI在放射学推理技能上的表现。

Method: 构建ReXVQA基准测试，包含五种核心放射学推理技能的任务，评估8种多模态大语言模型，并与人类专家进行对比研究。

Result: MedGemma模型表现最佳（83.24%准确率），超过人类专家（77.27%），揭示了AI与人类专家在性能模式上的差异。
...
Method: 通过视觉观察表示状态和动作，提供“动作等价物”以避免低层次连续性线索的干扰，基于部分可观察半MDP框架设计。
...
<details>
  <summary>Details</summary>
Motivation: 冰球检测因尺寸小、遮挡多、运动模糊等问题具有挑战性，球员行为（如身体朝向和视线）提供了强上下文线索。

Method: PLUCC包含上下文编码器、特征金字塔编码器和门控解码器，结合球员位置和朝向信息。

Result: 在PuckDataset上，PLUCC的平均精度提升12.2%，RSLE提升25%，优于基线方法。
...
Method: 使用视频Transformer模型，结合数据增强和10折分层交叉验证，在BdSLW60和BdSLW401数据集上进行训练和评估。

Result: VideoMAE模型在BdSLW60和BdSLW401数据集上分别达到95.5%和81.04%的准确率，表现最佳。

Conclusion: 视频Transformer模型在孟加拉手语识别中具有显著优势，适合大规模应用。
...
Conclusion: 激活最大化技术可灵活表征和调节人类视觉系统的响应，克服了其他方法的局限性。
...
Result: 实验表明，保护后的图像仍能被扩散模型成功编辑，且扰动可能增强编辑效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像滤波器经过精心归一化以避免失真，而深度学习中的卷积滤波器缺乏此类约束，导致在大气传输时响应失真。

Method: 提出滤波器归一化方法，结合可学习的缩放和偏移，类似于批量归一化，确保滤波器的大气等变性。

Result: 在人工和自然强度变化基准上取得显著改进，ResNet34甚至大幅超越CLIP。

Conclusion: 滤波器归一化不仅提升了性能，还正则化学习、促进多样性，并增强了鲁棒性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: VAR在图像生成中存在并行生成导致的质量下降、序列长度超线性增长以及采样计划固定的问题。

Method: HMAR采用马尔可夫过程和多步掩码生成，仅依赖前一尺度的信息，并分步生成子集。

Result: HMAR在ImageNet基准测试中表现优于VAR、扩散模型和自回归基线，训练和推理速度更快。
...
Method: 采用VIBA校准滚动快门RGB相机的高频轨迹，并结合高斯泼溅的物理图像形成模型处理传感器特性。

Result: 实验表明，VIBA和图像形成模型分别带来+1 dB的PSNR提升，总计+2 dB。

Conclusion: 提出的方法在多种光照条件下均表现优异，代码和数据集已开源。
...
Conclusion: 面部去正面化方法在大多数数据集上优于现有方法，但部分现有方法可能在小数据集上过拟合。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在资源受限的边缘设备上运行困难，FALO旨在解决这一问题。

Method: 将稀疏3D体素排列为1D序列，通过ConvDotMix块处理，结合大核卷积和隐式分组。

Result: 在nuScenes和Waymo基准测试中表现优异，推理速度比现有SOTA快1.6~9.8倍。

Conclusion: FALO适用于资源受限平台，实现了高效且高性能的3D检测。
...
Method: 结合分类与图像-文本对比学习，利用MLLMs生成文本，并集成数据不确定性学习。

Result: 在多个数据集上达到最先进性能，AUC提升显著。

Conclusion: AuthGuard通过语言引导和不确定性学习，显著提升了深度伪造检测的泛化性和解释性。
...
Method: 通过表示相似性（Centered Kernel Alignment）选择最佳子网络，迭代剪枝神经元和层。

Result: 在ResNet56和ResNet110上分别实现86.37%和95.82%的FLOPs减少，且模型性能保持或提升。

Conclusion: 该方法在高效剪枝的同时提升模型鲁棒性，为GreenAI和减少碳排放提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 智能终端设备因能量有限和低分辨率成像难以实现实时监测，需高效低功耗的裂缝检测方案。

Method: 使用SRCNN提升图像分辨率，SCU降低功耗，GAT模块融合多尺度特征增强检测鲁棒性。

Result: 在CrackVision12K上达到98.6%准确率，功耗仅5.6 mJ，比基线降低33%。

Conclusion: EECD-Net为资源受限环境提供可扩展、低功耗的实时裂缝检测方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过间接方式提升超分辨率性能，而LSK模块直接从频率角度增强图像质量。

Method: LSK模块设计为秩一矩阵，可分解为可合并的一维核，减少参数和计算量。

Result: 实验表明，LSK减少60%以上参数和计算量，同时提升模型性能，尤其在大比例放大时表现更优。

Conclusion: LSK模块是一种高效且可解释的超分辨率增强方法，适用于实际应用。
...
Method: 提出RACRO方法，通过推理引导的强化学习策略优化视觉描述生成，使其更符合推理需求。

Result: 在多项多模态数学和科学基准测试中达到最优性能，并支持更高级推理模型的即插即用。

Conclusion: RACRO方法有效提升了多模态推理的效率和适应性，为未来研究提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多分支CNN在多人体姿态估计中存在冗余结构和高延迟问题，且难以捕捉全局上下文信息。

Method: 设计了轻量级MobileViM Block和LARM模块，结合NPT-Op提取全局信息；引入SFusion模块融合多尺度信息。

Result: 在COCO和MPII数据集上，参数更少且性能更优，处理速度更快。

Conclusion: LGM-Pose通过单分支结构和创新模块设计，有效解决了轻量级姿态估计中的全局建模和效率问题。
...
Method: 通过深度点云渲染生成不可见区域掩码，结合用户编辑掩码构建复合掩码数据集，采用自迭代调优策略增强时间一致性，并引入时间包装模块提升生成质量。

Result: 方法在质量和多功能性上显著优于现有技术，支持基于提示的内容编辑，生成具有一致多视角的4D视频。

Conclusion: Follow-Your-Creation框架有效利用了基础模型的先验知识，实现了高质量的4D视频生成与编辑，具有强灵活性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法在具身智能中仅关注低层动作执行，忽视了高层规划和多级知识学习。

Method: 将学习过程分为高层指令和低层动作，提出HEC框架和Task-aware MoILE方法，利用视觉-文本嵌入聚类和双路由器选择LoRA专家，并通过SVD保留关键参数。

Result: 实验表明，该方法在减少旧任务遗忘方面优于其他方法，有效支持代理持续学习新任务。

Conclusion: HEC和Task-aware MoILE方法显著提升了具身智能的持续学习能力，尤其在知识保留和新任务学习方面表现优异。
...
Method: 利用视觉语言模型的常识推理能力，结合参数化生成器，通过自主验证循环迭代调整生成参数。

Result: 生成的化身支持姿势操作，质量优于现有文本和图像驱动系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度增强方法在不可靠区域检测和几何结构保持方面存在不足，需要一种更可靠的方法。

Method: 分为两阶段：随机估计阶段通过训练-推理域差距识别不可靠区域并推断几何结构；确定性细化阶段利用不确定性图增强结构一致性和像素级精度。

Result: 实验证明该方法在多种真实场景中有效，生成密集、无伪影的深度图，具有更高的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 传统图像匹配流程在复杂场景中表现不佳，深度学习显著提升了鲁棒性和准确性，因此需要系统梳理其变革。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI基准主要关注语言推理，忽视了非语言、多步视觉模拟的复杂性，因此需要开发一个专门评估空间认知能力的基准。

Method: STARE包含4K个任务，涵盖几何变换、空间推理和现实世界空间推理，通过多步视觉模拟来测试模型表现。

Result: 模型在简单2D任务上表现良好，但在复杂任务（如3D折叠和拼图）上接近随机水平；人类通过视觉模拟显著提升效率，而模型未能有效利用中间视觉信息。

Conclusion: 模型在复杂空间推理任务中表现不足，表明其在利用视觉模拟方面存在局限性，未来需改进多模态推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 生成模型在图像超分辨率中处理真实世界退化时，常导致文本结构失真，因此需要一种能够同时恢复自然细节和文本结构的方法。

Method: 提出TADiSR框架，结合文本感知注意力和联合分割解码器，并开发了合成高质量图像的全流程。

Result: 实验表明，TADiSR在超分辨率图像中显著提升了文本可读性，并在多个评估指标上达到最优性能。

Conclusion: TADiSR在真实场景中表现出强大的泛化能力，为图像超分辨率提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 扩散生成模型在高质量视频生成中表现优异，但推理速度慢和计算需求高限制了实际应用。现有方法中量化和稀疏化的简单结合会导致性能下降。

Method: FPSAttention通过统一的3D分块粒度、去噪步骤感知策略和优化的硬件友好内核，实现了量化和稀疏化的联合优化。

Result: 在720p分辨率下，FPSAttention实现了注意力操作7.09倍的内核加速和视频生成4.96倍的端到端加速，且生成质量未受影响。
...
<details>
  <summary>Details</summary>
Motivation: 传统表示学习假设解耦的独立特征轴是好的表示，但无法解释条件独立性。本文旨在克服这一问题，并推动对人类物体识别发展的理解。

Method: 通过结合特征提取和对象分割，将像素平移替换为特征平移，并将对象分割视为在相同变换下的特征分组。

Result: 在包含真实世界物体和背景的实用数据集上验证了所提方法的有效性。

Conclusion: 该方法有望为更一般的表示定义提供基础，并促进对人类在现实世界中物体识别发展的理解。
...
<details>
  <summary>Details</summary>
Motivation: 自解释模型在数据稀缺时表现不佳，主要由于参数不平衡和表示对齐问题。

Method: 采用Mixture of LoRA Experts (MoLE)进行参数高效适应，结合跨模块概念指导和几何感知概念判别损失。

Result: 在六个基准测试中，5-way 5-shot分类任务中相对提升了4.2%-8.7%。

Conclusion: FSPCC框架在提升准确性的同时增强了模型的可解释性，为透明视觉识别系统铺平了道路。
...
<details>
  <summary>Details</summary>
Motivation: 当前合成数据生成方法存在多对象掩码、分割不准确和类别标签错误等问题，限制了其有效性。

Method: Gen-n-Val包含两个代理：LD提示代理优化提示以生成高质量前景实例和分割掩码；数据验证代理过滤低质量合成数据。系统提示通过TextGrad优化，并使用图像协调技术组合多个实例。

Result: Gen-n-Val将无效合成数据从50%降至7%，在COCO实例分割中稀有类别的mAP提升1%，在开放词汇目标检测中mAP提升7.1%。

Conclusion: Gen-n-Val显著提升了实例分割和目标检测的性能，解决了合成数据生成中的关键问题。
...
<details>
  <summary>Details</summary>
Motivation: 传统插值和修复方法缺乏环境感知能力，而许多深度学习方法依赖详细场景数据，限制了泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有VLM安全方法在数据集覆盖和架构创新上存在不足，导致模型易受攻击且安全性有限。

Method: 引入HoliSafe数据集覆盖所有安全/不安全图像-文本组合，并提出SafeLLaVA模型，包含可学习安全元标记和专用安全头。
...
Method: 使用LlaVA-Next模型分析ImageNet类别的线性特征，并通过稀疏自编码器训练多模态特征。

Result: 发现图像和文本特征在深层逐渐共享，但初始表示较为分离。
...
<details>
  <summary>Details</summary>
Motivation: 预训练VLM在少样本适应任务中表现良好，但面对OOD测试数据时性能下降，因此需要提升其OOD泛化能力。

Method: 通过比较不同适应方法，发现部分微调视觉编码器效果最佳，并提出结合检索增强和对抗扰动的两阶段方法SRAPF。

Result: SRAPF在ImageNet OOD基准测试中实现了最先进的ID和OOD准确性。

Conclusion: 部分微调视觉编码器结合检索增强和对抗扰动是提升VLM少样本适应任务性能的有效方法。
...
<details>
  <summary>Details</summary>
Motivation: 预测ESD视频中的解剖轨迹对提升手术技能训练和简化学习过程有重要意义，但目前研究不足。模仿学习在处理不确定的未来动作、几何对称性和多样化手术场景时存在挑战。

Method: 提出iDPOE方法，通过联合状态动作分布建模专家行为，结合扩散模型进行策略学习，并嵌入等变性以提升几何对称性的泛化能力。还开发了前向过程引导的动作推理策略。
...
<details>
  <summary>Details</summary>
Motivation: 多数据集结合可提升泛化能力，但类语义和标注框不一致阻碍了效果。现有方法或假设共享标签分类，或需手动重标注，或生成统一标签空间，但可能不适用于固定目标标签空间。

Method: 提出LAT框架，通过训练数据集特定检测器生成伪标签，结合Privileged Proposal Generator和Semantic Feature Fusion模块，实现标签空间转移和特征细化。

Result: 在多个基准测试中，LAT显著提升目标域检测性能，最高提升4.8AP。

Conclusion: LAT无需共享标签空间或手动标注，同时解决了类级和标注框不一致问题。
...
<details>
  <summary>Details</summary>
Motivation: 研究发现VLMs在图像描述生成任务中容易受到后门攻击，攻击者通过注入微小扰动（如局部像素触发器或全局语义短语）控制模型生成恶意描述。这些攻击隐蔽且跨模态，难以检测和防御。

Method: SRD通过强化学习框架，利用Deep Q-Network学习对敏感图像区域施加离散扰动（如遮挡、颜色掩码），以破坏恶意路径的激活。同时设计了语义保真度评分作为奖励信号，评估输出的语义一致性和语言流畅性。
...
Method: 采用双流架构，分别通过物理估计器和胶囊聚类学习传输图和背景光，同时优化物理和感知质量。

Result: 在六个基准测试中，PSNR提升0.5dB，计算复杂度仅为现有方法的三分之一。

Conclusion: 该方法在物理约束和语义保留方面表现出色，计算效率高。
...
<details>
  <summary>Details</summary>
Motivation: 传统认为多模态大模型需要大量训练数据，但实际中仅有少数样本对推理能力提升有效，其余数据冗余且计算成本高。

Method: 提出RAP方法，包含两个互补估计器（CDE和ACE）识别关键样本，并通过DRM模块替换简单样本以提升复杂性。

Result: 在六个数据集上，RAP仅用9.3%数据即实现更优性能，计算成本降低43%。

Conclusion: RAP证明了高效数据选择对多模态推理的重要性，为减少数据冗余和计算成本提供了新思路。
...
Method: 提出了一种新的SSIM形式，使用加法组合组件，并通过实验优化参数组合。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中的视觉环境具有层次结构，双曲空间能更好地表示这种结构并保持特征空间的距离特性。

Method: 采用双曲空间表示层次特征关系，提出层次特征聚合机制和粗到细的搜索策略。

Result: 在多个基准数据集上，HypeVPR优于现有方法，检索速度提升高达5倍。

Conclusion: HypeVPR通过层次嵌入和高效搜索，为P2E VPR提供了更优的解决方案。
...
TL;DR: LotusFilter是一种后处理模块，用于多样化近似最近邻搜索（ANNS）结果，通过预计算邻近向量表快速删除冗余候选向量。


<details>
  <summary>Details</summary>
Motivation: 在RAG等应用中，ANNS结果可能过于相似，需要同时保证相似性和多样性。

Method: 预计算邻近向量表，并在过滤阶段贪婪查找以删除冗余向量。

Result: 在类似真实RAG场景中，LotusFilter运行速度快（0.02 ms/查询）。
...
TL;DR: SupeRANSAC是一种新型统一的RANSAC管道，旨在在多种视觉任务中实现一致的准确性，性能优于现有最佳方法。


<details>
  <summary>Details</summary>
Motivation: RANSAC及其变体在计算机视觉中是几何模型估计的黄金标准，但不同库的实现细节和问题特定优化对性能影响较大。

Method: 提出SupeRANSAC，详细分析了使RANSAC在特定视觉任务中有效的技术。

Result: SupeRANSAC在多个问题和数据集上显著优于现有技术，例如在基础矩阵估计上平均提高6 AUC点。

Conclusion: SupeRANSAC通过统一管道和优化技术，实现了在多种视觉任务中的一致高性能。
...
TL;DR: 论文介绍了MegaHan97K数据集，填补了中文超大类别字符识别领域的数据空白，支持最新GB18030-2022标准，包含97,455个类别，并解决了长尾分布问题。


<details>
  <summary>Details</summary>
Motivation: 中文字符类别庞大且不断扩展，现有数据集无法满足超大类别识别需求，阻碍了文化遗产保护和数字应用的发展。

Method: 构建了MegaHan97K数据集，包含手写、历史和合成三个子集，覆盖97,455个类别，支持GB18030-2022标准。

Result: 实验揭示了超大类别场景下的新挑战（如存储需求增加、形态相似字符识别等），同时为未来研究提供了机会。

Conclusion: MegaHan97K是目前OCR和模式识别领域类别最多的数据集，为超大类别字符识别研究奠定了基础。
...
Method: 提出Spike-TBR，通过整合脉冲神经元增强TBR的抗噪能力，评估了四种不同脉冲神经元的变体。

Result: 在噪声场景下表现优异，同时在干净数据上也有提升。

Conclusion: Spike-TBR填补了基于脉冲和基于帧处理的鸿沟，为事件驱动视觉应用提供了简单且抗噪的解决方案。
...
TL;DR: 论文提出了一种针对交通灯检测CNN的对抗性攻击方法，通过打印的补丁实现攻击。


<details>
  <summary>Details</summary>
Motivation: 现有研究较少关注交通灯检测器的攻击，本文填补了这一空白。

Method: 提出了一种威胁模型，通过在交通灯下方放置补丁进行攻击，并设计了训练策略。

Result: 实验展示了成功的通用对抗补丁攻击，包括红转绿的标签翻转攻击和象形图分类攻击。
...
TL;DR: DualX-VSR是一种新型的双轴时空Transformer模型，用于解决真实世界视频超分辨率任务中的像素级精度问题。
...
Method: 提出双轴时空注意力机制，沿正交方向整合时空信息，无需运动补偿。

Result: DualX-VSR在真实世界视频超分辨率任务中实现了高保真和卓越性能。

Conclusion: DualX-VSR通过简化结构和改进注意力机制，有效提升了视频超分辨率的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的感知系统在2D推理分割方面表现优异，但缺乏适用于3D推理分割的框架和结构。

Method: 引入SEG token和对象标识符，处理点云数据和文本提示，生成高精度的3D分割掩码。

Result: 在大规模ScanNet数据集上的实验验证了OpenMaskDINO3D在各种任务中的有效性。

Conclusion: OpenMaskDINO3D能够直接从自然语言指令生成准确的点云分割结果，为3D推理分割提供了新思路。
...
TL;DR: 提出了一种通过毒化训练数据在图像编辑过程中嵌入不可见触发器的后门攻击框架，利用现成的水印模型作为触发器，实验证明其攻击成功率较高。


<details>
  <summary>Details</summary>
Motivation: 现有研究多关注图像生成的后门攻击，而图像编辑领域的后门攻击较少，且现有方法多使用可见触发器，实用性不足。

Method: 利用现成的水印模型将不可见水印编码为后门触发器，通过毒化训练数据实现攻击。

Result: 在不同水印模型上实验，攻击成功率较高，且水印特性分析支持方法的有效性。

Conclusion: 提出的方法在图像编辑中实现了高效的后门攻击，且触发器不可见，具有实际应用潜力。
...
TL;DR: 论文提出了一种基于直觉驱动的规划方法，通过监督对比学习训练Transformer编码器，将棋盘状态嵌入到潜在空间中，实现无需深度搜索的走子选择。
...
Conclusion: 潜在规划方法可推广到其他完美信息游戏，为无需深度搜索的规划提供了新思路。
...
TL;DR: Anywhere3D-Bench是一个全面的3D视觉定位基准，涵盖四个不同级别的任务，揭示了当前模型在空间和部分级别任务上的局限性。


<details>
  <summary>Details</summary>
Motivation: 探索3D场景中超越对象级别的视觉定位任务，填补现有研究的空白。

Method: 构建Anywhere3D-Bench基准，评估现有3D视觉定位方法、LLMs和MLLMs的性能。

Result: 空间和部分级别任务表现最差，最佳模型OpenAI o4-mini在空间级别任务上仅达到23.57%准确率。
...
TL;DR: 本文提出了一种基于3D高斯泼溅（3DGS）的立体数据集生成方法，相比基于NeRF的方法更高效。通过结合显式3D重建几何和FoundationStereo模型的深度估计，展示了在零样本泛化基准测试中的竞争力。


<details>
  <summary>Details</summary>
Motivation: 探索一种高效且低成本的立体数据集生成方法，以替代传统的NeRF方法，并提升深度立体模型的微调速度和性能。

Method: 结合3DGS生成的几何重建和FoundationStereo模型的深度估计，通过专家知识转移生成高质量数据集。

Result: 3DGS生成的几何存在噪声和伪影，而FoundationStereo的视差估计更干净，在零样本泛化基准测试中表现更好。

Conclusion: 3DGS方法在低成本、高保真数据集生成和快速微调方面具有潜力，但在复杂场景中的鲁棒性仍需进一步研究。
...
TL;DR: 论文探讨了文化遗产数字化中3D摄影方法的多样性，强调应根据对象特性和未来用途选择合适的方法。


<details>
  <summary>Details</summary>
Motivation: 文化遗产数字化需求广泛，但现有方法无法适用于所有对象，需根据具体情况调整工具。

Method: 通过法国国家考古博物馆的藏品案例，分析不同3D摄影数字化方法的适用性。

Result: 研究表明，没有单一方法适用于所有对象，需结合遗产和数字领域专家的意见选择工具。

Conclusion: 应避免对3D数字化方法进行绝对分类，而是根据对象特性和数字孪生的未来用途选择最佳工具。
...
<details>
  <summary>Details</summary>
Motivation: 为猞猁的个体识别、姿态估计和实例分割提供首个大规模数据集，支持跨时空域的泛化测试。

Method: 数据集包含30k真实相机陷阱图像和100k合成图像，通过Unity和扩散模型生成，覆盖多样环境和姿态。

Result: 数据集包含219个独特个体，提供三种评估协议（地理感知、时间感知开放集和封闭集）。

Conclusion: CzechLynx将有助于评估先进模型和开发新方法，不仅限于个体动物重识别。
...
Method: CoSTeM框架结合形态学和形态动力学分支，分别提取局部结构特征和全局发育轨迹，整合静态和动态因素进行分级。

Result: 实验结果表明该设计优越，为AI辅助胚胎选择提供了方法论框架。

Conclusion: 该工作填补了胚胎评估的空白，数据集和源代码将公开。
...
TL;DR: 提出一种通过架构设计而非数据训练来增强图像质量评估模型鲁棒性的方法。


<details>
  <summary>Details</summary>
Motivation: 传统的数据驱动防御方法（如对抗训练）可能不是解决IQA模型脆弱性的最佳途径，探索通过设计模型内部结构来提升鲁棒性。

Method: 通过强制正交信息流和保持范数的操作来重塑模型内部结构，并结合剪枝和微调进一步稳定系统。

Result: 设计出的IQA架构能够抵御对抗攻击，且无需对抗训练或对原模型进行大幅修改。

Conclusion: 提出了一种新视角：通过设计而非数据优化来实现模型鲁棒性。
...
TL;DR: APVR框架通过分层视觉信息检索解决视频多模态大语言模型在小时级视频理解中的计算限制和信息提取效率问题。


<details>
  <summary>Details</summary>
Motivation: 当前基于视频的多模态大语言模型由于计算限制和长时序信息提取效率低，难以处理小时级视频理解。

Method: APVR框架包含两个互补组件：Pivot Frame Retrieval通过语义扩展和多模态置信度评分识别语义相关视频帧；Pivot Token Retrieval在选定帧内进行查询感知的注意力驱动令牌选择。

Result: 在LongVideoBench和VideoMME上的实验验证表明，APVR显著提升了性能，不仅优于无训练方法，也超越了基于训练的方法，同时具备即插即用的集成能力。

Conclusion: APVR通过双粒度方法实现了小时级视频的高效处理，同时保持了语义保真度，为视频理解提供了新的解决方案。
...
TL;DR: FEAT是一种高效的全维度注意力Transformer，通过空间-时间-通道注意力机制、线性复杂度设计和残差值引导模块，显著提升了动态医学视频合成的质量。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法在通道交互、计算复杂度和噪声处理方面存在不足，需要更高效的模型来提升动态医学视频合成的质量。
...
TL;DR: 比较了利用Segment Anything Model (SAM)进行无人机图像中树冠实例分割的方法，并提出了结合数字表面模型(DSM)的BalSAM模型，在特定场景下表现优异。
...
Method: 比较了SAM在三种森林类型中的应用，研究了DSM数据的整合，并提出了BalSAM模型。

Result: BalSAM在种植园场景中表现优于其他方法，但未经调整的SAM表现不及定制Mask R-CNN。

Conclusion: 端到端调优SAM和整合DSM数据是提升树冠实例分割模型的有效途径。
...
TL;DR: 该论文提出了TextVidBench，首个针对长视频文本问答的基准测试，并提出了改进大模型的方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集视频时长短、评估范围窄，难以评估多模态大语言模型的能力。

Method: 提出了IT-Rope机制、非均匀位置编码和轻量级微调方法。

Result: TextVidBench对现有模型提出了显著挑战，提出的方法有效提升了长视频文本理解能力。

Conclusion: TextVidBench为长视频文本问答提供了更真实的评估基准，提出的方法具有实用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有VARSR方法存在固定分辨率限制和大模型依赖问题，本文旨在通过创新组件解决这些问题。

Method: 提出分层图像标记化和DPO正则化项，训练量化器实现多尺度语义一致残差，并利用偏好优化训练VAR。

Result: 模型在单次前向传递中实现超分辨率，使用小模型（300M参数）和无外部数据，达到最先进结果。

Conclusion: 本文方法在效率和性能上优于现有VARSR，展示了多尺度标记化和偏好优化的潜力。
...
TL;DR: PATS是一种新型视频采样策略，通过保留完整的基础动作片段来提升多视角技能评估的准确性。
...
Result: 在EgoExo4D基准测试中，PATS在所有视角配置下均超越现有方法，尤其在动态运动领域表现突出。
...
TL;DR: 论文摘要介绍了单目深度估计的发展，重点讨论了深度学习在该领域的应用及其挑战，特别是泛化到未见过的领域的问题。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计的目标是从RGB图像中确定每个像素的深度。尽管深度学习推动了该领域的发展，但泛化到未见过的领域仍是一个挑战。

Method: 论文提到的方法包括多尺度融合网络、将回归任务重新定义为分类问题、引入额外先验知识、设计更有效的目标函数，以及使用仿射不变损失进行多数据集联合训练。

Result: Depth Anything在零样本单目深度估计中表现出色，能够从未见过的图像中提取结构信息，生成结构详细的深度图。

Conclusion: 尽管Depth Anything在度量深度估计上存在不足，但其在提取结构信息方面表现优异，为单目深度估计提供了新的可能性。
...
<details>
  <summary>Details</summary>
Motivation: 真实3D点云数据的获取和标注成本高且耗时，需要一种低成本、灵活的合成数据生成方法。

Method: 利用3D高斯泼溅和高斯不透明度场生成农业车辆的3D资产，并在模拟环境中通过LiDAR生成点云数据。
...
TL;DR: UAV4D框架解决了无人机拍摄动态场景的渲染挑战，通过结合3D基础模型和人体网格重建模型，实现了多行人动态场景的高质量渲染。


<details>
  <summary>Details</summary>
Motivation: 现有动态神经渲染方法难以处理无人机拍摄的单目相机、俯视角和多小移动行人场景，缺乏相关数据集支持。

Method: 结合3D基础模型和人体网格重建模型，利用SMPL模型和背景网格初始化高斯点，解决场景尺度模糊问题。
...
TL;DR: 提出了一种新型物理标注系统，用于生成自动化光学检测的训练数据，通过指针交互和投影界面提高标注效率和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统屏幕标注方法不够直观高效，无法充分利用人工检查员的专业知识，导致训练样本丢失。

Method: 使用校准的跟踪指针记录用户输入，结合投影界面提供视觉引导，将物理交互转化为标准化标注格式。
...
TL;DR: 本文综述了越南文档分析与识别（DAR）的现状，探讨了现有技术的局限性，并提出了利用大语言模型（LLMs）改进该领域的潜力。


<details>
  <summary>Details</summary>
Motivation: 越南文本识别因复杂的变音符号、声调变化和缺乏大规模标注数据集而面临挑战，传统OCR方法效果有限，深度学习受数据稀缺和泛化问题制约。

Method: 通过综述现有技术，分析LLMs和视觉语言模型在越南DAR中的应用潜力。

Result: LLMs和视觉语言模型在文本识别和文档理解方面表现出显著改进，但仍需解决领域适应、多模态学习和计算效率等问题。

Conclusion: 未来研究方向包括数据集开发、模型优化和多模态方法集成，以推动越南DAR的进步。
...
<details>
  <summary>Details</summary>
Motivation: 提升图像编辑工具的指令跟随能力和内容保留能力，特别是在真实图像输入场景下。

Method: 1. 开发了增强的数据整理流程和元信息嵌入策略；2. 引入了联合学习管道，结合扩散损失和奖励损失。

Result: 在测试基准中，SeedEdit 3.0 实现了 56.1% 的高可用率，优于之前版本和其他模型。
...
<details>
  <summary>Details</summary>
Motivation: 随着FAA工具的普及，其可信度问题（如偏见、隐私等）日益突出，需要研究解决。

Method: 通过研讨会形式，汇集研究者探讨FAA任务中的可信度挑战，如宏/微表情识别、面部动作单元检测等。

Result: 支持FG2025的伦理目标，推动可信FAA的研究与讨论。

Conclusion: 研讨会致力于推动FAA的可信度研究，促进伦理与技术结合。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散变压器（vDiT）在文本到视频生成方面取得了显著进展，但其高计算需求限制了实际部署。现有加速方法多依赖启发式规则，适用性有限。
...
TL;DR: 论文提出了DIM-CIM框架，用于无参考评估文本到图像模型的多样性和泛化能力，并通过COCO-DIMCIM基准测试发现模型在参数增加时泛化能力提升但默认模式多样性下降。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测模型在非理想条件下表现不稳定，容易被规避。受图像超分辨率领域退化模型的启发，作者希望开发一个更全面的伪造模型。

Method: 提出PMM，通过Poisson混合、多样化掩码、生成器伪影和干扰物扩展伪造空间，并在训练图像中添加强退化以增强检测器的泛化能力。

Result: 在DFDC和DFDCP数据集上，PMM分别比现有最佳模型（LAA backbone）提高了3.51%和6.21%的AUC。

Conclusion: PMM显著提升了检测模型对常见图像退化的鲁棒性，并改善了标准基准数据集上的性能，同时揭示了以往检测器的鲁棒性不足。
...
Result: VLMs仅能准确识别有限的基本对象属性，性能受对象位置影响，且难以理解对象间关系，未达到人类水平。
...
TL;DR: FRED数据集结合RGB和事件相机数据，专为无人机检测、跟踪和轨迹预测设计，包含7小时标注数据，涵盖多种挑战场景。


<details>
  <summary>Details</summary>
Motivation: 传统RGB相机在捕捉快速移动的无人机时受限，事件相机虽有优势，但现有基准缺乏精细时间分辨率或无人机特定运动模式。

Method: 提出FRED数据集，结合RGB视频和事件流，包含5种无人机模型及挑战性场景（如雨天和恶劣光照）。

Result: 数据集提供7小时密集标注轨迹，并制定详细评估协议和标准指标。
...
<details>
  <summary>Details</summary>
Motivation: 作者认为当前领域过于依赖神经网络，而早期基于模板匹配的方法具有更强的物理可解释性，因此尝试回归传统方法。

Method: 通过生成距离-时间图和多普勒-时间图，使用角点检测和多相主动轮廓模型分割微多普勒特征，再通过Mapper算法计算拓扑相似性。

Result: 数值模拟和实测实验验证了方法的有效性。

Conclusion: 该方法展示了非神经网络方法在穿墙雷达人体活动识别中的潜力。
...
Method: 提出TAO框架，将异常检测转化为像素级跟踪，结合分割和跟踪任务。
...
TL;DR: TAPFM是一种用于单GPU任务适应的新方法，通过优化特征表示和注意力权重，有效适应病理基础模型（PFMs）于临床任务。


<details>
  <summary>Details</summary>
Motivation: 由于全切片图像（WSIs）仅有弱标签，传统方法难以有效适应PFMs，需要多实例学习（MIL）范式。

Method: 提出TAPFM方法，利用视觉变换器（ViT）注意力进行MIL聚合，并优化特征和注意力权重，同时保持MIL聚合器和PFM的独立计算图。
...
TL;DR: 论文提出了一种多模态低秩适应方法（MokA），针对当前多模态微调方法的局限性，通过模态特定参数压缩单模态信息并增强跨模态交互，显著提升了多模态大语言模型的微调效果。


<details>
  <summary>Details</summary>
Motivation: 当前高效多模态微调方法多直接借鉴自LLMs，忽视了多模态场景的固有差异，导致无法充分利用所有模态。作者通过实证观察，提出单模态适应和跨模态适应是多模态微调的两个关键部分。

Method: 提出MokA方法，通过模态特定参数压缩单模态信息，同时显式增强跨模态交互，确保单模态和跨模态的协同适应。

Result: 在多种多模态场景（如视听文本、视觉文本、语音文本）和LLM骨干网络（如LLaMA2/3、Qwen2等）上的实验表明，MokA方法具有一致性的性能提升。

Conclusion: MokA为多模态大语言模型的高效适应提供了更具针对性的解决方案，为后续研究奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 解决在复杂环境中毫米波通信的可靠性问题，尤其是在城市或基础设施有限的场景中。

Method: 利用单目摄像头检测ArUco标记，实时估计到达角并动态调整金属反射板以优化信号反射。

Result: 实验结果显示，在60GHz频段下，接收信号强度平均增益为23dB，信号接收概率达0.89。
...
<details>
  <summary>Details</summary>
Motivation: 研究神经网络在多模态学习中的记忆特性，特别是跨模态记忆，以解决敏感信息泄露和知识获取效率问题。

Method: 引入合成人物数据集，训练模型在单一模态上并评估其在另一模态的表现，量化知识记忆和跨模态可转移性。

Result: 发现模态间知识可转移，但源模态和目标模态间存在显著差距，且这一差距在不同场景下普遍存在。

Conclusion: 提出了一种缓解跨模态差距的基线方法，希望促进更鲁棒的多模态学习技术发展。
...
TL;DR: DEGround模型通过共享DETR查询实现检测与定位，结合区域激活和查询调制模块，显著提升3D定位性能。


<details>
  <summary>Details</summary>
Motivation: 研究探讨3D定位是否充分受益于检测，发现现有方法表现不佳，提出改进方案。

Method: 提出DEGround框架，共享DETR查询，引入区域激活和查询调制模块。

Result: 在EmbodiedScan验证集上，DEGround比BIP3D模型准确率提升7.52%。

Conclusion: DEGround通过结合检测与语言指令理解，显著提升了3D定位性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法需要密集输入视图且计算成本高，而通用方法难以重建输入视锥之外的区域。OGGSplat旨在解决这些问题。

Method: 利用开放高斯的语义属性作为先验，结合RGB-语义一致性修复模块和双向控制扩散模型，逐步优化高斯参数。

Result: 建立了高斯外推（GO）基准测试，验证了OGGSplat在语义和生成质量上的优越性，并在智能手机拍摄的两视图图像上展示了良好的重建能力。

Conclusion: OGGSplat在稀疏视图下实现了语义一致且视觉合理的3D场景重建，为虚拟现实和具身AI应用提供了有效解决方案。
...
TL;DR: VLG模型通过视觉-语言-服装多模态合成服装，展示了在零样本情况下对未见服装风格和提示的迁移能力。
...
Result: 初步结果显示VLG在零样本情况下具有迁移潜力。

Conclusion: 多模态基础模型在时尚设计等专业领域具有有效适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有世界建模方法在训练难度、3D一致性和多阶段处理上存在不足，DSG-World通过双状态观测互补性解决了这些问题。

Method: 利用双状态观测构建双分割感知高斯场，通过双向光度和语义一致性约束，结合伪中间状态对称对齐和协作剪枝策略。

Result: 实验表明DSG-World在新视角和场景状态下具有强泛化能力，支持高保真渲染和对象级场景操作。

Conclusion: DSG-World为真实世界3D重建和模拟提供了一种高效且物理一致的方法。
...
TL;DR: MonkeyOCR是一种基于SRR三元范式的视觉语言模型，用于文档解析，在准确性和速度上均优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的文档解析方法要么过于复杂（如MinerU的模块化方法），要么效率低下（如大型多模态LLM），MonkeyOCR旨在通过SRR范式简化流程并提高效率。

Method: 采用Structure-Recognition-Relation (SRR) 三元范式，将文档解析分解为三个基本问题：结构（布局分析）、识别（内容识别）和关系（逻辑排序）。

Result: MonkeyOCR在多个指标上优于现有方法，特别是在公式和表格解析上表现突出，且3B参数模型在性能上超越更大的模型（如Qwen2.5-VL和Gemini 2.5 Pro）。

Conclusion: MonkeyOCR通过SRR范式实现了高效、可扩展的文档解析，并在性能和速度上均达到领先水平。
...
TL;DR: SAM-TTA提出了一种新的测试时适应框架，解决了SAM在医学图像分割中的输入和语义差异问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: SAM在医学图像分割中表现不佳，现有方法如MedSAM牺牲了泛化能力。SAM-TTA旨在保留SAM的泛化能力并提升其在医学图像中的性能。
...
TL;DR: MoViX是一个自监督的跨视角视频定位框架，用于GPS缺失的越野环境，通过学习和季节不变表示实现高精度定位。
...
Conclusion: MoViX在无需环境特定调优的情况下，实现了高精度定位，并展示了跨地理位置的泛化能力。
...
TL;DR: 本文提出了一种名为LeanPO的无参考方法，通过重新定义奖励机制和动态标签平滑策略，解决了Video-LLMs中偏好对齐技术导致的非目标响应概率意外增加的问题。


<details>
  <summary>Details</summary>
Motivation: 现有Video-LLMs采用的偏好对齐技术（如DPO）在训练过程中会导致目标和非目标响应的对数概率同时下降，从而无意中提升了非目标响应的概率。这一现象在处理视频内容的冗余复杂性时更为严重。

Method: 提出了LeanPO方法，通过将隐式奖励重新定义为策略模型对响应的平均似然，并结合自生成偏好数据管道和动态标签平滑策略，优化模型训练。

Result: 实验表明，LeanPO显著提升了最先进Video-LLMs的性能，且额外训练开销极小。

Conclusion: LeanPO为Video-LLMs提供了一种简单有效的偏好对齐解决方案，有助于实现可靠且高效的Video-LLMs。
...
TL;DR: 本文探讨了如何利用基础模型（FM）提升ID卡防伪检测（PAD）的泛化能力，特别是在面对不同国家ID卡时的表现。


<details>
  <summary>Details</summary>
Motivation: 当前PAD系统因隐私保护问题仅针对少数ID卡训练，导致在新国家ID卡上表现不佳，FM因其大数据训练可能改善这一问题。

Method: 通过零样本学习和微调两种方式，在两个ID卡数据集（智利ID和芬兰、西班牙、斯洛伐克ID）上测试FM的泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 解决真实场景中因光度不一致导致的重建质量下降问题，现有方法（外观编码和双边网格）各有局限性。
...
Method: 通过自监督编码器学习连续点速度场，将噪声点传输到目标位置，恢复部件姿态。

Result: 在六个基准测试中达到最先进性能，统一框架支持跨数据集联合训练，提升几何先验学习。
...
TL;DR: 论文提出了一种基于几何基础的长时空间记忆框架，以提升视频世界模型的长时一致性，解决了现有模型因时间窗口限制导致的场景遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有视频世界模型因时间上下文窗口有限，难以在场景重访时保持一致性，导致严重遗忘。受人类记忆机制启发，作者希望通过长时空间记忆提升模型的长时一致性。

Method: 提出了一种几何基础的长时空间记忆框架，包含存储和检索机制，并通过定制数据集训练和评估带有显式3D记忆机制的模型。

Result: 实验表明，该方法在质量、一致性和上下文长度上优于基线模型，为长时一致的世界生成提供了新思路。

Conclusion: 该框架通过引入长时空间记忆，显著提升了视频世界模型的长时一致性，为未来研究奠定了基础。
...
Method: 使用前馈Transformer预测查询射线的深度图、对象掩码和置信度分数，并通过多视角融合完成3D重建。
...
TL;DR: 论文提出了Vision Concept Transformer (VCT)和Stable Vision Concept Transformer (SVCT)，以解决现有概念瓶颈模型在医学领域中忽略图像特征和解释不稳定性的问题。


<details>
  <summary>Details</summary>
Motivation: 医学领域对透明度的需求促使研究者探索可解释AI（XAI）。现有概念瓶颈模型（CBMs）仅依赖概念特征，忽略了医学图像的固有特征，且在面对输入扰动时解释不稳定。

Method: VCT结合概念特征与图像特征；SVCT基于VCT，采用视觉变换器（ViT）作为主干，并引入Denoised Diffusion Smoothing以增强稳定性。

Result: 在四个医学数据集上的实验表明，VCT和SVCT在保持准确性的同时具有可解释性；SVCT在面对扰动时仍能提供稳定的解释。

Conclusion: VCT和SVCT在医学领域中实现了准确性与可解释性的平衡，SVCT尤其适用于需要稳定解释的场景。
...
TL;DR: EOC-Bench是一个用于评估多模态大语言模型在动态自我中心场景中对象认知能力的新基准，包含3,277个QA对，覆盖11个评估维度和3种视觉对象引用类型。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注静态场景，缺乏对动态交互中对象变化的评估，因此需要开发一个系统性的评估工具。

Method: 设计了混合格式的人工标注框架和多尺度时间准确性指标，用于评估不同时间类别的QA对。

Result: 对多种MLLM进行了全面评估，展示了EOC-Bench在提升模型对象认知能力方面的有效性。

Conclusion: EOC-Bench为提升MLLM在动态场景中的对象认知能力提供了重要工具，并为开发可靠的嵌入式系统核心模型奠定了基础。
...
TL;DR: 提出了一种新的Aligned Tokenizer (AliTok)，通过因果解码器建立单向依赖关系，解决了现有图像分词器在自回归模型中的建模问题，并在生成友好性和重建性能上表现优异。
...
TL;DR: SeedVR2是一种基于扩散模型的一步视频修复方法，通过对抗训练和动态窗口注意力机制，显著降低了计算成本，同时保持了高质量输出。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在视频修复中计算成本高，且现有的一步修复方法难以扩展到高分辨率视频。SeedVR2旨在解决这些问题。

Method: 提出动态窗口注意力机制和多种损失函数，包括特征匹配损失，以优化对抗训练。
...
TL;DR: Perceive Anything Model (PAM) 是一个高效的区域级视觉理解框架，通过结合 SAM 2 和 LLMs 实现对象分割与语义输出生成。


<details>
  <summary>Details</summary>
Motivation: 扩展 SAM 2 的功能，实现更全面的区域级视觉理解，包括对象分割和多样化的语义输出。

Method: 引入 Semantic Perceiver 将 SAM 2 的视觉特征转化为多模态 token，供 LLM 理解，并开发数据增强流程生成高质量数据集。

Result: PAM 在多种区域理解任务中表现优异，运行速度更快且内存消耗更低。

Conclusion: PAM 是一个高效且实用的解决方案，为未来区域级视觉理解研究提供了强基线。
...
<details>
  <summary>Details</summary>
Motivation: 解决对称物体或重复部分导致的语义匹配模糊问题。

Method: 训练适配器，利用3D感知链生成伪标签，并通过循环一致性和3D球形原型映射约束过滤错误标签。

Result: 在SPair-71k上实现了4%的绝对提升，且对类似监督需求的方法提升了7%。

Conclusion: 该方法减少了特定数据集标注需求，并展示了在其他数据源上的扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在基于示例图像进行材料编辑时存在局限性，MARBLE旨在通过CLIP空间和预训练模型改进材料编辑的精细控制。

Method: 通过找到去噪UNet中负责材料归因的块，并在CLIP空间中寻找材料混合方向，结合浅层网络预测细粒度材料属性变化方向。

Result: 实验表明，MARBLE能够实现材料混合和细粒度属性控制，并支持单次前向传递中的多次编辑和绘画应用。

Conclusion: MARBLE在材料编辑任务中表现出色，提供了更精细的控制和更高的灵活性。
...
TL;DR: ProJo4D提出了一种渐进式联合优化框架，用于解决从稀疏多视角视频中估计物理参数的问题，显著提升了4D场景理解的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏多视角视频输入时，由于顺序优化策略导致误差累积，限制了物理准确数字孪生等应用的效果。

Method: 采用渐进式联合优化框架，逐步增加联合优化的参数集，最终实现几何、外观、物理状态和材料属性的完全联合优化。

Result: 在PAC-NeRF和Spring-Gaus数据集上，ProJo4D在4D未来状态预测、未来状态的新视角渲染和材料参数估计方面优于现有方法。

Conclusion: ProJo4D通过渐进式联合优化，有效解决了物理参数估计的挑战，为物理基础的4D场景理解提供了实用解决方案。
...
TL;DR: 论文分析了3D视觉语言模型（VLMs）的性能差距，发现3D场景中心模型依赖3D编码器不足，并提出新数据集以促进真正的3D场景理解。


<details>
  <summary>Details</summary>
Motivation: 研究3D VLMs性能低于2D和3D物体中心模型的原因，并提出改进方法。

Method: 分类3D VLMs架构，分析性能差距，并提出新数据集3D Relevance Discrimination QA。
...
TL;DR: 提出了一种基于点图预测的PM-Loss正则化损失，用于改进深度图中物体边界处的几何平滑性，从而提升3D高斯泼溅的渲染质量。
...
Method: 通过预训练的transformer预测点图，并设计PM-Loss正则化损失来优化深度图，提升几何平滑性。

Result: 改进后的深度图显著提升了3D高斯泼溅的渲染效果，适用于多种架构和场景。

Conclusion: PM-Loss有效解决了深度图在物体边界处的几何平滑性问题，为3D高斯泼溅提供了更高质量的渲染结果。
...
TL;DR: 论文提出了CG-AV-Counting基准和AV-Reasoner模型，用于解决视频理解中的计数任务问题，并在多个基准测试中取得最佳效果。
...
Method: 提出CG-AV-Counting基准，包含1,027个多模态问题和5,845个标注线索；开发AV-Reasoner模型，采用GRPO和课程学习提升计数能力。

Result: AV-Reasoner在多个基准测试中达到最优效果，但语言空间推理在域外基准上未带来性能提升。

Conclusion: CG-AV-Counting和AV-Reasoner为视频计数任务提供了有效工具，但需进一步研究跨域推理问题。
...
TL;DR: MINT-CoT通过动态插入视觉标记到文本推理步骤中，提升了多模态数学推理能力。
...
Method: 提出MINT-CoT，通过Interleave Token动态选择数学图形中的视觉区域，并构建了54K的数据集和三阶段训练策略。
...
TL;DR: VideoMarathon是一个大规模的长视频指令跟随数据集，包含9700小时的长视频和3.3M高质量QA对，支持22种任务。基于此提出的Hour-LLaVA模型在长视频语言建模中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有长视频语言理解数据集的稀缺导致小时级视频训练不足，VideoMarathon填补了这一空白。

Method: 构建VideoMarathon数据集，并提出Hour-LLaVA模型，利用内存增强模块实现小时级视频训练和推理。

Result: Hour-LLaVA在多个长视频语言基准测试中表现最佳，验证了数据集和模型的高质量。

Conclusion: VideoMarathon和Hour-LLaVA为长视频语言理解提供了重要资源和方法，推动了该领域的发展。
...
TL;DR: VideoMolmo是一个多模态大模型，用于基于文本描述的细粒度时空定位，通过注意力机制和双向点传播提升一致性，并在多个任务中表现优异。
...
Result: 在多个任务中显著提升了时空定位准确性和推理能力。
...
TL;DR: 提出了一种生成室内空间去家具化复刻的流程，通过简化网格和全景图像处理，结合ControlNet修复技术，生成高质量的去家具化资产。


<details>
  <summary>Details</summary>
Motivation: 现有方法如神经辐射场（NeRF）生成的图像模糊且分辨率低，而RGB-D修复容易产生幻觉，因此需要一种更可靠的方法来生成高质量的去家具化室内场景。

Method: 首先从网格表示中分割并移除家具，扩展平面并填充孔洞，生成简化去家具化网格（SDM）。然后从SDM渲染的深度和法线图像中提取Canny边缘，作为ControlNet修复全景图像的引导信号。最后用修复后的全景图像为网格贴图。

Result: 该方法生成的去家具化资产质量高于依赖神经辐射场或RGB-D修复的方法，避免了模糊和幻觉问题。

Conclusion: 提出的流程通过结合几何引导和ControlNet修复，有效生成了高质量的去家具化室内场景复刻。
...
TL;DR: DirectLayout是一个基于LLM的框架，通过三阶段生成3D室内场景布局，解决了现有方法在开放词汇和细粒度指令对齐上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有3D室内场景布局生成方法因数据集有限，容易过拟合或依赖预定义约束，导致缺乏灵活性和用户指令对齐。

Method: DirectLayout分三阶段生成布局：BEV布局生成、3D空间提升和对象放置优化，结合CoT激活和奖励机制增强空间推理。

Result: 实验表明，DirectLayout在语义一致性、泛化性和物理合理性上表现优异。

Conclusion: DirectLayout通过LLM的空间推理能力，实现了高质量、灵活的3D场景布局生成。
...
TL;DR: 提出了一种新的任务（ORES）和框架（RAS），用于通过文本或文本加视觉实体的任意提示生成高质量的分割掩码组。


<details>
  <summary>Details</summary>
Motivation: 现有图像分割模型缺乏对基于语言和视觉的复杂查询的全面语义理解，限制了其在用户友好交互中的应用。

Method: 提出了RAS框架，通过掩码中心的大型多模态模型增强分割模型的多模态交互和理解能力。

Result: RAS在ORES、RES和GRES任务中表现出色。

Conclusion: ORES任务和RAS框架填补了现有分割模型的不足，提升了多模态交互能力。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决视频生成领域日益增长的计算成本问题，提出了高效的训练方法。

Method: 采用简约架构、多阶段训练策略和成本效益高的强化学习框架。
...
TL;DR: 论文研究了多模态大语言模型（MLLMs）中注意力机制的稀疏性，发现仅少数注意力头（约5%）对视觉理解有贡献，并提出了一种无需训练的框架来识别这些视觉头。基于此，作者提出了SparseMM优化策略，通过非对称计算预算加速MLLMs推理，同时保持性能。


<details>
  <summary>Details</summary>
Motivation: 探索MLLMs如何处理视觉输入，揭示注意力头的稀疏性现象，并利用这一现象优化推理效率。

Method: 设计了一种无需训练的框架来量化注意力头的视觉相关性，并提出了SparseMM优化策略，通过非对称计算预算加速推理。

Result: SparseMM在主流多模态基准测试中实现了优越的准确性与效率平衡，实现了1.38倍实时加速和52%内存减少。

Conclusion: 通过利用注意力头的稀疏性，SparseMM显著提升了MLLMs的推理效率，同时保持了性能，为多模态模型优化提供了新思路。
...
Result: 实现了高精度的直接和间接光传输效果，支持3D重建、视图合成和场景重光照。

Conclusion: 该系统在多视角光传播分析中表现出色，提供了新的逆向渲染能力。
...
Method: 提出FreeTimeGS，一种新型4D表示方法，允许高斯基元在任意时间和位置出现，并赋予其运动函数以减少时间冗余。

Result: 在多个数据集上的实验表明，该方法在渲染质量上大幅优于现有方法。
...
TL;DR: VideoMathQA是一个评估模型在视频中进行跨模态数学推理能力的基准测试，涵盖10个数学领域，强调多模态和时间维度的挑战。
...
Method: 通过专家标注构建高质量数据集，包含直接问题解决、概念迁移和深度教学理解三类推理挑战。

Result: 建立了系统化的评估框架，揭示了现有方法的局限性，并提供了多步推理的细粒度诊断。

Conclusion: VideoMathQA为跨模态和时间维度的数学推理任务提供了基准测试和评估工具，推动了相关研究的发展。
...
<details>
  <summary>Details</summary>
Motivation: 在条件设置下（如类别条件模型），流匹配的唯一性无法保证，导致生成结果模糊。本文旨在通过对比目标增强条件分离。

Method: 通过添加对比目标，最大化不同样本对预测流之间的差异，从而显式强制所有条件流的唯一性。

Result: 实验表明，对比流匹配在训练速度（提升9倍）、去噪步骤（减少5倍）和FID（降低8.9）方面均优于传统流匹配。

Conclusion: 对比流匹配是一种有效的扩展方法，显著提升了条件扩散模型的性能。
...
Method: 结合SFT和RFT训练，使用RefSpatial数据集和RefSpatial-Bench基准测试。

Result: SFT训练的RoboRefer在空间理解上达到89.6%的成功率，RFT训练的模型在基准测试中超越Gemini-2.5-Pro 17.4%。

Conclusion: RoboRefer在空间指代任务中表现卓越，可应用于多种机器人的动态任务。
...
TL;DR: SPARTA是一种通过点云估计角度依赖地形可通行性的方法，利用傅里叶基函数输出平滑分析函数，显著提高了成功率和计算效率。
...
Method: SPARTA通过几何结构网络输出1-Sphere上的平滑分析函数，利用傅里叶基函数预测风险分布，减少计算开销。

Result: 在高保真模拟中，SPARTA在40米巨石场中的成功率为91%（基线为73%），并在硬件测试中展示了泛化能力。

Conclusion: SPARTA通过几何结构和傅里叶基函数，高效解决了角度依赖的可通行性问题，适用于实际场景。
...
TL;DR: MineInsight是一个公开的多传感器、多光谱数据集，用于越野地雷检测，集成了多种视角和光谱范围的数据。


<details>
  <summary>Details</summary>
Motivation: 缺乏多样化和真实的数据集使得地雷检测算法的可靠验证成为研究社区的挑战。

Method: 数据集包含35个不同目标（15个地雷和20个常见物体），分布在三个不同轨道上，集成了双视角传感器扫描和多种光谱图像。

Result: 数据集包含约38,000个RGB帧、53,000个VIS-SWIR帧和108,000个LWIR帧，提供了目标位置估计。
...
Result: 合成数据训练的检测器在机器人足球场景中表现与真实数据训练的检测器相当，且结合两者能进一步提升性能。

Conclusion: 该方法为机器人领域提供了一种高效、可扩展的数据集生成方案，显著减少了人工标注需求。
...
### [132] [You Only Train Once](https://arxiv.org/abs/2506.04349)
*Christos Sakaridis*

Main category: cs.LG

TL;DR: YOTO方法通过一次性自动优化损失权重超参数，减少训练次数，优于传统网格搜索方法。


<details>
  <summary>Details</summary>
Motivation: 减少模型训练中多次运行的需求，特别是在损失选择和权重调整方面。

Method: 通过梯度优化将损失权重超参数视为网络参数，使用可微复合损失和软最大层满足约束。

Result: 在3D估计和语义分割任务中，YOTO优于最佳网格搜索模型。

Conclusion: YOTO是一种高效的一次性训练方法，适用于多损失优化问题。
...
### [133] [StatsMerging: Statistics-Guided Model Merging via Task-Specific Teacher Distillation](https://arxiv.org/abs/2506.04567)
*Ranjith Merugu,Bryan Bo Cao,Shubham Jain*

Main category: cs.LG

TL;DR: StatsMerging是一种基于统计学习的轻量级模型合并方法，通过权重分布统计指导合并，无需真实标签或测试样本。
...
Result: 在八个任务上验证了其有效性，优于现有技术。

Conclusion: StatsMerging在准确性、泛化性和鲁棒性上表现优异。
...
### [134] [Scaling Laws for Robust Comparison of Open Foundation Language-Vision Models and Datasets](https://arxiv.org/abs/2506.04598)
*Marianna Nezhurina,Tomer Porian,Giovanni Pucceti,Tommie Kerssies,Romain Beaumont,Mehdi Cherti,Jenia Jitsev*

Main category: cs.LG

TL;DR: 该论文展示了如何利用扩展定律（scaling laws）进行模型和数据集的比较，以决定预训练的最佳方法。通过对比CLIP和MaMMUT两种语言-视觉学习方法的扩展定律，发现MaMMUT在规模扩展和样本效率上优于CLIP。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于通过扩展定律的准确推导，避免仅基于单一参考尺度的测量导致的误导性结论，从而系统比较和改进开放基础模型及其数据集。

Method: 方法包括在广泛的模型和样本尺度范围内密集测量，推导CLIP和MaMMUT的扩展定律，并验证其在分类、检索和分割等下游任务中的一致性。

Result: 结果显示MaMMUT在规模扩展和样本效率上优于CLIP，并在多个数据集和任务中表现一致。同时，论文发布了预训练模型和代码。

Conclusion: 结论是扩展定律的准确推导为跨尺度模型和数据集比较提供了有效手段，为开放基础模型的系统改进铺平了道路。
...
### [135] [Exploring bidirectional bounds for minimax-training of Energy-based models](https://arxiv.org/abs/2506.04609)
...
<details>
  <summary>Details</summary>
Motivation: 能量模型（EBMs）虽然框架优雅，但训练困难，且现有方法（如变分下界）可能导致不稳定。

Method: 提出双向边界训练策略，包括基于生成器雅可比矩阵奇异值和互信息的下界，以及梯度惩罚和扩散过程的上界。

Result: 双向边界方法稳定了EBM训练，并实现了高质量的密度估计和样本生成。

Conclusion: 双向边界是一种有效的EBM训练策略，能够提升模型性能。
...
### [136] [Identifying and Understanding Cross-Class Features in Adversarial Training](https://arxiv.org/abs/2506.05032)
...
<details>
  <summary>Details</summary>
Motivation: 探索对抗训练的机制和动态，尤其是交叉类特征对鲁棒性的影响。

Method: 通过类间特征分析，结合理论模型和系统实验，研究AT过程中的特征学习动态。

Result: AT初期模型学习更多交叉类特征，鲁棒性最佳；后期则依赖类特定特征，导致鲁棒过拟合。

Conclusion: 研究为理解AT机制提供了新视角，并统一了软标签训练和鲁棒过拟合的解释。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法在潜在空间对齐中常涉及计算昂贵的似然评估和ODE求解，本文旨在解决这些问题。

Method: 通过预训练流模型捕获目标分布，并引入对齐损失来正则化潜在空间，避免直接计算似然和求解ODE。

Result: 理论证明对齐损失是最大化变分下界的替代目标，实验验证其在图像生成任务中的有效性。

Conclusion: 该框架为潜在空间对齐提供了新的理论支持和实用方法，显著降低了计算成本。
...
### [138] [HSSBench: Benchmarking Humanities and Social Sciences Ability for Multimodal Large Language Models](https://arxiv.org/abs/2506.03922)
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试过于关注STEM领域的垂直推理，忽视了HSS领域所需的横向跨学科思维和知识整合能力。

Method: 通过多领域专家与自动化代理协作的数据生成流程，创建了包含13,000多个样本的HSSBench基准测试。

Result: HSSBench对20多个主流MLLMs进行了测试，结果显示即使是先进模型也面临显著挑战。

Conclusion: HSSBench有望推动MLLMs在跨学科推理能力方面的进一步研究。
...
### [139] [ViCocktail: Automated Multi-Modal Data Collection for Vietnamese Audio-Visual Speech Recognition](https://arxiv.org/abs/2506.04635)
...
<details>
  <summary>Details</summary>
Motivation: 传统语音识别系统仅依赖音频特征，在噪声环境下表现不佳，而AVSR因其抗噪性受到关注。然而，大多数非英语语言的AVSR数据集稀缺，限制了模型的发展。

Method: 通过改进现有技术，提出了一种高效且易于操作的自动化数据收集方法，用于从原始视频生成AVSR数据集。

Result: 实验表明，自动收集的数据集能够支持强大的基线模型，在干净环境下与现有ASR竞争，在嘈杂环境（如鸡尾酒会）中表现更优。
...
### [140] [MMRefine: Unveiling the Obstacles to Robust Refinement in Multimodal Large Language Models](https://arxiv.org/abs/2506.04688)
...
Method: MMRefine通过六种不同场景和六种错误类型评估MLLMs的细化能力，并分析性能瓶颈。

Result: 实验揭示了阻碍细化性能的瓶颈和改进方向。
...
### [141] [ComfyUI-Copilot: An Intelligent Assistant for Automated Workflow Development](https://arxiv.org/abs/2506.05010)
...
<details>
  <summary>Details</summary>
Motivation: ComfyUI虽然灵活且用户友好，但对新用户仍存在文档不足、模型配置复杂和工作流设计困难等问题。

Method: 采用分层多智能体框架，包括中央助理代理和专用工作代理，结合知识库支持调试和部署。

Result: 离线评估和用户反馈表明，插件能准确推荐节点并加速工作流开发，降低新手门槛并提升老手效率。

Conclusion: ComfyUI-Copilot有效解决了ComfyUI的使用难题，并通过开源和演示视频展示了其实际价值。
...
### [142] [Parking, Perception, and Retail: Street-Level Determinants of Community Vitality in Harbin](https://arxiv.org/abs/2506.05080)
*HaoTian Lan*

Main category: cs.CL

TL;DR: 该研究提出了一种基于图像的可解释框架，分析中国哈尔滨社区街道的商业活力与车辆可达性、环境质量和行人感知的关系。


<details>
  <summary>Details</summary>
Motivation: 探讨社区街道的商业活力如何受街道特征（如停车密度、绿化、清洁度和街道宽度）影响，以提升城市设计和规划工具。

Method: 利用街景图像和多模态大语言模型（VisualGLM-6B）构建社区商业活力指数（CCVI），并通过GPT-4感知建模分析其与空间属性的关系。

Result: 研究发现适度车辆存在可提升商业可达性，但过度停车会降低步行性和满意度；绿化和清洁度显著提升满意度，但对定价影响较弱；街道宽度调节车辆影响。

Conclusion: 研究展示了AI辅助感知与城市形态分析结合的价值，为社区振兴提供了理论和工具支持，强调了车辆活动在商业中的条件性作用。
...
### [143] [Gradient Inversion Attacks on Parameter-Efficient Fine-Tuning](https://arxiv.org/abs/2506.04453)
*Hasin Us Sami,Swapneel Sen,Amit K. Roy-Chowdhury,Srikanth V. Krishnamurthy,Basak Guler*

Main category: eess.IV

TL;DR: 论文研究了联邦学习中参数高效微调（PEFT）的隐私问题，展示了通过恶意设计的预训练模型和适配器模块，攻击者可以通过梯度反演攻击重构用户的本地数据样本。


<details>
  <summary>Details</summary>
Motivation: 探讨在联邦学习中，如何通过PEFT机制保护用户隐私，同时揭示现有方法可能存在的隐私泄露风险。
...
### [144] [A Poisson-Guided Decomposition Network for Extreme Low-Light Image Enhancement](https://arxiv.org/abs/2506.04470)
...
Method: 采用编码器-解码器网络，集成Retinex分解和泊松去噪，通过泊松去噪损失函数处理信号依赖噪声，无需反射率和光照先验。

Result: 实验表明，该方法能显著提升低光条件下的可见性和亮度，同时保持图像结构和颜色一致性。

Conclusion: 该方法在低光图像去噪和增强中表现出高效性和实用性。
...
### [145] [DACN: Dual-Attention Convolutional Network for Hyperspectral Image Super-Resolution](https://arxiv.org/abs/2506.05041)
...
Method: DACN结合多头注意力和通道注意力，通过增强卷积捕获局部和全局特征依赖，并使用优化的损失函数确保光谱保真度。

Result: 在两个高光谱数据集上的实验表明，多头注意力和通道注意力的结合优于单独使用任一注意力机制。

Conclusion: DACN通过双注意力机制和优化损失函数，显著提升了高光谱图像超分辨率性能。
...
### [146] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
...
### [147] [DM-SegNet: Dual-Mamba Architecture for 3D Medical Image Segmentation with Global Context Modeling](https://arxiv.org/abs/2506.05297)
*Hangyu Ji*

Main category: eess.IV

TL;DR: DM-SegNet是一种双Mamba架构，通过方向性状态转换和解剖感知分层解码，解决了现有医学SSM在编码器-解码器不兼容的问题，显著提升了3D医学图像分割的精度。
...
Conclusion: DM-SegNet通过创新的双Mamba架构和解码策略，显著提升了3D医学图像分割的性能，适用于临床重要任务。
...
TL;DR: 该研究提出了一种基于本体的骨病诊断框架，结合了分层神经网络、视觉问答系统和多模态深度学习模型，旨在提高医学AI系统的诊断可靠性。
...
Result: 框架展示了在骨病诊断中的潜力，并具备扩展到其他疾病的标准化结构和可重用组件。

Conclusion: 虽然理论基础已建立，但实验验证因数据集和计算资源限制尚未完成，未来将扩展数据集并进行系统验证。
...
### [149] [Deep learning image burst stacking to reconstruct high-resolution ground-based solar observations](https://arxiv.org/abs/2506.04781)
*Christoph Schirninger,Robert Jarolim,Astrid M. Veronig,Christoph Kuckein*

Main category: astro-ph.SR

TL;DR: 提出一种基于深度学习的实时图像重建方法，用于处理地面太阳望远镜观测中的大气湍流问题。


<details>
  <summary>Details</summary>
Motivation: 地面太阳望远镜观测受大气湍流影响，现有方法在强湍流和高计算成本下表现不佳。

Method: 使用非配对图像到图像转换的深度学习模型，将100张短曝光图像实时重建为高质量图像。

Result: 该方法在感知质量上表现更优，尤其在参考图像存在伪影时更具鲁棒性。

Conclusion: 该方法能高效利用图像信息，在完整图像集下实现最佳重建效果。
...
### [150] [CMIE: Combining MLLM Insights with External Evidence for Explainable Out-of-Context Misinformation Detection](https://arxiv.org/abs/2505.23449)
*Fanxiao Li,Jiaying Wu,Canyuan He,Wei Zhou*

Main category: cs.MM

TL;DR: 论文提出了一种名为CMIE的新框架，用于检测图文不符的虚假信息，通过生成共存关系和关联评分机制，解决了现有多模态大语言模型在深层语义关联和噪声证据上的不足。


<details>
  <summary>Details</summary>
Motivation: 现有多模态大语言模型（MLLM）在检测图文不符虚假信息时，难以捕捉深层语义关联，且证据噪声影响准确性。

Method: 提出CMIE框架，包含共存关系生成（CRG）策略和关联评分（AS）机制，识别图文间的共存关系并选择性利用证据。

Result: 实验表明，CMIE在检测图文不符虚假信息上优于现有方法。

Conclusion: CMIE通过改进语义关联和证据利用，有效提升了虚假信息检测的准确性。
...
### [151] [SSIMBaD: Sigma Scaling with SSIM-Guided Balanced Diffusion for AnimeFace Colorization](https://arxiv.org/abs/2506.04283)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的噪声调度，可能损害感知一致性，因此需要一种更有效的方法来保持结构保真度和风格传递。

Method: 采用连续时间扩散模型，引入SSIMBaD（基于结构相似性的平衡扩散），通过sigma空间变换对齐感知退化。

Result: 在大规模动漫面部数据集上，方法在像素精度和感知质量上均优于现有技术，并能泛化到多样风格。

Conclusion: SSIMBaD框架在动漫面部上色任务中表现出色，提供了更平衡和忠实的重建效果。
...
### [152] [Handle-based Mesh Deformation Guided By Vision Language Model](https://arxiv.org/abs/2506.04562)
...
Method: 结合锥形奇点检测和VLM提示工程，通过多视角投票减少预测不确定性。

Result: 在基准测试中，方法生成的变形更符合用户意图，且变形失真低。
...
### [153] [VoxDet: Rethinking 3D Semantic Occupancy Prediction as Dense Object Detection](https://arxiv.org/abs/2506.04623)
*Wuyang Li,Zhu Yu,Alexandre Alahi*

Main category: cs.GR

TL;DR: VoxDet提出了一种实例为中心的3D语义占用预测框架，通过解耦任务提升实例级完整性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视实例级区分性，导致实例不完整和相邻模糊。VoxDet利用体素标签隐含的实例信息改进预测。

Method: VoxDet将体素预测解耦为偏移回归和语义预测两个子任务，使用空间解耦编码器和任务解耦预测器。

Result: 在相机和LiDAR输入上均达到SOTA，SemanticKITTI测试集IoU达63.0，排名第一。

Conclusion: VoxDet高效且性能优越，验证了实例中心化方法的有效性。
...
### [154] [A Fast Unsupervised Scheme for Polygonal Approximation](https://arxiv.org/abs/2506.04664)
*Bimal Kumar Ray*

Main category: cs.GR

TL;DR: 提出了一种快速且无监督的多边形近似方案，用于闭合数字曲线，速度优于现有方法，且在Rosin度量与美学方面表现优异。
...
Result: 方案在速度和Rosin度量上优于现有方法，且对几何变换具有鲁棒性。
