...
TL;DR: 论文提出了一种名为Recall的对抗性框架，用于测试图像生成模型（IGMs）在去学习技术中的鲁棒性，揭示了现有方法的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型（如Stable Diffusion）的发展，其生成能力引发了伦理、法律和社会问题。机器去学习（MU）被提出作为解决方案，但其鲁棒性尚未充分研究。

Method: Recall通过优化对抗性图像提示，利用扩散模型的多模态条件能力，测试去学习方法的鲁棒性。

Result: 实验表明，Recall在对抗性效果、计算效率和语义保真度上优于现有基线方法。
...
Result: 提出了针对生物医学图像分析的XAI方法综合框架，并探讨了多模态学习和视觉语言模型的新兴作用。
...
TL;DR: 本文研究了多模态大语言模型（MLLMs）中的幻觉现象，特别是由模态冲突引起的幻觉。作者定义了模态冲突，构建了MMMC数据集，并提出了三种方法来缓解这一问题。实验表明，强化学习方法效果最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管MLLMs在视觉语言任务中表现出色，但在实际场景中容易产生幻觉。本文从模态冲突的角度研究这一现象，揭示输入中不同模态的冲突如何直接导致幻觉。

Method: 作者提出了基于提示工程、监督微调和强化学习的三种方法，并在MMMC数据集上进行了实验。

Result: 实验结果显示，强化学习方法在缓解模态冲突导致的幻觉中表现最佳，监督微调方法也展现出稳定且有前景的效果。
...
<details>
  <summary>Details</summary>
Motivation: 在GNSS不可用的环境中，如搜索救援和威胁检测等应用需要自主的海上监视和目标船只识别。当目标船只仅通过视觉线索描述且位置未知时，无人机需依赖机载视觉在严格计算限制下扫描大范围区域。

Method: 使用YOLOv8检测视野内的所有船只，通过特征匹配和色调直方图距离分析判断是否为目标船只，并利用几何原理定位目标。

Result: 在MBZIRC2023竞赛的实际实验中验证了方法的可行性，同时评估了视角对检测精度和定位准确性的影响。

Conclusion: 该方法在GNSS不可用环境下实现了目标船只的自主检测和定位，为相关应用提供了有效解决方案。
...
TL;DR: 该论文提出了一种通过EEG信号与多级语义描述对齐的方法，实现了从脑电波到图像的高质量重建。


<details>
  <summary>Details</summary>
Motivation: EEG信号在空间细节上的局限性阻碍了图像重建，因此需要一种新的方法来绕过直接生成，通过语义描述作为中介。
...
TL;DR: 论文分析了当前视频生成模型的局限性，尤其是长视频（超过16秒）在角色一致性和场景布局上的问题，并提出了一种新的分类法和比较表。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在生成长视频时难以保持角色一致性和场景连贯性，尤其是多角色长视频。本文旨在通过系统研究32篇论文，找出关键架构和训练策略。

Method: 通过分析32篇视频生成论文，识别关键架构组件和训练策略，并构建新的分类法和比较表。

Result: 提出了一个全面的分类法，并总结了不同架构设计和性能特征的比较表。

Conclusion: 本文为长视频生成提供了系统性的研究框架，有助于未来改进角色一致性和场景连贯性。
...
TL;DR: 提出了一种基于颜色的轻量级方法CSCI，用于解决衣物变化重识别问题，无需额外标注或模型。
...
Method: 利用RGB信息，通过S2A自注意力机制分离颜色与身份特征。

Result: 在多个数据集上显著提升性能，如LTCC提升2.9%，PRCC提升5.0%。

Conclusion: 颜色是一种经济有效的解决方案，可用于衣物变化重识别中的外观偏差问题。
...
Method: 结合机器学习，通过文本提示进行灵活对象检测、逐帧图像分割和视频跟踪以确保时间一致性。

Result: 减少了手动工作量，加快了初步合成的速度，并提供了全面的分割数据。
...
TL;DR: 提出了一种多模态语言引导框架DisenQ，通过结构化文本监督解耦生物特征、运动和非生物特征，在活动生物识别任务中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 传统身份识别在多样化活动中面临生物特征与运动动态和外观变化纠缠的挑战，现有视觉数据辅助方法存在提取不准确问题。

Method: 引入DisenQ，一种统一查询变换器，利用结构化语言指导解耦特征，避免身份线索受外观和运动变化影响。

Result: 在三个活动视频基准测试中达到最优性能，并在传统视频识别任务中展现强泛化能力。

Conclusion: DisenQ框架通过语言引导有效解耦特征，提升了活动生物识别的准确性和泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LMMs在多语言能力评估方面不足，导致输出存在偏见和不公平。

Method: 构建包含6,875对图像-文本数据的数据集，使用Bias、Answer Relevancy和Faithfulness三个指标评估模型。

Result: 闭源模型（如GPT-4o和Gemini2.5）整体表现最佳，开源模型（如Gemma3和Qwen2.5）在跨语言泛化方面表现优异。

Conclusion: LinguaMark为多语言LMMs评估提供了标准化工具，促进未来研究。
...
Conclusion: MagiC为多模态认知提供了全面评估，并提出了新指标和分析方法。
...
TL;DR: ADIEE方法通过生成大规模数据集训练评分模型，显著提升了指令引导图像编辑的自动评估性能。


<details>
  <summary>Details</summary>
Motivation: 现有开源VLM模型在评估指令引导图像编辑时存在对齐问题，且缺乏透明度和成本效益高的专有模型，同时缺少公开训练数据集。

Method: 提出ADIEE方法生成大规模数据集（100K样本），并基于LLaVA-NeXT-8B模型定制解码器训练评分模型。

Result: 评分模型在多个基准测试中超越开源VLM和Gemini-Pro 1.5，与人类评分相关性提升17.24%，比较准确率最高提升9.35%。

Conclusion: ADIEE训练的评分模型能高效替代人工评估，并可作为奖励模型优化编辑模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉叙事系统在跨帧识别同一实体时表现不佳，导致不一致的引用和幻觉问题，缺乏明确的训练机制。

Method: 采用对比强化学习框架，结合合成负例和双组件奖励函数，优化Qwen Storyteller模型。

Result: 实体识别和连贯性显著提升，多项指标（如mAP、F1）均有两位数增长，结构化故事比例大幅提高。
...
TL;DR: PacGDC是一种标签高效技术，通过合成伪几何数据增强深度补全模型的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 减少对大规模标注数据的需求，提升深度补全模型的泛化性。

Method: 利用2D到3D投影中的形状和位置一致性，通过场景尺度操纵合成多样伪几何数据。

Result: 在零样本和少样本设置下，PacGDC在多个基准测试中表现出卓越的泛化能力。

Conclusion: PacGDC通过高效数据合成显著提升了深度补全模型的泛化性能。
...
Method: 引入两种机制：邻域对应损失和测地线对应算法，以增强自适应性和一致性。
...
TL;DR: 提出了一种多尺度注意力门移位模块（MSAGSM），用于增强体育视频中的精确事件检测，并在多个基准测试中取得了最先进的结果。


<details>
  <summary>Details</summary>
Motivation: 现有精确事件检测（PES）模型的时间模块在时间感受野和空间适应性方面存在局限，需要更高效的建模方法。
...
Result: 在MARS上达到91.73% mAP和97.32% Rank-1，iLIDS-VID上96.00% Rank-1和100.0% Rank-5。

Conclusion: KeyRe-ID通过关键点引导的全局-局部特征学习显著提升了视频行人重识别性能。
...
TL;DR: 提出了一种新的跨类别动物运动转移框架，专注于保留物种特有的习惯行为。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注人类运动，忽略了动物特有的习惯行为，因此需要一种能够保留这些行为的方法。

Method: 基于生成框架，引入习惯保留模块和类别特定习惯编码器，并结合大型语言模型（LLM）处理未观察到的物种。

Result: 通过新数据集DeformingThings4D-skl和实验验证了模型的有效性。

Conclusion: 该框架在跨类别动物运动转移中表现出色，能够有效保留习惯行为。
...
<details>
  <summary>Details</summary>
Motivation: 无约束照片集合易于获取但存在光照不一致和瞬态遮挡问题，传统分割方法无法解决这些问题。

Method: 集成多维特征嵌入和Spiky 3D Gaussian Cutter（SGC）技术，实现3D场景中的交互式分割。

Result: 实验表明，Seg-Wild在分割效果和重建质量上优于先前方法。
...
TL;DR: 提出了一种名为EPIC的高效提示交互策略，用于文本-图像分类任务，减少了计算资源消耗和可训练参数。
...
Result: 在UPMC-Food101和SNLI-VE数据集上表现优异，在MM-IMDB数据集上表现相当，同时减少了计算资源和可训练参数。
...
TL;DR: Corvid是一种增强思维链推理能力的多模态大语言模型，通过混合视觉编码器和精心设计的连接器提升跨模态对齐能力，并利用高质量数据集MCoT-Instruct-287K进行两阶段微调，显著提升了复杂推理任务的性能。
...
Result: Corvid在数学推理和科学问题解决任务中表现优异，优于同类参数规模的最先进模型。

Conclusion: Corvid通过增强的思维链推理能力和优化训练策略，显著提升了多模态大语言模型在复杂推理任务中的性能。
...
TL;DR: 论文提出了一个高分辨率3D异常检测数据集MiniShift和高效框架Simple3D，用于解决工业点云分析中细微异常检测的挑战。


<details>
  <summary>Details</summary>
Motivation: 工业点云分析需要高分辨率数据检测细微异常，但现有基准多关注低分辨率输入，导致检测效果受限。

Method: 提出可扩展的3D异常生成管道，创建MiniShift数据集；开发Simple3D框架，结合MSND和LFSA技术捕捉几何细节。
...
<details>
  <summary>Details</summary>
Motivation: 超声图像噪声大，影响自动分割效果，需要一种更鲁棒的方法。

Method: 提出AFSA模块和LGSA模块，分别处理相邻帧特征和局部全局特征融合。

Result: 在四个数据集上表现优于现有方法，推理速度更快。

Conclusion: DSANet有效提升超声视频分割的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: BWV是诊断黑色素瘤的关键特征，但现有研究有限，需要更高效的检测方法。

Method: 提出基于颜色阈值技术的成像算法生成标注数据集，设计自定义层的DCNN模型，并在多个数据集上训练和测试。

Result: 模型在不同数据集上表现优异，测试准确率最高达95.05%，XAI算法增强了决策可解释性。
...
TL;DR: Objectomaly是一个基于对象感知的OoD分割框架，通过三阶段方法解决边界不精确和背景噪声问题，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有OoD分割方法存在边界不精确、异常分数不一致和背景噪声导致的误报问题。
...
Method: 结合局部SFFT模块和DAFE模块，SFFT通过对齐低质量与高质量区域的局部统计分布增强结构，DAFE通过编码器表示对齐实现退化无关的特征提取。
...
TL;DR: 该论文提出了一种生成时间不可学习示例（TUEs）的新框架，以保护视频数据隐私，防止未经授权的深度跟踪器利用。


<details>
  <summary>Details</summary>
Motivation: 随着社交媒体的兴起，大量用户上传的视频被用作视觉对象跟踪（VOT）的训练数据，但视频数据隐私问题被忽视。许多私人视频未经授权被用于商业模型训练。

Method: 提出了一种生成时间不可学习示例（TUEs）的生成框架，并引入时间对比损失以增强TUEs的效果。

Result: 实验表明，该方法在视频数据隐私保护方面达到了最先进的性能，并具有强大的跨VOT模型、数据集和时间匹配任务的迁移性。

Conclusion: 该研究为视频数据隐私保护提供了有效的解决方案，解决了现有方法在视频任务中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 研究如何从散度最小化角度改进扩散模型的对齐问题，避免现有方法的次优均值优化。

Method: 提出DMPO方法，通过最小化反向KL散度对齐扩散模型，理论分析与RL优化方向一致。

Result: 实验表明DMPO在人类评估和自动指标上均优于现有方法，PickScore提升64.6%。

Conclusion: DMPO为扩散模型对齐提供了理论支持与实践优势，显著提升生成行为与期望输出的匹配度。
...
Method: GGMotion采用组图网络，提出新颖的径向场以捕捉时空依赖，并通过组内和组间交互模块捕获多尺度关节依赖，结合等变多层感知机更新关节特征。
...
<details>
  <summary>Details</summary>
Motivation: 动态场景的4D对象分割研究因缺乏大规模、精确标注的多视角视频数据集而受限，因此需要构建新的数据集。

Method: 构建了包含17个场景、7830张RGB图像及其分割掩码的MUVOD数据集，涵盖多种场景和相机配置。

Result: 数据集包含459个实例和73个类别，支持跨视角和跨时间帧的对象跟踪，并提供了评估指标和基线方法。

Conclusion: MUVOD数据集为动态场景分割提供了基准，并推动了该领域的研究进展。
...
Result: 在稀疏输入拟合连续场的时间插值中表现优异，同时在动态场景重建中达到与先进方法竞争的质量，且无需依赖线性混合蒙皮或尽可能刚性约束。

Conclusion: 该方法通过显式控制节点数量和低秩时变编码，有效提升了轨迹建模的空间一致性和时间插值性能。
...
TL;DR: MAPEX是一种基于多模态专家混合的遥感基础模型，通过模态感知剪枝技术优化任务性能。
...
Conclusion: MAPEX有效解决了模态不匹配问题，简化了微调和部署流程。
...
TL;DR: 论文研究了视觉语言模型（VLMs）在抽象推理任务中的线性推理瓶颈，提出了线性可分性天花板（LSC）概念，并发现该问题源于语言模型的推理路径而非感知能力。通过任务依赖的干预方法，证明了模型内存在强大的休眠推理路径。


<details>
  <summary>Details</summary>
Motivation: 现有VLMs在抽象推理任务中表现受限，主要原因是视觉嵌入的线性可分性不足。研究旨在揭示这一瓶颈的根源并提出解决方案。

Method: 引入LSC作为评估指标，通过线性分类器测试视觉嵌入的性能。采用任务依赖的干预方法（如激活现有路径或调整核心权重）进行实验。

Result: 发现线性推理瓶颈普遍存在，且源于语言模型的推理路径问题。通过干预方法可以部分解决，但复杂关系推理需要更深层次的权重调整。

Conclusion: 研究表明，稳健的推理需要针对性的对齐方法，而非仅改进表示学习。这为VLM分析提供了新视角。
...
TL;DR: DGKD-WLSS是一种结合扩散引导知识蒸馏和深度引导特征融合的新框架，用于解决弱监督低光语义分割问题，显著提升性能。
...
Method: 提出DGKD-WLSS框架，结合扩散引导知识蒸馏（DGKD）和深度引导特征融合（DGF2），通过去噪和几何先验增强特征学习。

Result: 实验表明DGKD-WLSS在低光条件下实现了最先进的性能。

Conclusion: DGKD-WLSS通过结合扩散和深度信息，有效解决了弱监督低光语义分割的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 工业异常检测中领域偏移问题导致模型泛化能力不足，NexViTAD旨在通过跨域知识迁移提升检测性能。
...
Conclusion: NexViTAD在跨域缺陷检测中实现了突破性进展，显著提升了模型泛化能力和检测精度。
...
<details>
  <summary>Details</summary>
Motivation: 洪水是常见的自然灾害，现有产品在覆盖范围和空间细节之间存在权衡，且常忽略洪水深度。HOTA旨在填补这一空白，提供及时、大范围的洪水信息。

Method: HOTA采用分层重叠平铺聚合策略，结合SegFormer模型和DEM差分方法，实现多尺度特征捕捉和洪水深度估计。
...
TL;DR: 本文提出了对SAM2框架的增强，通过分层运动估计策略和优化的内存银行，显著提升了视频目标跟踪的性能。
...
Result: 在LaSOT和LaSOText数据集上，大模型相对原始SAM2分别提升了9.6%和7.2%的AUC。

Conclusion: 该方法无需额外训练，低开销且高效，显著提升了长期跟踪性能。
...
TL;DR: LOSC方法通过整合图像级增强和时空一致性，提升了激光雷达扫描的开放词汇分割性能，显著优于现有零样本方法。


<details>
  <summary>Details</summary>
Motivation: 传统方法将图像语义反向投影到3D点云时，标签噪声大且稀疏，需要一种更鲁棒的方法。

Method: 通过整合标签以增强时空一致性和对图像级增强的鲁棒性，并训练3D网络。

Result: 在nuScenes和SemanticKITTI数据集上，LOSC在零样本开放词汇语义和全景分割任务中显著优于SOTA。
...
TL;DR: SpatialViz-Bench是一个多模态基准测试，用于评估MLLMs在空间可视化任务中的表现，揭示了模型与人类直觉不一致的行为。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法可能因与训练数据重叠而不可靠，需要专门的空间可视化评估工具。
...
TL;DR: ViLU是一个新的视觉语言不确定性量化框架，通过整合视觉和文本嵌入，实现了对预测错误的可靠检测。
...
Method: 利用多模态表示和二元分类器训练不确定性预测器，不依赖损失预测。

Result: 在多个数据集上显著优于现有方法，适用于后处理场景。

Conclusion: ViLU的架构和训练方法在不确定性量化中起到关键作用，代码已开源。
...
TL;DR: T-GVC是一种基于轨迹引导的生成视频编码框架，通过语义感知稀疏运动采样和轨迹对齐损失约束，在超低比特率下实现高质量视频重建。
...
Method: 采用语义感知稀疏运动采样管道，结合轨迹对齐损失约束的扩散过程，实现低比特率下的高效视频编码。
...
Method: 通过多任务学习、多模态输入和年龄加权损失三种策略，结合XAI技术分析注意力模式，并在AffectNet数据集上训练和验证。

Result: 结果显示，提出的策略显著提高了老年人群体的识别准确性，尤其是对易错表情（如中性、悲伤和愤怒）。

Conclusion: 年龄偏见可以通过简单的训练修改有效缓解，近似的人口统计标签对提升大规模情感计算系统的公平性具有价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法主要依赖细胞的空间特征，忽略了活细胞的时间动态变化，而药物分子模态可以补充图像模态的信息。

Method: 提出MolCLIP模型，结合细胞视频和分子模态，设计分子辅助CLIP框架和度量学习策略。

Result: 在MitoDataset上，药物识别和MoA识别的mAP分别提升了51.2%和20.5%。

Conclusion: MolCLIP通过多模态融合和时间动态分析，显著提升了药物机制识别的性能。
...
Result: ARNet显著减少了人工标注工作量，并在多个数据集中验证了其高性能和广泛适用性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在CoT推理中常忽略生成的中间依据，导致推理效果不佳，需改进多模态CoT的忠实性和准确性。

Method: 将多模态CoT重新定义为KL约束的奖励最大化问题，提出RED解码策略，结合视觉和依据信息生成下一令牌分布。

Result: RED在多个基准测试和LVLMs中显著优于标准CoT及其他解码方法，提升了推理性能。

Conclusion: RED为改进LVLMs的CoT推理提供了一种实用有效的方法，推动了更可靠的多模态系统发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有Mamba方法在水下深度估计任务中表现不佳，且数据集标签不可靠，导致估计不准确。

Method: 采用树感知扫描策略，构建最小生成树并灵活聚合空间拓扑特征，增强多尺度特征表示能力。

Result: Tree-Mamba在定性和定量评估中均优于现有方法，计算效率高。

Conclusion: Tree-Mamba方法有效解决了水下深度估计问题，并提供了可靠的基准数据集BlueDepth。
...
TL;DR: 提出了一种可训练的掩码预测器和帧内运动分析器，用于高效处理局部运动模糊，显著减少计算量并提升去模糊性能。
...
Method: 1. 使用可训练掩码预测器识别模糊区域，并在训练中排除清晰区域；2. 通过结构重参数化优化推理计算；3. 开发帧内运动分析器，将像素位移转化为运动轨迹，指导区域特异性模糊恢复。

Result: 在局部和全局模糊数据集上表现优于现有方法，计算量减少49%（如LMD-ViT）。

Conclusion: 该方法通过高效资源分配和自适应模糊恢复，显著提升了局部运动模糊的处理效果和计算效率。
...
TL;DR: CrossVLAD是一个新基准数据集，用于评估统一视觉语言模型（VLMs）的跨任务对抗攻击，并提出CRAFT方法提升攻击效果。


<details>
  <summary>Details</summary>
Motivation: 统一视觉语言模型（VLMs）的指令控制机制带来安全挑战，需评估对抗攻击在不同任务中的有效性。

Method: 提出CrossVLAD数据集和CRAFT方法，通过区域中心攻击框架提升跨任务攻击性能。

Result: CRAFT在Florence-2等VLMs上表现优于现有方法，成功率高。

Conclusion: CrossVLAD和CRAFT为统一VLMs的对抗攻击评估提供了有效工具。
...
TL;DR: 研究探讨了开源胸部X光数据集中是否存在数据集偏见，通过多种网络架构分析，旨在促进医学影像领域更可解释的研究。


<details>
  <summary>Details</summary>
Motivation: 医学影像因敏感性难以开源，导致某些数据集被广泛使用。研究希望验证这些数据集是否存在偏见，以确保AI方法关注病理而非捷径。

Method: 对NIH、CheXpert、MIMIC-CXR和PadChest数据集应用简单变换，并使用多种网络架构进行分析。

Result: 未明确提及具体结果，但旨在揭示数据集偏见。

Conclusion: 研究鼓励医学影像领域更多可解释研究和开源数据集的创建，代码将在接受后发布。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型仅适用于2D图像，无法直接扩展到3D体积数据（如CT或MRI），且现有3D方法通常计算复杂且耗时。
...
TL;DR: 提出了一种基于能量的解码方法，通过动态选择能量分数最低的隐藏状态层，有效减少视觉语言模型中的对象幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性，如依赖特定解码方法、复杂视觉输入修改或外部模型知识，而视觉语言模型在不同VQA数据集上的“Yes”比例失衡现象显著。

Method: 采用能量解码方法，动态选择隐藏状态层，以减少“Yes”比例偏差并提升性能。

Result: 在三个基准测试（POPE、MME、MMVP）上，准确率和F1分数均有提升，平均准确率提高4.82%，“Yes”比例差距减少8.81%。

Conclusion: 该方法简单有效，能显著减少偏差并提升模型性能，适用于多种视觉语言模型。
...
TL;DR: 提出了一种高频率双流脉冲神经网络（SNN），在THU EACT-50数据集上比之前工作提高了2%的准确率，并展示了其在早期动作触发中的应用。


<details>
  <summary>Details</summary>
Motivation: 早期识别人类活动对安全性和响应速度至关重要，事件视觉传感器的高时间分辨率和低延迟适合这一需求，但现有方法在早期预测能力上有限。

Method: 采用高频率双流SNN处理事件数据，并在THU EACT-50数据集上进行测试。

Result: 在THU EACT-50数据集上，最终准确率比之前工作提高了2%。

Conclusion: 该方法在早期动作识别中表现出色，并在实际应用中（如运动捕捉）展示了潜力。
...
TL;DR: SDST是一种用于视频时间定位的稀疏-密集侧调架构，通过引入基于参考的可变形自注意力机制，显著提升了性能并减少了参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖预训练模型的最终层特征，适应性有限，且忽略了MR的稀疏性。

Method: 提出SDST架构和基于参考的可变形自注意力机制，并首次将InternVideo2主干网络集成到侧调框架中。

Result: 在QVHighlights、TACoS和Charades-STA数据集上达到SOTA或高度竞争性结果，参数数量减少73%。

Conclusion: SDST通过稀疏-密集侧调和新型注意力机制，显著提升了视频时间定位的性能和效率。
...
TL;DR: X-RAFT模型通过改进RAFT光流模型，实现了在神经外科手术中不同光照条件下的高光谱图像配准，显著提高了荧光定量分析的准确性。


<details>
  <summary>Details</summary>
Motivation: 在动态手术环境中，荧光定量分析需要高精度的跨模态图像配准，但现有方法难以应对蓝光和白光条件下的光谱差异。

Method: 提出X-RAFT模型，采用独立编码器处理不同模态的图像，并通过自监督的流循环一致性进行微调。

Result: 与基线方法相比，X-RAFT将误差降低了36.6%，优于现有的跨模态光流方法（CrossRAFT）。

Conclusion: X-RAFT为荧光引导神经外科手术提供了更可靠的跨模态图像配准工具，未来将公开代码和模型。
...
<details>
  <summary>Details</summary>
Motivation: 增材制造中的几何不准确性会影响部件性能，传统DVC方法在配准和数据计算上存在挑战。

Method: 采用动态基于补丁的处理策略和引入二进制差异图（BDM）来评估配准精度。

Result: Dice Score提高了9.2%，体素匹配率提高了9.9%，交互时间从几天缩短到几分钟。

Conclusion: 该方法为基于深度学习的DVC方法奠定了基础，有望提高增材制造过程的可靠性和效率。
...
Result: 发现现有攻击方法无法生成不可感知的图像，且人类与自动化视觉系统感知不一致。

Conclusion: SCOOTER为无限制对抗攻击评估提供了标准化工具，揭示了人类与模型感知的差异。
...
<details>
  <summary>Details</summary>
Motivation: 探讨在数据集偏移下如何选择和应用校准技术，以提高分类模型的鲁棒性。

Method: 比较了多种后处理校准方法和训练中校准策略（如标签平滑），在多个分类任务和图像领域中进行实验。

Result: 发现熵正则化和标签平滑结合效果最佳；后处理校准器在少量OOD数据下表现最稳健；新校准方法未必优于简单方法；校准偏移常以ID校准为代价。

Conclusion: 在偏移下，校准前集成更有效；集成结合基础模型微调能获得最佳校准效果。
...
TL;DR: S	extsc{urprise}3D是一个新的数据集，专注于评估复杂3D场景中的语言引导空间推理分割，旨在解决现有数据集中空间关系理解的不足。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言研究中，空间推理能力未被充分探索，且现有数据集常依赖语义线索而非真正的空间关系。

Method: 通过构建包含200k+视觉语言对和89k+人工标注空间查询的数据集，避免对象名称的干扰，全面覆盖多种空间推理技能。

Result: 初步基准测试显示，当前最先进的3D视觉定位方法和3D-LLM面临显著挑战。

Conclusion: S	extsc{urprise}3D和3D-SRS旨在推动空间感知AI的发展，促进具身交互和机器人规划的进步。
...
TL;DR: 提出了一种基于3D卷积神经网络的端到端rPPG提取网络，通过差分帧融合模块和时序移位模块增强信号提取能力，动态混合损失函数减少过拟合，在复杂场景下表现出色。
...
TL;DR: ViaPT通过生成实例感知提示并结合数据集级提示，利用PCA保留关键信息，优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统视觉提示调优方法因数据集差异导致性能不佳，需改进。

Method: 提出ViaPT，结合实例感知提示和数据集级提示，利用PCA优化。

Result: 在34个数据集上表现优于现有方法。

Conclusion: ViaPT为视觉变换器提示优化提供了新范式。
...
TL;DR: 论文提出了一种名为协同提示（SyP）的新框架，用于处理多模态数据缺失时的视觉识别任务，通过动态适配器和协同提示策略提升性能。


<details>
  <summary>Details</summary>
Motivation: 现实应用中多模态数据常存在缺失或不全，导致性能下降，现有方法因静态提示和基本提示调优的局限性无法有效解决。

Method: 提出动态适配器计算自适应缩放因子生成动态提示，结合静态和动态提示的协同策略平衡模态信息。

Result: 在三个广泛使用的视觉识别数据集上显著优于现有方法，展示了在不同缺失率和条件下的鲁棒性。

Conclusion: SyP框架通过动态和协同提示策略有效处理多模态缺失问题，具有优越的适应性和可靠性。
...
TL;DR: 该研究首次将Vision Transformers (ViT)应用于无标记肺部肿瘤运动预测，比较了患者特异性（PS）和多患者（MP）模型的性能。
...
Method: 研究使用Vision Transformers (ViT)训练了PS和MP模型，分别针对个体化和泛化能力，并通过ADE和FDE指标评估性能。

Result: PS模型在T1数据上表现更优，但MP模型在T2数据上表现出更强的鲁棒性，无需重新训练即可达到可比性能。

Conclusion: PS模型精度更高，而MP模型在时间受限的临床环境中更具实用性。
...
TL;DR: 论文研究了内容拼图求解器在现实挑战中的鲁棒性，发现现有方法在缺失、边缘腐蚀和内容腐蚀等情况下性能下降，但深度学习模型通过数据增强可显著提升表现。


<details>
  <summary>Details</summary>
Motivation: 现有拼图求解器在现实应用（如文物或文件碎片重组）中缺乏对挑战性问题的评估。

Method: 引入三种拼图损坏类型（缺失、边缘腐蚀、内容腐蚀），评估启发式和深度学习求解器的表现。

Result: 标准求解器在损坏增加时性能迅速下降，但深度学习模型通过微调和数据增强显著提升鲁棒性，其中Positional Diffusion模型表现最佳。
...
TL;DR: SimCIS是一种简单但强大的基线方法，用于解决类增量/持续图像分割中的关键问题，如塑性损失和输入数据顺序依赖。


<details>
  <summary>Details</summary>
Motivation: 现有方法将掩码生成与持续学习过程解耦，导致塑性损失和输入数据顺序依赖问题。

Method: 通过直接选择图像特征进行查询分配，确保完美对齐以保留对象性，同时允许查询选择新类以促进塑性。

Result: SimCIS在各种分割任务、设置、分割和输入数据顺序中均优于现有方法。

Conclusion: SimCIS通过特征对齐和跨阶段一致性，有效解决了持续图像分割中的关键问题。
...
TL;DR: 3D-ADAM是一个大规模、高精度的3D异常检测数据集，旨在解决工业表面缺陷检测中数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 工业表面缺陷检测对提高制造效率至关重要，但现有数据集无法反映真实工业场景，限制了模型性能。

Method: 通过4种工业深度成像传感器采集14,120个高分辨率扫描数据，涵盖217个独特零件和27,346个缺陷实例。

Result: 3D-ADAM数据集在真实工业环境中捕获，展示了当前SOTA模型在处理复杂场景时的挑战。

Conclusion: 3D-ADAM为开发更鲁棒的3D异常检测模型提供了重要基准，推动现代制造业需求的发展。
...
Method: 开发了THUNDER基准测试工具，支持多种下游任务、特征分析和鲁棒性评估，比较了23个基础模型在16个数据集上的表现。

Result: THUNDER能够高效比较多种模型，并提供特征空间、鲁棒性和不确定性分析，代码已开源。

Conclusion: THUNDER为数字病理学领域提供了统一的评估框架，有助于推动模型比较和实际应用。
...
TL;DR: SLURPP是一种结合预训练潜在扩散模型和显式场景分解的网络架构，用于高效恢复水下图像，显著提升了速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的算法在复杂场景中计算量大且易产生不真实伪影，SLURPP旨在解决这些问题。

Method: 结合预训练潜在扩散模型和显式场景分解，并设计基于物理的水下图像合成流程生成训练数据。

Result: SLURPP比现有方法快200倍，PSNR提升约3 dB，在真实数据上也表现优异。

Conclusion: SLURPP通过新颖架构和合成数据，显著提升了水下图像恢复的效率和效果。
...
Method: MIRA框架包含校准的重新思考与重排模块和医学RAG框架，动态调整检索上下文并整合图像嵌入与医学知识库。

Result: 在公开医学VQA和报告生成基准测试中，MIRA显著提升了事实准确性和性能，达到新SOTA。

Conclusion: MIRA通过优化检索和知识整合，有效解决了多模态大语言模型在医学诊断中的事实不一致问题。
...
<details>
  <summary>Details</summary>
Motivation: 现代导航系统需要精确的位置估计，而VSLAM依赖可靠的特征点提取。资源受限平台需要高效且低计算需求的解决方案。
...
Result: 在五个数据集上的实验表明，该方法优于现有技术，实现了实时自监督适应。

Conclusion: CiCi框架在无需源数据的情况下，显著提升了rPPG模型的适应性，具有实际应用潜力。
...
Method: 结合了多目标追踪器（MouseTracks）、基于Transformer的ID分类器（Mouseformer）和轨迹关联器（MouseMap）的算法。
...
TL;DR: TinierHAR是一种超轻量级深度学习架构，结合了深度可分离卷积、GRU和时间聚合，在保持性能的同时显著提升了计算效率。
...
Method: 采用残差深度可分离卷积、门控循环单元（GRU）和时间聚合技术，构建了TinierHAR架构。

Result: 在14个公共HAR数据集上，TinierHAR参数减少2.7倍（相比TinyHAR）和43.3倍（相比DeepConvLSTM），计算量减少6.4倍和58.6倍，同时保持平均F1分数。

Conclusion: TinierHAR提供了高效HAR系统的设计指南，并开源了所有材料以促进边缘HAR研究。
...
TL;DR: 提出了一个全栈框架LongVILA，通过强化学习扩展视觉语言模型（VLMs）的长视频推理能力，包含数据集、训练管道和基础设施，并在多个基准测试中表现优异。
...
Method: 集成大规模数据集LongVideo-Reason、两阶段训练管道（CoT-SFT和RL）和高效训练基础设施MR-SP。

Result: LongVILA-R1-7B在长视频QA基准测试中表现优异，MR-SP系统训练速度提升2.1倍。

Conclusion: LongVILA-R1是VLMs长视频推理的重要进展，并开源了支持多模态RL训练的系统。
...
<details>
  <summary>Details</summary>
Motivation: 火星景观视频合成对任务演练和机器人模拟至关重要，但火星数据稀缺且与地球图像差异大。

Method: 1) M3arsSynth数据重建管道，从NASA的立体导航图像重建3D火星环境并渲染高保真视频；2) MarsGen视频生成器，基于3D结构合成新视频。

Result: 实验表明，该方法优于基于地球数据集训练的模型，具有更高的视觉保真度和3D结构一致性。

Conclusion: 该方法成功解决了火星视频合成的挑战，为任务演练和模拟提供了高质量工具。
...
<details>
  <summary>Details</summary>
Motivation: 视频扩散模型仅基于原始视频数据训练时，往往无法捕捉到几何感知的结构，因此需要一种方法来弥补这一差距。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试主要评估离线场景下的模型性能，而现实世界中的智能体需要在线处理增量观察并进行时空推理。OST-Bench旨在填补这一空白。

Method: 通过高效的数据收集流程，构建了包含1.4k场景和10k问答对的数据集，基于ScanNet、Matterport3D和ARKitScenes。评估了多个领先的MLLMs在在线时空推理任务中的表现。

Result: 现有MLLMs在复杂时空推理任务中表现不佳，尤其是在在线设置下，随着探索时间延长和记忆增长，准确性显著下降。
...
TL;DR: CLIP模型的表示存在局限性，无法区分物体属性绑定问题。研究发现数据属性（如低属性密度、不完整标注和显著性偏差）是影响CLIP学习绑定的关键因素。


<details>
  <summary>Details</summary>
Motivation: 解决CLIP模型在物体属性绑定问题上的局限性，探索数据属性对模型学习的影响。

Method: 使用合成数据集，分析数据属性（如属性密度、标注完整性和显著性偏差）对CLIP绑定能力的影响。

Result: 发现数据属性是影响CLIP绑定能力的关键，而非批量大小或硬负样本的增加。

Conclusion: 数据属性的优化是提升CLIP绑定能力的关键，而非传统方法如增加批量或硬负样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频大语言模型（LLMs）因时空令牌数量多导致计算复杂度呈二次方增长，STTM旨在解决这一问题。

Method: STTM通过四叉树结构进行多粒度空间令牌转换，并在时间维度上进行定向成对合并。

Result: 在六个视频QA基准测试中表现优异，50%令牌预算下速度提升2倍且精度仅下降0.5%。

Conclusion: STTM是一种高效且通用的令牌减少方法，支持跨问题KV缓存重用。
...
TL;DR: BASIC是一个多粒度评估框架，用于改进脑视觉解码方法的评估，通过结构保真度、推理对齐和上下文一致性来量化解码图像与真实图像的差异。


<details>
  <summary>Details</summary>
Motivation: 现有评估协议依赖粗粒度指标，缺乏神经科学基础，无法捕捉细粒度视觉差异，限制了模型的区分能力和解释性。

Method: BASIC框架结合了基于分割的分层指标（如前景、语义、实例和组件掩码）和多模态大语言模型提取的结构化场景表示（如对象、属性和关系），进行多粒度评估。

Result: 在多个刺激-神经影像数据集上对多种视觉解码方法进行了基准测试，展示了BASIC的区分性、可解释性和全面性。

Conclusion: BASIC为脑视觉解码方法提供了一个更精细、可解释和全面的评估基础。
...
TL;DR: KARL是一种基于Kolmogorov复杂性原理的单次自适应图像标记器，通过预测适当的标记数量来优化表示学习。
...
Method: KARL通过单次前向传播预测图像的标记数量，并在达到近似Kolmogorov复杂性时停止，训练过程类似于倒置强化学习。

Result: KARL性能与现有自适应标记器相当，但只需单次操作，并提供了关于编码器/解码器规模、连续与离散标记化等的扩展规律。
...
TL;DR: 提出了一种名为NickName的新方法，通过增强离散码本的表示能力，显著提升了VQ-VAEs的重建质量，并在多个基准测试中达到最优性能。
...
TL;DR: TreeBench是一个新的视觉推理基准测试，专注于复杂场景中的细微目标感知和可追溯证据评估，现有模型表现不佳。TreeVGR是一种新的训练范式，通过强化学习联合监督定位和推理，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有模型在视觉推理方面表现不佳，缺乏一个全面的评估基准来测试这些能力。

Method: 提出了TreeBench基准测试，基于三个原则构建，并开发了TreeVGR训练范式，使用强化学习联合监督定位和推理。

Result: TreeBench包含405个具有挑战性的视觉问答对，现有模型准确率低于60%。TreeVGR在多个基准测试上显著提升性能。
...
TL;DR: 论文研究了CLIP和大型多模态模型（LMMs）在训练数据中概念组合对组合泛化能力的影响，发现词共现统计量（PMI）与模型性能高度相关。
...
### [93] [Single-to-mix Modality Alignment with Multimodal Large Language Model for Document Image Machine Translation](https://arxiv.org/abs/2507.07572)
*Yupu Liang,Yaping Zhang,Zhiyang Zhang,Yang Zhao,Lu Xiang,Chengqing Zong,Yu Zhou*

Main category: cs.CL

TL;DR: M4Doc是一个新颖的单模态到多模态对齐框架，利用多模态大语言模型（MLLMs）提升文档图像机器翻译（DIMT）的性能，解决了训练数据有限和视觉-文本信息复杂交互的挑战。


<details>
  <summary>Details</summary>
Motivation: 文档图像机器翻译（DIMT）面临训练数据有限和视觉-文本信息复杂交互的挑战，导致泛化能力不足。

Method: M4Doc通过将仅图像编码器与预训练的多模态大语言模型（MLLMs）对齐，学习关键的视觉-文本关联，并在推理时绕过MLLM以保持计算效率。

Result: 实验表明，M4Doc显著提升了翻译质量，特别是在跨域泛化和复杂文档图像场景中。

Conclusion: M4Doc通过单模态到多模态对齐框架，有效提升了DIMT的性能，同时保持了计算效率。
...
### [94] [PyVision: Agentic Vision with Dynamic Tooling](https://arxiv.org/abs/2507.07998)
...
Conclusion: 动态工具生成使模型不仅能使用工具，还能发明工具，推动了更具自主性的视觉推理。
...
### [95] [Rainbow Artifacts from Electromagnetic Signal Injection Attacks on Image Sensors](https://arxiv.org/abs/2507.07773)
*Youqian Zhang,Xinyu Ji,Zhihao Wang,Qinhong Jiang*

Main category: cs.CR

TL;DR: 论文研究了针对图像传感器模拟域的电磁信号注入攻击，揭示了CMOS图像传感器中彩虹色伪影的攻击现象，并评估了对目标检测模型的影响。


<details>
  <summary>Details</summary>
Motivation: 图像传感器在安全和关键系统中广泛应用，但其视觉数据的完整性可能受到电磁攻击的威胁，现有数字完整性检查无法检测此类攻击。

Method: 通过精心调谐的电磁干扰诱导CMOS图像传感器产生彩虹色伪影，并分析其对图像信号处理流程和目标检测模型的影响。

Result: 攻击导致图像传感器捕获的图像出现伪影，并显著影响目标检测模型的预测准确性。

Conclusion: 研究揭示了视觉感知系统中一个关键且未被充分探索的漏洞，强调了需要针对物理层攻击开发更鲁棒的防御机制。
...
TL;DR: 提出了一种基于输入驱动的层丢弃方法，用于动态调整边缘和物联网环境中的计算资源。


<details>
  <summary>Details</summary>
Motivation: 在计算资源动态变化的边缘和物联网环境中，需要动态架构来适应不同的计算需求。现有层丢弃方法在层选择模式或架构修改方面存在局限性。

Method: 提出输入驱动的层丢弃方法，利用输入特征和轻量级层选择网络确定最佳处理层组合。

Result: 在4个语音和音频公共基准测试中，使用两种预训练基础模型，显著优于随机丢弃，并与早期退出方法效果相当或更好。

Conclusion: 输入驱动的层丢弃方法有效提升了动态模型的性能，适用于资源受限的环境。
...
### [97] [mmFlux: Crowd Flow Analytics with Commodity mmWave MIMO Radar](https://arxiv.org/abs/2507.07331)
*Anurag Pallaprolu,Winston Hurst,Yasamin Mostofi*

Main category: eess.SP

TL;DR: 提出了一种基于毫米波雷达提取人群运动模式和语义的新框架，结合光学流估计和噪声过滤技术，生成高保真流场，并通过几何图和局部分析提取关键语义。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以准确捕捉复杂人群运动模式和语义，尤其是在非结构化场景中。

Method: 结合光学流估计和统计形态学噪声过滤生成流场，将其转化为有向几何图，并通过局部Jacobian分析提取语义。

Result: 在21个实验中，框架成功重建了复杂人群的流结构，并准确推断出关键语义（如急转弯、分散等）。

Conclusion: 该框架在人群分析中表现出高保真和准确性，具有广泛应用潜力。
...
### [98] [Adaptive Attention Residual U-Net for curvilinear structure segmentation in fluorescence microscopy and biomedical images](https://arxiv.org/abs/2507.07800)
...
### [99] [LangNavBench: Evaluation of Natural Language Understanding in Semantic Navigation](https://arxiv.org/abs/2507.07299)
*Sonia Raychaudhuri,Enrico Cancelli,Tommaso Campari,Lamberto Ballan,Manolis Savva,Angel X. Chang*

Main category: cs.RO

TL;DR: LangNav是一个专注于语言理解的语义导航数据集，用于测试智能体在不同语言描述下的导航能力，并提出了MLFM方法，优于现有基线。
...
Result: MLFM在LangNav数据集上优于现有导航基线。

Conclusion: LangNav和MLFM为语言导航任务提供了更全面的评估和更优的解决方案。
...
### [100] [SD-GS: Structured Deformable 3D Gaussians for Efficient Dynamic Scene Reconstruction](https://arxiv.org/abs/2507.07465)
*Wei Yao,Shuzhao Xie,Letian Li,Weixiang Zhang,Zhixin Lai,Shiqi Dai,Ke Zhang,Zhi Wang*

Main category: cs.GR

TL;DR: SD-GS是一种高效的动态高斯溅射框架，通过可变形锚点网格和变形感知的密集化策略，显著降低了存储成本并提升了渲染速度。
...
Method: 引入可变形锚点网格作为层次化、内存高效的场景表示，并提出变形感知的密集化策略以优化动态区域的建模。
...
### [101] [Capture Stage Environments: A Guide to Better Matting](https://arxiv.org/abs/2507.07623)
*Hannah Dröge,Janelle Pfeifer,Saskia Rabich,Markus Plack,Reinhard Klein,Matthias B. Hullin*

Main category: cs.GR

TL;DR: 论文探讨了捕获阶段内容在图像抠图中的挑战，提出了改进工作流程的指南和高效管道，并通过扩散模型验证方法展示了其优势。


<details>
  <summary>Details</summary>
Motivation: 捕获阶段内容在图像抠图中表现不佳，现有算法难以应对其特殊性，因此需要研究改进方法。

Method: 提出了一种高效管道，无需大量标注即可适应最先进方法，并设计了基于扩散模型的验证方法。

Result: 展示了改进工作流程的优势，并通过验证方法证明了其有效性。

Conclusion: 论文为实践者提供了改进指南，并展示了适应捕获阶段内容的高效解决方案。
...
### [102] [RTR-GS: 3D Gaussian Splatting for Inverse Rendering with Radiance Transfer and Reflection](https://arxiv.org/abs/2507.07733)
...
<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅（3DGS）在新视角合成中表现出色，但在渲染反射物体时仍面临挑战，特别是在逆渲染和重光照方面。

Method: 通过结合前向渲染和延迟渲染的混合渲染模型，有效恢复几何结构，并分离高频和低频外观，减少球形谐波过拟合导致的伪影。
...
### [103] [Weighted Multi-Prompt Learning with Description-free Large Language Model Distillation](https://arxiv.org/abs/2507.07147)
*Sua Lee,Kyubum Shin,Jung Ho Park*

Main category: cs.LG

TL;DR: 提出了一种无需描述的提示学习方法DeMul，通过直接蒸馏LLM知识到提示中，提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖LLM生成描述，但存在高变异性与低可靠性问题。

Method: 采用描述无关的多提示学习，直接蒸馏LLM知识到连续向量提示中。

Result: 在11个识别数据集上表现优异。

Conclusion: DeMul方法通过消除描述提取步骤，提升了提示学习的鲁棒性和性能。
...
### [104] [ST-GRIT: Spatio-Temporal Graph Transformer For Internal Ice Layer Thickness Prediction](https://arxiv.org/abs/2507.07389)
...
### [105] [Resolving Token-Space Gradient Conflicts: Token Space Manipulation for Transformer-Based Multi-Task Learning](https://arxiv.org/abs/2507.07485)
...
<details>
  <summary>Details</summary>
Motivation: 多任务学习（MTL）中任务目标差异可能导致负迁移，现有方法因固定网络容量和结构限制适应性。
...
### [106] [Balancing the Past and Present: A Coordinated Replay Framework for Federated Class-Incremental Learning](https://arxiv.org/abs/2507.07712)
*Zhuang Qi,Lei Meng,Han Yu*

Main category: cs.LG

TL;DR: FedCBDR提出了一种联邦类增量学习中的类平衡数据回放方法，通过全局协调机制和任务感知温度缩放模块解决类不平衡问题，提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 联邦类增量学习中，数据回放方法因类不平衡问题（包括回放缓冲区内和回放与新任务类之间）而性能受限。

Method: FedCBDR包含两个关键模块：1) 全局视角数据回放模块，通过隐私保护方式重构先前任务的全局表示，指导类感知和重要性敏感采样；2) 任务感知温度缩放模块，根据任务动态调整logits温度，减少对多数类的过自信。

Result: 实验表明，FedCBDR在异构数据分布下实现了平衡的类采样，并在任务不平衡情况下提升了泛化能力，Top-1准确率比六种先进方法提高了2%-15%。

Conclusion: FedCBDR通过类平衡数据回放和任务感知温度缩放，有效解决了联邦类增量学习中的类不平衡问题，显著提升了模型性能。
...
### [107] [TRIX- Trading Adversarial Fairness via Mixed Adversarial Training](https://arxiv.org/abs/2507.07768)
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗训练方法对所有类别采用统一目标，忽略了类间脆弱性差异，导致对抗不公平性。强类别更鲁棒，而弱类别更易受攻击。
...
### [108] [Synchronizing Task Behavior: Aligning Multiple Tasks during Test-Time Training](https://arxiv.org/abs/2507.07778)
*Wooseong Jeong,Jegyeong Cho,Youngho Yoon,Kuk-Jin Yoon*

Main category: cs.LG

TL;DR: 提出了一种名为S4T的新方法，通过同步多任务处理来解决测试时训练（TTT）中任务行为不同步的问题。


<details>
  <summary>Details</summary>
Motivation: 解决在多任务和域偏移情况下，传统TTT方法因任务行为不同步而导致的性能下降问题。

Method: 提出S4T方法，通过预测跨域偏移的任务关系来同步任务。

Result: 在多个基准测试中，S4T优于现有TTT方法。

Conclusion: S4T通过同步任务关系，有效提升了多任务场景下的域适应性能。
...
### [109] [Semi-supervised learning and integration of multi-sequence MR-images for carotid vessel wall and plaque segmentation](https://arxiv.org/abs/2507.07496)
*Marie-Christine Pali,Christina Schwaiger,Malik Galijasevic,Valentin K. Ladenhauf,Stephanie Mangesius,Elke R. Gizewski*

Main category: eess.IV

TL;DR: 提出一种半监督深度学习方法来分割颈动脉血管壁和斑块，结合多序列MRI数据，解决标记数据稀缺和斑块形态复杂的问题。
...
### [110] [D-CNN and VQ-VAE Autoencoders for Compression and Denoising of Industrial X-ray Computed Tomography Images](https://arxiv.org/abs/2507.07704)
...
Method: 研究采用了两种网络架构（D-CNN和VQ-VAE）和不同压缩率，对砂岩样本的XCT数据进行压缩和恢复，并引入了一种对边缘保护敏感的指标。

Result: 结果表明，根据后续分析需求，需要选择不同的架构和压缩率以保留特定特征。

Conclusion: 研究结果为科学家在数据存储和分析需求方面提供了策略和指导。
...
### [111] [Compressive Imaging Reconstruction via Tensor Decomposed Multi-Resolution Grid Encoding](https://arxiv.org/abs/2507.07707)
...
### [112] [Breast Ultrasound Tumor Generation via Mask Generator and Text-Guided Network:A Clinically Controllable Framework with Downstream Evaluation](https://arxiv.org/abs/2507.07721)
...
Result: 在六个公共数据集上定量评估显示合成图像对乳腺癌诊断任务有显著提升，视觉图灵测试证实图像真实性。
...
### [113] [MeD-3D: A Multimodal Deep Learning Framework for Precise Recurrence Prediction in Clear Cell Renal Cell Carcinoma (ccRCC)](https://arxiv.org/abs/2507.07839)
*Hasaan Maqsood,Saif Ur Rehman Khan*

Main category: eess.IV

TL;DR: 提出了一种多模态深度学习框架，整合CT、MRI、病理切片、临床数据和基因组数据，以提高透明细胞肾细胞癌（ccRCC）复发预测的准确性。
...
Method: 采用多模态数据融合策略，结合CLAM（病理切片）、MeD-3D（CT/MRI）和MLP（临床/基因组数据）提取特征，并通过早期和晚期融合架构整合。

Result: 框架能够处理不完整数据，提升复发预测性能。
...
### [114] [ArteryX: Advancing Brain Artery Feature Extraction with Vessel-Fused Networks and a Robust Validation Framework](https://arxiv.org/abs/2507.07920)
*Abrar Faiyaz,Nhat Hoang,Giovanni Schifitto,Md Nasir Uddin*

Main category: eess.IV

TL;DR: ArteryX是一个基于MATLAB的半监督动脉评估框架，用于高效、准确地量化脑血管特征，解决了现有方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 脑血管病理对认知衰退和神经系统疾病有重要影响，但现有评估工具缺乏标准化和定量分析能力。

Method: ArteryX采用基于血管融合网络的标记方法，结合模拟框架验证特征提取的准确性。

Result: 在脑小血管病患者中验证显示，ArteryX对细微血管变化更敏感，性能优于现有半自动化方法。

Conclusion: ArteryX有望成为临床工作流程中的标准化工具，促进脑血管病理的早期检测和研究。
...
### [115] [Wrist bone segmentation in X-ray images using CT-based simulations](https://arxiv.org/abs/2507.07131)
*Youssef ElTantawy,Alexia Karantana,Xin Chen*

Main category: eess.IV

TL;DR: 利用CT生成的模拟X射线图像训练深度学习模型，用于真实X射线图像中的腕骨分割，解决了标注数据不足的问题。


<details>
  <summary>Details</summary>
Motivation: X射线图像分割对计算机辅助诊断至关重要，但高质量标注数据获取困难，尤其是腕骨分割因骨骼重叠更具挑战性。

Method: 通过CT体积生成大量模拟X射线图像及其标签，用于训练深度学习模型，并在模拟和真实图像上评估性能。

Result: 模拟数据集的Dice分数为0.80至0.92，真实X射线图像的定性分析也显示模型性能优越。
...
### [116] [Label-Efficient Chest X-ray Diagnosis via Partial CLIP Adaptation](https://arxiv.org/abs/2507.07254)
*Heet Nitinkumar Dalsania*

Main category: eess.IV

TL;DR: 提出了一种标签高效的胸部X光诊断策略，适用于真实医院场景，通过部分微调CLIP ViT-B/32模型，在少量标注数据下表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决医学影像标注数据稀缺、隐私和成本高的问题，模拟医院内部工作流程。

Method: 使用NIH Chest X-ray14数据集和预训练CLIP ViT-B/32模型，通过部分微调视觉编码器，评估零样本和少样本学习（1-16标注样本/类）。
...
### [117] [Computationally Efficient Information-Driven Optical Design with Interchanging Optimization](https://arxiv.org/abs/2507.07789)
*Eric Markley,Henry Pinkard,Leyla Kabuli,Nalini Singh,Laura Waller*

Main category: eess.IV

TL;DR: IDEAL-IO通过交替优化方法解决了IDEAL的高内存和长运行时间问题，提升了信息理论优化在成像系统设计中的实用性。


<details>
  <summary>Details</summary>
Motivation: IDEAL方法存在高内存使用、长运行时间和目标函数不匹配的问题，需要改进以提升实用性。

Method: 提出IDEAL-IO，通过交替优化将密度估计与光学参数优化解耦，减少内存和运行时间。

Result: IDEAL-IO在衍射光学、无透镜成像和快照3D显微镜应用中验证了其有效性，内存和运行时间减少6倍。
